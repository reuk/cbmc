...
int armcc_modet::doit()
{
  ...
  if(cmdline.isset('J'))
  {
    const std::list<std::string> &values= cmdline.get_values('J');
  ...
  {
    const std::list<std::string> &values= cmdline.get_values("preinclude=");
  ...
  if(cmdline.isset("default_extension="))
    compiler.object_file_extension= cmdline.get_value("default_extension=");
  else
  ...
  if(verbosity > 8)
  {
    ...
    for(it= compiler.library_paths.begin(); it != compiler.library_paths.end();
        it++)
    {
      std::cout << "  " << (*it) << std::endl;
    }
