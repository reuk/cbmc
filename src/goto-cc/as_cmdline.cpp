...
const char *goto_as_options_with_argument[] = {
    "--verbosity", "--function", "--native-assembler",
    "--print-rejected-preprocessed-source", NULL};

const char *as_options_without_argument[] = {"-a", // [-a[cdghlns][=file]]
                                             "--alternate",
                                             "-D",
                                             "--divide", // i386
                                             "-f",
...
const char *as_options_with_argument[] = {"--debug-prefix-map",
                                          "--defsym",
                                          "-I",
                                          "--listing-lhs-width",
                                          "--listing-lhs-width2",
...
bool as_cmdlinet::parse(int argc, const char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    for (const char **o = goto_as_options_with_argument; *o != NULL && !found;
         ++o) {
      std::string os(*o);
    ...
    if (has_prefix(argv_i, "-a")) // a-options
    {
      ...
      if (argv_i.size() > 2 && equal_pos != std::string::npos && equal_pos > 2)
        a_opts = argv_i.substr(2, equal_pos - 2);
      else if (argv_i.size() > 2 && equal_pos == std::string::npos)
        a_opts = argv_i.substr(2);

      for (std::string::const_iterator it = a_opts.begin(); it != a_opts.end();
           ++it) {
    ...
    for (const char **o = as_options_with_argument; *o != NULL && !found; ++o) {
      std::string os(*o);
