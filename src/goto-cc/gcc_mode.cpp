...
static std::string compiler_name(const cmdlinet &cmdline,
                                 const std::string &base_name) {
  if (cmdline.isset("native-compiler"))
    return cmdline.get_value("native-compiler");

  if (base_name == "bcc" || base_name.find("goto-bcc") != std::string::npos)
    return "bcc";

  std::string::size_type pos = base_name.find("goto-gcc");

  if (pos == std::string::npos || base_name == "goto-gcc" ||
      base_name == "goto-ld") {
#ifdef __FreeBSD__
    return "clang";
#else
...
static std::string linker_name(const cmdlinet &cmdline,
                               const std::string &base_name) {
...
gcc_modet::gcc_modet(goto_cc_cmdlinet &_cmdline, const std::string &_base_name,
...
bool gcc_modet::needs_preprocessing(const std::string &file) {
  if (has_suffix(file, ".c") || has_suffix(file, ".cc") ||
      has_suffix(file, ".cp") || has_suffix(file, ".cpp") ||
      has_suffix(file, ".CPP") || has_suffix(file, ".c++") ||
      has_suffix(file, ".C"))
    return true;
  else
    return false;
...
int gcc_modet::doit() {
  ...
  native_tool_name = act_as_ld ? linker_name(cmdline, base_name)
                               : compiler_name(cmdline, base_name);

  unsigned int verbosity = 1;

  bool act_as_bcc =
      base_name == "bcc" || base_name.find("goto-bcc") != std::string::npos;
  ...
  if (act_as_ld) {
  } else if (cmdline.isset('M') || cmdline.isset("MM") || cmdline.isset('E') ||
  ...
  if (cmdline.isset("m16") || cmdline.isset("m32") || cmdline.isset("mx32")) {
  ...
  if (cmdline.isset("i386-win32") || cmdline.isset("winx64")) {
  ...
  if (cmdline.isset("std")) {
    ...
    if (std_string == "gnu99" || std_string == "c99" ||
        std_string == "iso9899:1999" || std_string == "gnu9x" ||
        std_string == "c9x" || std_string == "iso9899:199x")
      config.ansi_c.set_c99();

    if (std_string == "gnu11" || std_string == "c11" || std_string == "gnu1x" ||
        std_string == "c1x")
      config.ansi_c.set_c11();

    if (std_string == "c++11" || std_string == "c++1x" ||
        std_string == "gnu++11" || std_string == "gnu++1x" ||
        std_string == "c++1y" || std_string == "gnu++1y")
...

  {
    ...
    for (goto_cc_cmdlinet::parsed_argvt::iterator arg_it =
             cmdline.parsed_argv.begin();
         arg_it != cmdline.parsed_argv.end(); arg_it++) {
      if (arg_it->is_infile_name) {
        ...
        if (language == "cpp-output" || language == "c++-cpp-output") {
          ...
          std::string new_name = get_base_name(arg_it->arg, true) + new_suffix;
          std::string dest = temp_dir(new_name);

          int exit_code = preprocess(language, arg_it->arg, dest, act_as_bcc);
...
int gcc_modet::preprocess(const std::string &language, const std::string &src,
  ...
  for (goto_cc_cmdlinet::parsed_argvt::const_iterator it =
           cmdline.parsed_argv.begin();
       it != cmdline.parsed_argv.end(); it++) {
    if (skip_next) {
      // skip
      skip_next = false;
...
int gcc_modet::gcc_hybrid_binary() {
  ...
  if (cmdline.isset('c')) {
    if (cmdline.isset('o')) {
      ...
      for (goto_cc_cmdlinet::parsed_argvt::const_iterator i_it =
               cmdline.parsed_argv.begin();
           i_it != cmdline.parsed_argv.end(); i_it++)
        if (i_it->is_infile_name && needs_preprocessing(i_it->arg))
  ...
  if (output_files.empty() ||
      (output_files.size() == 1 && output_files.front() == "/dev/null"))
    return EX_OK;
  ...
  for (std::list<std::string>::const_iterator it = output_files.begin();
...
int gcc_modet::asm_output(
    bool act_as_bcc, const std::list<std::string> &preprocessed_source_files) {
  {
    bool have_files = false;
  ...
  if (cmdline.isset('o')) {
    ...
    for (const std::string &s : preprocessed_source_files)
      output_files.insert(std::make_pair(s, get_base_name(s, true) + ".s"));
  }

  if (output_files.empty() ||
      (output_files.size() == 1 && output_files.begin()->second == "/dev/null"))
    return EX_OK;

  debug() << "Appending preprocessed sources to generate hybrid asm output"
          << eom;
