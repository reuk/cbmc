...
void xml_irep_convertt::resolve_references(const irept &cur) {
  ...
  forall_irep(iti, cur.get_sub()) resolve_references(*iti);

  forall_named_irep(iti, cur.get_named_sub()) resolve_references(iti->second);

  forall_named_irep(iti, cur.get_comments()) resolve_references(iti->second);
}
