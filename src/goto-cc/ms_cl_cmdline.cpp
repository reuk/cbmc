...
const char *non_ms_cl_options[] = {"--show-symbol-table",
                                   "--show-function-table",
                                   "--ppc-macos",
                                   "--i386-linux",
                                   "--i386-win32",
...
bool ms_cl_cmdlinet::parse(const std::vector<std::string> &options) {
  for (std::size_t i = 0; i < options.size(); i++) {
    ...
    if (std::string(options[i], 0, 2) == "--") {
      ...
      if (options[i] == "--verbosity" || options[i] == "--function") {
        if (i < options.size() - 1) {
    ...
    } else if (options[i] == "/link" || options[i] == "-link") {
...
void ms_cl_cmdlinet::process_response_file(const std::string &file) {
  ...
  if (!infile) {
    std::cerr << "failed to open response file `" << file << "'" << std::endl;
    return;
  }
  ...
  std::string line;
  getline(infile, line);
  if (line.size() >= 2 && line[0] == static_cast<char>(0xff) &&
...
#endif
  } else if (line.size() >= 3 && line[0] == static_cast<char>(0xef) &&
...
const char *ms_cl_flags[] = {"c", // compile only
                             NULL};
...
void ms_cl_cmdlinet::process_cl_option(const std::string &s) {
  ...
  // unrecognized option
  std::cout << "Warning: uninterpreted CL option `" << s << "'" << std::endl;
}
