...
const char *goto_ld_options_with_argument[] = {
    "--verbosity", "--native-compiler", "--native-linker", NULL};

const char *ld_options_with_argument[] = {"-a",
                                          "--architecture",
                                          "-A",
                                          "--format",
                                          "-b",
...
const char *ld_options_without_argument[] = {"--dc",
                                             "-d",
                                             "--dp",
                                             "--export-dynamic",
                                             "-E",
...
bool ld_cmdlinet::parse(int argc, const char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    for (const char **o = goto_ld_options_with_argument; *o != NULL && !found;
         ++o) {
      std::string os(*o);

      // separated?
      if (argv_i == os || (os.size() >= 3 && os[0] == '-' && os[1] == '-' &&
                           "-" + argv_i == os)) {
        found = true;
        if (i != argc - 1) {
          set(os, argv[i + 1]);
          ...
          set(os, ""); // end of command line
                 ...
                 has_prefix(argv_i, os + "=")) // concatenated, long
      {
        found = true;
    ...
    for (const char **o = ld_options_without_argument; *o != NULL && !found;
         o++) {
      std::string os(*o);
      ...
      if (argv_i == os || (os.size() >= 3 && os[0] == '-' && os[1] == '-' &&
        ...
        if (i != argc - 1) {
          ...
          set(os, ""); // end of command line
                 ...
                 has_prefix(argv_i, os)) // concatenated, short
      {
        found = true;
