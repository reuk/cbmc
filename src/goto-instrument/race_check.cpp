...
class w_guardst {
public:
  explicit w_guardst(symbol_tablet &_symbol_table)
...
void w_guardst::add_initialization(goto_programt &goto_program) const {
  ...
  for (std::list<irep_idt>::const_iterator it = w_guards.begin();
       it != w_guards.end(); it++) {
...
void race_check(value_setst &value_sets, symbol_tablet &symbol_table,
  ...
  Forall_goto_program_instructions(i_it, goto_program) {
    ...
    if (instruction.is_assign()) {
      ...
      forall_rw_set_w_entries(e_it, rw_set) {
        ...
        t->type = ASSIGN;
        t->code = code_assignt(w_guards.get_w_guard_expr(e_it->second),
                               e_it->second.guard);
...
void race_check(value_setst &value_sets, symbol_tablet &symbol_table,
  ...
  race_check(value_sets, symbol_table, L_M_ARG(goto_function) goto_program,
             w_guards);
...
void race_check(value_setst &value_sets, symbol_tablet &symbol_table,
  ...
  Forall_goto_functions(
      f_it, goto_functions) if (f_it->first != goto_functionst::entry_point() &&
                                f_it->first != CPROVER_PREFIX "initialize")
      race_check(value_sets, symbol_table,
                 L_M_ARG(f_it->second) f_it->second.body, w_guards);
