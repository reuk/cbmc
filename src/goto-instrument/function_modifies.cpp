...
void function_modifiest::get_modifies_lhs(
    ...
    const goto_programt::const_targett t, const exprt &lhs,
    modifiest &modifies) {
  if (lhs.id() == ID_symbol)
    modifies.insert(lhs);
    ...
    get_modifies_lhs(local_may_alias, t, to_if_expr(lhs).false_case(),
                     modifies);
  }
}
...
void function_modifiest::get_modifies(const local_may_aliast &local_may_alias,
  ...
  if (instruction.is_assign()) {
    ...
    get_modifies_function(code_function_call.function(), modifies);
  }
}
