...
void dump_ct::operator()(std::ostream &os)
{
      ...
      ++it)
  {
    ...
    if(
      symbol.is_type && symbol.location.get_function().empty() &&
  ...
  if(
    global_var_stream.str().find("NULL") != std::string::npos ||
  ...
  std::ostream &os)
{
  ...
  {
    const symbolt &symbol= ns.lookup(to_c_enum_tag_type(type).get_identifier());
    assert(symbol.is_type);
  ...
  std::ostream &os)
{
  ...
  */
      ...
      it++)
  {
    ...
    if(
      comp_type.id() == ID_code || comp.get_bool(ID_from_base) ||
      comp.get_is_padding())
      continue;
...
void dump_ct::convert_compound_enum(const typet &type, std::ostream &os)
{
  ...
  for(c_enum_typet::memberst::iterator it= members.begin(); it != members.end();
      ++it)
  {
    const irep_idt bn= it->get_base_name();

    if(
      declared_enum_constants.find(bn) != declared_enum_constants.end() ||
...
void dump_ct::init_system_library_map()
{
  ...
  // setjmp.h
  const char *setjmp_syms[]= {"_longjmp",
                              "_setjmp",
                              "longjmp",
                              "longjmperror",
                              "setjmp",
  ...
  const char *unistd_syms[]= {
    "_exit",   "access",    "alarm",     "chdir",     "chown",     "close",
    "dup",     "dup2",      "execl",     "execle",    "execlp",    "execv",
    "execve",  "execvp",    "fork",      "fpathconf", "getcwd",    "getegid",
    "geteuid", "getgid",    "getgroups", "getlogin",  "getpgrp",   "getpid",
    "getppid", "getuid",    "isatty",    "link",      "lseek",     "pathconf",
    "pause",   "pipe",      "read",      "rmdir",     "setgid",    "setpgid",
    "setsid",  "setuid",    "sleep",     "sysconf",   "tcgetpgrp", "tcsetpgrp",
    "ttyname", "ttyname_r", "unlink",    "write"};
...
bool dump_ct::ignore(const symbolt &symbol)
{
  ...
  if(
    has_prefix(name_str, CPROVER_PREFIX) || name_str == "__func__" ||
  ...
  if(
    (file_str == "gcc_builtin_headers_alpha.h" ||
  ...
  if(
    name_str == "__builtin_va_start" || name_str == "__builtin_va_end" ||
  ...
  local_static_declst &local_static_decls)
{
  ...
  if(
    (func.empty() || symbol.is_extern || symbol.value.is_not_nil()) &&
...
void dump_ct::cleanup_expr(exprt &expr)
{
  ...
  else if(expr.id() == ID_union)
  {
    ...
    if(
      !u.type().get_bool(ID_C_transparent_union) &&
    ...
    base_type_eq(expr.type(), expr.op0().type(), ns))
  {
    ...
    expr.id() == ID_code && to_code(expr).get_statement() == ID_function_call &&
