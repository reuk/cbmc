...
void dump_ct::operator()(std::ostream &os) {
  ...
  Forall_symbols(it, copied_symbol_table.symbols) {
    ...
    for (code_typet::parameterst::iterator it2 = parameters.begin();
         it2 != parameters.end(); ++it2) {
      typet &type = it2->type();

      if (type.id() == ID_symbol && type.get_bool(ID_C_transparent_union)) {
        symbolt new_type_sym = ns.lookup(to_symbol_type(type).get_identifier());
  ...
  Forall_symbols(it, copied_symbol_table.symbols) {
    ...
    if (symbol.is_type &&
      ...
      for (std::pair<unique_tagst::iterator, bool> unique_entry =
               unique_tags.insert(std::make_pair(new_tag, 0));
           !unique_entry.second;
           unique_entry = unique_tags.insert(std::make_pair(new_tag, 0))) {
        new_tag =
            new_tag_base + "$" + std::to_string(unique_entry.first->second);
  ...
  for (std::set<std::string>::const_iterator it = symbols_sorted.begin();
       it != symbols_sorted.end(); ++it) {
    const symbolt &symbol = ns.lookup(*it);
    const irep_idt &type_id = symbol.type.id();

    if (symbol.is_type && symbol.location.get_function().empty() &&
        (type_id == ID_struct || type_id == ID_incomplete_struct ||
      ...
      convert_global_variable(symbol, global_var_stream, local_static_decls);
  ...
  for (std::set<std::string>::const_iterator it = symbols_sorted.begin();
    ...
    convert_function_declaration(symbol, skip_function_main, func_decl_stream,
                                 func_body_stream, local_static_decls);
  }
  ...
  for (std::set<std::string>::const_iterator it = symbols_sorted.begin();
    ...
    if (symbol.is_type && (symbol.type.id() == ID_struct ||
      ...
      convert_compound_declaration(symbol, compound_body_stream);
  }

  for (std::set<std::string>::const_iterator it = system_headers.begin();
       it != system_headers.end(); ++it)
...
void dump_ct::convert_compound_declaration(const symbolt &symbol,
  ...
  if (symbol.type.id() == ID_struct || symbol.type.id() == ID_union ||
      symbol.type.id() == ID_c_enum)
...
void dump_ct::convert_compound(const typet &type, const typet &unresolved,
                               bool recursive, std::ostream &os) {
  if (type.id() == ID_symbol) {
    const symbolt &symbol = ns.lookup(to_symbol_type(type).get_identifier());
    assert(symbol.is_type);
...
void dump_ct::convert_compound(const struct_union_typet &type,
                               const typet &unresolved, bool recursive,
                               std::ostream &os) {
  ...
  constructor_args += "const " + type_to_string(compo.type()) + "& " +
  component_name;

  constructor_body += indent + indent + "this->"+component_name + " = " +
  component_name + ";\n";
  */
  ...
  for (struct_union_typet::componentst::const_iterator it =
           type.components().begin();
       it != type.components().end(); it++) {
    const struct_typet::componentt &comp = *it;
    const typet &comp_type = ns.follow(comp.type());

    if (comp_type.id() == ID_code || comp.get_bool(ID_from_base) ||
        comp.get_is_padding())
      continue;
    ...
    if (s.find("__CPROVER_bitvector") == std::string::npos) {
      ...
      if (t.get_width() <= config.ansi_c.long_long_int_width)
        struct_body << "long long int " << comp_name << " : " << t.get_width();
      else if (language->id() == "cpp")
        struct_body << "__signedbv<" << t.get_width() << "> " << comp_name;
      else
        struct_body << s;
      ...
      if (t.get_width() <= config.ansi_c.long_long_int_width)
        ...
        struct_body << "__unsignedbv<" << t.get_width() << "> " << comp_name;
      else
        struct_body << s;
    } else
      assert(false);
...
void dump_ct::convert_compound_enum(const typet &type, std::ostream &os) {
  assert(type.id() == ID_c_enum);

  const irept &tag = type.find(ID_tag);
  const irep_idt &name = tag.get(ID_C_base_name);

  if (tag.is_nil() || !converted_enum.insert(name).second)
    return;
  ...
  for (c_enum_typet::memberst::iterator it = members.begin();
       it != members.end(); ++it) {
    const irep_idt bn = it->get_base_name();

    if (declared_enum_constants.find(bn) != declared_enum_constants.end() ||
        copied_symbol_table.has_symbol(bn)) {
      std::string new_bn = id2string(name) + "$$" + id2string(bn);
      it->set_base_name(new_bn);
    }

    declared_enum_constants.insert(std::make_pair(bn, it->get_base_name()));
  }
...
#define ADD_TO_SYSTEM_LIBRARY(v, header)                                       \
  ...
  system_library_map.insert(std::make_pair(v[i], header))

void dump_ct::init_system_library_map() {
  // ctype.h
  const char *ctype_syms[] = {"isalnum", "isalpha", "isblank", "iscntrl",
  ...
  // fcntl.h
  const char *fcntl_syms[] = {"creat", "fcntl", "open"};
  ADD_TO_SYSTEM_LIBRARY(fcntl_syms, "fcntl.h");

  // locale.h
  const char *locale_syms[] = {"setlocale"};
  ADD_TO_SYSTEM_LIBRARY(locale_syms, "locale.h");

  // math.h
  const char *math_syms[] = {
      "acos",       "acosh",     "asin",    "asinh",     "atan",     "atan2",
      "atanh",      "cbrt",      "ceil",    "copysign",  "cos",      "cosh",
      "erf",        "erfc",      "exp",     "exp2",      "expm1",    "fabs",
      "fdim",       "floor",     "fma",     "fmax",      "fmin",     "fmod",
      "fpclassify", "frexp",     "hypot",   "ilogb",     "isfinite", "isinf",
      "isnan",      "isnormal",  "j0",      "j1",        "jn",       "ldexp",
      "lgamma",     "llrint",    "llround", "log",       "log10",    "log1p",
      "log2",       "logb",      "lrint",   "lround",    "modf",     "nan",
      "nearbyint",  "nextafter", "pow",     "remainder", "remquo",   "rint",
      "round",      "scalbln",   "scalbn",  "signbit",   "sin",      "sinh",
      "sqrt",       "tan",       "tanh",    "tgamma",    "trunc",    "y0",
      "y1",         "yn"};
  ADD_TO_SYSTEM_LIBRARY(math_syms, "math.h");

  // pthread.h
  const char *pthread_syms[] = {"pthread_cleanup_pop",
  ...
  // setjmp.h
  const char *setjmp_syms[] = {"_longjmp",     "_setjmp", "longjmp",
                               "longjmperror", "setjmp",  "siglongjmp",
                               "sigsetjmp"};
  ADD_TO_SYSTEM_LIBRARY(setjmp_syms, "setjmp.h");

  // stdio.h
  const char *stdio_syms[] = {
      "asprintf", "clearerr", "fclose", "fdopen", "feof", "ferror", "fflush",
      "fgetc", "fgetln", "fgetpos", "fgets", "fgetwc", "fgetws", "fileno",
      "fopen", "fprintf", "fpurge", "fputc", "fputs", "fputwc", "fputws",
      "fread", "freopen", "fropen", "fscanf", "fseek", "fsetpos", "ftell",
      "funopen", "fwide", "fwopen", "fwprintf", "fwrite", "getc", "getchar",
      "getdelim", "getline", "gets", "getw", "getwc", "getwchar", "mkdtemp",
      "mkstemp", "mktemp", "perror", "printf", "putc", "putchar", "puts",
      "putw", "putwc", "putwchar", "remove", "rewind", "scanf", "setbuf",
      "setbuffer", "setlinebuf", "setvbuf", "snprintf", "sprintf", "sscanf",
      "strerror", "swprintf", "sys_errlist", "sys_nerr", "tempnam", "tmpfile",
      "tmpnam", "ungetc", "ungetwc", "vasprintf", "vfprintf", "vfscanf",
      "vfwprintf", "vprintf", "vscanf", "vsnprintf", "vsprintf", "vsscanf",
  ...
  const char *stdlib_syms[] = {
      ...
      "atoll",   "bsearch",  "calloc", "div",      "exit",    "free",
      "getenv",  "labs",     "ldiv",   "llabs",    "lldiv",   "malloc",
      "mblen",   "mbstowcs", "mbtowc", "qsort",    "rand",    "realloc",
      "srand",   "strtod",   "strtof", "strtol",   "strtold", "strtoll",
      "strtoul", "strtoull", "system", "wcstombs", "wctomb"};
  ADD_TO_SYSTEM_LIBRARY(stdlib_syms, "stdlib.h");

  // string.h
  const char *string_syms[] = {"strcat",   "strncat", "strchr",  "strrchr",
  ...
  // time.h
  const char *time_syms[] = {"asctime", "asctime_r", "ctime", "ctime_r",
  ...
  const char *unistd_syms[] = {
      ...
      "close",     "dup",       "dup2",    "execl",     "execle",
      "execlp",    "execv",     "execve",  "execvp",    "fork",
      "fpathconf", "getcwd",    "getegid", "geteuid",   "getgid",
      "getgroups", "getlogin",  "getpgrp", "getpid",    "getppid",
      "getuid",    "isatty",    "link",    "lseek",     "pathconf",
      "pause",     "pipe",      "read",    "rmdir",     "setgid",
      "setpgid",   "setsid",    "setuid",  "sleep",     "sysconf",
      "tcgetpgrp", "tcsetpgrp", "ttyname", "ttyname_r", "unlink",
      "write"};
  ADD_TO_SYSTEM_LIBRARY(unistd_syms, "unistd.h");

  // sys/select.h
  const char *sys_select_syms[] = {"select"};
  ADD_TO_SYSTEM_LIBRARY(sys_select_syms, "sys/select.h");

  // sys/socket.h
  const char *sys_socket_syms[] = {"accept", "bind", "connect"};
  ADD_TO_SYSTEM_LIBRARY(sys_socket_syms, "sys/socket.h");

  // sys/stat.h
  const char *sys_stat_syms[] = {"fstat", "lstat", "stat"};
  ...
  */
  ...
  // sys/wait.h
  const char *sys_wait_syms[] = {"wait", "waitpid"};
...
bool dump_ct::ignore(const symbolt &symbol) {
  ...
  if (has_prefix(name_str, CPROVER_PREFIX) || name_str == "__func__" ||
      name_str == "__FUNCTION__" || name_str == "__PRETTY_FUNCTION__" ||
      name_str == "argc'" || name_str == "argv'" || name_str == "envp'" ||
      name_str == "envp_size'")
    return true;
  ...
  if (name_str == "__builtin_va_start" || name_str == "__builtin_va_end" ||
  ...
  system_library_mapt::const_iterator it = system_library_map.find(symbol.name);
...
void dump_ct::cleanup_decl(code_declt &decl, std::list<irep_idt> &local_static,
  ...
  code_blockt b;
  goto_program2codet p2s(irep_idt(), tmp, copied_symbol_table, b, local_static,
                         local_type_decls, system_headers);
  p2s();
...
void dump_ct::convert_global_variable(const symbolt &symbol, std::ostream &os,
                                      local_static_declst &local_static_decls) {
  ...
  if (!symbol.value.is_nil()) {
    std::set<std::string> symbols_sorted;
    for (find_symbols_sett::const_iterator it = syms.begin(); it != syms.end();
         ++it)
...
void dump_ct::convert_function_declaration(
    const symbolt &symbol, const bool skip_main, std::ostream &os_decl,
    std::ostream &os_body, local_static_declst &local_static_decls) {
  ...
  if (func_entry != goto_functions.function_map.end() &&
    ...
    goto_program2codet p2s(symbol.name, func_entry->second.body,
                           copied_symbol_table, b, local_static, type_decls,
                           system_headers);
    p2s();

    insert_local_static_decls(b, local_static, local_static_decls, type_decls);

    convertedt converted_c_bak(converted_compound);
    convertedt converted_e_bak(converted_enum);

    declared_enum_constants_mapt enum_constants_bak(declared_enum_constants);

    insert_local_type_decls(b, type_decls);
...
static bool find_block_position_rec(const irep_idt &identifier, codet &root,
                                    code_blockt *&dest,
  ...
  Forall_expr(it, operands) {
    ...
    if (it->id() == ID_code && to_code(*it).get_statement() != ID_skip)
      found =
          find_block_position_rec(identifier, to_code(*it), our_dest, before);
    else {
      find_symbols_sett syms;
...
void dump_ct::insert_local_static_decls(code_blockt &b,
  ...
  // other ones
  for (std::list<irep_idt>::const_reverse_iterator it = local_static.rbegin();
       it != local_static.rend(); ++it) {
    local_static_declst::const_iterator d_it = local_static_decls.find(*it);
...
void dump_ct::insert_local_type_decls(code_blockt &b,
  ...
  // look up last identifier first as its value may introduce the
  // other ones
  for (std::list<irep_idt>::const_reverse_iterator it = type_decls.rbegin();
       it != type_decls.rend(); ++it) {
...
void dump_ct::cleanup_expr(exprt &expr) {
  Forall_operands(it, expr) cleanup_expr(*it);

  cleanup_type(expr.type());

  if (expr.id() == ID_struct) {
    struct_typet type = to_struct_type(ns.follow(expr.type()));
    ...
    else if (u.op().id() == ID_constant && u.op().type().id() == ID_pointer &&
    ...
    if (!ns.lookup(fn.get_identifier(), s)) {
      ...
      if (parameters.size() == arguments.size()) {
        ...
        Forall_expr(it2, arguments) {
          ...
          if (type.id() == ID_union && type.get_bool(ID_C_transparent_union)) {
            if (it2->id() == ID_typecast && base_type_eq(it2->type(), type, ns))
              *it2 = to_typecast_expr(*it2).op();

            // add a typecast for NULL or 0
            if (it2->id() == ID_constant &&
                (it2->is_zero() ||
                 to_constant_expr(*it2).get_value() == ID_NULL)) {
  ...
  } else if (expr.id() == ID_constant && expr.type().id() == ID_signedbv) {
#if 0
...
void dump_ct::cleanup_type(typet &type) {
  Forall_subtypes(it, type) cleanup_type(*it);
  ...
  if (type.id() == ID_code) {
    ...
    for (code_typet::parameterst::iterator it = code_type.parameters().begin();
         it != code_type.parameters().end(); ++it)
...
void dump_c(const goto_functionst &src, const bool use_system_headers,
...
void dump_cpp(const goto_functionst &src, const bool use_system_headers,
              const namespacet &ns, std::ostream &out) {
