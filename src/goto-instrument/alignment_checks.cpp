...
void print_struct_alignment_problems(const symbol_tablet &symbol_table,
                                     std::ostream &out) {
  forall_symbols(it,
                 symbol_table.symbols) if (it->second.is_type &&
                                           it->second.type.id() == ID_struct) {
    ...
    for (struct_typet::componentst::const_iterator it_mem = components.begin();
      ...
      for (struct_typet::componentst::const_iterator it_next = it_mem;
           it_next != components.end(); it_next++) {
        ...
        if (it_mem != it_next && !it_next->get_is_padding()) {
          if (first_time_seen_in_struct) {
            ...
            out << std::endl
                << "WARNING: "
                << "declaration of structure " << str.find_type(ID_tag).pretty()
                << " at " << it->second.location << std::endl;
          }

          out << "members " << it_mem->get_pretty_name() << " and "
              << it_next->get_pretty_name() << " might interfere" << std::endl;
        }
      }
    }
  }
