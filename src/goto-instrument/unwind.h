...
public:
  ...
  void unwind(goto_programt &goto_program,
  ...
  void unwind(goto_programt &goto_program, const unwind_sett &unwind_set,
  ...
  jsont output_log_json() const { return unwind_log.output_log_json(); }
  ...
  struct unwind_logt {
    ...
    void cleanup(const goto_programt &goto_program) {
      forall_goto_program_instructions(it, goto_program) location_map.erase(it);
    }

    void insert(const goto_programt::const_targett target,
...
protected:
  int get_k(const irep_idt func, const unsigned loop_id, const int global_k,
            const unwind_sett &unwind_set) const;

  // copy goto program segment and redirect internal edges
  void copy_segment(const goto_programt::const_targett start,
