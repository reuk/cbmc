...
bool abstract_eventt::unsafe_pair_lwfence_param(
  ...
  bool lwsync_met) const
{
  ...
  if(
    operation == Fence || next.operation == Fence || operation == Lwfence ||
    next.operation == Lwfence || operation == ASMfence ||
    next.operation == ASMfence)
    return false;
  ...
  case RMO:
    return thread == next.thread &&
           ...
           !(variable == next.variable && operation == Write &&
             next.operation == Write) &&
           /* if posRW, fri maintained by the processor */
           !(variable == next.variable && operation == Read &&
             next.operation == Write);
...
bool abstract_eventt::unsafe_pair_asm(
  const abstract_eventt &next,
  memory_modelt model,
  unsigned char met) const
{
  ...
  case TSO:
    return (
      ...
      (met & 1) == 0);
  case PSO:
    return (thread == next.thread && operation == Write && (met & 3) == 0);
  case RMO:
    return thread == next.thread && (met & 15) == 0 &&
           /* if posWW, wsi maintained by the processor */
           !(variable == next.variable && operation == Write &&
             next.operation == Write) &&
           /* if posRW, fri maintained by the processor */
  ...
  case Power:
    return (thread == next.thread && (met & 15) == 0 &&
           ...
           (thread != next.thread && operation == Write &&
            next.operation == Read && variable == next.variable);
