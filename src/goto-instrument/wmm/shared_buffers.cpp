...
const shared_bufferst::varst &shared_bufferst::
operator()(const irep_idt &object) {
  ...
  vars.w_buff0_used =
      add(object, symbol.base_name, "$w_buff0_used", bool_typet());
  vars.w_buff1_used =
      add(object, symbol.base_name, "$w_buff1_used", bool_typet());

  vars.mem_tmp = add(object, symbol.base_name, "$mem_tmp", symbol.type);
  vars.flush_delayed =
      add(object, symbol.base_name, "$flush_delayed", bool_typet());
  ...
  for (unsigned cnt = 0; cnt < nb_threads; cnt++) {
    vars.r_buff0_thds.push_back(shared_bufferst::add(
        object, symbol.base_name, "$r_buff0_thd" + std::to_string(cnt),
        bool_typet()));
    vars.r_buff1_thds.push_back(shared_bufferst::add(
        object, symbol.base_name, "$r_buff1_thd" + std::to_string(cnt),
        bool_typet()));
  }
...
irep_idt shared_bufferst::add(const irep_idt &object, const irep_idt &base_name,
...
void shared_bufferst::assignment(goto_programt &goto_program,
                                 ...
                                 const irep_idt &id_lhs, const exprt &value) {
  const namespacet ns(symbol_table);
...
void shared_bufferst::write(goto_programt &goto_program,
  ...
  assignment(goto_program, target, source_location, vars.w_buff0,
  ...
  assignment(goto_program, target, source_location,
             vars.r_buff0_thds[current_thread], true_exprt());
}
...
void shared_bufferst::det_flush(goto_programt &goto_program,
  ...
  const exprt buff0_mine_expr =
      symbol_exprt(vars.r_buff0_thds[current_thread], bool_typet());
  const exprt buff1_mine_expr =
      symbol_exprt(vars.r_buff1_thds[current_thread], bool_typet());

  const exprt buff0_used_and_mine_expr =
      and_exprt(buff0_used_expr, buff0_mine_expr);
  const exprt buff1_used_and_mine_expr =
      and_exprt(buff1_used_expr, buff1_mine_expr);

  const exprt new_value_expr =
      if_exprt(buff0_used_and_mine_expr, buff0_expr,
               if_exprt(buff1_used_and_mine_expr, buff1_expr, lhs));
  ...
  assignment(
      goto_program, target, source_location, vars.w_buff0_used,
      if_exprt(buff0_used_and_mine_expr, false_exprt(), buff0_used_expr));
  ...
  assignment(goto_program, target, source_location, vars.w_buff1_used,
             if_exprt(buff0_or_buff1_used_and_mine_expr, false_exprt(),
                      buff1_used_expr));
    ...
    const bool tso_pso_rmo) // true: tso/pso/rmo; false: power
{
  ...
  try {
    ...
    // throw Boolean dice
    assignment(goto_program, target, source_location, choice0,
               nondet_bool_expr);
    assignment(goto_program, target, source_location, choice2,
               nondet_bool_expr);
    ...
    const symbol_exprt buff0_used_expr =
        symbol_exprt(vars.w_buff0_used, bool_typet());
    const symbol_exprt buff1_used_expr =
        symbol_exprt(vars.w_buff1_used, bool_typet());
    ...
    assignment(goto_program, target, source_location, vars.flush_delayed,
               delay_expr);
    ...
    if (tso_pso_rmo || !instrumented) {
      ...
      const exprt cond_134_expr = or_exprt(
          ...
          or_exprt(
              and_exprt(not_exprt(buff0_thd_expr), not_exprt(buff1_used_expr)),
              and_exprt(not_exprt(buff0_thd_expr), not_exprt(buff1_thd_expr))));
      ...
      // Updates
      // memory
      assignment(goto_program, target, source_location, object,
                 if_exprt(cond_134_expr, val_134_expr,
                          if_exprt(cond_267_expr, val_267_expr, val_5_expr)));
      // buff0
      assignment(goto_program, target, source_location, vars.w_buff0,
                 if_exprt(delay_expr, buff0_expr,
                          if_exprt(cond_134_expr, buff0_134_expr,
                                   if_exprt(cond_267_expr, buff0_267_expr,
                                            buff0_5_expr))));
      // buff1
      assignment(goto_program, target, source_location, vars.w_buff1,
                 if_exprt(delay_expr, buff1_expr,
                          if_exprt(cond_134_expr, buff1_134_expr,
                                   if_exprt(cond_267_expr, buff1_267_expr,
                                            buff1_5_expr))));
      // buff0_used
      assignment(goto_program, target, source_location, vars.w_buff0_used,
                 if_exprt(delay_expr, buff0_used_expr,
                          if_exprt(cond_134_expr, buff0_used_134_expr,
                                   if_exprt(cond_267_expr, buff0_used_267_expr,
                                            buff0_used_5_expr))));
      // buff1_used
      assignment(goto_program, target, source_location, vars.w_buff1_used,
                 if_exprt(delay_expr, buff1_used_expr,
                          if_exprt(cond_134_expr, buff1_used_134_expr,
                                   if_exprt(cond_267_expr, buff1_used_267_expr,
                                            buff1_used_5_expr))));
      // buff0_thd
      assignment(goto_program, target, source_location,
                 vars.r_buff0_thds[current_thread],
                 if_exprt(delay_expr, buff0_thd_expr,
                          if_exprt(cond_134_expr, buff0_thd_134_expr,
                                   if_exprt(cond_267_expr, buff0_thd_267_expr,
                                            buff0_thd_5_expr))));
      // buff1_thd
      assignment(goto_program, target, source_location,
                 vars.r_buff1_thds[current_thread],
                 if_exprt(delay_expr, buff1_thd_expr,
                          if_exprt(cond_134_expr, buff1_thd_134_expr,
                                   if_exprt(cond_267_expr, buff1_thd_267_expr,
                                            buff1_thd_5_expr))));
    }
    // POWER
    else {
      ...
      // throw Boolean dice
      assignment(goto_program, target, source_location, choice1,
                 nondet_bool_expr);
      ...
      const exprt cond_3_expr =
          and_exprt(buff0_used_expr, and_exprt(not_exprt(buff0_thd_expr),
      ...
      const exprt cond_4_expr =
          and_exprt(and_exprt(buff0_used_expr, not_exprt(buff1_thd_expr)),
                    and_exprt(buff1_used_expr, not_exprt(buff0_thd_expr)));
      const exprt val_4_expr = if_exprt(
          choice0_expr, lhs, if_exprt(choice1_expr, buff0_expr, buff1_expr));
      ...
      const exprt cond_5_expr =
          and_exprt(and_exprt(buff0_used_expr, buff1_thd_expr),
                    and_exprt(buff1_used_expr, not_exprt(buff0_thd_expr)));
      const exprt val_5_expr = if_exprt(choice0_expr, buff1_expr, buff0_expr);
      ...
      // memory
      assignment(goto_program, target, source_location, object,
                 if_exprt(cond_1_expr, val_1_expr,
                          if_exprt(cond_267_expr, val_267_expr,
                                   if_exprt(cond_4_expr, val_4_expr,
                                            if_exprt(cond_5_expr, val_5_expr,
                                                     val_3_expr)))));
      // buff0
      assignment(
          goto_program, target, source_location, vars.w_buff0,
          if_exprt(
              delay_expr, buff0_expr,
              if_exprt(cond_1_expr, buff0_1_expr,
                       if_exprt(cond_267_expr, buff0_267_expr,
                                if_exprt(cond_4_expr, buff0_4_expr,
                                         if_exprt(cond_5_expr, buff0_5_expr,
                                                  buff0_3_expr))))));
      // buff1
      assignment(
          goto_program, target, source_location, vars.w_buff1,
          if_exprt(
              delay_expr, buff1_expr,
              if_exprt(cond_1_expr, buff1_1_expr,
                       if_exprt(cond_267_expr, buff1_267_expr,
                                if_exprt(cond_4_expr, buff1_4_expr,
                                         if_exprt(cond_5_expr, buff1_5_expr,
                                                  buff1_3_expr))))));
      // buff0_used
      assignment(
          goto_program, target, source_location, vars.w_buff0_used,
          if_exprt(delay_expr, buff0_used_expr,
                   if_exprt(cond_1_expr, buff0_used_1_expr,
                            if_exprt(cond_267_expr, buff0_used_267_expr,
                                     if_exprt(cond_4_expr, buff0_used_4_expr,
                                              if_exprt(cond_5_expr,
                                                       buff0_used_5_expr,
                                                       buff0_used_3_expr))))));
      // buff1_used
      assignment(
          goto_program, target, source_location, vars.w_buff1_used,
          if_exprt(delay_expr, buff1_used_expr,
                   if_exprt(cond_1_expr, buff1_used_1_expr,
                            if_exprt(cond_267_expr, buff1_used_267_expr,
                                     if_exprt(cond_4_expr, buff1_used_4_expr,
                                              if_exprt(cond_5_expr,
                                                       buff1_used_5_expr,
                                                       buff1_used_3_expr))))));
      // buff0_thd
      assignment(
          goto_program, target, source_location,
          vars.r_buff0_thds[current_thread],
          if_exprt(
              delay_expr, buff0_thd_expr,
              if_exprt(cond_1_expr, buff0_thd_1_expr,
                       if_exprt(cond_267_expr, buff0_thd_267_expr,
                                if_exprt(cond_4_expr, buff0_thd_4_expr,
                                         if_exprt(cond_5_expr, buff0_thd_5_expr,
                                                  buff0_thd_3_expr))))));
      // buff1_thd
      assignment(
          goto_program, target, source_location,
          vars.r_buff1_thds[current_thread],
          if_exprt(
              delay_expr, buff1_thd_expr,
              if_exprt(cond_1_expr, buff1_thd_1_expr,
                       if_exprt(cond_267_expr, buff1_thd_267_expr,
                                if_exprt(cond_4_expr, buff1_thd_4_expr,
                                         if_exprt(cond_5_expr, buff1_thd_5_expr,
                                                  buff1_thd_3_expr))))));
    }
  } catch (std::string s) {
    message.warning() << s << messaget::eom;
...
bool shared_bufferst::is_buffered(
    const namespacet &ns, const symbol_exprt &symbol_expr, bool is_write
    ...
    if (instruction.is_assign()) {
      try {
        ...
        forall_rw_set_w_entries(
            w_it,
            rw_set) if (shared_buffers.is_buffered(ns, w_it->second.symbol_expr,
                                                   false))
        ...
        forall_rw_set_r_entries(e_it, rw_set) {
          ...
          shared_buffers.flush_read(goto_program, i_it, source_location,
                                    e_it->second.object);
          ...
          if (shared_buffers.is_buffered(ns, e_it->second.symbol_expr, false))
            shared_buffers.nondet_flush(
                ...
                current_thread, (model == TSO || model == PSO || model == RMO));
        }
        ...
        forall_rw_set_w_entries(e_it, rw_set) {
          ...
          if (model == RMO || model == Power) {
            forall_rw_set_r_entries(
                r_it,
                rw_set) if (shared_buffers.is_buffered(ns,
                                                       r_it->second.symbol_expr,
                                                       true)) {
              shared_buffers.delay_read(goto_program, i_it, source_location,
                                        r_it->second.object,
                                        e_it->second.object);
            }
          }
          ...
          if (shared_buffers.is_buffered(ns, e_it->second.symbol_expr, true)) {
            shared_buffers.write(goto_program, i_it, source_location,
            ...
            if (model == RMO || model == Power) {
              forall_rw_set_r_entries(
                  r_it,
                  rw_set) if (shared_buffers.affected_by_delay_set
                                  .find(r_it->second.object) !=
                              shared_buffers.affected_by_delay_set.end()) {
                shared_buffers.message.debug()
                    << "second: " << r_it->second.object << messaget::eom;
                const varst &vars = (shared_buffers)(r_it->second.object);

                shared_buffers.message.debug()
                    << "writer " << e_it->second.object << " reads "
                ...
                symbol_exprt to_replace_expr =
                    symbol_exprt(r_it->second.object, vars.type);
                symbol_exprt new_read_expr = symbol_exprt(
                    vars.read_delayed_var, pointer_typet(vars.type));
                symbol_exprt read_delayed_expr =
                    symbol_exprt(vars.read_delayed, bool_typet());

                // One extra non-deterministic choice needed
                irep_idt choice1 =
                    shared_buffers.choice(instruction.function, "1");
                const symbol_exprt choice1_expr =
                    symbol_exprt(choice1, bool_typet());
                ...
                shared_buffers.assignment(goto_program, i_it, source_location,
                                          choice1, nondet_bool_expr);

                exprt rhs = if_exprt(
                    read_delayed_expr,
                    if_exprt(choice1_expr,
        ...
        forall_rw_set_r_entries(
            e_it,
            rw_set) if (shared_buffers.is_buffered(ns, e_it->second.symbol_expr,
                                                   false)) {
          shared_buffers.message.debug()
              << "flush restore: " << e_it->second.object << messaget::eom;
          const varst vars = (shared_buffers)(e_it->second.object);
          const exprt delayed_expr =
              symbol_exprt(vars.flush_delayed, bool_typet());
          const symbol_exprt mem_value_expr =
              symbol_exprt(vars.mem_tmp, vars.type);
          const exprt cond_expr =
              if_exprt(delayed_expr, mem_value_expr, e_it->second.symbol_expr);
      ...
      for (std::set<irep_idt>::iterator s_it = past_writes.begin();
        ...
        shared_buffers.det_flush(goto_program, i_it, source_location, *s_it,
                                 current_thread);
      }
