...
void data_dpt::dp_analysis(const datat &read, bool local_read,
                           const datat &write, bool local_write) {
  const_iterator it;

  for (it = begin(); it != end(); ++it) {
    if (local_read && it->id == read.id) {
      insert(datat(write.id, (local_write ? source_locationt() : write.loc),
                   it->eq_class));
      continue;
    }

    if (local_write && it->id == write.id) {
      insert(datat(read.id, (local_read ? source_locationt() : read.loc),
                   it->eq_class));
      continue;
    }
  }
...
void data_dpt::print(messaget &message) {
  ...
  for (std::map<unsigned, std::set<source_locationt>>::const_iterator m_it =
           classed.begin();
