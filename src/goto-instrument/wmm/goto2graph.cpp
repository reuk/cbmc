...
bool inline instrumentert::local(const irep_idt &id)
{
  ...
  if(
    has_prefix(identifier, "symex_invalid") ||
...
/* break the flow (def 1) or add full barrier (def 2) */
    ...
    else if(instruction.is_function_call())
    {
      visit_cfg_function_call(
        value_sets, i_it, model, no_dependencies, replicate_body);
    }
...
{
  if(
    instrumenter.map_function_graph.find(id_function) !=
  ...
  if(in_pos.find(targ) != in_pos.end())
  {
    ...
    else
    {
      ...
      for(goto_programt::instructionst::iterator cur= i_it; cur != targ; --cur)
      {
...
event_idt
alt_copy_segment(wmm_grapht &alt_egraph, event_idt begin, event_idt end)
{
  ...
  ) const
{
  ...
  for(goto_programt::const_targett cur= targ; cur != i_it; ++cur)
  {
    ...
    forall_rw_set_r_entries(r_it, rw_set)
    {
      ...
      if(
        id2string(var).find("[]") != std::string::npos &&
  ...
  goto_programt::const_targett i_it)
{
  ...
  if(in_pos[targ].empty())
  {
    ...
    if(
      ...
      new_targ->source_location.get_file() != targ->source_location.get_file())
      return;
  }
  ...
  goto_programt::const_targett i_it)
{
  ...
  else
  {
    ...
    for(goto_programt::const_targett cur= i_it; cur != targ; --cur)
    {
      for(const auto &in : cur->incoming_edges)
      {
        if(in_pos.find(in) != in_pos.end() && updated.find(in) != updated.end())
        {
          /* out_pos[in].insert(in_pos[in])*/
          add_all_pos(it1, out_pos[in], in_pos[in]);
        }
  ...
  )
{
  ...
  forall_rw_set_r_entries(r_it, rw_set)
  {
    ...
    /* for unknown writes */
    for(std::set<event_idt>::const_iterator id_it= unknown_write_nodes.begin();
  ...
  bool hide_internals)
{
      ...
      it++)
  {
        ...
        it_e++)
    {
      ...
      if(
        var_to_instr.find(ev.variable) != var_to_instr.end() &&
...
void instrumentert::print_outputs(memory_modelt model, bool hide_internals)
{
  ...
  if(!set_of_cycles.empty())
    print_outputs_local(
      set_of_cycles, dot, ref, output, all, table, model, hide_internals);
  else if(num_sccs != 0)
  {
    for(unsigned i= 0; i < num_sccs; i++)
    {
...
{
  unsigned scc= 0;
  set_of_cycles_per_SCC.resize(
    num_sccs, std::set<event_grapht::critical_cyclet>());
