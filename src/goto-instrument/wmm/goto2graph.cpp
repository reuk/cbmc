...
bool inline instrumentert::local(const irep_idt &id) {
  ...
  if (identifier == CPROVER_PREFIX "alloc" ||
      ...
      identifier == "sys_nerr" || has_prefix(identifier, "__unbuffered_"))
    return true;
...
unsigned instrumentert::goto2graph_cfg(value_setst &value_sets,
  ...
  for (std::map<event_idt, event_idt>::const_iterator it =
           map_vertex_gnode.begin();
       it != map_vertex_gnode.end(); it++) {
    const std::size_t sg = subgraph_index[it->second];
    egraph_SCCs[sg].insert(it->first);
  }

  message.status() << "Number of threads detected: " << visitor.max_thread
                   << messaget::eom;
  ...
  message.statistics() << "Number of goto-instructions: " << instr_counter
                       << messaget::eom;
...
void instrumentert::cfg_visitort::visit_cfg_function(
    ...
    value_setst &value_sets, memory_modelt model, bool no_dependencies,
  ...
  Forall_goto_program_instructions(
      i_it, instrumenter.goto_functions.function_map[function].body) {
...
void inline instrumentert::cfg_visitort::visit_cfg_reference_function(
  ...
  goto_programt::instructionst &body =
      instrumenter.goto_functions.function_map[id_function].body.instructions;
  ...
  if (in_pos.find(targ) != in_pos.end()) {
    ...
    if (updated.find(targ) != updated.end()) {
      ...
      for (goto_programt::instructionst::iterator cur = i_it; cur != targ;
           --cur) {
...
event_idt alt_copy_segment(wmm_grapht &alt_egraph, event_idt begin,
                           event_idt end) {
...
void inline instrumentert::cfg_visitort::visit_cfg_body(
    goto_programt::const_targett i_it, loop_strategyt replicate_body,
    value_setst &value_sets
#ifdef LOCAL_MAY
    ,
    local_may_aliast &local_may
...
void inline instrumentert::cfg_visitort::visit_cfg_duplicate(
  ...
  if (in_pos[targ].empty()) {
    ...
    for (; new_targ != fun.body.instructions.end(); ++new_targ) {
...
void inline instrumentert::cfg_visitort::visit_cfg_backedge(
  ...
  if (updated.find(targ) != updated.end()) {
      ...
      for (std::set<nodet>::const_iterator from = in_pos[i_it].begin();
        ...
        if (from->first != to->first) {
          if (egraph[from->first].thread != egraph[to->first].thread)
            continue;
          instrumenter.message.debug()
              << from->first << "-po->" << to->first << messaget::eom;
    ...
    /* connects NEXT nodes following the targets -- bwd analysis */
    for (goto_programt::const_targett cur = i_it; cur != targ; --cur) {
      for (const auto &in : cur->incoming_edges) {
        if (in_pos.find(in) != in_pos.end() &&
            updated.find(in) != updated.end()) {
          /* out_pos[in].insert(in_pos[in])*/
...
void instrumentert::cfg_visitort::visit_cfg_goto(
    goto_programt::instructionst::iterator i_it, loop_strategyt replicate_body,
    value_setst &value_sets
#ifdef LOCAL_MAY
    ,
    local_may_aliast &local_may
...
void instrumentert::cfg_visitort::visit_cfg_function_call(
    value_setst &value_sets, goto_programt::instructionst::iterator i_it,
    memory_modelt model, bool no_dependencies, loop_strategyt replicate_body) {
  const goto_programt::instructiont &instruction = *i_it;
  std::set<nodet> s;
  for (const auto &in : instruction.incoming_edges)
    if (in_pos.find(in) != in_pos.end())
  ...
  try {
    ...
    instrumenter.message.warning()
        ...
        << "(function " << fun_id << "; .cpp) " << s << messaget::eom;
  }
}
...
void instrumentert::cfg_visitort::visit_cfg_lwfence(
    goto_programt::instructionst::iterator i_it) {
  const goto_programt::instructiont &instruction = *i_it;
  const abstract_eventt new_fence_event(abstract_eventt::Lwfence, thread, "f",
                                        instrumenter.unique_id++,
                                        instruction.source_location, false);
    ...
    if (in_pos.find(in) != in_pos.end()) {
      for (const auto &node : in_pos[in]) {
        if (egraph[node.first].thread != thread)
          continue;
        instrumenter.message.debug()
            << node.first << "-po->" << new_fence_node << messaget::eom;
...
void instrumentert::cfg_visitort::visit_cfg_asm_fence(
  ...
  const abstract_eventt new_fence_event(
      abstract_eventt::ASMfence, thread, "asm", instrumenter.unique_id++,
      instruction.source_location, false, WRfence, WWfence, RRfence, RWfence,
      WWcumul, RWcumul, RRcumul);
  const event_idt new_fence_node = egraph.add_node();
  egraph[new_fence_node](new_fence_event);
  const event_idt new_fence_gnode = egraph_alt.add_node();
  egraph_alt[new_fence_gnode] = new_fence_event;
...
void instrumentert::cfg_visitort::visit_cfg_assign(
    value_setst &value_sets, namespacet &ns,
  ...
  forall_rw_set_r_entries(r_it, rw_set) {
    ...
    const abstract_eventt new_read_event(
        abstract_eventt::Read, thread, id2string(read),
        instrumenter.unique_id++, instruction.source_location, local(read));
      ...
      if (in_pos.find(in) != in_pos.end()) {
        for (const auto &node : in_pos[in]) {
          if (egraph[node.first].thread != thread)
            continue;
          instrumenter.message.debug()
              << node.first << "-po->" << new_read_node << messaget::eom;
    ...
    const std::pair<id2nodet::iterator, id2nodet::iterator> with_same_var =
        map_writes.equal_range(read);
    for (id2nodet::iterator id_it = with_same_var.first;
      ...
      if (egraph[id_it->second].thread != new_read_event.thread) {
        instrumenter.message.debug()
            << id_it->second << "<-com->" << new_read_node << messaget::eom;
    ...
    for (std::set<event_idt>::const_iterator id_it =
         ...
         id_it != unknown_write_nodes.end(); ++id_it)
      if (egraph[*id_it].thread != new_read_event.thread) {
        instrumenter.message.debug()
            << *id_it << "<-com->" << new_read_node << messaget::eom;
        std::map<event_idt, event_idt>::const_iterator entry =
  ...
  forall_rw_set_w_entries(w_it, rw_set) {
    ...
    const abstract_eventt new_write_event(
        abstract_eventt::Write, thread, id2string(write),
        instrumenter.unique_id++, instruction.source_location, local(write));
    ...
    if (previous != std::numeric_limits<event_idt>::max()) {
      instrumenter.message.debug()
          << previous << "-po->" << new_write_node << messaget::eom;
        ...
        if (in_pos.find(in) != in_pos.end()) {
          for (const auto &node : in_pos[in]) {
            if (egraph[node.first].thread != thread)
              continue;
            instrumenter.message.debug()
                << node.first << "-po->" << new_write_node << messaget::eom;
    ...
    const std::pair<id2nodet::iterator, id2nodet::iterator> r_with_same_var =
    ...
    for (id2nodet::iterator idr_it = r_with_same_var.first;
      ...
      if (egraph[idr_it->second].thread != new_write_event.thread) {
        instrumenter.message.debug()
            << idr_it->second << "<-com->" << new_write_node << messaget::eom;
        std::map<event_idt, event_idt>::const_iterator entry =
    ...
    const std::pair<id2nodet::iterator, id2nodet::iterator> w_with_same_var =
    ...
    for (id2nodet::iterator idw_it = w_with_same_var.first;
      ...
      if (egraph[idw_it->second].thread != new_write_event.thread) {
        instrumenter.message.debug()
            << idw_it->second << "<-com->" << new_write_node << messaget::eom;
        std::map<event_idt, event_idt>::const_iterator entry =
    ...
    for (std::set<event_idt>::const_iterator id_it =
      ...
      if (egraph[*id_it].thread != new_write_event.thread) {
        instrumenter.message.debug()
            << *id_it << "<-com->" << new_write_node << messaget::eom;
        std::map<event_idt, event_idt>::const_iterator entry =
            instrumenter.map_vertex_gnode.find(*id_it);
        assert(entry != instrumenter.map_vertex_gnode.end());
    ...
    /* for unknown reads */
    for (std::set<event_idt>::const_iterator id_it = unknown_read_nodes.begin();
         id_it != unknown_read_nodes.end(); ++id_it)
      if (egraph[*id_it].thread != new_write_event.thread) {
        instrumenter.message.debug()
            << *id_it << "<-com->" << new_write_node << messaget::eom;
        std::map<event_idt, event_idt>::const_iterator entry =
  ...
  if (!no_dependencies) {
    forall_rw_set_w_entries(write_it, rw_set)
      ...
      instrumenter.message.debug()
          << "dp: Write:" << write << "; Read:" << read << messaget::eom;
...
void instrumentert::cfg_visitort::visit_cfg_fence(
    goto_programt::instructionst::iterator i_it) {
  const goto_programt::instructiont &instruction = *i_it;
  const abstract_eventt new_fence_event(abstract_eventt::Fence, thread, "F",
                                        instrumenter.unique_id++,
                                        instruction.source_location, false);
  const event_idt new_fence_node = egraph.add_node();
  egraph[new_fence_node](new_fence_event);
...
void inline instrumentert::add_instr_to_interleaving(
    goto_programt::instructionst::iterator it, goto_programt &interleaving) {
  if (it->is_return() || it->is_throw() || it->is_catch() || it->is_skip() ||
      it->is_dead() || it->is_start_thread() || it->is_end_thread())
    return;

  if (it->is_atomic_begin() || it->is_atomic_end()) {
...
bool instrumentert::is_cfg_spurious(const event_grapht::critical_cyclet &cyc) {
  ...
  for (event_grapht::critical_cyclet::const_iterator e_it = cyc.begin();
    ...
    Forall_goto_functions(f_it, goto_functions) {
      forall_goto_program_instructions(
          p_it,
          f_it->second.body) if (p_it->source_location == current_location) {
    ...
    if ((++e_it) != cyc.end() || !exists_n) {
      ...
      /* add this instruction to the interleaving */
      Forall_goto_program_instructions(
          i_it, *current_po) if (i_it->source_location == current_location) {
  ...
  /* if a goto points to a label outside from this interleaving, replace it
     by an assert 0 */
  Forall_goto_program_instructions(int_it,
                                   interleaving) if (int_it->is_goto()) {
    for (const auto &t : int_it->targets) {
      bool target_in_cycle = false;

      forall_goto_program_instructions(targ, interleaving) if (targ == t) {
...
void inline instrumentert::print_outputs_local(
    const std::set<event_grapht::critical_cyclet> &set, std::ofstream &dot,
    std::ofstream &ref, std::ofstream &output, std::ofstream &all,
    std::ofstream &table, memory_modelt model, bool hide_internals) {
  ...
  if (render_po_aligned) {
    for (unsigned i = 0; i <= max_thread; i++)
      if (!same_po[i].empty()) {
        dot << "{rank=same; thread_" << i << "[shape=plaintext, label=\"thread "
            << i << "\"];";
  ...
  table << std::string(80, '-');
  for (std::map<std::string, std::string>::const_iterator m_it =
           map_id2var.begin();
       m_it != map_id2var.end(); ++m_it) {
...
void instrumentert::print_outputs(memory_modelt model, bool hide_internals) {
  ...
  if (!set_of_cycles.empty())
    print_outputs_local(set_of_cycles, dot, ref, output, all, table, model,
                        hide_internals);
  else if (num_sccs != 0) {
    for (unsigned i = 0; i < num_sccs; i++) {
...
void instrumentert::collect_cycles_by_SCCs(memory_modelt model) {
  ...
  for (std::vector<std::set<event_idt>>::const_iterator it =
           egraph_SCCs.begin();
...
public:
  ...
  pthread_argumentt(instrumentert &_instr, memory_modelt _mem,
...
void instrumentert::collect_cycles_by_SCCs(memory_modelt model) {
  ...
  for (std::vector<std::set<unsigned>>::const_iterator it = egraph_SCCs.begin();
    ...
    if (it->size() >= 4) {
      ...
      int rc =
          pthread_create(&threads[scc++], NULL, collect_cycles_in_thread, &arg);

      message.status() << (rc != 0 ? "Failure " : "Success ")
                       << "in creating thread for SCC #" << scc - 1
                       << messaget::eom;
    }
