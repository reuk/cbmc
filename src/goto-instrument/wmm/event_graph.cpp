...
static const char *colour_map[NB_COLOURS]= {"red",
                                            "blue",
                                            "black",
                                            "green",
                                            "yellow",
  ...
  std::set<event_idt> &visited)
{
  ...
  for(wmm_grapht::edgest::const_iterator it= po_out(node_id).begin();
      it != po_out(node_id).end();
      ++it)
  {
  ...
  for(wmm_grapht::edgest::const_iterator it= com_out(node_id).begin();
      it != com_out(node_id).end();
      ++it)
  {
...
void event_grapht::explore_copy_segment(
  std::set<event_idt> &explored,
  event_idt begin,
  event_idt end) const
{
...
event_idt event_grapht::copy_segment(event_idt begin, event_idt end)
{
  ...
  if(
    ...
    begin_event.source_location.get_function() !=
      end_event.source_location.get_function())
    return end;

  if(
    duplicated_bodies.find(std::make_pair(begin_event, end_event)) !=
  ...
  message.status() << "tries to duplicate between "
                   << begin_event.source_location << " and "
                   << end_event.source_location << messaget::eom;
  ...
  for(std::set<event_idt>::const_iterator it_i= covered.begin();
      ...
      ++it_i)
  {
    for(event_idt it_j= 0; it_j < size(); ++it_j)
    {
      /* skips potential back-edges */
      if(it_j >= *it_i)
        continue;
...
bool event_grapht::critical_cyclet::is_unsafe(memory_modelt model, bool fast)
{
  ...
  for(th_it= begin(); th_it != end() && thread == egraph[*th_it].thread;
      ++th_it)
  ...
  for(; it != end() && next != end(); ++next, ++it)
  {
    ...
    if(s_it == n_it)
    {
      ...
      if(
        first.unsafe_pair(second, model) &&
        (first.thread != second.thread || egraph.are_po_ordered(*it, *s_it)))
      {
        ...
        if(
          first.variable == second.variable && first.thread == second.thread &&
    ...
    else
    {
      ...
      if(
        first.unsafe_pair_lwfence(second, model) &&
        (first.thread != second.thread || egraph.are_po_ordered(*it, *s_it)))
      {
        const_iterator before_first;
        const_iterator after_second;
  ...
  /* strong fence -- this pair is safe */
  if(
    egraph[back()].operation == abstract_eventt::Fence ||
  ...
  bool fast)
{
  ...
  for(const_iterator it= begin(); it != end() && ++it != end(); it++)
  {
    ...
    for(; s_it != end() && egraph[*s_it].operation == abstract_eventt::ASMfence;
        s_it++)
    ...
    if(first.thread != second.thread && model == Power)
    {
      ...
      const_iterator AC_it= ++s_it;
      --s_it;
      for(; AC_it != end() && egraph[*AC_it].thread == second.thread; AC_it++)
        if(
          egraph[*AC_it].operation == abstract_eventt::ASMfence &&
      ...
      if(AC_it == end() && egraph[front()].thread == second.thread)
      {
        for(AC_it= begin(); !(egraph[*AC_it] == first) &&
                            egraph[*AC_it].thread == second.thread;
            AC_it++)
          if(
            egraph[*AC_it].operation == abstract_eventt::ASMfence &&
            egraph[*AC_it].is_cumul() &&
      ...
      for(; BC_it != begin() && egraph[*BC_it].thread == first.thread; BC_it--)
      {
        if(
          egraph[*BC_it].operation == abstract_eventt::ASMfence &&
      ...
      if(BC_it == begin() && egraph[back()].thread == first.thread)
      {
        for(BC_it= end(); !(egraph[*BC_it] == second) &&
                          egraph[*BC_it].thread == first.thread;
            BC_it--)
          if(
            egraph[*BC_it].operation == abstract_eventt::ASMfence &&
            egraph[*BC_it].is_cumul() &&
    ...
    else
    {
      ...
      /* one (or more) lwfence between the pair */
      if(
        first.unsafe_pair_asm(second, model, fences_met) &&
        (first.thread != second.thread || egraph.are_po_ordered(*it, *s_it)))
      {
        if(fast)
          return true;
  ...
  /* strong fence -- this pair is safe */
  if(
    egraph[back()].operation == abstract_eventt::ASMfence ||
...
bool event_grapht::critical_cyclet::is_not_uniproc() const
{
  ...
  for(; it != end(); ++it)
  {
    const abstract_eventt &it_evt= egraph[*it];
    if(
      it_evt.variable != var && it_evt.operation != abstract_eventt::Fence &&
      it_evt.operation != abstract_eventt::Lwfence &&
      it_evt.operation != abstract_eventt::ASMfence)
      break;
  }
...
bool event_grapht::critical_cyclet::is_not_thin_air() const
{
  ...
  for(const_iterator it= begin(); it != end(); ++it)
  {
    ...
    /* rf */
    if(
      current.operation == abstract_eventt::Write &&
...
std::string event_grapht::critical_cyclet::print_unsafes() const
{
      ...
      ++it)
  {
      ...
      (last.thread != first.thread || it->first > it->second))
    {
      ...
      last.thread == first.thread && last.operation != abstract_eventt::Fence)
    {
      name+= " Po";
      name+= (last.variable == first.variable ? "s" : "d") +
             last.get_operation() + first.get_operation();
    }
  }
  ...
  critical_cyclet &reduced) const
{
  ...
  for(first_it= begin(); first_it != end(); ++first_it)
  {
    ...
    if(
      prev_it != end() && egraph[*prev_it].thread != first.thread &&
      !first.is_fence())
      break;
    prev_it= first_it;
  }
  ...
  memory_modelt model) const
{
    ...
    if(prev_it != reduced.end())
    {
      ...
      else if(cur.operation == abstract_eventt::Fence)
      {
        ...
        while(true)
        {
          ...
          if(
            cand.operation != abstract_eventt::Fence &&
        ...
        assert(
          succ.operation == abstract_eventt::Read ||
        ...
        (prev.thread != cur.thread || *prev_it > *cur_it))
      {
        ...
        prev.thread == cur.thread && prev.operation != abstract_eventt::Fence &&
        prev.operation != abstract_eventt::Lwfence &&
        prev.operation != abstract_eventt::ASMfence)
      {
        const data_dpt &dep= egraph.map_data_dp[cur.thread];

        if(prev.operation == abstract_eventt::Read && dep.dp(prev, cur))
        {
          name+= " DpData";
          name+=
            (prev.variable == cur.variable ? "s" : "d") + cur.get_operation();
        }
        else
        {
          name+= " Po";
  ...
  if(
    first.operation == abstract_eventt::Fence ||
    ...
    first.operation == abstract_eventt::ASMfence)
  {
    ...
    for(; it != reduced.end(); ++it)
    {
      ...
      if(
        ...
        cand.operation == abstract_eventt::Fence ||
        (cand.operation == abstract_eventt::ASMfence && cand.fence_value() & 1))
        cand_name= (model == Power ? " Sync" : " MFence");
    }
    ...
    assert(
      ...
      succ.operation == abstract_eventt::Write);
    name+= cand_name;
    name+= (last.variable == succ.variable ? "s" : "d") + last.get_operation() +
           succ.get_operation();
  }
  ...
  else if(last.thread == first.thread)
  {
    ...
    if(last.operation == abstract_eventt::Read && dep.dp(last, first))
    {
      name+= " DpData";
      name+=
        (last.variable == first.variable ? "s" : "d") + first.get_operation();
    }
    else
    {
      name+= " Po";
  ...
  memory_modelt model) const
{
  ...
  for(const_iterator cur_it= begin(); cur_it != end(); ++cur_it)
  {
    ...
    if(prev_it != end())
    {
        ...
        cur.operation == abstract_eventt::Write)
      {
        ...
        prev.variable == cur.variable &&
        prev.operation == abstract_eventt::Write &&
        cur.operation == abstract_eventt::Write && prev.thread != cur.thread)
      {
        /* we prefer to write Po rather than Wsi */
        str << cur.id << "[label=\"";
        ...
        prev.thread == cur.thread && prev.operation != abstract_eventt::Fence)
      {
        str << cur.id << "[label=\"";
        str << "Po"
            << (prev.variable == cur.variable ? "s" : "d") +
                 prev.get_operation() + cur.get_operation();
      }
      else
        str << cur.id << "[label=\"?";
    ...
    first.operation == abstract_eventt::Write)
  {
    ...
    last.variable == first.variable &&
    last.operation == abstract_eventt::Write &&
    first.operation == abstract_eventt::Write && last.thread != first.thread)
  {
    /* we prefer to write Po rather than Wsi */
    str << first.id << "[label=\"";
