...
static const char *colour_map[NB_COLOURS] = {
    ...
    "orange",    "blueviolet", "cyan",   "cadetblue", "magenta",
    "palegreen", "deeppink",   "indigo", "olivedrab"};
...
event_idt event_grapht::copy_segment(event_idt begin, event_idt end) {
  ...
  if (begin_event.source_location.get_file() !=
...
bool event_grapht::critical_cyclet::check_AC(
    const_iterator s_it, const abstract_eventt &first,
...
bool event_grapht::critical_cyclet::check_BC(
    const_iterator it, const abstract_eventt &first,
    const abstract_eventt &second) const {
  bool BC = false;
...
bool event_grapht::critical_cyclet::is_unsafe(memory_modelt model, bool fast) {
  ...
  if (s_it == begin()) {
    ...
    if (first.unsafe_pair(second, model) &&
        (first.thread != second.thread ||
         egraph.are_po_ordered(back(), *s_it))) {
      ...
      if (first.variable == second.variable && first.thread == second.thread &&
          egraph[*before_first].thread != first.thread &&
          egraph[*after_second].thread != second.thread) {
        /* not unsafe */
      } else {
    ...
    if (first.thread != second.thread && model == Power) {
      ...
      if (AC_it == end() && egraph[front()].thread == second.thread) {
        for (AC_it = begin(); !(egraph[*AC_it] == first) &&
          ...
          if (egraph[*AC_it].operation == abstract_eventt::ASMfence &&
              egraph[*AC_it].is_cumul() &&
              egraph[*AC_it].is_corresponding_fence(egraph[*it],
                                                    egraph[*s_it])) {
            AC = true;
            break;
          }
      ...
      for (; BC_it != begin() && egraph[*BC_it].thread == first.thread;
           BC_it--) {
      ...
      if (BC_it == begin() && egraph[back()].thread == first.thread) {
        for (BC_it = end(); !(egraph[*BC_it] == second) &&
          ...
          if (egraph[*BC_it].operation == abstract_eventt::ASMfence &&
              egraph[*BC_it].is_cumul() &&
              egraph[*BC_it].is_corresponding_fence(egraph[*it],
                                                    egraph[*s_it])) {
            BC = true;
            break;
          }
...
bool event_grapht::critical_cyclet::is_not_uniproc() const {
  ...
  for (; it != end(); ++it) {
    const abstract_eventt &it_evt = egraph[*it];
    if (it_evt.variable != var && it_evt.operation != abstract_eventt::Fence &&
        it_evt.operation != abstract_eventt::Lwfence &&
        it_evt.operation != abstract_eventt::ASMfence)
      break;
  }
...
std::string event_grapht::critical_cyclet::print_detail(
    ...
    std::map<std::string, std::string> &map_var2id, memory_modelt model) const {
  std::string cycle;
...
std::string event_grapht::critical_cyclet::print_all(
    memory_modelt model, std::map<std::string, std::string> &map_id2var,
    std::map<std::string, std::string> &map_var2id, bool hide_internals) const {
  std::string cycle;
...
std::string
event_grapht::critical_cyclet::print_name(const critical_cyclet &reduced,
  ...
  for (const_iterator cur_it = reduced.begin(); cur_it != reduced.end();
       ++cur_it) {
    const abstract_eventt &cur = egraph[*cur_it];
