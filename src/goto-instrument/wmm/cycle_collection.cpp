...
void event_grapht::graph_explorert::collect_cycles(
    std::set<critical_cyclet> &set_of_cycles, memory_modelt model) {
  ...
  for (std::list<event_idt>::const_iterator st_it = order->begin();
       st_it != order->end(); ++st_it) {
    event_idt source = *st_it;
    egraph.message.debug() << "explore " << egraph[source].id << messaget::eom;
    backtrack(set_of_cycles, source, source, false, max_po_trans, false, false,
              false, "", model);
...
event_grapht::graph_explorert::extract_cycle(event_idt vertex, event_idt source,
                                             unsigned number) {
  ...
  while (!stack.empty()) {
    ...
    egraph.message.debug() << "extract: "
                           ...
                           << egraph[current_vertex].local << messaget::eom;
...
bool event_grapht::graph_explorert::backtrack(
    std::set<critical_cyclet> &set_of_cycles, event_idt source,
    event_idt vertex,
  ...
  egraph.message.debug() << "bcktck " << egraph[vertex].id << "#" << vertex
                         ...
                         << " lw:" << lwfence_met << " unsafe:" << unsafe_met
                         << messaget::eom;
  bool f = false;
    ...
    if (!point_stack.empty()) {
      ...
      unsafe_met_updated |= prev_vertex.unsafe_pair(this_vertex, model) &&
                            !(prev_vertex.thread == this_vertex.thread &&
                              egraph.map_data_dp[this_vertex.thread].dp(
    ...
    if (!get_com_only) {
      ...
      for (wmm_grapht::edgest::const_iterator w_it =
        ...
        if (w == source && point_stack.size() >= 4 &&
          ...
          f |= backtrack(set_of_cycles, source, w, unsafe_met_updated, po_trans,
                         same_var_pair_updated, false, has_to_be_unsafe_updated,
  ...
  if (skip_tracked.find(vertex) == skip_tracked.end()) // 25 oct
    if (not_thin_air && !get_com_only && (po_trans > 1 || po_trans == 0) &&
        !point_stack.empty() &&
        egraph.are_po_ordered(point_stack.top(), vertex) &&
        this_vertex.operation != abstract_eventt::Fence &&
        (this_vertex.operation != abstract_eventt::Lwfence ||
      ...
      for (wmm_grapht::edgest::const_iterator w_it =
           ...
           w_it != egraph.po_out(vertex).end(); w_it++) {
        const event_idt w = w_it->first;
        f |= backtrack(set_of_cycles, source, w, unsafe_met /*_updated*/,
                       (po_trans == 0 ? 0 : po_trans - 1),
                       same_var_pair /*_updated*/, is_lwfence, has_to_be_unsafe,
                       avoid_at_the_end, model);
      }
