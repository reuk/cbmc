...
void weak_memory(memory_modelt model, value_setst &value_sets,
                 symbol_tablet &symbol_table, goto_functionst &goto_functions,
                 bool SCC, instrumentation_strategyt event_strategy,
                 unsigned unwinding_bound, bool no_cfg_kill,
                 bool no_dependencies, loop_strategyt duplicate_body,
                 unsigned input_max_var, unsigned input_max_po_trans,
                 bool render_po, bool render_file, bool render_function,
                 bool cav11_option, bool hide_internals,
                 message_handlert &message_handler, bool ignore_arrays) {
  ...
  if (SCC) {
    ...
    if (total_cycles == 0) {
      message.status() << "program safe -- no need to instrument"
                       << messaget::eom;
      return;
    }
  } else {
    instrumenter.collect_cycles(model);
    message.status() << "cycles collected: "
  ...
  for (std::set<irep_idt>::iterator it =
       ...
       it != shared_buffers.affected_by_delay_set.end(); it++)
  ...
  Forall_goto_functions(f_it, goto_functions) remove_skip(f_it->second.body);
