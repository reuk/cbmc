...
public:
  ...
  public:
    ...
    bool is_unsafe_fast(memory_modelt model) { return is_unsafe(model, true); }

    void compute_unsafe_pairs(memory_modelt model) { is_unsafe(model); }
    ...
    struct delayt {
      ...
      explicit delayt(event_idt _first) : first(_first), is_po(true) {}
    ...
    void print_dot(std::ostream &str, unsigned colour,
                   memory_modelt model) const;
...
protected:
  ...
  protected:
    ...
    virtual inline bool filtering(event_idt u) { return false; }

    virtual inline std::list<event_idt> *
    order_filtering(std::list<event_idt> *order) {
      return order;
    }
  ...
  public:
    graph_explorert(event_grapht &_egraph, unsigned _max_var,
        ...
        : egraph(_egraph), max_var(_max_var), max_po_trans(_max_po_trans),
          cycle_nb(0) {}
    ...
    critical_cyclet extract_cycle(event_idt vertex, event_idt source,
                                  unsigned number_of_cycles);

    bool backtrack(std::set<critical_cyclet> &set_of_cycles, event_idt source,
                   event_idt vertex, bool unsafe_met, event_idt po_trans,
                   bool same_var_pair, bool lwsync_met, bool has_to_be_unsafe,
  ...
  public:
    ...
    bool filtering(event_idt u) { return filter.find(u) == filter.end(); }
...
public:
  explicit event_grapht(messaget &_message)
      : filter_thin_air(true), filter_uniproc(true), message(_message) {}
  ...
  std::size_t size() const { return po_graph.size(); }

  const wmm_grapht::edgest &po_in(event_idt n) const { return po_graph.in(n); }
  ...
  void remove_po_edge(event_idt a, event_idt b) { po_graph.remove_edge(a, b); }
  ...
  bool is_local(event_idt a) { return operator[](a).local; }
  ...
  /* Tarjan 1972 adapted and modified for events + po-transitivity */
  void collect_cycles(std::set<critical_cyclet> &set_of_cycles,
                      memory_modelt model, const std::set<event_idt> &filter) {
