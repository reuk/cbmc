    ...
    if(find_second_event(first))
    {
      ...
      try
      {
        ...
        OUTPUT(
          res,
          "fence",
          first_event.source_location.get_file(),
          first_event.source_location.get_line(),
          first_event.variable,
  ...
  event_idt current)
{
  ...
  for(wmm_grapht::edgest::const_iterator it= egraph.po_out(current).begin();
      it != egraph.po_out(current).end();
      ++it)
  {
