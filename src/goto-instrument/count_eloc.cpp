...
static void
collect_eloc(const goto_functionst &goto_functions, working_dirst &dest)
{
  forall_goto_functions(f_it, goto_functions)
  {
    forall_goto_program_instructions(it, f_it->second.body)
    {
      ...
      if(!file.empty() && !it->source_location.is_built_in())
...
void print_path_lengths(const goto_functionst &goto_functions)
{
  ...
  if(
    start == goto_functions.function_map.end() ||
  ...
  if(n_loops > 0)
    std::cout << "Loop information: " << n_loops << " loops, " << loop_ins
