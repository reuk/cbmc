...
int goto_instrument_parse_optionst::doit() {
  ...
  try {
    ...
    if (cmdline.isset("custom-bitvector-analysis")) {
      ...
      custom_bitvector_analysist custom_bitvector_analysis;
      custom_bitvector_analysis(goto_functions, ns);
      custom_bitvector_analysis.check(ns, goto_functions,
                                      cmdline.isset("xml-ui"), std::cout);
    ...
    if (cmdline.isset("show-rw-set")) {
      ...
      std::cout << rw_set_functiont(value_set_analysis, ns, goto_functions,
                                    main);
      return 0;
    }
    ...
    if (cmdline.args.size() == 2) {
      ...
      if (write_goto_binary(cmdline.args[1], symbol_table, goto_functions,
                            get_message_handler()))
        return 1;
      else
        return 0;
...
void goto_instrument_parse_optionst::do_indirect_call_and_rtti_removal(
  ...
  remove_function_pointers(get_message_handler(), symbol_table, goto_functions,
...
void goto_instrument_parse_optionst::do_remove_const_function_pointers_only() {
  ...
  remove_function_pointers(
      get_message_handler(), symbol_table, goto_functions,
      cmdline.isset("pointer-check"),
...
void goto_instrument_parse_optionst::get_goto_program() {
  ...
  if (read_goto_binary(cmdline.args[0], symbol_table, goto_functions,
                       get_message_handler()))
    throw 0;
...
void goto_instrument_parse_optionst::instrument_goto_program() {
  ...
  if (cmdline.isset("skip-loops")) {
    ...
    if (skip_loops(goto_functions, cmdline.get_value("skip-loops"),
                   get_message_handler()))
      throw 0;
  }
  ...
  if (cmdline.isset("model-argc-argv")) {
    ...
    if (model_argc_argv(symbol_table, goto_functions, max_argc,
                        get_message_handler()))
      throw 0;
  }
  ...
  if (cmdline.isset("remove-pointers") || cmdline.isset("race-check") ||
    ...
    if (cmdline.isset("mm")) {
      ...
      if (model != Unknown)
        weak_memory(model, value_set_analysis, symbol_table, goto_functions,
                    cmdline.isset("scc"), inst_strategy, unwind_loops,
                    !cmdline.isset("cfg-kill"),
                    cmdline.isset("no-dependencies"), loops, max_var,
                    max_po_trans, !cmdline.isset("no-po-rendering"),
                    cmdline.isset("render-cluster-file"),
                    cmdline.isset("render-cluster-function"),
                    cmdline.isset("cav11"), cmdline.isset("hide-internals"),
                    get_message_handler(), cmdline.isset("ignore-arrays"));
    }

    // Interrupt handler
    if (cmdline.isset("isr")) {
      status() << "Instrumenting interrupt handler" << eom;
      interrupt(value_set_analysis, symbol_table, goto_functions,
...
void goto_instrument_parse_optionst::help() {
  std::cout
      << "\n"
         ...
         " --continue-as-loops          add loop for remaining iterations "
         "after unwound part\n" // NOLINT(*)
         ...
         " --check-invariant function   instruments invariant checking "
         "function\n"
         ...
         " --skip-loops <loop-ids>      add gotos to skip selected loops "
         "during execution\n" // NOLINT(*)
         "\n"
         ...
         " --function-inline <function> transitively inline all calls "
         "<function> makes\n" // NOLINT(*)
         " --no-caching                 disable caching of intermediate "
         "results during transitive function inlining\n" // NOLINT(*)
      ...
      HELP_REMOVE_CONST_FUNCTION_POINTERS
         ...
         " --model-argc-argv <n>        model up to <n> command line "
         "arguments\n"
         "\n"
         "Other options:\n"
