...
void goto_program2codet::operator()()
{
  ...
  forall_goto_program_instructions(target, goto_program)
    target= convert_instruction(
      target, goto_program.instructions.end(), toplevel_block);
...
void goto_program2codet::build_loop_map()
{
      ...
      ++l_it)
  {
    ...
    for(natural_loopst::natural_loopt::const_iterator it= l_it->second.begin();
        it != l_it->second.end();
        ++it)
      if((*it)->is_goto())
      {
        if(
          (*it)->get_target() == loop_start &&
  ...
  codet &dest)
{
  ...
  {
    if(
      has_prefix(id2string(*it), "__CPROVER_ASYNC_") ||
  ...
  codet &dest)
{
  ...
  if(
    !ret.has_return_value() || ret.return_value().id() != ID_side_effect ||
  ...
  codet &dest)
{
  ...
  // current boundary
  if(
    next != upper_bound && move_to_dest && !next->is_target() &&
  ...
  goto_programt::const_targett &default_target)
{
  ...
  for(; cases_it != upper_bound && cases_it != first_target; ++cases_it)
  {
      ...
      cases_it->guard.is_true())
    {
      ...
      if(
        first_target == goto_program.instructions.end() ||
      ...
      cases_it->is_goto() && !cases_it->is_backwards_goto() &&
      (cases_it->guard.id() == ID_equal || cases_it->guard.id() == ID_or))
    {
      exprt::operandst eqs;
  ...
  goto_programt::const_targett default_target)
{
  for(cases_listt::const_iterator it= cases.begin(); it != cases.end(); ++it)
  {
    ...
    if(
      it->case_last->is_goto() && it->case_last->guard.is_true() &&
      it->case_last->get_target() == default_target)
      continue;
  ...
  codet &dest)
{
  ...
  if(
    target->is_backwards_goto() || eq_cand.id() != ID_equal ||
  ...
  for(cases_listt::const_iterator it= cases.begin(); it != cases.end(); ++it)
    if(
      it->case_last != goto_program.instructions.end() &&
  ...
  for(goto_programt::const_targett it= first_target; it != target; ++it)
    if(
      processed_locations.find(it->location_number) ==
  ...
  codet &dest)
{
  ...
  goto_programt::const_targett after_loop= loop_end;
  for(++after_loop; after_loop != goto_program.instructions.end(); ++after_loop)
  {
    cfg_dominatorst::cfgt::entry_mapt::const_iterator i_entry=
...
void goto_program2codet::cleanup_code(codet &code, const irep_idt parent_stmt)
{
  if(code.get_statement() == ID_decl)
  {
    ...
    if(
      code.operands().size() == 2 && code.op1().id() == ID_side_effect &&
  ...
  else if(statement == ID_dowhile)
  {
    ...
    // do stmt while(false) is just stmt
      ...
      do_while.cond().is_false() && do_while.body().get_statement() != ID_block)
      code= do_while.body();
  }
}
  ...
  const irep_idt parent_stmt)
{
  ...
  if(operands.empty() && parent_stmt != ID_nil)
    ...
    operands.size() == 1 && parent_stmt != ID_nil &&
...
void goto_program2codet::remove_const(typet &type)
{
  ...
  else if(type.id() == ID_struct || type.id() == ID_union)
  {
    ...
    for(struct_union_typet::componentst::iterator it= c.begin(); it != c.end();
        ++it)
...
void goto_program2codet::cleanup_expr(exprt &expr, bool no_typecast)
{
  ...
  if(no_typecast && (expr.id() == ID_address_of || expr.id() == ID_member))
  {
    ...
    !no_typecast && (expr.id() == ID_union || expr.id() == ID_struct ||
    ...
    if(statement == ID_nondet)
    {
      ...
      if(id == "")
      {
        ...
        if(expr.type().get(ID_C_c_type) != "")
        {
          ...
          if(
            symbol_table.symbols.find("nondet_" + id2string(suffix)) ==
