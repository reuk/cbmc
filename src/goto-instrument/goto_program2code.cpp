...
void goto_program2codet::operator()() {
  ...
  // convert
  forall_goto_program_instructions(target, goto_program) target =
      convert_instruction(target, goto_program.instructions.end(),
                          toplevel_block);
...
void goto_program2codet::build_loop_map() {
  ...
  for (natural_loopst::loop_mapt::const_iterator l_it = loops.loop_map.begin();
       l_it != loops.loop_map.end(); ++l_it) {
    ...
    for (natural_loopst::natural_loopt::const_iterator it =
             l_it->second.begin();
         it != l_it->second.end(); ++it)
      if ((*it)->is_goto()) {
...
void goto_program2codet::build_dead_map() {
  ...
  // record last dead X
  forall_goto_program_instructions(target, goto_program) if (target->is_dead())
...
void goto_program2codet::scan_for_varargs() {
  ...
  forall_goto_program_instructions(target,
                                   goto_program) if (target->is_assign()) {
  ...
  if (!va_list_expr.empty()) {
    ...
    symbol_tablet::symbolst::iterator it = symbol_table.symbols.find(func_name);
    ...
    for (code_typet::parameterst::iterator it2 = parameters.begin();
      ...
      const symbol_exprt arg = ns.lookup(it2->get_identifier()).symbol_expr();
...
goto_programt::const_targett goto_program2codet::convert_instruction(
    ...
    goto_programt::const_targett upper_bound, codet &dest) {
  ...
  case ATOMIC_END: {
    ...
    f.function() =
        symbol_exprt(target->is_atomic_begin() ? "__CPROVER_atomic_begin"
...
void goto_program2codet::convert_labels(goto_programt::const_targett target,
  ...
  if (target->is_target()) {
    ...
    latest_block =
        &to_code_label(to_code(latest_block->operands().back())).code();
  }

  for (goto_programt::instructiont::labelst::const_iterator it =
           target->labels.begin();
       it != target->labels.end(); ++it) {
...
goto_program2codet::convert_assign(goto_programt::const_targett target,
...
goto_programt::const_targett goto_program2codet::convert_assign_varargs(
  ...
  if (r.id() == ID_constant &&
    ...
    dest.move_to_operands(f);
             ...
             to_side_effect_expr(r).get_statement() ==
    ...
    if (next != upper_bound && next->is_assign()) {
...
void goto_program2codet::convert_assign_rec(const code_assignt &assign,
  ...
  if (assign.rhs().id() == ID_array) {
    const array_typet &type = to_array_type(ns.follow(assign.rhs().type()));
...
goto_programt::const_targett
goto_program2codet::convert_return(goto_programt::const_targett target,
                                   goto_programt::const_targett upper_bound,
                                   codet &dest) {
  const code_returnt &ret = to_code_return(target->code);

  // add return instruction unless original code was missing a return
  if (!ret.has_return_value() || ret.return_value().id() != ID_side_effect ||
  ...
  if (next != upper_bound && next->is_goto() && !next->is_target())
    target = next;
...
goto_programt::const_targett
goto_program2codet::convert_decl(goto_programt::const_targett target,
                                 goto_programt::const_targett upper_bound,
                                 codet &dest) {
  ...
  bool move_to_dest =
      &toplevel_block == &dest ||
      (entry != dead_map.end() && upper_bound->location_number > entry->second);

  // move back initialising assignments into the decl, unless crossing the
  // current boundary
  if (next != upper_bound && move_to_dest && !next->is_target() &&
      (next->is_assign() || next->is_function_call())) {
    exprt lhs = next->is_assign() ? to_code_assign(next->code).lhs()
                                  : to_code_function_call(next->code).lhs();
    if (lhs == symbol && va_list_expr.find(lhs) == va_list_expr.end()) {
      if (next->is_assign())
...
goto_programt::const_targett
goto_program2codet::convert_do_while(goto_programt::const_targett target,
...
goto_programt::const_targett
goto_program2codet::convert_goto(goto_programt::const_targett target,
                                 goto_programt::const_targett upper_bound,
                                 codet &dest) {
...
goto_programt::const_targett
goto_program2codet::convert_goto_while(goto_programt::const_targett target,
                                       goto_programt::const_targett loop_end,
                                       codet &dest) {
  assert(loop_end->is_goto() && loop_end->is_backwards_goto());
  ...
  if (w.body().has_operands() &&
    ...
    f.swap(w);
  } else if (w.body().has_operands() && w.cond().is_true()) {
...
goto_programt::const_targett
goto_program2codet::get_cases(goto_programt::const_targett target,
                              goto_programt::const_targett upper_bound,
                              const exprt &switch_var, cases_listt &cases,
  ...
  for (; cases_it != upper_bound && cases_it != first_target; ++cases_it) {
    if (cases_it->is_goto() && !cases_it->is_backwards_goto() &&
      ...
      cases.push_back(
          caset(goto_program, nil_exprt(), cases_it, default_target));
      unique_targets.insert(default_target);

      ++cases_it;
      break;
    } else if (cases_it->is_goto() && !cases_it->is_backwards_goto() &&
      ...
      for (exprt::operandst::const_reverse_iterator e_it = eqs.rbegin();
        ...
        cases.push_back(caset(goto_program, to_equal_expr(*e_it).rhs(),
                              cases_it, cases_it->get_target()));
...
bool goto_program2codet::set_block_end_points(
    goto_programt::const_targett upper_bound, const cfg_dominatorst &dominators,
    cases_listt &cases, std::set<unsigned> &processed_locations) {
  std::map<goto_programt::const_targett, std::size_t> targets_done;

  for (cases_listt::iterator it = cases.begin(); it != cases.end(); ++it) {
    ...
    for (goto_programt::const_targett case_end = it->case_start;
      ...
      const cfg_dominatorst::cfgt::nodet &n = dominators.cfg[i_entry->second];
...
bool goto_program2codet::remove_default(
    const cfg_dominatorst &dominators, const cases_listt &cases,
    goto_programt::const_targett default_target) {
  for (cases_listt::const_iterator it = cases.begin(); it != cases.end();
       ++it) {
    // ignore empty cases
    ...
    if (last->case_start == default_target && it == --last) {
      // ignore dead instructions for the following checks
      goto_programt::const_targett next_case = it->case_last;
      for (++next_case; next_case != goto_program.instructions.end();
           ++next_case) {
        cfg_dominatorst::cfgt::entry_mapt::const_iterator i_entry =
    ...
    if (it->case_last->is_goto() && it->case_last->guard.is_true() &&
        it->case_last->get_target() == default_target)
      continue;
...
goto_programt::const_targett goto_program2codet::convert_goto_switch(
  ...
  if (target->is_backwards_goto() || eq_cand.id() != ID_equal ||
  ...
  goto_programt::const_targett default_target = goto_program.instructions.end();

  goto_programt::const_targett cases_start = get_cases(
      target, upper_bound, s.value(), cases, first_target, default_target);
  ...
  if (set_block_end_points(upper_bound, dominators, cases,
                           processed_locations)) {
  ...
  for (cases_listt::const_iterator it = cases.begin(); it != cases.end(); ++it)
    if (it->case_last != goto_program.instructions.end() &&
  ...
  for (goto_programt::const_targett it = first_target; it != target; ++it)
    if (processed_locations.find(it->location_number) ==
      ...
      const cfg_dominatorst::cfgt::nodet &n = dominators.cfg[it_entry->second];
...
goto_programt::const_targett
goto_program2codet::convert_goto_if(goto_programt::const_targett target,
                                    goto_programt::const_targett upper_bound,
                                    codet &dest) {
...
goto_programt::const_targett goto_program2codet::convert_goto_break_continue(
    goto_programt::const_targett target, codet &dest) {
  ...
  // 1: ...
  goto_programt::const_targett next = target;
  for (++next; next != goto_program.instructions.end(); ++next) {
    cfg_dominatorst::cfgt::entry_mapt::const_iterator i_entry =
  ...
  if (target->get_target() == loop_end && loop_last_stack.back().second) {
    code_continuet cont;
  ...
  goto_programt::const_targett after_loop = loop_end;
  for (++after_loop; after_loop != goto_program.instructions.end();
       ++after_loop) {
    cfg_dominatorst::cfgt::entry_mapt::const_iterator i_entry =
...
goto_programt::const_targett
goto_program2codet::convert_goto_goto(goto_programt::const_targett target,
                                      codet &dest) {
  ...
  for (goto_programt::instructiont::labelst::const_iterator it =
           target->get_target()->labels.begin();
       it != target->get_target()->labels.end(); ++it) {
    if (has_prefix(id2string(*it), "__CPROVER_ASYNC_") ||
        has_prefix(id2string(*it), "__CPROVER_DUMP_L"))
      continue;
...
goto_programt::const_targett goto_program2codet::convert_start_thread(
  ...
  if (thread_start->is_function_call() &&
      to_code_function_call(to_code(thread_start->code)).arguments().size() ==
          1 &&
...
goto_programt::const_targett
goto_program2codet::convert_throw(goto_programt::const_targett target,
                                  codet &dest) {
...
goto_programt::const_targett
goto_program2codet::convert_catch(goto_programt::const_targett target,
                                  goto_programt::const_targett upper_bound,
                                  codet &dest) {
...
void goto_program2codet::add_local_types(const typet &type) {
  if (type.id() == ID_symbol) {
    ...
    if (full_type.id() == ID_pointer || full_type.id() == ID_array) {
      add_local_types(full_type.subtype());
    } else if (full_type.id() == ID_struct || full_type.id() == ID_union) {
...
void goto_program2codet::cleanup_code(codet &code, const irep_idt parent_stmt) {
  if (code.get_statement() == ID_decl) {
    if (va_list_expr.find(code.op0()) != va_list_expr.end())
      code.op0().type().id(ID_gcc_builtin_va_list);

    if (code.operands().size() == 2 && code.op1().id() == ID_side_effect &&
      ...
      Forall_operands(it, code) cleanup_expr(*it, true);
    ...
    while (call.lhs().is_not_nil() && call.lhs().id() == ID_typecast)
...
void goto_program2codet::cleanup_function_call(
    const exprt &function, code_function_callt::argumentst &arguments) {
...
void goto_program2codet::cleanup_code_block(codet &code,
  ...
  for (exprt::operandst::size_type i = 0;
       operands.size() > 1 && i < operands.size();) // no ++i
  {
    ...
    // merge nested blocks, unless there are declarations in the inner block
      ...
      bool has_decl = false;
      forall_operands(it2, *it) if (it2->id() == ID_code &&
                                    to_code(*it2).get_statement() == ID_decl) {
        has_decl = true;
        break;
      }

      if (!has_decl) {
        operands.insert(operands.begin() + i + 1, it->operands().begin(),
                        it->operands().end());
  ...
  if (operands.empty() && parent_stmt != ID_nil)
    code = code_skipt();
  else if (operands.size() == 1 && parent_stmt != ID_nil &&
...
void goto_program2codet::remove_const(typet &type) {
  ...
  if (type.id() == ID_symbol) {
    ...
    for (struct_union_typet::componentst::iterator it = c.begin();
         it != c.end(); ++it)
...
static bool has_labels(const codet &code) {
  ...
  forall_operands(it, code) if (it->id() == ID_code &&
                                has_labels(to_code(*it))) return true;
...
static bool move_label_ifthenelse(exprt &expr, exprt &label_dest) {
  ...
  if (label.get_label().empty() || label.code().get_statement() != ID_skip)
    return false;
...
void goto_program2codet::cleanup_code_ifthenelse(codet &code,
  ...
  if (code.get_statement() == ID_ifthenelse && i_t_e.else_case().is_not_nil() &&
  ...
  if (code.get_statement() == ID_ifthenelse && i_t_e.else_case().is_nil() &&
...
void goto_program2codet::cleanup_expr(exprt &expr, bool no_typecast) {
  ...
  if (no_typecast && (expr.id() == ID_address_of || expr.id() == ID_member)) {
    Forall_operands(it, expr) cleanup_expr(*it, false);
  } else if (!no_typecast &&
             (expr.id() == ID_union || expr.id() == ID_struct ||
              expr.id() == ID_array || expr.id() == ID_vector)) {
    Forall_operands(it, expr) cleanup_expr(*it, true);
  } else {
    Forall_operands(it, expr) cleanup_expr(*it, no_typecast);
  }

  // work around transparent union argument
  if (expr.id() == ID_union && ns.follow(expr.type()).id() != ID_union) {
    expr = to_union_expr(expr).op();
  }

  // try to get rid of type casts, revealing (char)97 -> 'a'
  if (expr.id() == ID_typecast && to_typecast_expr(expr).op().is_constant())
    simplify(expr, ns);

  if (expr.id() == ID_union || expr.id() == ID_struct) {
    if (no_typecast)
      return;
  ...
  } else if (expr.id() == ID_array || expr.id() == ID_vector) {
    if (no_typecast || expr.get_bool(ID_C_string_constant))
      return;
  ...
  } else if (expr.id() == ID_isnan || expr.id() == ID_sign)
    ...
    if (expr.type().id() == ID_floatbv) {
      ...
      expr = from_integer(expr.is_true() ? 1 : 0,
    ...
    if (ns.follow(expr.type()).id() == ID_c_bit_field)
      ...
      assert(expr.type().id() != ID_union && expr.type().id() != ID_struct);
    }
    ...
    if (expr.type().id() != ID_code) {
      ...
      if (symbol.is_static_lifetime && symbol.type.id() != ID_code &&
          !symbol.is_extern && !symbol.location.get_function().empty() &&
