...
code_function_callt function_to_call(symbol_tablet &symbol_table,
  ...
  if (s_it == symbol_table.symbols.end()) {
    ...
    function_type.parameters().push_back(code_typet::parametert(p));
  ...
  if (s_it->second.type.id() != ID_code ||
      ...
      to_code_type(s_it->second.type).parameters()[0].type().id() !=
          ID_pointer) {
  ...
  call.arguments()[0] =
      typecast_exprt(address_of_exprt(index_exprt(
...
void function_enter(symbol_tablet &symbol_table,
                    goto_functionst &goto_functions, const irep_idt &id) {
  Forall_goto_functions(f_it, goto_functions) {
    ...
    goto_programt::targett t = body.insert_before(body.instructions.begin());
    t->make_function_call(function_to_call(symbol_table, id, f_it->first));
...
void function_exit(symbol_tablet &symbol_table, goto_functionst &goto_functions,
                   const irep_idt &id) {
  Forall_goto_functions(f_it, goto_functions) {
    // don't instrument our internal functions
    if (has_prefix(id2string(f_it->first), CPROVER_PREFIX))
