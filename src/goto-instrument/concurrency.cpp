...
public:
  concurrency_instrumentationt(value_setst &_value_sets,
...
protected:
  ...
  void collect(const goto_programt &goto_program,
...
void concurrency_instrumentationt::instrument(goto_programt &goto_program,
  ...
  for (goto_programt::instructionst::iterator it =
    ...
    if (it->is_assign()) {
      ...
      Forall_expr(it, code.arguments()) instrument(*it);
    }
  }
}
...
void concurrency(value_setst &value_sets, class symbol_tablet &symbol_table,
                 goto_functionst &goto_functions) {
  concurrency_instrumentationt concurrency_instrumentation(value_sets,
                                                           symbol_table);
