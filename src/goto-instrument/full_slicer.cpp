...
void full_slicert::add_dependencies(const cfgt::nodet &node, queuet &queue,
...
void full_slicert::add_function_calls(const cfgt::nodet &node, queuet &queue,
  ...
  cfgt::entry_mapt::const_iterator entry = cfg.entry_map.find(begin_function);
  assert(entry != cfg.entry_map.end());

  for (cfgt::edgest::const_iterator it = cfg[entry->second].in.begin();
       it != cfg[entry->second].in.end(); ++it)
...
void full_slicert::add_decl_dead(const cfgt::nodet &node, queuet &queue,
...
void full_slicert::add_jumps(
  ...
  for (jumpst::iterator it = jumps.begin(); it != jumps.end();) // no ++it
  {
    ...
    for (; !lex_succ->is_end_function(); ++lex_succ) {
      cfgt::entry_mapt::const_iterator entry = cfg.entry_map.find(lex_succ);
      assert(entry != cfg.entry_map.end());
    ...
    const cfg_post_dominatorst::cfgt::nodet &n = pd.cfg[e->second];
    ...
    if (n.dominators.find(lex_succ) == n.dominators.end()) {
      ...
      for (cfg_dominatorst::target_sett::const_iterator d_it =
               n.dominators.begin();
           d_it != n.dominators.end(); ++d_it) {
        cfgt::entry_mapt::const_iterator entry = cfg.entry_map.find(*d_it);
        assert(entry != cfg.entry_map.end());
        ...
        if (cfg[entry->second].node_required) {
          ...
          const cfg_post_dominatorst::cfgt::nodet &n2 = pd.cfg[e2->second];
...
void full_slicert::fixedpoint(goto_functionst &goto_functions, queuet &queue,
                              jumpst &jumps, decl_deadt &decl_dead,
                              const dependence_grapht &dep_graph) {
...
void full_slicert::operator()(goto_functionst &goto_functions,
  ...
  Forall_goto_functions(f_it,
                        goto_functions) if (f_it->second.body_available()) {
    Forall_goto_program_instructions(i_it, f_it->second.body) {
      ...
      if (!i_it->is_end_function() && // always retained
        ...
        for (std::set<unsigned>::const_iterator req_it =
             ...
             req_it != cfg[e].required_by.end(); ++req_it) {
...
void property_slicer(goto_functionst &goto_functions, const namespacet &ns,
