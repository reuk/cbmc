...
public:
  ...
  void output(std::ostream &out) {
    for (block_mapt::const_iterator b_it = block_map.begin();
         b_it != block_map.end(); b_it++)
      out << b_it->first->source_location << " -> " << b_it->second << '\n';
  }
};
...
bool is_condition(const exprt &src) {
  ...
  if (src.id() == ID_and || src.id() == ID_or || src.id() == ID_not ||
      src.id() == ID_implies)
    return false;
...
std::set<exprt>
replacement_conjunction(const std::set<exprt> &replacement_exprs,
...
bool is_mcdc_pair(const exprt &e1, const exprt &e2, const exprt &c,
                  const std::set<exprt> &conditions, const exprt &decision) {
...
bool has_mcdc_pair(const exprt &c, const std::set<exprt> &expr_set,
                   const std::set<exprt> &conditions, const exprt &decision) {
  ...
  Forall_goto_program_instructions(i_it, goto_program) {
...

      {
        ...
        if (blocks_done.insert(block_nr).second) {
          ...
          if (!source_location.get_file().empty() &&
            ...
            i_it->source_location.set_comment(comment);
            i_it->source_location.set(ID_coverage_criterion,
                                      coverage_criterion);
            i_it->source_location.set_property_class(property_class);
    ...
    case coverage_criteriont::BRANCH:
      ...
      if (i_it->is_goto() && !i_it->guard.is_true()) {
        ...
        std::string true_comment = "function " + id2string(i_it->function) +
                                   " block " + b + " branch true";
        std::string false_comment = "function " + id2string(i_it->function) +
                                    " block " + b + " branch false";
    ...
    case coverage_criteriont::MCDC:
      ...
      //    affect the outcome of the decision.
      {
        ...
        std::set<exprt> both;
        std::set_union(conditions.begin(), conditions.end(), decisions.begin(),
                       decisions.end(), inserter(both, both.end()));
...
bool instrument_cover_goals(const cmdlinet &cmdline,
  ...
  for (const auto &criterion_string : criteria_strings) {
    ...
    if (criterion_string == "assertion" || criterion_string == "assertions") {
    ...
    else if (criterion_string == "condition" ||
             criterion_string == "conditions")
      ...
      msg.error() << "unknown coverage criterion " << '\'' << criterion_string
                  << '\'' << messaget::eom;
      return true;
    }
