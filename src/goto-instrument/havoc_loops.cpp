...
public:
  ...
  havoc_loopst(function_modifiest &_function_modifies,
...
protected:
  ...
  void build_havoc_code(const goto_programt::targett loop_head,
                        const modifiest &modifies, goto_programt &dest);
...
void havoc_loopst::build_havoc_code(const goto_programt::targett loop_head,
                                    ...
                                    goto_programt &dest) {
  for (modifiest::const_iterator m_it = modifies.begin();
       m_it != modifies.end(); m_it++) {
    exprt lhs = *m_it;
...
void havoc_loopst::havoc_loop(const goto_programt::targett loop_head,
  ...
  for (loopt::const_iterator l_it = loop.begin(); l_it != loop.end(); l_it++) {
    ...
    if (instruction.is_goto()) {
      for (goto_programt::targetst::iterator t_it = instruction.targets.begin();
           t_it != instruction.targets.end(); t_it++) {
        if (*t_it == loop_head)
...
void havoc_loopst::get_modifies(const loopt &loop, modifiest &modifies) {
  for (loopt::const_iterator i_it = loop.begin(); i_it != loop.end(); i_it++) {
    ...
    if (instruction.is_assign()) {
      ...
      if (lhs.is_not_nil())
        function_modifies.get_modifies_lhs(local_may_alias, *i_it, lhs,
                                           modifies);
