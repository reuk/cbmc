...
void goto_unwindt::unwind(
    goto_programt &goto_program, const goto_programt::const_targett loop_head,
    const goto_programt::const_targett loop_exit, const unsigned k,
    const unwind_strategyt unwind_strategy,
    std::vector<goto_programt::targett> &iteration_points) {
  ...
  if (k != 0) {
    ...
    for (goto_programt::targett t = goto_program.const_cast_target(loop_head);
         t != loop_iter; t++) {
      if (!t->is_goto())
        continue;
...
int goto_unwindt::get_k(const irep_idt func, const unsigned loop_id,
                        const int global_k,
...
void goto_unwindt::unwind(goto_programt &goto_program,
                          const unwind_sett &unwind_set, const int k,
                          const unwind_strategyt unwind_strategy) {
  assert(k >= -1);
...
jsont goto_unwindt::unwind_logt::output_log_json() const {
  ...
  for (location_mapt::const_iterator it = location_map.begin();
    ...
    object["originalLocationNumber"] =
        json_numbert(std::to_string(location_number));
    object["newLocationNumber"] =
