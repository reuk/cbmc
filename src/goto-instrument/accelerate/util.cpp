 ...
 */
bool is_signed(const typet &t) { return t.id() == ID_signedbv; }
...
typet join_types(const typet &t1, const typet &t2) {
  ...
  if (is_bitvector(t1) && is_bitvector(t2)) {
    ...
    if (is_unsigned(b1) && is_unsigned(b2)) {
      ...
      std::size_t signed_width =
          is_signed(t1) ? b1.get_width() : b2.get_width();
      std::size_t unsigned_width =
          is_signed(t1) ? b2.get_width() : b1.get_width();
      // unsigned_width++;
  ...
  std::cerr << "Tried to join types: " << t1.pretty() << " and " << t2.pretty()
            << std::endl;
