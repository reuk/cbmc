...
public:
  path_acceleratort(patht &_path, goto_programt &pure, goto_programt &overflow,
                    std::set<exprt> &changed, std::set<exprt> &dirty)
      : path(_path), changed_vars(changed), dirty_vars(dirty) {
  ...
  path_acceleratort(const path_acceleratort &that)
      : path(that.path), changed_vars(that.changed_vars),
