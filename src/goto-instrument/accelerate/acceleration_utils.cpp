...
void acceleration_utilst::gather_rvalues(const exprt &expr, expr_sett &rvalues)
{
  if(
...
void acceleration_utilst::find_modified(const patht &path, expr_sett &modified)
{
...
exprt acceleration_utilst::precondition(patht &path)
{
  ...
  for(patht::reverse_iterator r_it= path.rbegin(); r_it != path.rend(); ++r_it)
  {
    ...
    if(t->is_assign())
    {
      ...
      if(
        lhs.id() == ID_symbol || lhs.id() == ID_index ||
...
void acceleration_utilst::push_nondet(exprt &expr)
{
  ...
  if(expr.id() == ID_not && expr.op0().id() == ID_nondet)
  {
    ...
    expr.id() == ID_equal || expr.id() == ID_lt || expr.id() == ID_gt ||
      ...
      ++a_it)
  {
        ...
        ++it)
    {
      if(it->array == array)
      {
        ...
        if(
          index.max_degree(loop_counter) > 1 ||
    ...
    if(!nonlinear_index)
    {
          ...
          ++it)
      {
        ...
        {
          max_idx= minus_exprt(
            index.to_expr(), from_integer(1, index.to_expr().type()));
  ...
  scratch_programt &program)
{
  ...
  for(goto_programt::instructionst::iterator it= body.begin(); it != body.end();
      ++it)
  {
    if(it->is_assign())
    {
  ...
  polynomialt &polynomial)
{
      ...
      ++it)
  {
    ...
    for(std::map<exprt, int>::iterator it= degrees.begin(); it != degrees.end();
        ++it)
    {
