...
void acceleration_utilst::gather_rvalues(const exprt &expr,
                                         expr_sett &rvalues) {
  if (expr.id() == ID_symbol || expr.id() == ID_index ||
      expr.id() == ID_member || expr.id() == ID_dereference) {
    rvalues.insert(expr);
  } else {
    forall_operands(it, expr) { gather_rvalues(*it, rvalues); }
  }
}

void acceleration_utilst::find_modified(const goto_programt &body,
                                        expr_sett &modified) {
  forall_goto_program_instructions(it, body) find_modified(it, modified);
}

void acceleration_utilst::find_modified(
    const goto_programt::instructionst &instructions, expr_sett &modified) {
  for (goto_programt::instructionst::const_iterator it = instructions.begin();
...
void acceleration_utilst::find_modified(
    const natural_loops_mutablet::natural_loopt &loop, expr_sett &modified) {
...
bool acceleration_utilst::check_inductive(
    std::map<exprt, polynomialt> polynomials, patht &path) {
  ...
  codet inc_loop_counter = code_assignt(
      loop_counter,
...
void acceleration_utilst::stash_polynomials(
    scratch_programt &program, std::map<exprt, polynomialt> &polynomials,
    substitutiont &substitution, patht &path) {
  expr_sett modified;
...
void acceleration_utilst::stash_variables(scratch_programt &program,
  ...
  for (expr_sett::iterator it = modified.begin(); it != modified.end(); ++it) {
  ...
  for (find_symbols_sett::iterator it = vars.begin(); it != vars.end(); ++it) {
...
exprt acceleration_utilst::precondition(patht &path) {
  ...
  for (patht::reverse_iterator r_it = path.rbegin(); r_it != path.rend();
       ++r_it) {
...
void acceleration_utilst::abstract_arrays(exprt &expr,
  ...
  if (expr.id() == ID_index || expr.id() == ID_dereference) {
    ...
    Forall_operands(it, expr) { abstract_arrays(*it, abstractions); }
  }
}

void acceleration_utilst::push_nondet(exprt &expr) {
  Forall_operands(it, expr) { push_nondet(*it); }

  if (expr.id() == ID_not && expr.op0().id() == ID_nondet) {
    expr = side_effect_expr_nondett(expr.type());
  } else if (expr.id() == ID_equal || expr.id() == ID_lt ||
             expr.id() == ID_gt || expr.id() == ID_le || expr.id() == ID_ge) {
    if (expr.op0().id() == ID_nondet || expr.op1().id() == ID_nondet) {
      expr = side_effect_expr_nondett(expr.type());
    }
  }
}
...
bool acceleration_utilst::do_assumptions(
    std::map<exprt, polynomialt> polynomials, patht &path, exprt &guard) {
...
#ifdef DEBUG
  std::cout << "Found " << array_assignments.size() << " array assignments"
            << std::endl;
#endif
...
// bail out altogether :-(
  ...
  for (polynomial_array_assignmentst::iterator it = poly_assignments.begin();
    ...
    array_operands.push_back(
        equal_exprt(index_exprt(it->array, stashed_index.to_expr()),
  ...
  exprt k_bound =
      and_exprt(binary_relation_exprt(from_integer(0, k.type()), ID_le, k),
...
// Couldn't convert the index -- bail out.
#ifdef DEBUG
      std::cout << "Couldn't convert index: " << expr2c(index_expr.index(), ns)
                << std::endl;
#endif
      return false;
    }
    ...
    std::cout << "Converted index to: "
              << expr2c(poly_assignment.index.to_expr(), ns) << std::endl;
#endif
...
// Couldn't conver the RHS -- bail out.
      ...
      std::cout << "Converted RHS to: "
                << expr2c(poly_assignment.value.to_expr(), ns) << std::endl;
#endif
...
bool acceleration_utilst::do_nonrecursive(
    ...
    std::map<exprt, polynomialt> &polynomials, exprt &loop_counter,
    substitutiont &substitution, expr_sett &nonrecursive,
    scratch_programt &program) {
  ...
  for (expr_sett::iterator it = nonrecursive.begin(); it != nonrecursive.end();
       ++it) {
    exprt e = *it;
    state[e] = e;
  }
...
// out now.
  ...
  for (expr_sett::iterator it = array_writes.begin(); it != array_writes.end();
       ++it) {
    const exprt &lhs = *it;
...
#ifdef DEBUG
      std::cout << "Failed to assign a nonrecursive array: " << expr2c(lhs, ns)
...
bool acceleration_utilst::assign_array(const exprt &lhs, const exprt &rhs,
...
// alternation, and that's too expensive.  Bail out.
#ifdef DEBUG
    std::cout << "Bailing out on nonlinear index: " << expr2c(idx, ns)
              << std::endl;
#endif
    return false;
  }
  ...
  if (stride > 0) {
    replace_expr(loop_counter, from_integer(0, loop_counter.type()),
                 lower_bound);
...
// happen to much so just bail out.
    ...
    not_touched = or_exprt(binary_relation_exprt(k, "<", lower_bound),
    ...
    not_touched = or_exprt(
        or_exprt(binary_relation_exprt(k, "<", lower_bound),
...
void acceleration_utilst::gather_array_accesses(const exprt &e,
                                                expr_sett &arrays) {
  if (e.id() == ID_index || e.id() == ID_dereference) {
    arrays.insert(e.op0());
  }

  forall_operands(it, e) { gather_array_accesses(*it, arrays); }
}
...
void acceleration_utilst::extract_polynomial(
  ...
  for (std::set<std::pair<expr_listt, exprt>>::iterator it =
           coefficients.begin();
    ...
    for (expr_listt::iterator it = terms.begin(); it != terms.end(); ++it) {
      exprt term = *it;
