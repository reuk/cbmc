...
void overflow_instrumentert::add_overflow_checks() {
  goto_programt::targett init_overflow =
  ...
  Forall_goto_program_instructions(it, program) { add_overflow_checks(it); }
}

void overflow_instrumentert::add_overflow_checks(
    goto_programt::targett t, goto_programt::targetst &added) {
...
void overflow_instrumentert::add_overflow_checks(
    goto_programt::targett t, const exprt &expr,
...
void overflow_instrumentert::overflow_expr(const exprt &expr,
                                           expr_sett &cases) {
  forall_operands(it, expr) { overflow_expr(*it, cases); }
  ...
  if (expr.id() == ID_typecast) {
    ...
    if (type.id() == ID_signedbv) {
      ...
      if (old_type.id() == ID_signedbv) {
        // signed -> unsigned
        cases.insert(binary_relation_exprt(expr.op0(), ID_lt,
    ...
    if (type.id() == ID_signedbv) {
      ...
      cases.insert(
          equal_exprt(expr.op0(), to_signedbv_type(type).smallest_expr()));
    }
...
void overflow_instrumentert::overflow_expr(const exprt &expr, exprt &overflow) {
  ...
  for (expr_sett::iterator it = cases.begin(); it != cases.end(); ++it) {
    if (overflow == false_exprt()) {
      overflow = *it;
    } else {
