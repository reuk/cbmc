...
void scratch_programt::append(goto_programt::instructionst &new_instructions) {
  instructions.insert(instructions.end(), new_instructions.begin(),
...
static void fix_types(exprt &expr) {
  Forall_operands(it, expr) { fix_types(*it); }

  if (expr.id() == ID_equal || expr.id() == ID_notequal || expr.id() == ID_gt ||
...
void scratch_programt::append_path(patht &path) {
  for (patht::iterator it = path.begin(); it != path.end(); ++it) {
...
void scratch_programt::append_loop(goto_programt &program,
  ...
  for (goto_programt::targett t = instructions.begin(); t != instructions.end();
       ++t) {
