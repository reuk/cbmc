...
bool polynomial_acceleratort::accelerate(patht &loop,
  ...
  for (patht::iterator it = loop.begin(); it != loop.end(); ++it) {
  ...
  for (expr_sett::iterator it = targets.begin(); it != targets.end(); ++it) {
  ...
  for (expr_sett::iterator it = targets.begin(); it != targets.end(); ++it) {
...
#ifdef DEBUG
      std::cout << "Found nonrecursive expression: " << expr2c(target, ns)
                << std::endl;
#endif
    ...
    if (target.id() == ID_index || target.id() == ID_dereference) {
...
#ifdef DEBUG
      std::cout << "Failed to fit a polynomial for " << expr2c(target, ns)
                << std::endl;
#endif
...
bool polynomial_acceleratort::fit_polynomial_sliced(
    goto_programt::instructionst &body, exprt &var, expr_sett &influence,
  ...
  for (expr_sett::iterator it = influence.begin(); it != influence.end();
       ++it) {
    std::cout << expr2c(*it, ns) << std::endl;
  }
#endif
  ...
  for (std::vector<expr_listt>::iterator it = parameters.begin();
    ...
    symbolt coeff =
        utils.fresh_symbol("polynomial::coeff", signedbv_typet(width));
...
#ifdef DEBUG
  std::cout << "Fitting polynomial over " << values.size() << " variables"
            << std::endl;
#endif
...
bool polynomial_acceleratort::fit_const(goto_programt::instructionst &body,
                                        exprt &target, polynomialt &poly) {
  return false;

  scratch_programt program(symbol_table);

  program.append(body);
  program.add_instruction(ASSERT)->guard =
      equal_exprt(target, not_exprt(target));
...
void polynomial_acceleratort::assert_for_values(
    scratch_programt &program, std::map<exprt, int> &values,
    std::set<std::pair<expr_listt, exprt>> &coefficients, int num_unwindings,
    goto_programt::instructionst &loop_body, exprt &target,
  ...
  for (std::map<exprt, int>::iterator it = values.begin(); it != values.end();
       ++it) {
...
void polynomial_acceleratort::cone_of_influence(
    goto_programt::instructionst &orig_body, exprt &target,
    goto_programt::instructionst &body, expr_sett &cone) {
  utils.gather_rvalues(target, cone);

  for (goto_programt::instructionst::reverse_iterator r_it = orig_body.rbegin();
       r_it != orig_body.rend(); ++r_it) {
    if (r_it->is_assign()) {
      ...
      for (expr_sett::iterator s_it = lhs_syms.begin(); s_it != lhs_syms.end();
           ++s_it) {
...
void polynomial_acceleratort::stash_polynomials(
    ...
    substitutiont &substitution, goto_programt::instructionst &body) {
  expr_sett modified;
