...
public:
  acceleration_utilst(symbol_tablet &_symbol_table,
      ...
      : symbol_table(_symbol_table), ns(symbol_table),
        goto_functions(_goto_functions), loop_counter(_loop_counter) {}

  acceleration_utilst(symbol_tablet &_symbol_table,
                      const goto_functionst &_goto_functions)
      : symbol_table(_symbol_table), ns(symbol_table),
        goto_functions(_goto_functions), loop_counter(nil) {}
  ...
  void stash_variables(scratch_programt &program, expr_sett modified,
                       substitutiont &substitution);
  void stash_polynomials(scratch_programt &program,
                         std::map<exprt, polynomialt> &polynomials,
                         std::map<exprt, exprt> &stashed, patht &path);

  exprt precondition(patht &path);
  void abstract_arrays(exprt &expr, expr_mapt &abstractions);
  void push_nondet(exprt &expr);

  bool do_assumptions(std::map<exprt, polynomialt> polynomials, patht &body,
                      exprt &guard);
  ...
  bool do_arrays(goto_programt::instructionst &loop_body,
                 ...
                 substitutiont &substitution, scratch_programt &program);
  ...
  bool expr2poly(exprt &expr, std::map<exprt, polynomialt> &polynomials,
                 polynomialt &poly);

  bool do_nonrecursive(goto_programt::instructionst &loop_body,
                       std::map<exprt, polynomialt> &polynomials,
                       exprt &loop_counter, substitutiont &substitution,
                       expr_sett &nonrecursive, scratch_programt &program);
  bool assign_array(const exprt &lhs, const exprt &rhs,
                    const exprt &loop_counter, scratch_programt &program);
  ...
  void find_modified(const goto_programt::instructionst &instructions,
                     expr_sett &modified);
  void find_modified(const natural_loops_mutablet::natural_loopt &loop,
                     expr_sett &modified);
