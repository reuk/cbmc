...
safety_checkert::resultt bmc_all_propertiest::operator()() {
  ...
  forall_goto_functions(f_it, goto_functions) forall_goto_program_instructions(
      i_it, f_it->second.body) if (i_it->is_assert())
...
void bmc_all_propertiest::report(const cover_goalst &cover_goals) {
  ...
  case ui_message_handlert::PLAIN: {
    ...
    for (const auto &goal_pair : goal_map)
      status() << "[" << goal_pair.first << "] " << goal_pair.second.description
               << ": " << goal_pair.second.status_string() << eom;
