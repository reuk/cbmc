...
public:
  explicit coverage_recordt(const std::string &node_id)
      : xml(node_id), lines_covered(0), lines_total(0), branches_covered(0),
        branches_total(0) {}
...
public:
  ...
  const irep_idt &get_file() const { return file_name; }
...
protected:
  ...
  void compute_coverage_lines(const goto_programt &goto_program,
...
goto_program_coverage_recordt::goto_program_coverage_recordt(
    const namespacet &ns, goto_functionst::function_mapt::const_iterator gf_it,
    const symex_coveraget::coveraget &coverage)
  ...
  compute_coverage_lines(gf_it->second.body, file_name, coverage,
                         coverage_lines_map);
  ...
  xml.set_attribute("line-rate", rate(lines_covered, lines_total));
  ...
  for (const auto &cov_line : coverage_lines_map) {
    ...
    if (cov_line.second.conditions.empty())
      ...
      std::ostringstream oss;
      oss << rate(total_taken, number * 2, true) << " (" << total_taken << '/'
          << number * 2 << ')';
...
void goto_program_coverage_recordt::compute_coverage_lines(
    const goto_programt &goto_program, const irep_idt &file_name,
    const symex_coveraget::coveraget &coverage, coverage_lines_mapt &dest) {
...
void symex_coveraget::compute_overall_coverage(
    const goto_functionst &goto_functions, coverage_recordt &dest) const {
  ...
  forall_goto_functions(gf_it, goto_functions) {
    ...
    for (xmlt::elementst::const_iterator it =
         ...
         it != func_cov.xml.elements.front().elements.end(); ++it)
  ...
  for (file_recordst::const_iterator it = file_records.begin();
    ...
    class_xml.set_attribute("line-rate",
                            rate(f_cov.lines_covered, f_cov.lines_total));
    class_xml.set_attribute("branch-rate",
                            rate(f_cov.branches_covered, f_cov.branches_total));
