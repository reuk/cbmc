...
cbmc_parse_optionst::cbmc_parse_optionst(int argc, const char **argv)
    : parse_options_baset(CBMC_OPTIONS, argc, argv), xml_interfacet(cmdline),
...
::cbmc_parse_optionst::cbmc_parse_optionst(int argc, const char **argv,
    ...
    : parse_options_baset(CBMC_OPTIONS + extra_options, argc, argv),
      xml_interfacet(cmdline), language_uit(cmdline, ui_message_handler),
      ui_message_handler(cmdline, "CBMC " CBMC_VERSION) {}
...
int cbmc_parse_optionst::get_goto_program(const optionst &options,
  ...
  try {
    ...
    for (const auto &bin : binaries) {
      ...
      if (read_object_and_link(bin, symbol_table, goto_functions,
                               get_message_handler())) {
        return 6;
      }
    }
...
bool cbmc_parse_optionst::process_goto_program(
    const optionst &options, goto_functionst &goto_functions) {
  try {
    namespacet ns(symbol_table);
    ...
    if (cmdline.isset("string-abstraction"))
      string_instrumentation(symbol_table, get_message_handler(),
                             goto_functions);
    ...
    if (cmdline.isset("cover")) {
      if (instrument_cover_goals(cmdline, symbol_table, goto_functions,
                                 get_message_handler()))
        return true;
    }
...
void cbmc_parse_optionst::help() {
  ...
  std::cout
      << "* *              Daniel Kroening, Edmund Clarke             * *\n"
         ...
         "analysis\n" // NOLINT(*)
         " --symex-coverage-report f    generate a Cobertura XML coverage "
         "report in f\n" // NOLINT(*)
         " --property id                only check one specific property\n"
         " --stop-on-fail               stop analysis once a failed property "
         "is detected\n" // NOLINT(*)
      ...
      << ")\n" // NOLINT(*)
         ...
         " --round-to-nearest           rounding towards nearest even "
         "(default)\n"
         ...
         "criterion CC\n" // NOLINT(*)
         " --mm MM                      memory consistency model for "
         "concurrent programs\n" // NOLINT(*)
         "\n"
         ...
         "                              (use --show-loops to get the loop "
         "IDs)\n"
         ...
         " --yices                      use Yices\n"
         " --z3                         use Z3\n"
         " --refine                     use refinement procedure "
         "(experimental)\n"
