...
void bmct::do_conversion() {
  ...
  if (!bmc_constraints.empty()) {
    ...
    forall_expr_list(it, bmc_constraints) prop_conv.set_to_true(*it);
  }
}
...
safety_checkert::resultt bmct::run(const goto_functionst &goto_functions) {
  ...
  if (mm.empty() || mm == "sc")
    memory_model =
        std::unique_ptr<memory_model_baset>(new memory_model_sct(ns));
  else if (mm == "tso")
    memory_model =
  ...
  try {
    if (options.get_option("slice-by-trace") != "") {
      ...
      symex_slice_by_trace.slice_by_trace(options.get_option("slice-by-trace"),
                                          equation);
    }
...
bmct::stop_on_fail(const goto_functionst &goto_functions,
                   prop_convt &prop_conv) {
  ...
  case decision_proceduret::D_SATISFIABLE:
    if (options.get_bool_option("trace")) {
      if (options.get_bool_option("beautify"))
        counterexample_beautificationt()(dynamic_cast<bv_cbmct &>(prop_conv),
                                         equation, ns);
