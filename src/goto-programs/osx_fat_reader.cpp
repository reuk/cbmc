...
osx_fat_readert::osx_fat_readert(std::ifstream &in) : has_gb_arch(false) {
#ifdef __APPLE__
  ...
  for (unsigned i = 0; !has_gb_arch && i < narch; ++i) {
    ...
    assert(sizeof(fa.cputype) == 4 && sizeof(fa.cpusubtype) == 4 &&
           sizeof(fa.size) == 4);
    ...
    has_gb_arch = cputype == CPU_TYPE_HPPA &&
                  cpusubtype == CPU_SUBTYPE_HPPA_7100LC && size > 0;
  }
#endif
}
