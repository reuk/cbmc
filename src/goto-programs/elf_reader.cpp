...
elf_readert::elf_readert(std::istream &_in) : in(_in) {
  // read 32-bit header
  in.read(reinterpret_cast<char *>(&elf32_header), sizeof(elf32_header));

  if (!in)
    throw "failed to read ELF header";

  if (elf32_header.e_ident[0] != 0x7f || elf32_header.e_ident[1] != 'E' ||
      elf32_header.e_ident[2] != 'L' || elf32_header.e_ident[3] != 'F')
  ...
  if (elf_class == ELF32) {
    ...
    if (elf32_header.e_shoff == 0 || elf32_header.e_shnum == 0)
    ...
    for (unsigned i = 0; i < elf32_section_header_table.size(); i++) {
      ...
      // read section header
      in.read(reinterpret_cast<char *>(&elf32_section_header_table[i]),
              sizeof(Elf32_Shdr));
    }
    ...
    // read 64-bit header
    in.seekg(0);
    in.read(reinterpret_cast<char *>(&elf64_header), sizeof(elf64_header));
    ...
    // get offset for section header
    if (elf64_header.e_shoff == 0 || elf64_header.e_shnum == 0)
    ...
    for (unsigned i = 0; i < elf64_section_header_table.size(); i++) {
      ...
      // read section header
      in.read(reinterpret_cast<char *>(&elf64_section_header_table[i]),
              sizeof(Elf64_Shdr));
    }
