...
bool goto_convertt::has_function_call(const exprt &expr) {
  forall_operands(it, expr) if (has_function_call(*it)) return true;
...
void goto_convertt::remove_assignment(side_effect_exprt &expr,
  ...
  if (statement == ID_assign) {
  ...
  } else if (statement == ID_assign_plus || statement == ID_assign_minus ||
    ...
    if (statement == ID_assign_plus)
      ...
      error().source_location = expr.find_source_location();
      error() << "assignment `" << statement << "' not yet supproted" << eom;
      throw 0;
    }
...
void goto_convertt::remove_pre(side_effect_exprt &expr, goto_programt &dest,
                               bool result_is_used) {
  ...
  assert(statement == ID_preincrement || statement == ID_predecrement);
  ...
  } else if (op_type.id() == ID_c_enum || op_type.id() == ID_c_enum_tag) {
    rhs.copy_to_operands(expr.op0(), from_integer(1, signed_int_type()));
    rhs.op0().make_typecast(signed_int_type());
    rhs.type() = signed_int_type();
    rhs.make_typecast(op_type);
...
void goto_convertt::remove_post(side_effect_exprt &expr, goto_programt &dest,
                                bool result_is_used) {
  ...
  if (expr.operands().size() != 1) {
    error().source_location = expr.find_source_location();
    error() << "postincrement/postdecrement must have one operand" << eom;
    throw 0;
  }

  const irep_idt statement = expr.get_statement();

  assert(statement == ID_postincrement || statement == ID_postdecrement);
...
void goto_convertt::replace_new_object(const exprt &object, exprt &dest) {
  if (dest.id() == "new_object")
    ...
    Forall_operands(it, dest) replace_new_object(object, *it);
}
...
void goto_convertt::remove_cpp_new(side_effect_exprt &expr, goto_programt &dest,
                                   bool result_is_used) {
  codet call;
...
void goto_convertt::remove_malloc(side_effect_exprt &expr, goto_programt &dest,
                                  bool result_is_used) {
  codet call;
  ...
  if (result_is_used) {
    ...
    new_symbol.base_name =
        "malloc_value$" + std::to_string(++temporary_counter);
    new_symbol.type = expr.type();
    new_symbol.name = tmp_symbol_prefix + id2string(new_symbol.base_name);
...
void goto_convertt::remove_temporary_object(side_effect_exprt &expr,
                                            goto_programt &dest,
                                            bool result_is_used) {
  if (expr.operands().size() != 1 && !expr.operands().empty()) {
    error().source_location = expr.find_source_location();
...
void goto_convertt::remove_statement_expression(side_effect_exprt &expr,
  ...
  if (code.operands().empty()) {
    error().source_location = expr.find_source_location();
    error() << "statement_expression takes non-empty block as operand" << eom;
    throw 0;
  }
  ...
  symbolt &new_symbol = new_tmp_symbol(expr.type(), "statement_expression",
  ...
  if (last.get(ID_statement) == ID_expression) {
    ...
    error() << "statement_expression expects expression as "
            << "last statement, but got `" << last.get(ID_statement) << "'"
            << eom;
    throw 0;
  }
...
void goto_convertt::remove_push_catch(side_effect_exprt &expr,
                                      goto_programt &dest) {
...
void goto_convertt::remove_side_effect(side_effect_exprt &expr,
  ...
  else if (statement == ID_assign || statement == ID_assign_plus ||
