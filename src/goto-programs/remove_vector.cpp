...
static bool have_to_remove_vector(const exprt &expr) {
  ...
  forall_operands(it, expr) if (have_to_remove_vector(*it)) return true;
...
static void remove_vector(exprt &expr) {
  ...
  Forall_operands(it, expr) remove_vector(*it);
...
static void remove_vector(symbol_tablet &symbol_table) {
  Forall_symbols(it, symbol_table.symbols) remove_vector(it->second);
}
...
static void remove_vector(goto_functionst &goto_functions) {
  Forall_goto_functions(it, goto_functions) remove_vector(it->second);
}
