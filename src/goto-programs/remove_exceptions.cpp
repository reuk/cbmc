...
protected:
  ...
  void
  add_exceptional_returns(const goto_functionst::function_mapt::iterator &);

  void
  instrument_exception_handler(const goto_functionst::function_mapt::iterator &,
                               const goto_programt::instructionst::iterator &);

  void instrument_throw(const goto_functionst::function_mapt::iterator &,
                        const goto_programt::instructionst::iterator &,
                        const stack_catcht &, std::vector<exprt> &);

  void
  instrument_function_call(const goto_functionst::function_mapt::iterator &,
                           const goto_programt::instructionst::iterator &,
                           const stack_catcht &, std::vector<exprt> &);
...
void remove_exceptionst::add_exceptional_returns(
  ...
  forall_goto_program_instructions(
      instr_it,
      goto_program) if (instr_it->is_throw() || instr_it->is_function_call()) {
  ...
  if (add_exceptional_var) {
    ...
    t_null->code = code_assignt(lhs_expr_null, rhs_expr_null);
...
void remove_exceptionst::instrument_exception_handler(
  ...
  if (symbol_table.has_symbol(id2string(function_id) + EXC_SUFFIX)) {
    ...
    t_exc->code = code_assignt(typecast_exprt(exception, rhs_expr_exc.type()),
                               rhs_expr_exc);
...
void remove_exceptionst::instrument_throw(
  ...
  for (std::size_t i = stack_catch.size(); i-- > 0;) {
    for (std::size_t j = stack_catch[i].size(); j-- > 0;) {
      goto_programt::targett new_state_pc = stack_catch[i][j].second;
...
void remove_exceptionst::instrument_function_call(
    const goto_functionst::function_mapt::iterator &func_it,
    const goto_programt::instructionst::iterator &instr_it,
    const stack_catcht &stack_catch, std::vector<exprt> &locals) {
  ...
  if (symbol_table.has_symbol(id2string(callee_id) + EXC_SUFFIX)) {
    ...
    for (std::size_t i = stack_catch.size(); i-- > 0;) {
      for (std::size_t j = stack_catch[i].size(); j-- > 0;) {
        ...
        symbol_typet type(stack_catch[i][j].first);
        type_exprt expr(type);
        binary_predicate_exprt check_instanceof(callee_exc, ID_java_instanceof,
                                                expr);
    ...
    equal_exprt eq_null(callee_exc,
...
void remove_exceptionst::operator()(goto_functionst &goto_functions) {
  Forall_goto_functions(it, goto_functions) add_exceptional_returns(it);
  Forall_goto_functions(it, goto_functions) instrument_exceptions(it);
}
