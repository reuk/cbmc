...
json_objectt
show_goto_functions_jsont::convert(const goto_functionst &goto_functions) {
  ...
  for (const auto &function_entry : goto_functions.function_map) {
    ...
    if (function.body_available()) {
      ...
      for (const goto_programt::instructiont &instruction :
        ...
        function.body.output_instruction(ns, function_name, instruction_builder,
                                         instruction);
        ...
        if (instruction.code.operands().size() > 0) {
          ...
          for (const exprt &operand : instruction.code.operands()) {
            ...
            no_comments_irep_converter.convert_from_irep(operand,
                                                         operand_object);
        ...
        if (!instruction.guard.is_true()) {
          ...
          no_comments_irep_converter.convert_from_irep(instruction.guard,
                                                       guard_object);
...
void show_goto_functions_jsont::
operator()(const goto_functionst &goto_functions, std::ostream &out,
           bool append) {
  if (append) {
    out << ",\n";
  }
