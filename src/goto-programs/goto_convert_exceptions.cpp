...
void goto_convertt::convert_msc_leave(const codet &code, goto_programt &dest) {
...
void goto_convertt::convert_java_try_catch(const codet &code,
                                           goto_programt &dest) {
...
void goto_convertt::convert_try_catch(const codet &code, goto_programt &dest) {
...
void goto_convertt::convert_CPROVER_throw(const codet &code,
  ...
  {
    goto_programt::targett t_set_exception = dest.add_instruction(ASSIGN);
  ...
  if (targets.throw_set) {
    // need to process destructor stack
    unwind_destructor_stack(code.source_location(), targets.throw_stack_size,
                            dest);
...
symbol_exprt goto_convertt::exception_flag() {
  ...
  symbol_tablet::symbolst::const_iterator s_it = symbol_table.symbols.find(id);
...
void goto_convertt::unwind_destructor_stack(
    const source_locationt &source_location, std::size_t final_stack_size,
    goto_programt &dest) {
  unwind_destructor_stack(source_location, final_stack_size, dest,
                          targets.destructor_stack);
}

void goto_convertt::unwind_destructor_stack(
    const source_locationt &source_location, std::size_t final_stack_size,
    goto_programt &dest, destructor_stackt &destructor_stack) {
