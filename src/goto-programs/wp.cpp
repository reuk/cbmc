...
bool has_nondet(const exprt &dest) {
  forall_operands(it, dest) if (has_nondet(*it)) return true;
...
void approximate_nondet_rec(exprt &dest, unsigned &count) {
  ...
  Forall_operands(it, dest) approximate_nondet_rec(*it, count);
}
...
aliasingt aliasing(const exprt &e1, const exprt &e2, const namespacet &ns) {
  ...
  if (e1.id() == ID_dereference && e1.operands().size() == 1 &&
      e1.op0().id() == ID_address_of && e1.op0().operands().size() == 1)
    return aliasing(e1.op0().op0(), e2, ns);

  if (e2.id() == ID_dereference && e2.operands().size() == 1 &&
...
void substitute_rec(exprt &dest, const exprt &what, const exprt &by,
                    const namespacet &ns) {
  if (dest.id() != ID_address_of)
    Forall_operands(it, dest) substitute_rec(*it, what, by, ns);

  // possibly substitute?
  if (dest.id() == ID_member || dest.id() == ID_index ||
...
exprt wp_assign(const code_assignt &code, const exprt &post,
                const namespacet &ns) {
  exprt pre = post;
...
exprt wp_assume(const code_assumet &code, const exprt &post,
                const namespacet &ns) {
