...
static bool is_empty(const goto_programt &goto_program) {
  forall_goto_program_instructions(
      it, goto_program) if (!it->is_skip() || !it->labels.empty() ||
                            !it->code.is_nil()) return false;
...
static void finish_catch_push_targets(goto_programt &dest) {
  ...
  Forall_goto_program_instructions(it, dest) {
    if (it->is_catch()) {
      ...
      if (exceptions.is_nil() && it->code.has_operands())
      ...
      if (!exception_list.empty()) {
        ...
        if (handlers.is_nil() && it->code.has_operands())
        ...
        for (const auto &handler : handler_list) {
          std::map<irep_idt, goto_programt::targett>::const_iterator
              handler_it = label_targets.find(handler.id());
  ...
  for (const auto &g_it : targets.gotos) {
    ...
    if (i.code.get_statement() == "non-deterministic-goto") {
      ...
      forall_irep(it, destinations.get_sub()) {
        labelst::const_iterator l_it = targets.labels.find(it->id_string());
      ...
      if (!stack_is_prefix) {
        ...
        if (unwind_to_size < goto_stack.size()) {
          status() << "Adding goto-destructor code on jump to " << goto_label
                   << eom;
...
void goto_convertt::finish_computed_gotos(goto_programt &goto_program) {
  for (auto &g_it : targets.computed_gotos) {
    ...
    for (const auto &label : targets.labels) {
      ...
      goto_programt::targett t = goto_program.insert_after(g_it);
...
void goto_convertt::copy(const codet &code, goto_program_instruction_typet type,
                         goto_programt &dest) {
...
void goto_convertt::convert_switch_case(const code_switch_caset &code,
  ...
  if (code.is_default())
    ...
    if (cases_entry == targets.cases_map.end()) {
      ...
      cases_entry = targets.cases_map
                        .insert(std::make_pair(target, --targets.cases.end()))
                        .first;
    }
...
void goto_convertt::convert_gcc_switch_case_range(const codet &code,
                                                  goto_programt &dest) {
...
void goto_convertt::convert(const codet &code, goto_programt &dest) {
  ...
  if (statement == ID_block)
    ...
    if (assertion.is_false()) {
      ...
      error() << "static assertion " << get_string_constant(code.op1()) << eom;
      throw 0;
    ...
    forall_operands(it, code) convert(to_code(*it), dest);
  } else
...
void goto_convertt::convert_block(const code_blockt &code,
  ...
  if (!dest.empty() && dest.instructions.back().is_goto() &&
...
void goto_convertt::convert_decl(const code_declt &code, goto_programt &dest) {
...
void goto_convertt::convert_decl_type(const codet &code, goto_programt &dest) {
  // we remove these
}
...
void goto_convertt::convert_assign(const code_assignt &code,
  ...
  if (rhs.id() == ID_side_effect && rhs.get(ID_statement) == ID_function_call) {
    ...
    Forall_operands(it, rhs) clean_expr(*it, dest);
  ...
  } else if (rhs.id() == ID_side_effect && rhs.get(ID_statement) == ID_malloc) {
    // just preserve
    Forall_operands(it, rhs) clean_expr(*it, dest);
...
void goto_convertt::convert_for(const code_fort &code, goto_programt &dest) {
...
void goto_convertt::convert_while(const code_whilet &code,
  ...
  generate_conditional_branch(boolean_negate(cond), z, source_location,
                              tmp_branch);
...
void goto_convertt::convert_switch(const code_switcht &code,
  ...
  forall_operands(it, code) if (it != code.operands().begin())
...
void goto_convertt::convert_break(const code_breakt &code,
  ...
  unwind_destructor_stack(code.source_location(), targets.break_stack_size,
                          dest);
...
void goto_convertt::convert_return(const code_returnt &code,
  ...
  if (!code.operands().empty() && code.operands().size() != 1) {
    error().source_location = code.find_source_location();
...
void goto_convertt::convert_continue(const code_continuet &code,
  ...
  // need to process destructor stack
  unwind_destructor_stack(code.source_location(), targets.continue_stack_size,
                          dest);
...
void goto_convertt::convert_non_deterministic_goto(const codet &code,
                                                   goto_programt &dest) {
  convert_goto(code, dest);
}
...
void goto_convertt::convert_specc_notify(const codet &code,
                                         goto_programt &dest) {
#if 0
...
void goto_convertt::convert_specc_event(const exprt &op,
                                        std::set<irep_idt> &events) {
  if (op.id() == ID_or || op.id() == ID_and) {
    forall_operands(it, op) convert_specc_event(*it, events);
...
void goto_convertt::convert_specc_wait(const codet &code, goto_programt &dest) {
#if 0
...
void goto_convertt::convert_specc_par(const codet &code, goto_programt &dest) {
  copy(code, OTHER, dest);
}
...
void goto_convertt::convert_start_thread(const codet &code,
  ...
  goto_programt::targett start_thread = dest.add_instruction(START_THREAD);
...
void goto_convertt::convert_bp_enforce(const codet &code, goto_programt &dest) {
  if (code.operands().size() != 2) {
    error().source_location = code.find_source_location();
  ...
  if (!op.is_true()) {
    ...
    Forall_goto_program_instructions(it, tmp) {
      if (it->is_assign()) {
        ...
        it->code.op1() = and_exprt(it->code.op1(), constraint);
      }
    }
  }
...
void goto_convertt::convert_bp_abortif(const codet &code, goto_programt &dest) {
  if (code.operands().size() != 1) {
    error().source_location = code.find_source_location();
...
void goto_convertt::convert_ifthenelse(const code_ifthenelset &code,
  ...
  if (code.cond().id() == ID_and && code.cond().operands().size() == 2 &&
      (needs_cleaning(code.cond().op0()) ||
       needs_cleaning(code.cond().op1())) &&
      !has_else) {
...
void goto_convertt::collect_operands(const exprt &expr, const irep_idt &id,
                                     std::list<exprt> &dest) {
  if (expr.id() != id) {
    dest.push_back(expr);
  } else {
    // left-to-right is important
    forall_operands(it, expr) collect_operands(*it, id, dest);
  }
}
...
void goto_convertt::generate_ifthenelse(const exprt &guard,
  ...
  if (is_empty(false_case) && is_size_one(true_case) &&
  ...
  if (is_size_one(true_case) && true_case.instructions.back().is_assert() &&
  ...
  // similarly, do guarded assertions directly
  if (is_size_one(false_case) && false_case.instructions.back().is_assert() &&
  ...
  if (is_empty(true_case))
    return generate_ifthenelse(boolean_negate(guard), false_case, true_case,
  ...
  // v: if(!c) goto z/y;
  goto_programt tmp_v;
  generate_conditional_branch(boolean_negate(guard), has_else ? y : z,
                              source_location, tmp_v);
...
static bool has_and_or(const exprt &expr) {
  forall_operands(it, expr) if (has_and_or(*it)) return true;
...
void goto_convertt::generate_conditional_branch(
    const exprt &guard, goto_programt::targett target_true,
    const source_locationt &source_location, goto_programt &dest) {
  if (has_and_or(guard) && needs_cleaning(guard)) {
    ...
    generate_conditional_branch(guard, target_true, target_false,
                                source_location, dest);
...
void goto_convertt::generate_conditional_branch(
  ...
  if (guard.id() == ID_not) {
    ...
    // simply swap targets
    generate_conditional_branch(guard.op0(), target_false, target_true,
                                source_location, dest);
    return;
  }
  ...
  if (guard.id() == ID_and) {
    ...
    forall_expr_list(it, op) generate_conditional_branch(
    ...
    forall_expr_list(it, op)
        generate_conditional_branch(*it, target_true, source_location, dest);
...
bool goto_convertt::get_string_constant(const exprt &expr, irep_idt &value) {
  ...
  if (expr.id() == ID_address_of && expr.operands().size() == 1 &&
      expr.op0().id() == ID_index && expr.op0().operands().size() == 2) {
    ...
    if (index_op.id() == ID_string_constant)
      ...
      std::string result;
      forall_operands(it, index_op) if (it->is_constant()) {
...
irep_idt goto_convertt::get_string_constant(const exprt &expr) {
  ...
  if (get_string_constant(expr, result)) {
    ...
    error() << "expected string constant, but got: " << expr.pretty() << eom;
...
symbolt &
goto_convertt::new_tmp_symbol(const typet &type, const std::string &suffix,
                              goto_programt &dest,
                              const source_locationt &source_location) {
  symbolt &new_symbol =
      get_fresh_aux_symbol(type, tmp_symbol_prefix, "tmp_" + suffix,
                           source_location, irep_idt(), symbol_table);
...
void goto_convertt::make_temp_symbol(exprt &expr, const std::string &suffix,
                                     goto_programt &dest) {
...
void goto_convert(const codet &code, symbol_tablet &symbol_table,
                  goto_programt &dest, message_handlert &message_handler) {
...
void goto_convert(symbol_tablet &symbol_table, goto_programt &dest,
