...
symbol_exprt goto_convertt::make_compound_literal(const exprt &expr,
  ...
  symbolt &new_symbol =
      get_fresh_aux_symbol(expr.type(), tmp_symbol_prefix, "literal",
...
bool goto_convertt::needs_cleaning(const exprt &expr) {
  if (expr.id() == ID_dereference || expr.id() == ID_side_effect ||
      expr.id() == ID_compound_literal || expr.id() == ID_comma)
    return true;
  ...
  forall_operands(it, expr) if (needs_cleaning(*it)) return true;
...
void goto_convertt::clean_expr(exprt &expr, goto_programt &dest,
                               bool result_is_used) {
  // this cleans:
  ...
  if (expr.id() == ID_and || expr.id() == ID_or) {
    ...
    generate_ifthenelse(if_expr.cond(), tmp_true, tmp_false, source_location,
                        dest);
    ...
    if (statement == ID_gcc_conditional_expression) {
      ...
      remove_statement_expression(to_side_effect_expr(expr), dest,
                                  result_is_used);
      return;
      ...
      if (to_side_effect_expr_function_call(expr).function().id() ==
              ID_symbol &&
          to_symbol_expr(to_side_effect_expr_function_call(expr).function())
    ...
    if (tmp.instructions.empty()) {
      error().source_location = expr.find_source_location();
      error() << "no side-effects in quantified expressions allowed" << eom;
      throw 0;
    }
    return;
  ...
  Forall_operands(it, expr) clean_expr(*it, dest);
...
void goto_convertt::clean_expr_address_of(exprt &expr, goto_programt &dest) {
  ...
  if (expr.id() == ID_compound_literal) {
    ...
    Forall_operands(it, expr) clean_expr_address_of(*it, dest);
}
