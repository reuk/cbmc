...
protected:
  ...
  void do_function_calls(goto_functionst &goto_functions,
                         goto_programt &goto_program);

  bool restore_returns(goto_functionst::function_mapt::iterator f_it);

  void undo_function_calls(goto_functionst &goto_functions,
                           goto_programt &goto_program);
};
...
void remove_returnst::do_function_calls(goto_functionst &goto_functions,
                                        goto_programt &goto_program) {
  Forall_goto_program_instructions(i_it, goto_program) {
      ...
      if (old_type.return_type() != empty_typet()) {
        ...
        if (function_call.lhs().is_not_nil()) {
          ...
          if (f_it->second.body_available()) {
            ...
            return_value.set_identifier(id2string(function_id) +
                                        RETURN_VALUE_SUFFIX);
            rhs = return_value;
          } else {
...
bool remove_returnst::restore_returns(
  ...
  symbol_tablet::symbolst::iterator rv_it = symbol_table.symbols.find(rv_name);
...
void remove_returnst::undo_function_calls(goto_functionst &goto_functions,
                                          goto_programt &goto_program) {
...
void remove_returnst::restore(goto_functionst &goto_functions) {
  ...
  Forall_goto_functions(it, goto_functions) unmodified =
      restore_returns(it) && unmodified;
