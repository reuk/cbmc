...
public:
  ...
  */
  ...
  public:
    ...
    instructiont():
      instructiont(NO_INSTRUCTION_TYPE)
    {
    }
...
// It's not expensive to copy a std::list with move semantics, and we probably
// get RVO anyway
template <class codeT, class guardT>
template <typename Target>
std::list<Target> goto_program_templatet<codeT, guardT>::get_successors(
  Target target) const
{
  // This method is written in this way to restrict mutable state to the
