...
protected:
  ...
  // strings
  void do_sprintf(goto_programt &dest, goto_programt::targett it,
                  code_function_callt &call);
  void do_snprintf(goto_programt &dest, goto_programt::targett it,
                   code_function_callt &call);
  void do_strcat(goto_programt &dest, goto_programt::targett it,
                 code_function_callt &call);
  void do_strncmp(goto_programt &dest, goto_programt::targett it,
                  code_function_callt &call);
  void do_strchr(goto_programt &dest, goto_programt::targett it,
                 code_function_callt &call);
  void do_strrchr(goto_programt &dest, goto_programt::targett it,
                  code_function_callt &call);
  void do_strstr(goto_programt &dest, goto_programt::targett it,
                 code_function_callt &call);
  void do_strtok(goto_programt &dest, goto_programt::targett it,
                 code_function_callt &call);
  void do_strerror(goto_programt &dest, goto_programt::targett it,
                   code_function_callt &call);
  void do_fscanf(goto_programt &dest, goto_programt::targett it,
                 code_function_callt &call);

  void do_format_string_read(goto_programt &dest,
  ...
  void do_format_string_write(goto_programt &dest,
                              goto_programt::const_targett target,
                              const code_function_callt::argumentst &arguments,
                              unsigned format_string_inx,
                              unsigned argument_start_inx,
  ...
  bool is_string_type(const typet &t) const {
    return (t.id() == ID_pointer || t.id() == ID_array) &&
           (t.subtype().id() == ID_signedbv ||
            t.subtype().id() == ID_unsignedbv) &&
           (to_bitvector_type(t.subtype()).get_width() ==
            config.ansi_c.char_width);
  }

  void invalidate_buffer(goto_programt &dest,
                         goto_programt::const_targett target,
                         const exprt &buffer, const typet &buf_type,
...
void string_instrumentationt::operator()(goto_programt &dest) {
  Forall_goto_program_instructions(it, dest) instrument(dest, it);
}
...
void string_instrumentationt::do_snprintf(goto_programt &dest,
  ...
  if (arguments.size() < 3) {
    error().source_location = target->source_location;
    error() << "snprintf expected to have three or more arguments" << eom;
    throw 0;
  }
...
void string_instrumentationt::do_format_string_read(
    goto_programt &dest, goto_programt::const_targett target,
    const code_function_callt::argumentst &arguments,
    unsigned format_string_inx, unsigned argument_start_inx,
    const std::string &function_name) {
  const exprt &format_arg = arguments[format_string_inx];

  if (format_arg.id() == ID_address_of && format_arg.op0().id() == ID_index &&
    ...
    for (const auto &token : token_list) {
      ...
      if (token.type != format_tokent::TEXT &&
          token.type != format_tokent::UNKNOWN)
        args++;

      if (find(token.flags.begin(), token.flags.end(),
               format_tokent::ASTERISK) != token.flags.end())
  ...
  } else // non-const format string
  {
    ...
    for (unsigned i = 2; i < arguments.size(); i++) {
      ...
      if (arguments[i].id() != ID_string_constant && is_string_type(arg_type)) {
        goto_programt::targett assertion = dest.add_instruction();
        assertion->source_location = target->source_location;
        assertion->source_location.set_property_class("string");
        std::string comment("zero-termination of string argument of ");
...
void string_instrumentationt::do_strerror(goto_programt &dest,
...

  {
    ...
    assumption1->make_assumption(
        binary_relation_exprt(symbol_size.symbol_expr(), ID_notequal,
  ...
  index.copy_to_operands(symbol_buf.symbol_expr(),
...
void string_instrumentationt::invalidate_buffer(
    goto_programt &dest, goto_programt::const_targett target,
    const exprt &buffer, const typet &buf_type, const mp_integer &limit) {
