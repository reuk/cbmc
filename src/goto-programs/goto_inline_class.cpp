    ...
    else
    {
      ...
      if(!base_type_eq(par_type, actual.type(), ns))
      {
           ...
           f_partype.subtype() == f_acttype.subtype()))
        {
          ...
          (f_partype.id() == ID_signedbv || f_partype.id() == ID_unsignedbv ||
           f_partype.id() == ID_bool) &&
          (f_acttype.id() == ID_signedbv || f_acttype.id() == ID_unsignedbv ||
  ...
  const exprt &constrain)
{
  ...
  parameter_assignments(
    target, identifier, goto_function.type, arguments, tmp1);
  ...
  if(goto_function.body_available())
  {
    if(transitive)
    {
      ...
      insert_function_body(
        cached, dest, target, lhs, function, arguments, constrain);
    ...
    else
    {
      ...
      insert_function_body(
        goto_function, dest, target, lhs, function, arguments, constrain);
    }
  }
  ...
  const bool force_full)
{
      ...
      c_it++)
  {
    ...
    expand_function_call(
      goto_program, new_inline_map, transitive, force_full, call.first);
  }
...
jsont goto_inlinet::goto_inline_logt::output_inline_log_json() const
{
  ...
  for(log_mapt::const_iterator it= log_map.begin(); it != log_map.end(); it++)
  {
