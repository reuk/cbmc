...
void goto_inlinet::parameter_assignments(
  ...
  for (code_typet::parameterst::const_iterator it2 = parameter_types.begin();
       it2 != parameter_types.end(); it2++) {
    ...
    if (actual.is_nil()) {
      ...
      if (!base_type_eq(par_type, actual.type(), ns)) {
        ...
        if ((f_partype.id() == ID_pointer && f_acttype.id() == ID_pointer) ||
          ...
          error() << "function call: argument `" << identifier
                  ...
                  << actual.type().pretty() << "', parameter is `"
                  << from_type(ns, identifier, par_type) << "'" << eom;
          throw 0;
        }
      }
...
void replace_location(exprt &dest, const source_locationt &new_location) {
  Forall_operands(it, dest) replace_location(*it, new_location);
...
void goto_inlinet::insert_function_body(const goto_functiont &goto_function,
                                        goto_programt &dest,
  ...
  if (adjust_function)
    Forall_goto_program_instructions(i_it, body) i_it->function =
        target->function;

  replace_return(body, lhs, constrain);

  goto_programt tmp1;
  parameter_assignments(target, identifier, goto_function.type, arguments,
                        tmp1);
  ...
  inline_log.add_segment(tmp, begin_location_number, end_location_number,
                         call_location_number, identifier);
...
void goto_inlinet::insert_function_nobody(goto_programt &dest, const exprt &lhs,
                                          goto_programt::targett target,
                                          const symbol_exprt &function,
                                          const exprt::operandst &arguments) {
  assert(is_call(target));
  ...
  if (goto_function.body_available()) {
    if (transitive) {
      const goto_functiont &cached =
          goto_inline_transitive(identifier, goto_function, force_full);
...
void goto_inlinet::get_call(goto_programt::const_targett it, exprt &lhs,
                            exprt &function, exprt::operandst &arguments,
                            exprt &constrain) {
  assert(is_call(it));
...
void goto_inlinet::goto_inline(const irep_idt identifier,
  ...
  goto_inline_nontransitive(identifier, goto_function, inline_map, force_full);
}
...
void goto_inlinet::goto_inline_nontransitive(const irep_idt identifier,
  ...
  for (call_listt::const_iterator c_it = call_list.begin();
    ...
    const inline_mapt &new_inline_map = transitive ? inline_mapt() : inline_map;

    expand_function_call(goto_program, new_inline_map, transitive, force_full,
                         call.first);
  }
...
const goto_inlinet::goto_functiont &
goto_inlinet::goto_inline_transitive(const irep_idt identifier,
  ...
  for (goto_programt::targetst::iterator c_it = call_list.begin();
       c_it != call_list.end(); c_it++) {
    expand_function_call(goto_program, inline_mapt(), true, force_full, *c_it);
  }
...
bool goto_inlinet::is_ignored(const irep_idt id) const {
  return id == "__CPROVER_cleanup" || id == "__CPROVER_set_must" ||
         id == "__CPROVER_set_may" || id == "__CPROVER_clear_must" ||
         id == "__CPROVER_clear_may" || id == "__CPROVER_cover";
}
...
void goto_inlinet::output_inline_map(std::ostream &out,
  ...
  for (inline_mapt::const_iterator it = inline_map.begin();
    ...
    if (f_it != goto_functions.function_map.end() && !call_list.empty()) {
...
void goto_inlinet::goto_inline_logt::cleanup(
  ...
  forall_goto_program_instructions(it, goto_program) log_map.erase(it);
}
...
void goto_inlinet::goto_inline_logt::cleanup(
  ...
  for (goto_functionst::function_mapt::const_iterator it = function_map.begin();
...
void goto_inlinet::goto_inline_logt::add_segment(
    const goto_programt &goto_program, const unsigned begin_location_number,
    const unsigned end_location_number, const unsigned call_location_number,
...
jsont goto_inlinet::goto_inline_logt::output_inline_log_json() const {
  ...
  for (log_mapt::const_iterator it = log_map.begin(); it != log_map.end();
       it++) {
