...
bool string_abstractiont::build_wrap(const exprt &object, exprt &dest,
                                     bool write) {
  // debugging
  ...
  if (!type_eq(dest.type(), a_t, ns) &&
      !(dest.type().id() == ID_array && a_t.id() == ID_pointer &&
        type_eq(dest.type().subtype(), a_t.subtype(), ns))) {
    warning() << "warning: inconsistent abstract type for " << object.pretty()
              << eom;
    return true;
  }
...
bool string_abstractiont::is_ptr_string_struct(const typet &type) const {
  return type.id() == ID_pointer && type_eq(type.subtype(), string_struct, ns);
}
...
string_abstractiont::string_abstractiont(symbol_tablet &_symbol_table,
    ...
    : messaget(_message_handler), arg_suffix("#strarg"), sym_suffix("#str$fcn"),
      symbol_table(_symbol_table), ns(_symbol_table), temporary_counter(0) {
  struct_typet s;
...
void string_abstractiont::operator()(goto_functionst &dest) {
  ...
  goto_functionst::function_mapt::iterator m_it =
      dest.function_map.find(dest.entry_point());
...
void string_abstractiont::add_str_arguments(
    const irep_idt &name, goto_functionst::goto_functiont &fct) {
  ...
  code_typet::parameterst &parameters = to_code_type(fct.type).parameters();
  ...
  symb_parameters.insert(symb_parameters.end(), str_args.begin(),
                         str_args.end());
}
...
void string_abstractiont::add_argument(code_typet::parameterst &str_args,
  ...
  typet final_type = is_ptr_argument(type) ? type : pointer_typet(type);
...
void string_abstractiont::abstract(goto_programt &dest) {
  ...
  Forall_goto_program_instructions(it, dest) it = abstract(dest, it);
...
void string_abstractiont::declare_define_locals(goto_programt &dest) {
  ...
  Forall_goto_program_instructions(it, dest) if (it->is_decl())
      // same name may exist several times due to inlining, make sure the first
      // declaration is used
      available_decls.insert(
          std::make_pair(to_code_decl(it->code).get_identifier(), it));
...
exprt string_abstractiont::make_val_or_dummy_rec(
    goto_programt &dest, goto_programt::targett ref_instr,
  ...
  if (eff_type.id() == ID_array || eff_type.id() == ID_pointer) {
    const typet &source_subt = is_ptr_string_struct(eff_type)
                                   ? source_type
                                   : ns.follow(source_type.subtype());
    symbol_exprt sym_expr = add_dummy_symbol_and_value(
        dest, ref_instr, symbol, irep_idt(), eff_type.subtype(), source_subt);
    ...
    if (eff_type.id() == ID_array)
      ...
      return address_of_exprt(sym_expr);
             ...
             (eff_type.id() == ID_struct &&
              !type_eq(eff_type, string_struct, ns))) {
    ...
    for (struct_union_typet::componentst::const_iterator it =
             s_components.begin();
         it != s_components.end() && it2 != components.end(); ++it) {
      if (it->get_name() != it2->get_name())
        continue;

      const typet &eff_sub_type = ns.follow(it2->type());
      if (eff_sub_type.id() == ID_pointer || eff_sub_type.id() == ID_array ||
          eff_sub_type.id() == ID_struct || eff_sub_type.id() == ID_union) {
        symbol_exprt sym_expr =
            add_dummy_symbol_and_value(dest, ref_instr, symbol, it2->get_name(),
...
symbol_exprt string_abstractiont::add_dummy_symbol_and_value(
    goto_programt &dest, goto_programt::targett ref_instr,
    const symbolt &symbol, const irep_idt &component_name, const typet &type,
  ...
  new_symbol.pretty_name =
      id2string(symbol.pretty_name.empty() ? symbol.base_name
                                           : symbol.pretty_name) +
      suffix;
...
string_abstractiont::abstract(goto_programt &dest, goto_programt::targett it) {
  switch (it->type) {
  case ASSIGN:
...
goto_programt::targett
string_abstractiont::abstract_assign(goto_programt &dest,
...
void string_abstractiont::abstract_function_call(
    goto_programt &dest, goto_programt::targett target) {
  ...
  for (code_typet::parameterst::const_iterator it2 = formal_params.begin();
    ...
    if (str_args.back().type().id() == ID_array &&
      ...
      assert(type_eq(str_args.back().type().subtype(), abstract_type.subtype(),
                     ns));
...
bool string_abstractiont::has_string_macros(const exprt &expr) {
  if (expr.id() == "is_zero_string" || expr.id() == "zero_string_length" ||
      expr.id() == "buffer_size")
    return true;

  forall_operands(it, expr) if (has_string_macros(*it)) return true;
...
void string_abstractiont::replace_string_macros(
    exprt &expr, bool lhs, const source_locationt &source_location) {
  if (expr.id() == "is_zero_string") {
    ...
    Forall_operands(it, expr) replace_string_macros(*it, lhs, source_location);
}
...
exprt string_abstractiont::build(const exprt &pointer, whatt what, bool write,
  ...
  if (what == LENGTH || what == SIZE) {
    ...
    if (pointer_offset.is_not_nil() && !pointer_offset.is_zero())
...
const typet &string_abstractiont::build_abstraction_type(const typet &type) {
  ...
  return abstraction_types_map
      .insert(std::make_pair(eff_type, map_entry->second))
      .first->second;
}
...
const typet &string_abstractiont::build_abstraction_type_rec(
    const typet &type, const abstraction_types_mapt &known) {
  const typet &eff_type = ns.follow(type);
  ...
  ::std::pair<abstraction_types_mapt::iterator, bool> map_entry(
      abstraction_types_map.insert(::std::make_pair(eff_type, nil_typet())));
  if (!map_entry.second)
    return map_entry.first->second;

  if (eff_type.id() == ID_array || eff_type.id() == ID_pointer) {
    // char* or void* or char[]
    if (is_char_type(eff_type.subtype()) || eff_type.subtype().id() == ID_empty)
      ...
      if (!subt.is_nil()) {
        if (eff_type.id() == ID_array)
          ...
          map_entry.first->second = pointer_typet(subt);
      }
    }
...
bool string_abstractiont::build_if(const if_exprt &o_if, exprt &dest,
                                   bool write) {
...
bool string_abstractiont::build_array(const array_exprt &object, exprt &dest,
                                      bool write) {
...
bool string_abstractiont::build_pointer(const exprt &object, exprt &dest,
                                        bool write) {
...
bool string_abstractiont::build_symbol_constant(const mp_integer &zero_length,
                                                ...
                                                exprt &dest) {
  irep_idt base = "$string_constant_str_" + integer2string(zero_length) + "_" +
                  integer2string(buf_size);
...
goto_programt::targett
string_abstractiont::abstract_pointer_assign(goto_programt &dest,
                                             goto_programt::targett target) {
  code_assignt &assign = to_code_assign(target->code);
...
goto_programt::targett
string_abstractiont::abstract_char_assign(goto_programt &dest,
                                          goto_programt::targett target) {
  code_assignt &assign = to_code_assign(target->code);
...
goto_programt::targett string_abstractiont::char_assign(
    goto_programt &dest, goto_programt::targett target, const exprt &new_lhs,
  ...
  move_lhs_arithmetic(assignment2->code.op0(), assignment2->code.op1());
...
goto_programt::targett
string_abstractiont::value_assignments(goto_programt &dest,
                                       goto_programt::targett target,
                                       const exprt &lhs, const exprt &rhs) {
  if (rhs.id() == ID_if)
  ...
  if (lhs.type().id() == ID_array) {
    ...
    for (mp_integer i = 0; i < size; ++i)
      target = value_assignments(
          dest, target, index_exprt(lhs, from_integer(i, a_size.type())),
    ...
    for (const auto &comp : struct_union_type.components()) {
      ...
      target = value_assignments(
          dest, target, member_exprt(lhs, comp.get_name(), comp.type()),
...
goto_programt::targett string_abstractiont::value_assignments_if(
    goto_programt &dest, goto_programt::targett target, const exprt &lhs,
    const if_exprt &rhs) {
  goto_programt tmp;
...
goto_programt::targett string_abstractiont::value_assignments_string_struct(
...

  {
    ...
    assignment->code = code_assignt(member(lhs, IS_ZERO), member(rhs, IS_ZERO));
    assignment->code.add_source_location() = target->source_location;
    assignment->function = target->function;
    assignment->source_location = target->source_location;
  }
  ...
  {
    goto_programt::targett assignment = tmp.add_instruction(ASSIGN);
    assignment->code = code_assignt(member(lhs, LENGTH), member(rhs, LENGTH));
    assignment->code.add_source_location() = target->source_location;
    assignment->function = target->function;
    assignment->source_location = target->source_location;
  }
  ...
  {
    goto_programt::targett assignment = tmp.add_instruction(ASSIGN);
    assignment->code = code_assignt(member(lhs, SIZE), member(rhs, SIZE));
    assignment->code.add_source_location() = target->source_location;
    assignment->function = target->function;
    assignment->source_location = target->source_location;
  }
...
exprt string_abstractiont::member(const exprt &a, whatt what) {
  ...
  result.struct_op() =
      a.type().id() == ID_pointer ? dereference_exprt(a, string_struct) : a;

  switch (what) {
  case IS_ZERO:
    result.set_component_name("is_zero");
    break;
  case SIZE:
    result.set_component_name("size");
    break;
  case LENGTH:
    result.set_component_name("length");
    break;
  }
