  ...
  const typet &source_type)
{
    ...
    (eff_type.id() == ID_struct && !type_eq(eff_type, string_struct, ns)))
  {
    ...
    const struct_union_typet::componentst &s_components= su_source.components();
        ...
        ++it)
    {
      ...
      if(
        eff_sub_type.id() == ID_pointer || eff_sub_type.id() == ID_array ||
  ...
  const typet &source_type)
{
  ...
  new_symbol.pretty_name=
    id2string(
      symbol.pretty_name.empty() ? symbol.base_name : symbol.pretty_name) +
    suffix;
  ...
  goto_programt::targett target)
{
      ...
      it2++, it1++)
  {
    ...
    if(
      str_args.back().type().id() == ID_array &&
...
{
  if(
    expr.id() == "is_zero_string" || expr.id() == "zero_string_length" ||
    expr.id() == "buffer_size")
    return true;
...
bool string_abstractiont::build(const exprt &object, exprt &dest, bool write)
{
  ...
  if(object.id() == ID_typecast)
  {
    ...
    return !(
      type_eq(dest.type(), abstract_type, ns) ||
...
bool string_abstractiont::build_if(
  const if_exprt &o_if,
  exprt &dest,
  bool write)
{
...
bool string_abstractiont::build_array(
  const array_exprt &object,
  exprt &dest,
  bool write)
{
...
bool string_abstractiont::build_pointer(
  const exprt &object,
  exprt &dest,
  bool write)
{
  ...
  if(ptr.pointer.id() == ID_address_of)
  {
    ...
    ptr.pointer.id() == ID_symbol && is_char_type(object.type().subtype()))
...
bool string_abstractiont::build_symbol(const symbol_exprt &sym, exprt &dest)
{
  ...
  if(
    current_args.find(symbol.name) != current_args.end() &&
  ...
  goto_programt::targett target)
{
  ...
  bool unknown=
    (abstract_type != build_abstraction_type(rhsp->type()) ||
  ...
  goto_programt::targett target)
{
  ...
  if(lhs.id() == ID_index)
  {
    ...
    if(!build_wrap(i_lhs.array(), new_lhs, true))
    {
      ...
      if_exprt min_expr(
        binary_relation_exprt(new_length, ID_lt, i2), new_length, i2);
  ...
  else if(lhs.id() == ID_dereference)
  {
    ...
    if(!build_wrap(ptr.pointer, new_lhs, true))
    {
      ...
      return char_assign(
        ...
        new_lhs,
        i2,
        ptr.offset.is_nil() ? from_integer(0, build_type(LENGTH)) : ptr.offset);
    }
  }
