...
void compute_address_taken_functions(const exprt &src,
  ...
  forall_operands(it, src) compute_address_taken_functions(*it, address_taken);

  if (src.id() == ID_address_of && src.type().id() == ID_pointer &&
...
void compute_functions(const exprt &src, std::set<irep_idt> &address_taken) {
  forall_operands(it, src) compute_functions(*it, address_taken);

  if (src.type().id() == ID_code && src.id() == ID_symbol)
...
void compute_called_functions(const goto_functionst &goto_functions,
  ...
  while (!working_queue.empty()) {
    ...
    forall_goto_program_instructions(i_it, program) {
      if (i_it->is_function_call()) {
        compute_functions(to_code_function_call(i_it->code).function(),
                          working_queue);
      }
    }
  }
