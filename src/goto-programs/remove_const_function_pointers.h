...
public:
  ...
  remove_const_function_pointerst(message_handlert &message_handler,
...
private:
  ...
  bool try_resolve_function_calls(const expressionst &exprs,
                                  functionst &out_functions);

  bool try_resolve_index_of_function_call(const index_exprt &index_expr,
                                          functionst &out_functions);

  bool try_resolve_member_function_call(const member_exprt &member_expr,
                                        functionst &out_functions);

  bool
  try_resolve_address_of_function_call(const address_of_exprt &address_expr,
                                       functionst &out_functions);

  bool
  try_resolve_dereference_function_call(const dereference_exprt &deref_expr,
                                        functionst &out_functions);

  bool try_resolve_typecast_function_call(const typecast_exprt &typecast_expr,
                                          functionst &out_functions);

  // recursive functions for dealing with the auxiliary elements
  bool try_resolve_expression(const exprt &expr,
                              expressionst &out_resolved_expression,
                              bool &out_is_const);

  bool try_resolve_index_of(const index_exprt &index_expr,
                            expressionst &out_expressions, bool &out_is_const);

  bool try_resolve_member(const member_exprt &member_expr,
                          expressionst &out_expressions, bool &out_is_const);

  bool try_resolve_dereference(const dereference_exprt &deref_expr,
                               expressionst &out_expressions,
                               bool &out_is_const);

  bool try_resolve_typecast(const typecast_exprt &typecast_expr,
                            expressionst &out_expressions, bool &out_is_const);

  bool is_const_expression(const exprt &expression) const;
  bool is_const_type(const typet &type) const;

  bool try_resolve_index_value(const exprt &index_value_expr,
                               mp_integer &out_array_index);

  exprt get_component_value(const struct_exprt &struct_expr,
