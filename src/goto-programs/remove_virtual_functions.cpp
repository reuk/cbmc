...
class remove_virtual_functionst {
public:
  remove_virtual_functionst(const symbol_tablet &_symbol_table,
...
protected:
  ...
  void remove_virtual_function(goto_programt &goto_program,
                               goto_programt::targett target);

  class functiont {
  public:
    functiont() {}
    explicit functiont(const irep_idt &_class_id) : class_id(_class_id) {}
  ...
  exprt get_method(const irep_idt &class_id,
...
remove_virtual_functionst::remove_virtual_functionst(
    const symbol_tablet &_symbol_table, const goto_functionst &goto_functions)
...
void remove_virtual_functionst::remove_virtual_function(
  ...
  for (auto it = functions.crbegin(), itend = functions.crend(); it != itend;
       ++it) {
    const auto &fun = *it;
...
void remove_virtual_functionst::get_child_functions_rec(
    const irep_idt &this_id, const symbol_exprt &last_method_defn,
    const irep_idt &component_name, functionst &functions,
    std::set<irep_idt> &visited) const {
  ...
  for (const auto &child : findit->second.children) {
    ...
    get_child_functions_rec(child, function.symbol_expr, component_name,
                            functions, visited);
  }
}
...
void remove_virtual_functionst::get_functions(const exprt &function,
  ...
  get_child_functions_rec(class_id, root_function.symbol_expr, component_name,
                          functions, visited);
...
exprt remove_virtual_functionst::get_method(
    const irep_idt &class_id, const irep_idt &component_name) const {
  irep_idt id = id2string(class_id) + "." + id2string(component_name);
...
void remove_virtual_functions(const symbol_tablet &symbol_table,
  ...
  remove_virtual_functionst rvf(symbol_table, goto_functions);
...
void remove_virtual_functions(goto_modelt &goto_model) {
  remove_virtual_functions(goto_model.symbol_table, goto_model.goto_functions);
}
