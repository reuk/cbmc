...
goto_convert_functionst::goto_convert_functionst(
    symbol_tablet &_symbol_table, goto_functionst &_functions,
    message_handlert &_message_handler)
    : goto_convertt(_symbol_table, _message_handler), functions(_functions) {}
...
void goto_convert_functionst::goto_convert() {
  ...
  forall_symbols(it, symbol_table.symbols) {
    if (!it->second.is_type && !it->second.is_macro &&
        it->second.type.id() == ID_code &&
        (it->second.mode == ID_C || it->second.mode == ID_cpp ||
         it->second.mode == ID_java || it->second.mode == "jsil"))
...
void goto_convert_functionst::add_return(
  ...
  if (!f.body.instructions.empty()) {
    ...
    while (true) {
      ...
      if (last_instruction->is_goto() && last_instruction->guard.is_true())
        return;
...
void goto_convert_functionst::convert_function(const irep_idt &identifier) {
  ...
  if (symbol.value.is_nil() ||
      symbol.value.id() ==
          "compiled") /* goto_inline may have removed the body */
    return;

  if (symbol.value.id() != ID_code) {
    error().source_location = symbol.value.find_source_location();
    error() << "got invalid code for function `" << identifier << "'" << eom;
    throw 0;
  }
  ...
  // do function tags
  Forall_goto_program_instructions(i_it, f.body) i_it->function = identifier;
...
void goto_convert(symbol_tablet &symbol_table, goto_modelt &goto_model,
...
void goto_convert(symbol_tablet &symbol_table, goto_functionst &functions,
                  message_handlert &message_handler) {
  goto_convert_functionst goto_convert_functions(symbol_table, functions,
                                                 message_handler);
