...
void graphml_witnesst::remove_l0_l1(exprt &expr) {
  ...
  Forall_operands(it, expr) remove_l0_l1(*it);
}
...
std::string graphml_witnesst::convert_assign_rec(const irep_idt &identifier,
  ...
  if (assign.rhs().id() == ID_array) {
    ...
    for (const auto &comp : components) {
      if (comp.type().id() == ID_code || comp.get_is_padding() ||
...
void graphml_witnesst::operator()(const symex_target_equationt &equation) {
       ...
       it++, step_nr++) // we cannot replace this by a ranged for
  {
    ...
    ++next;
    if (next != equation.SSA_steps.end() && next->is_assignment() &&
       ...
       it != equation.SSA_steps.end();) // no ++it
  {
    ...
    case goto_trace_stept::GOTO: {
      ...
      if ((it->is_assignment() || it->is_decl()) && it->ssa_rhs.is_not_nil() &&
