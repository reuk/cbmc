  ...
  const code_assignt &assign)
{
  ...
  else if(assign.rhs().id() == ID_struct || assign.rhs().id() == ID_union)
  {
      ...
      !base_type_eq(assign.lhs().type(), assign.rhs().type(), ns))
    {
      ...
      assign.lhs().id() == ID_byte_extract_little_endian ||
    ...
    exprt::operandst::const_iterator it= assign.rhs().operands().begin();
    for(const auto &comp : components)
    {
      ...
      member_exprt member(assign.lhs(), comp.get_name(), it->type());
      if(!result.empty())
        result+= ' ';
  ...
  goto_tracet::stepst::const_iterator &it)
{
  ...
  if(
    source_location.is_nil() || source_location.get_file().empty() ||
    source_location.is_built_in() || source_location.get_line().empty())
    return true;
...
void graphml_witnesst::operator()(const goto_tracet &goto_trace)
{
      ...
      it != goto_trace.steps.end();) // no ++it
  {
    ...
    goto_tracet::stepst::const_iterator next= it;
    for(++next; next != goto_trace.steps.end() &&
    ...
    const std::size_t to=
      next == goto_trace.steps.end() ? sink : step_to_node[next->step_nr];
    ...
    case goto_trace_stept::GOTO:
    {
      ...
      if(
        it->type == goto_trace_stept::ASSIGNMENT &&
        it->lhs_object_value.is_not_nil() && it->full_lhs.is_not_nil())
      {
        if(
          !it->lhs_object_value.is_constant() ||
          !it->lhs_object_value.has_operands() ||
          !has_prefix(
            id2string(it->lhs_object_value.op0().get(ID_value)), "INVALID-"))
        {
      ...
      else if(it->type == goto_trace_stept::GOTO && it->pc->is_goto())
      {
        xmlt &val= edge.new_element("data");
        val.set_attribute("key", "sourcecode");
        const std::string cond= from_expr(ns, "", it->cond_expr);
        const std::string neg_cond= from_expr(ns, "", not_exprt(it->cond_expr));
...
void graphml_witnesst::operator()(const symex_target_equationt &equation)
{
      ...
      it++, step_nr++) // we cannot replace this by a ranged for
  {
    ...
    const graphmlt::node_indext node= graphml.add_node();
    graphml[node].node_name= std::to_string(it->source.pc->location_number) +
      ...
      it != equation.SSA_steps.end();) // no ++it
  {
    ...
    for(++next, ++next_step_nr; next != equation.SSA_steps.end() &&
    ...
    const std::size_t to=
      next == equation.SSA_steps.end() ? sink : step_to_node[next_step_nr];
    ...
    case goto_trace_stept::GOTO:
    {
      ...
      else if(it->is_goto() && it->source.pc->is_goto())
      {
        xmlt &val= edge.new_element("data");
        val.set_attribute("key", "sourcecode");
        const std::string cond= from_expr(ns, "", it->cond_expr);
