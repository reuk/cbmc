...
void goto_convertt::do_prob_uniform(const exprt &lhs, const exprt &function,
  ...
  if (to_integer(arguments[0], lb) || to_integer(arguments[1], ub)) {
    error().source_location = function.find_source_location();
...
void goto_convertt::do_prob_coin(const exprt &lhs, const exprt &function,
                                 const exprt::operandst &arguments,
                                 goto_programt &dest) {
  const irep_idt &identifier = function.get(ID_identifier);
  ...
  if (to_integer(arguments[0], num) || to_integer(arguments[1], den)) {
    error().source_location = function.find_source_location();
    error() << "error converting operands" << eom;
    throw 0;
  }
...
void goto_convertt::do_printf(const exprt &lhs, const exprt &function,
                              const exprt::operandst &arguments,
                              goto_programt &dest) {
...
void goto_convertt::do_scanf(const exprt &lhs, const exprt &function,
                             const exprt::operandst &arguments,
                             goto_programt &dest) {
  const irep_idt &f_id = function.get(ID_identifier);
...
void goto_convertt::do_input(const exprt &lhs, const exprt &function,
                             const exprt::operandst &arguments,
                             goto_programt &dest) {
  codet input_code;
...
void goto_convertt::do_output(const exprt &lhs, const exprt &function,
                              const exprt::operandst &arguments,
                              goto_programt &dest) {
  codet output_code;
...
void goto_convertt::do_atomic_begin(const exprt &lhs, const exprt &function,
                                    const exprt::operandst &arguments,
                                    goto_programt &dest) {
  if (lhs.is_not_nil()) {
...
void goto_convertt::do_atomic_end(const exprt &lhs, const exprt &function,
                                  const exprt::operandst &arguments,
                                  goto_programt &dest) {
  if (lhs.is_not_nil()) {
    error().source_location = lhs.find_source_location();
...
void goto_convertt::do_cpp_new(const exprt &lhs, const side_effect_exprt &rhs,
                               goto_programt &dest) {
  if (lhs.is_nil()) {
    error().source_location = lhs.find_source_location();
  ...
  t_n->code = code_assignt(lhs, typecast_exprt(tmp_symbol_expr, lhs.type()));
...
void set_class_identifier(struct_exprt &expr, const namespacet &ns,
...
void goto_convertt::do_java_new(const exprt &lhs, const side_effect_exprt &rhs,
                                goto_programt &dest) {
  if (lhs.is_nil()) {
    error().source_location = lhs.find_source_location();
  ...
  set_class_identifier(to_struct_expr(zero_object), ns,
...
void goto_convertt::do_java_new_array(const exprt &lhs,
  ...
  if (lhs.is_nil()) {
    error().source_location = lhs.find_source_location();
    error() << "do_java_new_array without lhs is yet to be implemented" << eom;
    throw 0;
  }
  ...
  exprt zero_element = zero_initializer(data.type().subtype(), location, ns,
                                        get_message_handler());
  ...
  if (rhs.operands().size() >= 2) {
    ...
    dereference_exprt deref_expr(plus_exprt(data, tmp_i),
                                 data.type().subtype());
    ...
    code_assignt assign_subarray(deref_expr,
...
void goto_convertt::do_array_set(const exprt &lhs, const exprt &function,
                                 const exprt::operandst &arguments,
                                 goto_programt &dest) {
  if (arguments.size() != 2) {
    error().source_location = function.find_source_location();
...
void goto_convertt::do_array_copy(const exprt &lhs, const exprt &function,
                                  const exprt::operandst &arguments,
                                  goto_programt &dest) {
  if (arguments.size() != 2) {
    error().source_location = function.find_source_location();
...
void goto_convertt::do_array_equal(const exprt &lhs, const exprt &function,
                                   const exprt::operandst &arguments,
                                   goto_programt &dest) {
  if (arguments.size() != 2) {
    error().source_location = function.find_source_location();
  ...
  if (arg0_type.id() != ID_pointer || arg1_type.id() != ID_pointer) {
    error().source_location = function.find_source_location();
  ...
  if (lhs.is_not_nil()) {
    ...
    assignment.lhs() = lhs;
    assignment.rhs() =
        binary_exprt(lhs_array, ID_array_equal, rhs_array, lhs.type());
...
void goto_convertt::do_function_call_symbol(const exprt &lhs,
  ...
  if (identifier == CPROVER_PREFIX "assume" ||
    ...
    if (arguments.size() != 1 && arguments.size() != 2) {
      error().source_location = function.find_source_location();
      error() << "`" << identifier << "' expected to have one or two arguments"
              << eom;
      throw 0;
    }
    ...
    t->source_location.set("user-provided", true);
    t->source_location.set_property_class(ID_assertion);
    t->source_location.set_comment("assertion at " +
    ...
    t->source_location.set("user-provided", true);
    t->source_location.set_property_class(ID_assertion);
    t->source_location.set_comment("assertion " +
                                   id2string(from_expr(ns, "", t->guard)));
    ...
    t->guard = arguments[0];
    t->source_location = function.source_location();
    t->source_location.set("user-provided",
  ...
  */
  {
    do_printf(lhs, function, arguments, dest);
  } else if (identifier == "__assert_fail" || identifier == "_assert" ||
