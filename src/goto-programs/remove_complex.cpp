...
static bool have_to_remove_complex(const exprt &expr) {
  ...
  forall_operands(it, expr) if (have_to_remove_complex(*it)) return true;
...
static bool have_to_remove_complex(const typet &type) {
  if (type.id() == ID_struct || type.id() == ID_union) {
    ...
    for (struct_union_typet::componentst::const_iterator it =
         ...
         it != struct_union_type.components().end(); it++)
      if (have_to_remove_complex(it->type()))
        return true;
  } else if (type.id() == ID_pointer || type.id() == ID_vector ||
             type.id() == ID_array)
...
static void remove_complex(exprt &expr) {
  ...
  if (expr.id() == ID_typecast) {
    ...
    if (expr.op0().type().id() == ID_complex) {
      if (expr.type().id() == ID_complex) {
        ...
        unary_exprt tmp(ID_complex_real, expr.op0(),
                        expr.op0().type().subtype());
  ...
  Forall_operands(it, expr) remove_complex(*it);
...
void remove_complex(symbol_tablet &symbol_table) {
  Forall_symbols(it, symbol_table.symbols) remove_complex(it->second);
}
...
static void remove_complex(goto_functionst &goto_functions) {
  Forall_goto_functions(it, goto_functions) remove_complex(it->second);
}
