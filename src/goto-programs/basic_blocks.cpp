...
void basic_blocks(goto_programt &goto_program, unsigned max_block_size) {
  ...
  for (goto_programt::instructionst::iterator it =
           goto_program.instructions.begin();
       it != goto_program.instructions.end();) // intentionally no it++
  {
    // goto's and empty code are left unchanged
    if (it->is_goto() || it->is_dead() || it->is_assert() || it->is_assume() ||
        it->is_atomic_begin() || it->is_atomic_end() || it->is_end_thread() ||
        it->is_start_thread() || it->is_end_function() || it->is_location() ||
        it->is_skip() || it->is_function_call() || it->is_decl())
      it++;
