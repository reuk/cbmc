...
public:
  explicit remove_static_init_loopst(const symbol_tablet &_symbol_table)
      : symbol_table(_symbol_table) {}

  void unwind_enum_static(const goto_functionst &goto_functions,
                          optionst &options);
...
void remove_static_init_loopst::unwind_enum_static(
    const goto_functionst &goto_functions, optionst &options) {
  size_t unwind_max = 0;
    ...
    for (const auto &ins : p.instructions) {
      ...
      if (ins.is_backwards_goto()) {
        ...
        if (!(function_name.mode == ID_java && has_suffix(fname, java_clinit)))
          continue;
        assert(class_prefix_length != std::string::npos &&
        ...
        if (unwinds > 0) {
          ...
          if (set != "")
            newset = ",";
          newset += id2string(ins.function) + "." + std::to_string(loop_id) +
