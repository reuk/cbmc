  ...
  for (const auto &step : goto_trace.steps) {
    ...
    case goto_trace_stept::DECL: {
      ...
      if (step.lhs_object_value.type().is_not_nil())
        type_string = from_type(ns, identifier, step.lhs_object_value.type());
    ...
    case goto_trace_stept::FUNCTION_RETURN: {
      std::string tag = (step.type == goto_trace_stept::FUNCTION_CALL)
                            ? "function_call"
                            : "function_return";
      ...
      xml_function.set_attribute("display_name",
    ...
    default:
      if (source_location != previous_source_location) {
        ...
        if (xml_location.name != "") {
          ...
          xml_location_only.set_attribute("thread",
                                          std::to_string(step.thread_nr));
          xml_location_only.set_attribute("step_nr",
                                          std::to_string(step.step_nr));
