...
protected:
  ...
  void process_instruction(goto_programt::instructiont &instruction,
                           goto_programt &dest);

  void process_function(goto_functionst::goto_functiont &);

  void gcc_asm_function_call(const irep_idt &function_base_name,
                             const codet &code, goto_programt &dest);
};
...
void remove_asmt::process_instruction(goto_programt::instructiont &instruction,
  ...
  if (flavor == ID_gcc) {
    ...
    for (const auto &instruction : assembler_parser.instructions) {
      ...
      if (command == "fstcw" || command == "fnstcw" ||
          command == "fldcw") // x86
      {
        gcc_asm_function_call("__asm_" + id2string(command), code, tmp_dest);
      } else if (command == "mfence" || command == "lfence" ||
                 command == "sfence") // x86
      {
        gcc_asm_function_call("__asm_" + id2string(command), code, tmp_dest);
...
void remove_asmt::operator()() {
  Forall_goto_functions(it, goto_functions) { process_function(it->second); }
}
