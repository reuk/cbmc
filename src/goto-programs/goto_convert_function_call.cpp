...
void goto_convertt::convert_function_call(
    const code_function_callt &function_call, goto_programt &dest) {
  do_function_call(function_call.lhs(), function_call.function(),
                   function_call.arguments(), dest);
}
...
void goto_convertt::do_function_call(const exprt &lhs, const exprt &function,
  ...
  Forall_expr(it, new_arguments) clean_expr(*it, dest);
  ...
  if (new_function.id() == ID_if) {
    ...
    do_function_call_symbol(new_lhs, to_symbol_expr(new_function),
                            new_arguments, dest);
