...
void interpretert::show_state() {
  ...
  if (PC == function->second.body.instructions.end()) {
    std::cout << "End of function `" << function->first << "'" << std::endl;
  } else
    function->second.body.output_instruction(ns, function->first, std::cout,
                                             PC);
...
void interpretert::execute_assign() {
  ...
  if (!rhs.empty()) {
    ...
    if (size != rhs.size())
      std::cout << "!! failed to obtain rhs (" << rhs.size() << " vs. " << size
                << ")" << std::endl;
    else
      assign(address, rhs);
  }
...
void interpretert::assign(mp_integer address,
  ...
  for (unsigned i = 0; i < rhs.size(); i++, ++address) {
    if (address < memory.size()) {
      ...
      std::cout << "** assigning " << cell.identifier << "[" << cell.offset
                << "]:=" << rhs[i] << std::endl;
      cell.value = rhs[i];
    }
  }
...
void interpretert::execute_function_call() {
  ...
  if (function_call.lhs().is_not_nil())
    return_value_address = evaluate_address(function_call.lhs());
  else
