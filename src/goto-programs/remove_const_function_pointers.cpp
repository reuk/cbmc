...
/// To take a function call on a function pointer, and if possible resolve it
/// to a small collection of possible values.
///
...
/// To take a function call on a function pointer, and if possible resolve it
/// to a small collection of possible values. It will resolve function pointers
...
///
/// \returns Returns true if it was able to resolve the call, false if not. If
///   it returns true, out_functions will be populated by all the possible
///   values the function pointer could be.
...
///
/// \returns Returns a modified version of the expression, with all const
///   symbols resolved to their actual values.
...
///
/// \returns The expression value of the symbol.
...
///
/// \returns Returns true if it was able to resolve the expression to some
...
///
/// \returns Returns true if able to resolve each of the expressions down to
...
/// its array and squash its index If we can get a precise number for the
/// index, we try_resolve_function_call on its value otherwise
...
///
/// \returns Returns true if it was able to resolve the index expression to
...
/// \param out_functions: The functions this expression could be
///
/// \returns Returns true if it was able to resolve the member expression to
///   some specific functions. If this is the case, out_functions will contain
///   the possible functions.
...
/// \param out_functions: The functions this expression could be
///
/// \returns Returns true if it was able to resolve the address_of expression
///   to some specific functions. If this is the case, out_functions will
...
/// \param out_functions: The functions this expression could be
///
/// \returns Returns true if it was able to resolve the dereference expression
///   to some specific functions. If this is the case, out_functions will
///   contain the possible functions.
...
/// \param out_functions: The functions this expression could be
///
/// \returns Returns true if it was able to resolve the typecast expression to
///   some specific functions. If this is the case, out_functions will contain
///   the possible functions.
...
///
/// \returns Returns true providing the squashing went OK (note it may not have
...
///
/// \returns Returns true if was able to find a constant value for the index
...
/// To squash an index access by first finding the array it is accessing Then
/// if the index can be resolved, return the squashed value. If the index can't
/// be determined then squash each value in the array and return them all.
///
/// \param index_expr: The index expression to  to resolve
/// \param out_expressions: The expressions this expression could be
/// \param out_is_const: Is the squashed expression constant
///
/// \returns Returns true if it was able to squash the index expression If this
...
/// \param out_is_const: Is the squashed expression constant
///
/// \returns Returns true if it was able to squash the member expression If
///   this is the case, out_expressions will contain the possible values this
///   member could return The out_is_const will return whether the struct is
///   const.
...
/// dereference is dereferencing. Then return the squashed value of the
/// relevant component.
///
...
/// \param out_is_const: Is the squashed expression constant
///
/// \returns Returns true if it was able to squash the dereference expression
///   If this is the case, out_expressions will contain the possible values
///   this dereference could return The out_is_const will return whether the
///   object that gets dereferenced is constant.
...
/// \param out_is_const: Is the squashed expression constant
///
/// \returns Returns true if it was able to squash the typecast expression If
...
///
/// \returns Returns true if the type of the expression is constant.
...
///
/// \returns Returns true if the type has ID_C_constant or is an array since
...
///
/// \returns Returns the value of a specific component for a given struct
///   expression.
