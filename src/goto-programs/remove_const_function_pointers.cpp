  ...
  const exprt &expression) const
{
  if(expression.id() == ID_symbol)
  {
    ...
    {
      const symbolt &symbol= symbol_table.lookup(expression.get(ID_identifier));
  ...
  for(const exprt &value : exprs)
  {
    ...
    if(resolved_value)
    {
      out_functions.insert(
        potential_out_functions.begin(), potential_out_functions.end());
    }
    else
    {
  ...
  functionst &out_functions)
{
  ...
  bool resolved= try_resolve_function_call(typecast_expr.op(), typecast_values);
  ...
  bool &out_is_const)
{
  ...
  {
    const dereference_exprt &deref= to_dereference_expr(simplified_expr);
    resolved= try_resolve_dereference(
      deref, resolved_expressions, is_resolved_expression_const);
  }
  else if(simplified_expr.id() == ID_typecast)
  {
    typecast_exprt typecast_expr= to_typecast_expr(simplified_expr);
    resolved= try_resolve_typecast(
  ...
  bool &out_is_const)
{
  ...
  bool typecast_const;
  bool resolved=
    try_resolve_expression(typecast_expr.op(), typecast_values, typecast_const);

  if(resolved)
  {
    out_expressions.insert(
      out_expressions.end(), typecast_values.begin(), typecast_values.end());
