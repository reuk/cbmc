...
bool read_bin_goto_object_v3(std::istream &in, const std::string &filename,
  ...
  for (std::size_t i = 0; i < count; i++) {
    ...
    for (std::size_t i = 0; i < ins_count; i++) {
      ...
      if (instruction.is_target() &&
          rev_target_map
                  .insert(rev_target_map.end(),
                          std::make_pair(instruction.target_number, itarget))
                  ->second != itarget)
        assert(false);
...
bool read_bin_goto_object(std::istream &in, const std::string &filename,
...

  {
    ...
    case 3:
      return read_bin_goto_object_v3(in, filename, symbol_table, functions,
                                     message_handler, irepconverter);
      break;
