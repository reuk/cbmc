...
public:
  ...
  void set_line_no(unsigned _line_no) { line_no = _line_no; }

  void set_file(const irep_idt &file) {
    source_location.set_file(file);
    source_location.set_working_directory(get_current_working_directory());
  }

  irep_idt get_file() const { return source_location.get_file(); }

  unsigned get_line_no() const { return line_no; }

  unsigned get_column() const { return column; }

  void set_column(unsigned _column) { column = _column; }
  ...
  void advance_column(unsigned token_width) { column += token_width; }
