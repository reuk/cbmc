...
bool simplify_exprt::simplify_boolean(exprt &expr) {
  ...
  if (expr.id() == ID_implies) {
    if (operands.size() != 2 || operands.front().type().id() != ID_bool ||
    ...
    if (expr.id() == ID_and || expr.id() == ID_or) {
      ...
      forall_operands(
          it, expr) if (it->id() == ID_not && it->operands().size() == 1 &&
                        it->type().id() == ID_bool) expr_set.insert(it->op0());

      // now search for a

      if (!expr_set.empty()) {
        forall_operands(it, expr) {
          if (it->id() != ID_not && expr_set.find(*it) != expr_set.end()) {
