...
// this is safe for static objects
  ...
  char operator[](size_t i) const { return as_string()[i]; }

  // the pointer is guaranteed to be stable
  const char *c_str() const { return as_string().c_str(); }

  size_t size() const { return as_string().size(); }
  ...
  unsigned get_no() const { return no; }

  size_t hash() const { return no; }
...
inline size_t hash_string(const dstringt &s) { return s.hash(); }
