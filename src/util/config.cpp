...
void configt::ansi_ct::set_16() { set_LP32(); }
...
void configt::ansi_ct::set_32() { set_ILP32(); }
...
void configt::ansi_ct::set_arch_spec_mips(const irep_idt &subarch) {
  if (subarch == "mipsel" || subarch == "mips" || subarch == "mipsn32el" ||
      subarch == "mipsn32") {
    set_ILP32();
    long_double_width = 8 * 8;
  } else {
  ...
  if (subarch == "mipsel" || subarch == "mipsn32el" || subarch == "mips64el")
    endianness = endiannesst::IS_LITTLE_ENDIAN;
  else
    endianness = endiannesst::IS_BIG_ENDIAN;
...
void configt::set_arch(const irep_idt &arch) {
  ...
  else if (arch == "arm64" || arch == "armel" || arch == "armhf" ||
           arch == "arm")
    ansi_c.set_arch_spec_arm(arch);
  else if (arch == "mips64el" || arch == "mipsn32el" || arch == "mipsel" ||
           arch == "mips64" || arch == "mipsn32" || arch == "mips")
...
bool configt::set(const cmdlinet &cmdline) {
  ...
  if (cmdline.isset("i386-linux")) {
    os = "linux";
    arch = "i386";
  } else if (cmdline.isset("i386-win32") || cmdline.isset("win32")) {
    os = "windows";
    arch = "i386";
...
static irep_idt string_from_ns(const namespacet &ns, const std::string &what) {
  ...
  if (tmp.id() != ID_address_of || tmp.operands().size() != 1 ||
      tmp.op0().id() != ID_index || tmp.op0().operands().size() != 2 ||
      tmp.op0().op0().id() != ID_string_constant) {
    throw "symbol table configuration entry `" + id2string(id) +
...
static unsigned unsigned_from_ns(const namespacet &ns,
  ...
  if (to_integer(to_constant_expr(tmp), int_value))
    throw "failed to convert symbol table configuration entry `" +
        id2string(id) + "'";
...
void configt::set_from_symbol_table(const symbol_tablet &symbol_table) {
  ...
  if (symbol_table.symbols.find(CPROVER_PREFIX
                                "architecture_"
