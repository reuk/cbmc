...
#ifdef SUB_IS_LIST
static inline bool
named_subt_order(const std::pair<irep_namet, irept> &a, const irep_namet &b)
{
  return a.first < b;
}
...
void irept::nonrecursive_destructor(dt *old_data)
{
  ...
  while(!stack.empty())
  {
    ...
    {
      stack.reserve(
        stack.size() + d->named_sub.size() + d->comments.size() +
        d->sub.size());
      ...
      for(named_subt::iterator it= d->comments.begin(); it != d->comments.end();
          it++)
      {
        stack.push_back(it->second.data);
        it->second.data= &empty_d;
...
bool irept::operator==(const irept &other) const
{
  ...
  if(
    id() != other.id() || get_sub() != other.get_sub() || // recursive call
...
std::size_t irept::hash() const
{
  ...
  forall_irep(it, sub)
    result= hash_combine(result, it->hash());
...
std::size_t irept::full_hash() const
{
  ...
  forall_irep(it, sub)
    result= hash_combine(result, it->full_hash());
