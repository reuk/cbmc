...
#ifdef SUB_IS_LIST
static inline bool named_subt_order(const std::pair<irep_namet, irept> &a,
                                    const irep_namet &b) {
  return a.first < b;
}

static inline irept::named_subt::const_iterator
named_subt_lower_bound(const irept::named_subt &s, const irep_namet &id) {
  return std::lower_bound(s.begin(), s.end(), id, named_subt_order);
}

static inline irept::named_subt::iterator
named_subt_lower_bound(irept::named_subt &s, const irep_namet &id) {
  return std::lower_bound(s.begin(), s.end(), id, named_subt_order);
}
#endif
...
void irept::nonrecursive_destructor(dt *old_data) {
  ...
  while (!stack.empty()) {
    ...
    if (d->ref_count == 0) {
      stack.reserve(stack.size() + d->named_sub.size() + d->comments.size() +
                    d->sub.size());

      for (named_subt::iterator it = d->named_sub.begin();
           it != d->named_sub.end(); it++) {
        stack.push_back(it->second.data);
        it->second.data = &empty_d;
      }

      for (named_subt::iterator it = d->comments.begin();
           it != d->comments.end(); it++) {
        stack.push_back(it->second.data);
        it->second.data = &empty_d;
      }

      for (subt::iterator it = d->sub.begin(); it != d->sub.end(); it++) {
...
bool irept::get_bool(const irep_namet &name) const { return get(name) == ID_1; }
...
bool irept::full_eq(const irept &other) const {
...

  {
    ...
    for (; i1_it != i1_named_sub.end(); i1_it++, i2_it++)
      if (i1_it->first != i2_it->first || !i1_it->second.full_eq(i2_it->second))
        return false;
  }
...
int irept::compare(const irept &i) const {
  ...
  const subt::size_type size = get_sub().size(), i_size = i.get_sub().size();
  if (size < i_size)
    return -1;
  if (size > i_size)
    return 1;

  {
    ...
    for (it1 = get_sub().begin(), it2 = i.get_sub().begin();
         it1 != get_sub().end() && it2 != i.get_sub().end(); it1++, it2++) {
      r = it1->compare(*it2);
      if (r != 0)
        return r;
    }
...
bool irept::operator<(const irept &other) const { return ordering(other); }
...
std::size_t irept::full_hash() const {
  ...
  result =
      hash_finalize(result, named_sub.size() + sub.size() + comments.size());
