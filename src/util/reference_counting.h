...
public:
  ...
  explicit reference_counting(const T &other) : d(NULL) { write() = other; }
  ...
  void swap(reference_counting &other) { std::swap(other.d, d); }
...
public:
  dt *get_d() const { return d; }
};

template <class T> void reference_counting<T>::remove_ref(dt *old_d) {
  if (old_d == NULL)
    return;
...
template <class T> void reference_counting<T>::detatch() {
#ifdef REFERENCE_COUNTING_DEBUG
...
template <class T>
inline bool operator!=(const reference_counting<T> &i1,
