...
std::size_t to_be_merged_irept::hash() const {
  ...
  forall_named_irep(it, named_sub) {
    ...
    result = hash_combine(result,
                          static_cast<const merged_irept &>(it->second).hash());
  }
...
const merged_irept &merged_irepst::merged(const irept &irep) {
  ...
  forall_irep(it, src_sub) dest_sub.push_back(merged(*it)); // recursive call
  ...
  return static_cast<const merged_irept &>(
