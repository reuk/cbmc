...
public:
  ...
  */
  ...
  bool is_static_lifetime() const { return get_bool(ID_C_static_lifetime); }

  void set_static_lifetime() { return set(ID_C_static_lifetime, true); }

  void clear_static_lifetime() { remove(ID_C_static_lifetime); }

  bool is_thread_local() const { return get_bool(ID_C_thread_local); }

  void set_thread_local() { return set(ID_C_thread_local, true); }

  void clear_thread_local() { remove(ID_C_thread_local); }
};
...
public:
  ...
  unary_exprt(const irep_idt &_id, const exprt &_op) : exprt(_id, _op.type()) {
    copy_to_operands(_op);
  }

  unary_exprt(const irep_idt &_id, const typet &_type) : exprt(_id, _type) {
    operands().resize(1);
  }
...
public:
  ...
  explicit abs_exprt(const exprt &_op) : unary_exprt(ID_abs, _op, _op.type()) {}
};
...
public:
  ...
  explicit predicate_exprt(const irep_idt &_id) : exprt(_id, bool_typet()) {}
...
public:
  ...
  unary_predicate_exprt(const irep_idt &_id, const exprt &_op)
      : unary_exprt(_id, _op, bool_typet()) {}
...
public:
  ...
  explicit sign_exprt(const exprt &_op) : unary_predicate_exprt(ID_sign, _op) {}
};
...
public:
  ...
  binary_exprt(const irep_idt &_id, const typet &_type) : exprt(_id, _type) {
    operands().resize(2);
  }
...
public:
  ...
  binary_predicate_exprt(const exprt &_op0, const irep_idt &_id,
                         const exprt &_op1)
      : binary_exprt(_op0, _id, _op1, bool_typet()) {}
};
...
public:
  ...
  binary_relation_exprt(const exprt &_lhs, const irep_idt &_id,
                        const exprt &_rhs)
      : binary_predicate_exprt(_lhs, _id, _rhs) {}

  exprt &lhs() { return op0(); }

  const exprt &lhs() const { return op0(); }

  exprt &rhs() { return op1(); }

  const exprt &rhs() const { return op1(); }
};
...
public:
  ...
  exprt &array() { return op0(); }

  const exprt &array() const { return op0(); }

  exprt &index() { return op1(); }

  const exprt &index() const { return op1(); }
};
...
public:
  ...
  explicit array_of_exprt(const exprt &_what, const array_typet &_type)
      : unary_exprt(ID_array_of, _what, _type) {}

  exprt &what() { return op0(); }

  const exprt &what() const { return op0(); }
};
...
public:
  ...
  explicit array_exprt(const array_typet &_type) : exprt(ID_array, _type) {}
};
...
public:
  ...
  explicit vector_exprt(const vector_typet &_type) : exprt(ID_vector, _type) {}
};
...
public:
  ...
  explicit union_exprt(const typet &_type) : unary_exprt(ID_union, _type) {}

  explicit union_exprt(const irep_idt &_component_name, const exprt &_value,
                       const typet &_type)
      : unary_exprt(ID_union, _value, _type) {
    set_component_name(_component_name);
  }

  irep_idt get_component_name() const { return get(ID_component_name); }
...
public:
  ...
  explicit struct_exprt(const typet &_type) : exprt(ID_struct, _type) {}
};
...
public:
  ...
  explicit complex_exprt(const exprt &_real, const exprt &_imag,
                         const complex_typet &_type)
      : binary_exprt(_real, ID_complex, _imag, _type) {}

  exprt real() { return op0(); }

  const exprt &real() const { return op0(); }

  exprt imag() { return op1(); }

  const exprt &imag() const { return op1(); }
};
...
public:
  ...
  exprt &object() { return op0(); }

  const exprt &object() const { return op0(); }
...
public:
  ...
  exprt &valid() { return op1(); }

  const exprt &valid() const { return op1(); }
};
...
public:
  ...
  floatbv_typecast_exprt(const exprt &op, const exprt &rounding,
                         const typet &_type)
      : binary_exprt(ID_floatbv_typecast, _type) {
  ...
  exprt &rounding_mode() { return op1(); }

  const exprt &rounding_mode() const { return op1(); }
};
...
public:
  ...
  and_exprt(const exprt &op0, const exprt &op1) : exprt(ID_and, bool_typet()) {
...
public:
  ...
  explicit bitnot_exprt(const exprt &op) : unary_exprt(ID_bitnot, op) {}
};
...
public:
  ...
  shift_exprt(const exprt &_src, const irep_idt &_id,
  ...
  exprt &distance() { return op1(); }

  const exprt &distance() const { return op1(); }
};
...
public:
  ...
  exprt &times() { return op0(); }

  const exprt &times() const { return op0(); }

  exprt &op() { return op1(); }

  const exprt &op() const { return op1(); }
};
...
public:
  ...
  extractbit_exprt(const exprt &_src, const exprt &_index)
      : binary_predicate_exprt(_src, ID_extractbit, _index) {}

  extractbit_exprt(const exprt &_src, const std::size_t _index);

  exprt &src() { return op0(); }

  exprt &index() { return op1(); }

  const exprt &src() const { return op0(); }
...
public:
  ...
  extractbits_exprt(const exprt &_src, const exprt &_upper, const exprt &_lower,
                    const typet &_type)
      : exprt(ID_extractbits, _type) {
    copy_to_operands(_src, _lower, _upper);
  }

  extractbits_exprt(const exprt &_src, const std::size_t _upper,
                    const std::size_t _lower, const typet &_type);

  exprt &src() { return op0(); }

  exprt &upper() { return op1(); }

  exprt &lower() { return op2(); }

  const exprt &src() const { return op0(); }

  const exprt &upper() const { return op1(); }

  const exprt &lower() const { return op2(); }
};
...
public:
  ...
  address_of_exprt() : exprt(ID_address_of, pointer_typet()) {
    operands().resize(1);
  }
...
public:
  ...
  not_exprt() : exprt(ID_not, bool_typet()) { operands().resize(1); }
...
public:
  ...
  explicit dereference_exprt(const exprt &op) : exprt(ID_dereference) {
    copy_to_operands(op);
  }
  ...
  dereference_exprt() : exprt(ID_dereference) { operands().resize(1); }

  exprt &pointer() { return op0(); }

  const exprt &pointer() const { return op0(); }
};
...
public:
  ...
  if_exprt() : exprt(ID_if) { operands().resize(3); }

  exprt &cond() { return op0(); }

  const exprt &cond() const { return op0(); }

  exprt &true_case() { return op1(); }

  const exprt &true_case() const { return op1(); }

  exprt &false_case() { return op2(); }

  const exprt &false_case() const { return op2(); }
};
...
public:
  ...
  with_exprt() : exprt(ID_with) { operands().resize(3); }

  exprt &old() { return op0(); }

  const exprt &old() const { return op0(); }

  exprt &where() { return op1(); }

  const exprt &where() const { return op1(); }

  exprt &new_value() { return op2(); }

  const exprt &new_value() const { return op2(); }
};
...
public:
  explicit index_designatort(const exprt &_index) : exprt(ID_index_designator) {
    copy_to_operands(_index);
  }

  const exprt &index() const { return op0(); }

  exprt &index() { return op0(); }
};
...
public:
  ...
  explicit update_exprt(const typet &_type) : exprt(ID_update, _type) {
    operands().resize(3);
  }
  ...
  // 2) index_designator
  // as defined above
  exprt::operandst &designator() { return op1().operands(); }

  const exprt::operandst &designator() const { return op1().operands(); }
...
public:
  ...
  member_exprt() : exprt(ID_member) { operands().resize(1); }
  ...
  const exprt &struct_op() const { return op0(); }

  // will go away, use compound()
  exprt &struct_op() { return op0(); }

  const exprt &compound() const { return op0(); }

  exprt &compound() { return op0(); }
};
...
public:
  ...
  ieee_float_op_exprt(const exprt &_lhs, const irep_idt &_id, const exprt &_rhs,
                      const exprt &_rm)
      : exprt(_id) {
  ...
  exprt &rounding_mode() { return op2(); }

  const exprt &rounding_mode() const { return op2(); }
};
...
public:
  ...
  argumentst &arguments() { return op1().operands(); }

  const argumentst &arguments() const { return op1().operands(); }
};
...
public:
  ...
  explicit concatenation_exprt(const exprt &_op0, const exprt &_op1,
                               const typet &_type)
      : exprt(ID_concatenation, _type) {
    copy_to_operands(_op0, _op1);
  }
...
public:
  ...
  symbol_exprt &symbol() { return static_cast<symbol_exprt &>(op0()); }

  const symbol_exprt &symbol() const {
    return static_cast<const symbol_exprt &>(op0());
  }

  exprt &value() { return op1(); }

  const exprt &value() const { return op1(); }

  exprt &where() { return op2(); }

  const exprt &where() const { return op2(); }
};
