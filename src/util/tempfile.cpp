...
int my_mkstemps(char *template_str, int suffix_len) {
  ...
  char *XXXXXX_pos = template_str + template_length - 6 - suffix_len;
...
std::string get_temporary_file(const std::string &prefix,
  ...
  std::string t_template = std::string(lpTempPathBuffer) + prefix +
...
temporary_filet::~temporary_filet() { unlink(name.c_str()); }
