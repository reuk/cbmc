...
void cout_message_handlert::print(unsigned level, const std::string &message) {
  if (verbosity >= level) {
    std::cout << message << '\n';
...
void cerr_message_handlert::print(unsigned level, const std::string &message) {
  if (verbosity >= level)
    std::cerr << message << '\n' << std::flush;
}
...
void console_message_handlert::print(unsigned level,
  ...
  if (GetConsoleMode(out_handle, &consoleMode)) {
    ...
    WriteConsoleW(out_handle, wide_message.c_str(), (DWORD)wide_message.size(),
                  &number_written, NULL);
