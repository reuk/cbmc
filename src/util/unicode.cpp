...
void utf16_append_code(unsigned int code, bool swap_bytes,
                       std::wstring &result) {
  // we do not treat 0xD800 to 0xDFFF, although
  // they are not valid unicode symbols

  if (code < 0xFFFF) { // code is encoded as one UTF16 character
...
std::wstring utf8_to_utf16(const std::string &in, bool swap_bytes) {
  ...
  while (i < in.size()) {
    ...
    if (c <= 0x7F) {
      ...
      code = c;
    } else if (c <= 0xDF && i < in.size()) { // in other cases, we need to read
                                             // the right number of chars and
                                             // decode
