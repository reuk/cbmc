...
static bool
is_dereference_integer_object(const exprt &expr, mp_integer &address)
{
  if(expr.id() == ID_dereference && expr.operands().size() == 1)
  {
    if(
      expr.op0().id() == ID_typecast && expr.op0().operands().size() == 1 &&
      expr.op0().op0().is_constant() && !to_integer(expr.op0().op0(), address))
      return true;
  ...
  if(expr.id() == ID_index)
  {
    if(expr.operands().size() == 2)
    {
      ...
      if(is_dereference_integer_object(expr.op0(), address))
      {
        ...
        if(!to_integer(expr.op1(), index) && step_size != -1)
        {
...
bool simplify_exprt::simplify_pointer_offset(exprt &expr)
{
  ...
  else if(ptr.id() == ID_typecast) // pointer typecast
  {
    ...
    else if(op_type.id() == ID_signedbv || op_type.id() == ID_unsignedbv)
    {
      ...
      else
      {
        ...
        {
          if(
            tmp.op0().id() == ID_typecast && tmp.op0().operands().size() == 1 &&
            tmp.op0().op0().id() == ID_address_of)
          {
            ...
            tmp.op1().id() == ID_typecast && tmp.op1().operands().size() == 1 &&
  ...
  else if(ptr.id() == ID_plus) // pointer arithmetic
  {
    ...
    mp_integer element_size= pointer_offset_size(pointer_type.subtype(), ns);
  ...
  else if(ptr.id() == ID_constant && ptr.get(ID_value) == ID_NULL)
  {
...
bool simplify_exprt::simplify_inequality_address_of(exprt &expr)
{
  ...
  if(tmp0.id() == ID_typecast)
    tmp0= expr.op0().op0();
  if(tmp0.op0().id() == ID_index && to_index_expr(tmp0.op0()).index().is_zero())
    tmp0= address_of_exprt(to_index_expr(tmp0.op0()).array());
  exprt tmp1= expr.op1();
  if(tmp1.id() == ID_typecast)
    tmp1= expr.op1().op0();
  if(tmp1.op0().id() == ID_index && to_index_expr(tmp1.op0()).index().is_zero())
  ...
  if(tmp0.op0().id() == ID_symbol && tmp1.op0().id() == ID_symbol)
  {
    bool equal= tmp0.op0().get(ID_identifier) == tmp1.op0().get(ID_identifier);
