...
template <class keyT, class valueT, class predT = std::equal_to<keyT>,
...
public:
  ...
  sharing_nodet() : data(empty_data) { _sn_assert(data.use_count() >= 2); }
  ...
  subt &get_sub() { return write().sub; }

  const subt &get_sub() const { return read().sub; }

  containert &get_container() { return write().con; }

  const containert &get_container() const { return read().con; }
  ...
  void remove_leaf(const key_type &k) {
    ...
    for (typename containert::const_iterator it = c.begin(); it != c.end();
         it++) {
  ...
  void clear() { *this = self_type(); }

  bool shares_with(const self_type &other) const { return data == other.data; }

  void swap(self_type &other) { data.swap(other.data); }
