...
member_offset_iterator::member_offset_iterator(const struct_typet &_type,
    ...
    : current({0, 0}), type(_type), ns(_ns), bit_field_bits(0) {}
...
mp_integer member_offset(const struct_typet &type, const irep_idt &member,
                         const namespacet &ns) {
  ...
  for (struct_typet::componentst::const_iterator it = components.begin();
       it != components.end() && offsets->second != -1; ++it, ++offsets) {
...
exprt member_offset_expr(const member_exprt &member_expr,
  ...
  if (type.id() == ID_struct)
    return member_offset_expr(to_struct_type(type),
...
exprt member_offset_expr(const struct_typet &type, const irep_idt &member,
                         const namespacet &ns) {
  const struct_typet::componentst &components = type.components();
...
exprt size_of_expr(const typet &type, const namespacet &ns) {
  if (type.id() == ID_array) {
    ...
    return from_integer(32 / 8, signedbv_typet(config.ansi_c.pointer_width));
  } else
    return nil_exprt();
}
...
mp_integer compute_pointer_offset(const exprt &expr, const namespacet &ns) {
  if (expr.id() == ID_symbol) {
    if (is_ssa_expr(expr))
      return compute_pointer_offset(to_ssa_expr(expr).get_original_expr(), ns);
    else
      return 0;
    ...
    if (o != -1) {
      ...
      return o + member_offset(to_struct_type(type),
                               expr.get(ID_component_name), ns);
    }
...
exprt build_sizeof_expr(const constant_exprt &expr, const namespacet &ns) {
  ...
  if (type_size < 0 || to_integer(expr, val) || val < type_size ||
      (type_size == 0 && val > 0))
    return nil_exprt();
...
bool get_subexpression_at_offset(exprt &result, mp_integer offset,
  ...
  if (source_type.id() == ID_struct) {
    ...
    while (offsets->first < components.size() && offsets->second != -1 &&
      ...
      auto nextit = offsets;
      ++nextit;
      if ((offsets->first + 1) == components.size() ||
          offset < nextit->second) {
        // This field might be, or might contain, the answer.
        result = member_exprt(result, components[offsets->first].get_name(),
                              components[offsets->first].type());
        return get_subexpression_at_offset(result, offset - offsets->second,
                                           target_type, ns);
      }
      ++offsets;
    }
...
bool get_subexpression_at_offset(exprt &result, const exprt &offset,
