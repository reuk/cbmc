...
public:
  ...
  public:
    ...
    const irep_idt &get_name() const { return get(ID_name); }

    void set_name(const irep_idt &name) { return set(ID_name, name); }

    const irep_idt &get_base_name() const { return get(ID_base_name); }

    void set_base_name(const irep_idt &base_name) {
      return set(ID_base_name, base_name);
    }

    const irep_idt &get_access() const { return get(ID_access); }

    void set_access(const irep_idt &access) { return set(ID_access, access); }

    const irep_idt &get_pretty_name() const { return get(ID_pretty_name); }

    void set_pretty_name(const irep_idt &name) {
      return set(ID_pretty_name, name);
    }

    bool get_anonymous() const { return get_bool(ID_anonymous); }

    void set_anonymous(bool anonymous) { return set(ID_anonymous, anonymous); }

    bool get_is_padding() const { return get_bool(ID_C_is_padding); }
...
public:
  class_typet() : struct_typet() { set(ID_C_class, true); }
  ...
  componentst &methods() { return (methodst &)(add(ID_methods).get_sub()); }

  bool is_class() const { return get_bool(ID_C_class); }

  irep_idt default_access() const {
    return is_class() ? ID_private : ID_public;
  }

  const irept::subt &bases() const { return find(ID_bases).get_sub(); }

  irept::subt &bases() { return add(ID_bases).get_sub(); }

  void add_base(const typet &base) { bases().push_back(exprt(ID_base, base)); }
...
inline const tag_typet &to_tag_type(const typet &type) {
  assert(type.id() == ID_c_enum_tag || type.id() == ID_struct_tag ||
...
public:
  ...
  const irept::subt &elements() const { return find(ID_elements).get_sub(); }

  irept::subt &elements() { return add(ID_elements).get_sub(); }
};
...
public:
  ...
  public:
    ...
    const exprt &default_value() const { return find_expr(ID_C_default_value); }

    bool has_default_value() const { return default_value().is_not_nil(); }

    exprt &default_value() { return add_expr(ID_C_default_value); }
    ...
    void set_base_name(const irep_idt &name) { set(ID_C_base_name, name); }

    const irep_idt &get_identifier() const { return get(ID_C_identifier); }

    const irep_idt &get_base_name() const { return get(ID_C_base_name); }

    bool get_this() const { return get_bool(ID_C_this); }

    void set_this() { set(ID_C_this, true); }
  };
  ...
  bool is_KnR() const { return get_bool(ID_C_KnR); }

  void make_ellipsis() { add(ID_parameters).set(ID_ellipsis, true); }

  void remove_ellipsis() { add(ID_parameters).remove(ID_ellipsis); }

  typedef std::vector<parametert> parameterst;

  const typet &return_type() const { return find_type(ID_return_type); }

  typet &return_type() { return add_type(ID_return_type); }
  ...
  bool get_inlined() const { return get_bool(ID_C_inlined); }

  void set_inlined(bool value) { set(ID_C_inlined, value); }
...
public:
  ...
  exprt &size() { return static_cast<exprt &>(add(ID_size)); }

  bool is_complete() const { return size().is_not_nil(); }

  bool is_incomplete() const { return size().is_nil(); }
};
 ...
 * \ingroup gr_std_types
*/
inline const incomplete_array_typet &
to_incomplete_array_type(const typet &type) {
  assert(type.id() == ID_array);
...
public:
  ...
  std::size_t get_width() const { return get_size_t(ID_width); }

  void set_width(std::size_t width) { set(ID_width, width); }
};
...
public:
  ...
  void set_integer_bits(std::size_t b) { set(ID_integer_bits, b); }
};
...
public:
  ...
  void set_f(std::size_t b) { set(ID_f, b); }
};
...
public:
  ...
  signedbv_typet difference_type() const { return signedbv_typet(get_width()); }
};
...
public:
  reference_typet() { set(ID_C_reference, true); }

  // this one will go away; use the one with width
  explicit reference_typet(const typet &_subtype) : pointer_typet(_subtype) {
