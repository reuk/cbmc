...
public:
  ...
  bool operator>=(const rationalt &n) const { return !(*this < n); }

  bool operator>(const rationalt &n) const { return !(*this <= n); }

  bool is_zero() const { return numerator.is_zero(); }

  bool is_one() const { return !is_zero() && numerator == denominator; }

  bool is_negative() const { return !is_zero() && numerator.is_negative(); }

  void invert();

  const mp_integer &get_numerator() const { return numerator; }

  const mp_integer &get_denominator() const { return denominator; }
};
