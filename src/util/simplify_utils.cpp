...
struct saj_tablet {
  ...
  const irep_idt type_ids[10];
    ...
    {ID_plus,
     {ID_integer, ID_natural, ID_real, ID_complex, ID_rational, ID_unsignedbv,
      ID_signedbv, ID_fixedbv, irep_idt()}},
    {ID_mult,
     {ID_integer, ID_natural, ID_real, ID_complex, ID_rational, ID_unsignedbv,
      ID_signedbv, ID_fixedbv, irep_idt()}},
    ...
    {ID_bitand,
     {ID_unsignedbv, ID_signedbv, ID_floatbv, ID_fixedbv, irep_idt()}},
    {ID_bitor,
     {ID_unsignedbv, ID_signedbv, ID_floatbv, ID_fixedbv, irep_idt()}},
    {ID_bitxor,
     {ID_unsignedbv, ID_signedbv, ID_floatbv, ID_fixedbv, irep_idt()}},
...
bool sort_and_join(exprt &expr) {
  ...
  forall_operands(
      it, expr) if (!sort_and_join(saj_entry, it->type().id())) return true;
  ...
  forall_operands(it, expr) {
    if (it->id() == expr.id()) {
      ...
      forall_operands(it2, *it) new_ops.push_back(*it2);
