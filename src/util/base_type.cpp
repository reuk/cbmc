...
void base_type_rec(typet &type, const namespacet &ns,
                   std::set<irep_idt> &symb) {
  if (type.id() == ID_symbol || type.id() == ID_c_enum_tag ||
      type.id() == ID_struct_tag || type.id() == ID_union_tag) {
    const symbolt *symbol;

    if (!ns.lookup(type.get(ID_identifier), symbol) && symbol->is_type &&
        !symbol->type.is_nil()) {
      type = symbol->type;
    ...
    if (subtype.id() == ID_symbol || subtype.id() == ID_c_enum_tag ||
...
void base_type(exprt &expr, const namespacet &ns) {
  ...
  Forall_operands(it, expr) base_type(*it, ns);
}
...
bool base_type_eqt::base_type_eq_rec(const typet &type1, const typet &type2) {
  ...
  // loop avoidance
  if ((type1.id() == ID_symbol || type1.id() == ID_c_enum_tag ||
       type1.id() == ID_struct_tag || type1.id() == ID_union_tag) &&
      type2.id() == type1.id()) {
    // already in same set?
    if (identifiers.make_union(type1.get(ID_identifier),
  ...
  if (type2.id() == ID_symbol || type2.id() == ID_c_enum_tag ||
  ...
  if (type1.id() == ID_struct || type1.id() == ID_union) {
    ...
    return base_type_eq_rec(to_incomplete_array_type(type1).subtype(),
...
bool base_type_eqt::base_type_eq_rec(const exprt &expr1, const exprt &expr2) {
  ...
  for (exprt::operandst::const_iterator it1 = expr1_op.begin(),
                                        it2 = expr2_op.begin();
       it1 != expr1_op.end() && it2 != expr2_op.end(); ++it1, ++it2)
