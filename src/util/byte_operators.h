...
public:
  ...
  byte_extract_exprt(irep_idt _id, const exprt &_op, const exprt &_offset,
                     const typet &_type)
      : exprt(_id, _type) {
...
to_byte_extract_little_endian_expr(const exprt &expr) {
  assert(expr.id() == ID_byte_extract_little_endian &&
...
public:
  ...
  byte_extract_big_endian_exprt(const exprt &_op, const exprt &_offset,
                                const typet &_type)
...
to_byte_extract_big_endian_expr(const exprt &expr) {
  assert(expr.id() == ID_byte_extract_big_endian &&
         expr.operands().size() == 2);
...
public:
  ...
  byte_update_exprt(irep_idt _id, const typet &_type) : exprt(_id, _type) {
    operands().resize(3);
  }

  byte_update_exprt(irep_idt _id, const exprt &_op, const exprt &_offset,
                    const exprt &_value)
      : exprt(_id, _op.type()) {
...
public:
  ...
  byte_update_little_endian_exprt(const exprt &_op, const exprt &_offset,
                                  const exprt &_value)
...
to_byte_update_little_endian_expr(const exprt &expr) {
  assert(expr.id() == ID_byte_update_little_endian &&
...
public:
  ...
  byte_update_big_endian_exprt(const exprt &_op, const exprt &_offset,
                               const exprt &_value)
