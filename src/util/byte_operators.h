...
{
  assert(
    expr.id() == ID_byte_extract_little_endian && expr.operands().size() == 2);
...
{
  assert(
    expr.id() == ID_byte_extract_big_endian && expr.operands().size() == 2);
...
{
  assert(
    expr.id() == ID_byte_update_little_endian && expr.operands().size() == 3);
...
{
public:
  byte_update_big_endian_exprt() : byte_update_exprt(ID_byte_update_big_endian)
  {
  }
...
inline byte_update_big_endian_exprt &to_byte_update_big_endian_expr(exprt &expr)
{
  assert(expr.id() == ID_byte_update_big_endian && expr.operands().size() == 3);
