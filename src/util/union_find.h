...
public:
  ...
  void swap(unsigned_union_find &other) { other.nodes.swap(nodes); }
  ...
  bool same_set(size_type a, size_type b) const { return find(a) == find(b); }
...
public:
  ...
  bool make_union(typename numbering<T>::const_iterator it_a,
    ...
    size_type na = it_a - numbering<T>::begin(),
              nb = it_b - numbering<T>::begin();
    bool is_union = find_number(na) == find_number(nb);
    uuf.make_union(na, nb);
    return is_union;
  }
  ...
  bool same_set(const T &a, const T &b) const {
    ...
    bool have_na = !subt::get_number(a, na), have_nb = !subt::get_number(b, nb);
  ...
  bool same_set(typename numbering<T>::const_iterator it_a,
    ...
    return uuf.same_set(it_a - numbering<T>::begin(),
                        it_b - numbering<T>::begin());
  }
  ...
  size_type find_number(size_type a) const { return uuf.find(a); }

  size_type find_number(const T &a) { return uuf.find(number(a)); }

  bool is_root_number(size_type a) const { return uuf.is_root(a); }
  ...
  void isolate(const T &a) { uuf.isolate(number(a)); }
