...
void exprt::copy_to_operands(const exprt &expr) { operands().push_back(expr); }
...
bool exprt::is_constant() const { return id() == ID_constant; }
...
bool exprt::is_boolean() const { return type().id() == ID_bool; }
...
bool exprt::sum(const exprt &expr) {
  ...
  if (type_id == ID_integer || type_id == ID_natural) {
    set(ID_value, integer2string(string2integer(get_string(ID_value)) +
    ...
    set(ID_value,
        integer2binary(binary2integer(get_string(ID_value), false) +
...
void exprt::visit(expr_visitort &visitor) {
  ...
  while (!stack.empty()) {
    ...
    Forall_operands(it, expr) stack.push(&(*it));
  }
}
