...
bool simplify_exprt::simplify_typecast(exprt &expr)
{
  ...
  if(
    op_type.id() == ID_pointer && expr.op0().is_constant() &&
  ...
  if(
    expr_type.id() == ID_pointer && expr.op0().id() == ID_typecast &&
  ...
  if(
    expr_type.id() == ID_pointer && expr.op0().is_constant() &&
    (to_constant_expr(expr.op0()).get_value() == ID_NULL ||
  ...
  if(operand.is_constant())
  {
    ...
    else if(op_type_id == ID_bool)
    {
      ...
      else if(expr_type_id == ID_c_enum_tag)
      {
        ...
        expr_type_id == ID_pointer && operand.is_false() &&
        config.ansi_c.NULL_is_zero)
      {
  ...
  bool &new_truth)
{
  ...
  if(truth && cond.id() == ID_lt && expr.id() == ID_lt)
  {
      ...
      expr.op1().is_constant() && cond.op1().type() == expr.op1().type())
    {
      ...
      {
        if(
          string2integer(cond.op1().get_string(ID_value)) >=
      ...
      else if(type_id == ID_unsignedbv)
      {
        ...
        if(
          binary2integer(cond.op1().get_string(ID_value), false) >=
      ...
      else if(type_id == ID_signedbv)
      {
        const mp_integer i1, i2;
        if(
          binary2integer(cond.op1().get_string(ID_value), true) >=
  ...
  else if(expr.id() == ID_typecast)
  {
    ...
    else if(op_type.id() == ID_signedbv || op_type.id() == ID_unsignedbv)
    {
      ...
      if(tmp.id() == ID_plus && tmp.operands().size() == 2)
      {
        ...
        if(
          cand.id() == ID_typecast && cand.operands().size() == 1 &&
          cand.op0().id() == ID_address_of)
        {
          ...
          cand.id() == ID_typecast && cand.operands().size() == 1 &&
          cand.op0().id() == ID_plus && cand.op0().operands().size() == 2 &&
  ...
  bool little_endian)
{
  ...
  else if(type.id() == ID_c_enum)
  {
    ...
    return bits2expr(
      bits, ns.follow_tag(to_c_enum_tag_type(type)), little_endian);
...
bool simplify_exprt::simplify_byte_update(byte_update_exprt &expr)
{
  ...
  if(
    expr.id() == expr.op().id() && expr.offset() == expr.op().op1() &&
  ...
  if(offset.is_zero() && val_size > 0 && root_size > 0 && val_size >= root_size)
  {
  ...
  if(value.id() == ID_with)
  {
    ...
    if(with.old().id() == ID_byte_extract_little_endian)
    {
      ...
      if(tp.id() == ID_struct)
      {
        ...
        // is this a bit field?
        if(
          struct_type.get_component(component_name).type().id() ==
          ID_c_bit_field)
        {
  ...
  if(op_type.id() == ID_struct)
  {
    ...
    for(const auto &component : components)
    {
      mp_integer m_offset= member_offset(struct_type, component.get_name(), ns);
      ...
      if(
        m_offset < offset_int || (m_offset == offset_int && update_size > 0 &&
                                  m_size_bytes > update_size))
      {
        ...
        update_size > 0 && m_offset + m_size_bytes > offset_int + update_size)
      {
...
// #define DEBUGX
  ...
  if(expr.id() == ID_typecast)
    ...
    expr.id() == ID_equal || expr.id() == ID_notequal || expr.id() == ID_gt ||
    expr.id() == ID_lt || expr.id() == ID_ge || expr.id() == ID_le)
    ...
    expr.id() == ID_implies || expr.id() == ID_iff || expr.id() == ID_or ||
    expr.id() == ID_xor || expr.id() == ID_and)
