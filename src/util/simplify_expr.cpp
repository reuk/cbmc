...
public:
  ...
  containert &container() { return container_normal; }
};
...
bool simplify_exprt::simplify_typecast(exprt &expr) {
  ...
  if (op_type.id() == ID_pointer && expr.op0().is_constant() &&
  ...
  if (expr_type.id() == ID_pointer && expr.op0().id() == ID_typecast &&
  ...
  if (expr_type.id() == ID_bool) {
    ...
    inequality.id(op_type.id() == ID_floatbv ? ID_ieee_float_notequal
                                             : ID_notequal);
  ...
  if (expr_type.id() == ID_c_bool && op_type.id() != ID_bool) {
  ...
  if (expr_type.id() == ID_pointer && expr.op0().is_constant() &&
      (to_constant_expr(expr.op0()).get_value() == ID_NULL ||
  ...
  // (T1 *)(T2 *)x -> (T1 *)x
  if (expr_type.id() == ID_pointer && expr.op0().id() == ID_typecast &&
      op_type.id() == ID_pointer && expr.op0().operands().size() == 1) {
    exprt tmp;
  ...
  if (config.ansi_c.NULL_is_zero &&
      ...
      expr.op0().id() == ID_plus && expr.op0().operands().size() == 2 &&
      expr.op0().op0().id() == ID_typecast &&
      expr.op0().op0().operands().size() == 1 &&
      expr.op0().op0().op0().is_zero() && op_type.id() == ID_pointer) {
  ...
  if ((expr_type.id() == ID_signedbv || expr_type.id() == ID_unsignedbv) &&
      (op_type.id() == ID_signedbv || op_type.id() == ID_unsignedbv)) {
    bool enlarge = to_bitvector_type(expr_type).get_width() >
    ...
    if (!enlarge) {
      ...
      if (op_id == ID_plus || op_id == ID_minus || op_id == ID_mult ||
          op_id == ID_unary_minus || op_id == ID_bitxor || op_id == ID_bitor ||
          op_id == ID_bitand) {
  ...
  if (operand.is_constant()) {
    ...
    if (op_type_id == ID_integer || op_type_id == ID_natural) {
      // from integer to ...
      ...
      if (expr_type_id == ID_unsignedbv || expr_type_id == ID_signedbv ||
          ...
          expr_type_id == ID_rational || expr_type_id == ID_c_bool ||
          expr_type_id == ID_c_enum || expr_type_id == ID_c_bit_field) {
        if (operand.is_true()) {
      ...
      } else if (expr_type_id == ID_pointer && operand.is_false() &&
                 config.ansi_c.NULL_is_zero) {
        expr = null_pointer_exprt(to_pointer_type(expr_type));
        return false;
      }
    } else if (op_type_id == ID_unsignedbv || op_type_id == ID_signedbv ||
               op_type_id == ID_c_bit_field || op_type_id == ID_c_bool) {
      ...
      if (expr_type_id == ID_unsignedbv || expr_type_id == ID_signedbv ||
          expr_type_id == ID_bv || expr_type_id == ID_c_bit_field) {
        expr = from_integer(int_value, expr_type);
      ...
      if (expr_type_id == ID_fixedbv) {
        // int to float
        const fixedbv_typet &f_expr_type = to_fixedbv_type(expr_type);
      ...
      if (expr_type_id == ID_floatbv) {
        // int to float
        const floatbv_typet &f_expr_type = to_floatbv_type(expr_type);
  ...
  } else if (operand.id() == ID_typecast) // typecast of typecast
  {
    ...
    if (operand.operands().size() == 1 && op_type_id == expr_type_id &&
...
bool simplify_exprt::simplify_dereference(exprt &expr) {
  ...
  // rewrite *(&a[0] + x) to a[x]
    ...
    const address_of_exprt &address_of = to_address_of_expr(pointer.op0());
    if (address_of.object().id() == ID_index) {
      ...
      if (ns.follow(old.array().type()).id() == ID_array) {
        index_exprt idx(old.array(), plus_exprt(old.index(), pointer.op1()),
...
bool simplify_exprt::simplify_if_implies(exprt &expr, const exprt &cond,
  ...
  if (truth && cond.id() == ID_lt && expr.id() == ID_lt) {
    if (cond.op0() == expr.op0() && cond.op1().is_constant() &&
    ...
    if (cond.op1() == expr.op1() && cond.op0().is_constant() &&
...
bool simplify_exprt::simplify_if_recursive(exprt &expr, const exprt &cond,
                                           bool truth) {
  ...
  Forall_operands(it, expr) result =
      simplify_if_recursive(*it, cond, truth) && result;
...
bool simplify_exprt::simplify_if_conj(exprt &expr, const exprt &cond) {
  ...
  Forall_operands(it, expr) result = simplify_if_conj(*it, cond) && result;
...
bool simplify_exprt::simplify_if_disj(exprt &expr, const exprt &cond) {
  ...
  Forall_operands(it, expr) result = simplify_if_disj(*it, cond) && result;
...
bool simplify_exprt::simplify_if_branch(exprt &trueexpr, exprt &falseexpr,
                                        const exprt &cond) {
  bool tresult = true;
  bool fresult = true;
...
bool simplify_exprt::simplify_if(if_exprt &expr) {
  ...
  if (((truevalue.id() == ID_struct && falsevalue.id() == ID_struct) ||
    ...
    exprt::operandst::const_iterator f_it = falsevalue_copy.operands().begin();
...
bool simplify_exprt::simplify_with(exprt &expr) {
  ...
  if (op0_type.id() == ID_struct) {
    if (expr.op0().id() == ID_struct || expr.op0().id() == ID_constant) {
      while (expr.operands().size() > 1) {
        const irep_idt &component_name = expr.op1().get(ID_component_name);

        if (!to_struct_type(op0_type).has_component(component_name))
          return result;

        std::size_t number =
            to_struct_type(op0_type).component_number(component_name);
    ...
    if (expr.op0().id() == ID_array || expr.op0().id() == ID_constant) {
      while (expr.operands().size() > 1) {
        mp_integer i;
...
bool simplify_exprt::simplify_update(exprt &expr) {
  ...
  for (const auto &e : designator) {
    ...
    if (e.id() == ID_index_designator && value_ptr->id() == ID_array) {
      mp_integer i;
    ...
    } else if (e.id() == ID_member_designator && value_ptr->id() == ID_struct) {
      const irep_idt &component_name = e.get(ID_component_name);

      if (!to_struct_type(value_ptr_type).has_component(component_name))
        return true;

      std::size_t number =
          to_struct_type(value_ptr_type).component_number(component_name);
...
bool simplify_exprt::simplify_object(exprt &expr) {
  if (expr.id() == ID_plus) {
    if (expr.type().id() == ID_pointer) {
      ...
      Forall_operands(it, expr) if (ns.follow(it->type()).id() == ID_pointer) {
        exprt tmp = *it;
        expr.swap(tmp);
    ...
    if (op_type.id() == ID_pointer) {
      ...
      if (tmp.id() == ID_plus && tmp.operands().size() == 2) {
        ...
        if (cand.id() == ID_typecast && cand.operands().size() == 1 &&
            cand.op0().id() == ID_address_of) {
          expr = cand.op0();
          return false;
        } else if (cand.id() == ID_typecast && cand.operands().size() == 1 &&
                   cand.op0().id() == ID_plus &&
...
exprt simplify_exprt::bits2expr(const std::string &bits, const typet &_type,
                                bool little_endian) {
  ...
  if (type.id() == ID_unsignedbv || type.id() == ID_signedbv ||
    ...
    return bits2expr(bits, ns.follow_tag(to_c_enum_tag_type(type)),
                     little_endian);
    ...
    for (const auto &component : components) {
      ...
      std::string comp_bits = std::string(bits, integer2size_t(m_offset_bits),
...
std::string simplify_exprt::expr2bits(const exprt &expr, bool little_endian) {
...
bool simplify_exprt::simplify_byte_extract(byte_extract_exprt &expr) {
  ...
  if (offset == 0 && base_type_eq(expr.type(), expr.op().type(), ns) &&
  ...
  if (expr.op().id() == ID_array_of && expr.op().op0().id() == ID_constant) {
    std::string const_bits = expr2bits(
        expr.op().op0(), byte_extract_id() == ID_byte_extract_little_endian);
    ...
    std::string el_bits = std::string(const_bits, integer2size_t(offset * 8),
                                      integer2size_t(el_size));

    exprt tmp = bits2expr(el_bits, expr.type(),
                          expr.id() == ID_byte_extract_little_endian);
  ...
  if (expr.op().id() == ID_array_of && (offset * 8) % el_size == 0 &&
  ...
  if (op_type.id() == ID_array) {
    ...
    for (const typet *op_type_ptr = &op_type; op_type_ptr->id() == ID_array;
      ...
      if (base_type_eq(expr.type(), op_type_ptr->subtype(), ns)) {
        if (offset % el_bytes == 0) {
          ...
          result =
              index_exprt(result, from_integer(offset, expr.offset().type()));
        ...
        if (sub_size > 0) {
          ...
          result =
              index_exprt(result, from_integer(index, expr.offset().type()));
        }
      }
    }
    ...
    for (const auto &component : components) {
      ...
      if (offset * 8 == m_offset_bits && el_size == m_size &&
...
bool simplify_exprt::simplify_byte_update(byte_update_exprt &expr) {
  ...
  if (expr.id() == expr.op().id() && expr.offset() == expr.op().op1() &&
  ...
  if (offset.is_zero() && val_size > 0 && root_size > 0 &&
      val_size >= root_size) {
    byte_extract_exprt be(expr.id() == ID_byte_update_little_endian
  ...
  if (op_type.id() == ID_struct) {
    ...
    mp_integer update_size = pointer_offset_size(value.type(), ns);
    ...
    for (const auto &component : components) {
      ...
      // member?
      if (m_offset < 0 || m_size_bits <= 0 || m_size_bits % 8 != 0) {
        result_expr.make_nil();
        break;
      }
      ...
      else if (update_size > 0 && m_offset >= offset_int + update_size)
        break;
      ...
      if (m_offset < offset_int || (m_offset == offset_int && update_size > 0 &&
                                    m_size_bytes > update_size)) {
        byte_update_exprt v(
            ID_byte_update_little_endian,
            member_exprt(root, component.get_name(), component.type()),
            from_integer(offset_int - m_offset, offset.type()), value);
        ...
        byte_extract_exprt v(ID_byte_extract_little_endian, value,
    ...
    Forall_operands(it, result) {
      ...
      if (offset_int * 8 < m_offset_bits + el_size) {
        ...
        byte_extract_exprt new_val(
            byte_extract_id(), value, from_integer(val_offset, offset.type()),
            array_typet(unsignedbv_typet(8),
                        from_integer(bytes_req, offset.type())));

        *it = byte_update_exprt(
            expr.id(), *it,
            from_integer(offset_int + val_offset - m_offset_bits / 8,
                         offset.type()),
            new_val);
...
bool simplify_exprt::simplify_node_preorder(exprt &expr) {
  ...
  if (expr.id() == ID_address_of) {
    ...
    if (expr.has_operands()) {
      Forall_operands(it, expr) if (!simplify_rec(*it)) // recursive call
          result = false;
    }
  }
...
// #define DEBUGX
  ...
  if (expr.id() == ID_typecast)
    result = simplify_typecast(expr) && result;
           ...
           expr.id() == ID_gt || expr.id() == ID_lt || expr.id() == ID_ge ||
           expr.id() == ID_le)
    ...
    result = simplify_not(expr) && result;
           ...
           expr.id() == ID_or || expr.id() == ID_xor || expr.id() == ID_and)
...
#else
  if (!local_replace_map.empty() && !replace_expr(local_replace_map, tmp)) {
    simplify_rec(tmp);
    result = false;
  }
#endif
