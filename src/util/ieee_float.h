...
public:
  ...
  explicit ieee_float_spect(const floatbv_typet &type) { from_type(type); }
...
public:
  ...
  explicit ieee_floatt(const ieee_float_spect &_spec)
      : rounding_mode(ROUND_TO_EVEN), spec(_spec), sign_flag(false),
        exponent(0), fraction(0), NaN_flag(false), infinity_flag(false) {}

  explicit ieee_floatt(const floatbv_typet &type)
      : rounding_mode(ROUND_TO_EVEN), spec(ieee_float_spect(type)),
        sign_flag(false), exponent(0), fraction(0), NaN_flag(false),
        infinity_flag(false) {}

  ieee_floatt()
      : rounding_mode(ROUND_TO_EVEN), sign_flag(false), exponent(0),
        fraction(0), NaN_flag(false), infinity_flag(false) {}
  ...
  void negate() { sign_flag = !sign_flag; }

  void set_sign(bool _sign) { sign_flag = _sign; }
  ...
  std::string to_ansi_c_string() const { return format(format_spect()); }
