...
public:
  ...
  bool is_string() const { return kind == J_STRING; }

  bool is_number() const { return kind == J_NUMBER; }

  bool is_object() const { return kind == J_OBJECT; }

  bool is_array() const { return kind == J_ARRAY; }

  bool is_true() const { return kind == J_TRUE; }

  bool is_false() const { return kind == J_FALSE; }

  bool is_null() const { return kind == J_NULL; }

  jsont() : kind(J_NULL) {}

  void output(std::ostream &out) const { output_rec(out, 0); }
...
public:
  ...
  void resize(std::size_t size) { array.resize(size); }
...
public:
  ...
  jsont &operator[](const std::string &key) { return object[key]; }
