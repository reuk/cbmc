...
void replace_symbolt::insert(const symbol_exprt &old_expr,
                             const exprt &new_expr) {
  expr_map.insert(
      std::pair<irep_idt, exprt>(old_expr.get_identifier(), new_expr));
}
...
bool replace_symbolt::replace(exprt &dest) const {
  ...
  if (dest.id() == ID_symbol) {
    expr_mapt::const_iterator it = expr_map.find(dest.get(ID_identifier));
  ...
  Forall_operands(it, dest) if (!replace(*it)) result = false;
...
bool replace_symbolt::have_to_replace(const exprt &dest) const {
  ...
  forall_operands(it, dest) if (have_to_replace(*it)) return true;
...
bool replace_symbolt::replace(typet &dest) const {
  ...
  Forall_subtypes(it, dest) if (!replace(*it)) result = false;
  ...
  if (dest.id() == ID_struct || dest.id() == ID_union) {
    ...
    type_mapt::const_iterator it = type_map.find(dest.get(ID_identifier));
...
bool replace_symbolt::have_to_replace(const typet &dest) const {
  ...
  forall_subtypes(it, dest) if (have_to_replace(*it)) return true;
