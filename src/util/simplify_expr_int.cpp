...
{
  if(
    expr.type().id() == ID_unsignedbv && expr.operands().size() == 1 &&
  ...
  for(exprt::operandst::iterator it= operands.begin(); it != operands.end();)
  {
    ...
    if(it->is_constant() && it->type() == expr.type())
    {
      ...
      if(c_sizeof_type.is_nil())
        c_sizeof_type= static_cast<const typet &>(it->find(ID_C_c_sizeof_type));
...
bool simplify_exprt::simplify_plus(exprt &expr)
{
  ...
  else
  {
    ...
    Forall_expr(it, operands)
      if(it->id() == ID_unary_minus && it->operands().size() == 1)
...
bool simplify_exprt::simplify_minus(exprt &expr)
{
  ...
  if(
    is_number(expr.type()) && is_number(operands[0].type()) &&
    is_number(operands[1].type()))
  {
    ...
    expr.type().id() == ID_pointer && operands[0].type().id() == ID_pointer &&
    is_number(operands[1].type()))
  {
    ...
    is_number(expr.type()) && operands[0].type().id() == ID_pointer &&
...
bool simplify_exprt::simplify_inequality_constant(exprt &expr)
{
  ...
  if(expr.op1().type().id() == ID_pointer)
  {
      ...
      expr.op1().get(ID_value) == ID_NULL)
    {
      ...
      if(expr.op0().id() == ID_address_of && expr.op0().operands().size() == 1)
      {
        if(
