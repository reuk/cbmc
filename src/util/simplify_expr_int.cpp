...
bool simplify_exprt::simplify_bswap(exprt &expr) {
  if (expr.type().id() == ID_unsignedbv && expr.operands().size() == 1 &&
      expr.op0().type() == expr.type() && expr.op0().is_constant()) {
  ...
  if (ns.follow(expr.type()).id() == ID_floatbv) {
    ...
    Forall_expr(it, operands) {
      ...
      if (next != operands.end()) {
        if (it->type() == next->type() && it->is_constant() &&
            next->is_constant()) {
          it->sum(*next);
          operands.erase(next);
        }
    ...
    // count the constants
    size_t count = 0;
    forall_operands(it, expr) if (is_number(it->type()) && it->is_constant())
        count++;
    ...
    Forall_expr(it, operands) if (it->id() == ID_unary_minus &&
                                  it->operands().size() == 1)
...
bool simplify_exprt::simplify_minus(exprt &expr) {
  ...
  if (is_number(expr.type()) && is_number(operands[0].type()) &&
    ...
    expr.swap(tmp);
    return false;
  } else if (is_number(expr.type()) && operands[0].type().id() == ID_pointer &&
...
bool simplify_exprt::simplify_bitwise(exprt &expr) {
  ...
  if (expr.type().id() != ID_bool) {
    ...
    forall_operands(it, expr) {
      if (it->id() == ID_typecast && it->operands().size() == 1 &&
  ...
  if (expr.operands().size() == 2 && expr.op0() == expr.op1()) {
    ...
    if (expr.id() == ID_lshr) {
      ...
      if (distance >= 0) {
        ...
        mp_integer new_value =
            value >> distance; // NOLINT(whitespace/operators)
        expr = from_integer(new_value, expr.type());
        return false;
      }
...
bool simplify_exprt::simplify_extractbits(exprt &expr) {
  ...
  if (expr.op0().is_constant()) {
    ...
    std::string extracted_value = svalue.substr(
        width - integer2size_t(start) - 1, integer2size_t(start - end + 1));
...
bool simplify_exprt::simplify_unary_minus(exprt &expr) {
  ...
  if (operand.id() == ID_unary_minus) {
    ...
    if (type_id == ID_integer || type_id == ID_signedbv ||
...
bool simplify_exprt::simplify_bitnot(exprt &expr) {
  ...
  if (expr.type().id() == ID_bv || expr.type().id() == ID_unsignedbv ||
...
bool simplify_exprt::simplify_inequality(exprt &expr) {
  ...
  if (tmp0.id() == ID_pointer_object && tmp1.id() == ID_pointer_object &&
      (expr.id() == ID_equal || expr.id() == ID_notequal))
...
bool simplify_exprt::eliminate_common_addends(exprt &op0, exprt &op1) {
  ...
  if (op0.id() == ID_plus) {
    ...
    Forall_operands(it, op0) if (!eliminate_common_addends(*it, op1)) result =
        false;
    ...
    Forall_operands(it, op1) if (!eliminate_common_addends(op0, *it)) result =
        false;
...
bool simplify_exprt::simplify_inequality_not_constant(exprt &expr) {
  ...
  if (ok0 && ok1) {
    ...
    forall_value_list(it0, values0) forall_value_list(it1, values1) {
      bool tmp;
...
bool simplify_exprt::simplify_inequality_constant(exprt &expr) {
  ...
  if (expr.op1().type().id() == ID_pointer) {
    ...
    if (expr.id() == ID_equal && expr.op1().is_constant() &&
  ...
  if (expr.op0().id() == ID_typecast && expr.op0().operands().size() == 1 &&
