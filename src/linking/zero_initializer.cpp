...
protected:
  ...
  std::string to_string(const exprt &src) { return expr2c(src, ns); }

  std::string to_string(const typet &src) { return type2c(src, ns); }

  exprt zero_initializer_rec(const typet &type,
...
exprt zero_initializert::zero_initializer_rec(
  ...
  if (type_id == ID_unsignedbv || type_id == ID_signedbv ||
      type_id == ID_pointer || type_id == ID_c_enum ||
      type_id == ID_incomplete_c_enum || type_id == ID_c_bit_field ||
      type_id == ID_bool || type_id == ID_c_bool || type_id == ID_floatbv ||
      type_id == ID_fixedbv) {
    exprt result = from_integer(0, type);
    result.add_source_location() = source_location;
    return result;
  } else if (type_id == ID_rational || type_id == ID_real) {
    ...
    if (array_type.size().is_nil()) {
      ...
      exprt tmpval =
          zero_initializer_rec(array_type.subtype(), source_location);
    ...
    if (!found) {
      ...
      value.op() = zero_initializer_rec(component.type(), source_location);
    }
    ...
    return zero_initializer_rec(ns.follow_tag(to_c_enum_tag_type(type)),
                                source_location);
  } else if (type_id == ID_struct_tag) {
    return zero_initializer_rec(ns.follow_tag(to_struct_tag_type(type)),
                                source_location);
  } else if (type_id == ID_union_tag) {
    return zero_initializer_rec(ns.follow_tag(to_union_tag_type(type)),
                                source_location);
  } else if (type_id == ID_string) {
    return constant_exprt(irep_idt(), type);
  } else {
    error().source_location = source_location;
    error() << "failed to zero-initialize `" << to_string(type) << "'" << eom;
    throw 0;
  }
}
