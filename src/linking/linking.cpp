  ...
  const typet &type) const
{
  ...
  else if(followed.id() == ID_pointer)
  {
    ...
    followed.id() == ID_incomplete_struct ||
    ...
    else
    {
      for(std::size_t i= 0; i < components1.size(); ++i)
      {
        ...
        else if(!base_type_eq(subtype1, subtype2, ns))
        {
          ...
          conflict_path= member_exprt(conflict_path, components1[i].get_name());
          ...
          else
          {
            ...
            if(depth > 0)
            {
              ...
              detailed_conflict_report_rec(
                old_symbol, new_symbol, subtype1, subtype2, depth - 1, c);
              msg.swap(msg_bak);
            }
          }
...
irep_idt linkingt::rename(const irep_idt id)
{
  ...
  while(true)
  {
    ...
    if(
      main_symbol_table.symbols.find(new_identifier) !=
...
void linkingt::duplicate_code_symbol(symbolt &old_symbol, symbolt &new_symbol)
{
  ...
  if(!base_type_eq(old_symbol.type, new_symbol.type, ns))
  {
    ...
    if(!old_symbol.location.get_function().empty() && old_symbol.value.is_nil())
    {
      ...
      !new_symbol.location.get_function().empty() && new_symbol.value.is_nil())
    {
      // issue a warning
      link_warning(
        old_symbol,
    ...
    // handle (incomplete) function prototypes
      ...
      base_type_eq(old_t.return_type(), new_t.return_type(), ns) &&
    ...
    // aliasing may alter the type
      ...
      (new_symbol.is_macro && new_symbol.value.is_not_nil() &&
  ...
  adjust_type_infot &info)
{
  ...
  if(
    t1.id() == ID_symbol || t1.id() == ID_struct_tag ||
    t1.id() == ID_union_tag || t1.id() == ID_c_enum_tag)
  {
    ...
    t2.id() == ID_symbol || t2.id() == ID_struct_tag ||
  ...
  else if(t1.id() == ID_array && t2.id() == ID_pointer)
  {
    ...
    (t1.id() == ID_incomplete_struct && t2.id() == ID_struct) ||
    (t1.id() == ID_incomplete_union && t2.id() == ID_union))
  {
    ...
    (t1.id() == ID_struct && t2.id() == ID_incomplete_struct) ||
    ...
    !adjust_object_type_rec(t1.subtype(), t2.subtype(), info))
  {
    ...
    if(
      ...
      (old_size.is_zero() && new_size.is_not_nil()) || info.old_symbol.is_weak)
    {
      info.set_to_new= true; // store new type
    }
    else if(new_size.is_nil() || new_size.is_zero() || info.new_symbol.is_weak)
    {
...
void linkingt::duplicate_object_symbol(symbolt &old_symbol, symbolt &new_symbol)
{
  // both are variables
  ...
  if(!base_type_eq(old_symbol.type, new_symbol.type, ns))
  {
    ...
    if(failed)
    {
      ...
      if(
        old_type.id() == ID_struct || old_type.id() == ID_union ||
    ...
    !new_symbol.value.get_bool(ID_C_zero_initializer))
  {
    if(
      old_symbol.value.is_nil() ||
      old_symbol.value.get_bool(ID_C_zero_initializer) || old_symbol.is_weak)
    {
      // new_symbol wins
      old_symbol.value= new_symbol.value;
      old_symbol.is_macro= new_symbol.is_macro;
...
void linkingt::duplicate_type_symbol(symbolt &old_symbol, symbolt &new_symbol)
{
  ...
  if(
    old_symbol.type.id() == ID_incomplete_struct &&
  ...
  if(
    old_symbol.type.id() == ID_incomplete_union &&
  ...
  {
    if(
      to_array_type(old_symbol.type).size().is_nil() &&
    ...
    if(
      to_array_type(new_symbol.type).size().is_nil() &&
