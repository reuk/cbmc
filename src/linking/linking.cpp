...
void linkingt::detailed_conflict_report_rec(const symbolt &old_symbol,
                                            ...
                                            const typet &type1,
                                            const typet &type2, unsigned depth,
                                            exprt &conflict_path) {
#ifdef DEBUG
  ...
  if (t1.id() != t2.id())
    ...
    if (depth > 0 && !base_type_eq(t1.subtype(), t2.subtype(), ns)) {
      conflict_path = dereference_exprt(conflict_path);

      detailed_conflict_report_rec(old_symbol, new_symbol, t1.subtype(),
                                   t2.subtype(), depth - 1, conflict_path);
    ...
    if (components1.size() != components2.size()) {
      ...
      for (std::size_t i = 0; i < components1.size(); ++i) {
        ...
        if (components1[i].get_name() != components2[i].get_name()) {
          ...
          if (depth > 0 && parent_types.find(t1) == parent_types.end())
            detailed_conflict_report_rec(old_symbol, new_symbol, subtype1,
                                         subtype2, depth - 1, conflict_path);
          else {
    ...
    if (parameters1.size() != parameters2.size()) {
      ...
      if (depth > 0)
        detailed_conflict_report_rec(old_symbol, new_symbol, return_type1,
                                     return_type2, depth - 1, conflict_path);
      else
  ...
  if (!msg.empty()) {
    ...
    error() << "reason for conflict at "
            << expr_to_string(ns, "", conflict_path) << ": " << msg << '\n';
...
void linkingt::link_error(const symbolt &old_symbol, const symbolt &new_symbol,
...
void linkingt::link_warning(const symbolt &old_symbol,
                            const symbolt &new_symbol, const std::string &msg) {
  warning().source_location = new_symbol.location;

  warning() << "warning: " << msg << " \"" << old_symbol.display_name() << "\""
            << '\n';
...
irep_idt linkingt::rename(const irep_idt id) {
  ...
  while (true) {
    irep_idt new_identifier = id2string(id) + "$link" + std::to_string(++cnt);
...
void linkingt::duplicate_code_symbol(symbolt &old_symbol, symbolt &new_symbol) {
  // Both are functions.
  if (!base_type_eq(old_symbol.type, new_symbol.type, ns)) {
    ...
    if (!old_symbol.location.get_function().empty() &&
      ...
      // issue a warning
      link_warning(old_symbol, new_symbol,
    ...
    // replace weak symbols
      ...
      if (new_symbol.value.is_nil() || old_symbol.value.is_not_nil()) {
    ...
    else if ((new_symbol.is_macro && new_symbol.value.is_not_nil() &&
              old_symbol.value.is_nil()) ||
             (old_symbol.is_macro && old_symbol.value.is_not_nil() &&
              new_symbol.value.is_nil())) {
      link_warning(old_symbol, new_symbol,
    ...
    // mismatch on number of parameters is definitively an error
              ...
              new_symbol.value.is_not_nil() && !old_t.has_ellipsis()) ||
             (old_t.parameters().size() > new_t.parameters().size() &&
              old_symbol.value.is_not_nil() && !new_t.has_ellipsis())) {
      link_error(old_symbol, new_symbol,
      ...
      code_typet::parameterst::const_iterator n_it = new_t.parameters().begin(),
                                              o_it = old_t.parameters().begin();
      for (;
           o_it != old_t.parameters().end() && n_it != new_t.parameters().end();
           ++o_it, ++n_it) {
        if (!base_type_eq(o_it->type(), n_it->type(), ns))
          conflicts.push_back(std::make_pair(o_it->type(), n_it->type()));
      }
      ...
      while (!conflicts.empty()) {
        ...
        // _GNU_SOURCE consistent
                 ...
                 (t2.id() == ID_union && (t2.get_bool(ID_C_transparent_union) ||
                                          conflicts.front().second.get_bool(
          ...
          for (union_typet::componentst::const_iterator it =
               ...
               !found && it != union_type.components().end(); it++)
      ...
      if (!conflicts.empty()) {
        detailed_conflict_report(old_symbol, new_symbol,
                                 conflicts.front().first,
                                 conflicts.front().second);

        link_error(old_symbol, new_symbol, "conflicting function declarations");
      } else {
  ...
  if (!new_symbol.value.is_nil()) {
    if (old_symbol.value.is_nil()) {
      ...
      link_error(old_symbol, new_symbol, "duplicate definition of function");
  }
}
...
bool linkingt::adjust_object_type_rec(const typet &t1, const typet &t2,
                                      adjust_type_infot &info) {
  if (base_type_eq(t1, t2, ns))
    return false;

  if (t1.id() == ID_symbol || t1.id() == ID_struct_tag ||
    ...
    return false;
  } else if (t2.id() == ID_symbol || t2.id() == ID_struct_tag ||
  ...
  if (t1.id() == ID_pointer) {
...
#else
    link_warning(info.old_symbol, info.new_symbol,
                 "conflicting pointer types for variable");
#endif
    ...
    if ((old_size.is_nil() && new_size.is_not_nil()) ||
      ...
      info.set_to_new = true; // store new type
    } else if (new_size.is_nil() || new_size.is_zero() ||
      ...
      if (!simplify_expr(eq, ns).is_true())
        link_error(info.old_symbol, info.new_symbol,
    ...
    struct_union_typet::componentst::const_iterator it1 = components1.begin(),
                                                    it2 = components2.begin();
    for (; it1 != components1.end() && it2 != components2.end(); ++it1, ++it2) {
...
bool linkingt::adjust_object_type(const symbolt &old_symbol,
                                  const symbolt &new_symbol, bool &set_to_new) {
...
void linkingt::duplicate_object_symbol(symbolt &old_symbol,
  ...
  if (!base_type_eq(old_symbol.type, new_symbol.type, ns)) {
    ...
    bool failed = adjust_object_type(old_symbol, new_symbol, set_to_new);
    ...
    if (failed) {
      ...
      if (old_type.id() == ID_struct || old_type.id() == ID_union ||
          old_type.id() == ID_array || old_type.id() == ID_c_enum)
        detailed_conflict_report(old_symbol, new_symbol, old_symbol.type,
                                 new_symbol.type);

      link_error(old_symbol, new_symbol, "conflicting types for variable");
    } else if (set_to_new)
      old_symbol.type = new_symbol.type;
  }
  ...
  if (!new_symbol.value.is_nil() &&
    ...
    if (old_symbol.value.is_nil() ||
      ...
      // try simplifier
      exprt tmp_old = old_symbol.value, tmp_new = new_symbol.value;
...
void linkingt::duplicate_non_type_symbol(symbolt &old_symbol,
  ...
  if (is_code_old_symbol != is_code_new_symbol)
    link_error(old_symbol, new_symbol, "conflicting definition for symbol");
...
void linkingt::duplicate_type_symbol(symbolt &old_symbol, symbolt &new_symbol) {
  ...
  if (old_symbol.type.id() == ID_array && new_symbol.type.id() == ID_array &&
...
void linkingt::do_type_dependencies(id_sett &needs_to_be_renamed) {
  ...
  forall_symbols(s_it, src_symbol_table.symbols) {
    if (s_it->second.is_type) {
      ...
      for (find_symbols_sett::const_iterator it = symbols_used.begin();
           it != symbols_used.end(); it++) {
  ...
  for (id_sett::const_iterator d_it = needs_to_be_renamed.begin();
       d_it != needs_to_be_renamed.end(); d_it++)
    queue.push(*d_it);
  ...
  while (!queue.empty()) {
    ...
    for (id_sett::const_iterator d_it = u.begin(); d_it != u.end(); d_it++)
      if (needs_to_be_renamed.insert(*d_it).second) {
        queue.push(*d_it);
#ifdef DEBUG
        debug() << "LINKING: needs to be renamed (dependency): " << *d_it
                << eom;
#endif
      }
  }
...
void linkingt::rename_symbols(const id_sett &needs_to_be_renamed) {
  ...
  for (id_sett::const_iterator it = needs_to_be_renamed.begin();
       it != needs_to_be_renamed.end(); it++) {
...
#ifdef DEBUG
    debug() << "LINKING: renaming " << *it << " to " << new_identifier << eom;
#endif
...
void linkingt::copy_symbols() {
  ...
  Forall_symbols(s_it, src_symbol_table.symbols) {
    ...
    if (s_it->first != s_it->second.name) {
      ...
      symbol_tablet::symbolst::iterator m_it =
          main_symbol_table.symbols.find(s_it->first);
  ...
  for (id_sett::const_iterator i_it = collisions.begin();
       i_it != collisions.end(); i_it++) {
...
#ifdef DEBUG
      debug() << "LINKING: needs to be renamed: " << s_it->first << eom;
#endif
    }
  }
...
bool linking(symbol_tablet &dest_symbol_table, symbol_tablet &new_symbol_table,
  ...
  linkingt linking(dest_symbol_table, new_symbol_table, message_handler);
