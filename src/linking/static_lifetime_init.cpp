...
bool static_lifetime_init(symbol_tablet &symbol_table,
  ...
  forall_symbols(it, symbol_table.symbols) symbols.insert(id2string(it->first));
  ...
  for (const std::string &id : symbols) {
    ...
    if (identifier == CPROVER_PREFIX "constant_infinity_uint" ||
        ...
        identifier == "argc'" || identifier == "argv'" ||
        identifier == "envp'" || identifier == "envp_size'")
      continue;
    ...
    if (type.id() == ID_incomplete_struct || type.id() == ID_incomplete_union)
    ...
    if (symbol.value.is_nil()) {
      try {
        namespacet ns(symbol_table);
        rhs =
            zero_initializer(symbol.type, symbol.location, ns, message_handler);
