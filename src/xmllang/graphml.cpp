...
static bool build_graph_rec(
    const xmlt &xml, name_mapt &name_to_node,
    std::map<std::string, std::map<std::string, std::string>> &defaults,
    graphmlt &dest, std::string &entrynode) {
  if (xml.name == "node") {
    ...
    for (std::map<std::string, std::string>::const_iterator it =
             edge_defaults.begin();
         it != edge_defaults.end(); ++it) {
      bool found = false;
      for (xmlt::elementst::const_iterator e_it = xml.elements.begin();
           e_it != xml.elements.end() && !found; ++e_it)
...
static bool build_graph(const xmlt &xml, graphmlt &dest,
  ...
  const bool err =
      build_graph_rec(xml, name_to_node, defaults, dest, entrynode);
...
bool write_graphml(const graphmlt &src, std::ostream &os) {
  ...
  graphml.set_attribute("xmlns:xsi",
