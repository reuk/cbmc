...
bool path_symext::propagate(const exprt &src) {
  ...
  if (src.is_constant())
    ...
    forall_operands(it, src) if (!propagate(*it)) return false;
    return true;
...
void path_symext::symex_va_arg_next(path_symex_statet &state, const exprt &lhs,
                                    const side_effect_exprt &code) {
  if (code.operands().size() != 1)
...
void path_symext::assign_rec(path_symex_statet &state, exprt::operandst &guard,
                             const exprt &ssa_lhs, const exprt &ssa_rhs) {
  ...
  if (ssa_lhs.id() == ID_symbol) {
    ...
    for (std::size_t i = 0; i < components.size(); i++) {
      exprt new_rhs =
          ssa_rhs.is_nil()
              ? ssa_rhs
              : simplify_expr(member_exprt(ssa_rhs, components[i].get_name(),
                                           components[i].type()),
                              state.var_map.ns);
    ...
    for (std::size_t i = 0; i < operands.size(); i++) {
      exprt new_rhs =
          ssa_rhs.is_nil()
              ? ssa_rhs
              : simplify_expr(index_exprt(ssa_rhs,
...
void path_symext::function_call_rec(
    path_symex_statet &state, const code_function_callt &call,
    const exprt &function, std::list<path_symex_statet> &further_states) {
#ifdef DEBUG
