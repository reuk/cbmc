  ...
  const side_effect_exprt &code)
{
...

  {
    ...
    {
      object_type=
        array_typet(c_sizeof_type_rec(tmp_size.op0()), tmp_size.op1());
    }
    else
    {
...
static irep_idt
get_old_va_symbol(const path_symex_statet &state, const exprt &src)
{
  ...
  const side_effect_exprt &code)
{
  ...
  exprt rhs=
    zero_initializer(lhs.type(), code.source_location(), state.var_map.ns);
...
// std::cout << "ssa_lhs_type: " << ssa_lhs_type.id() << std::endl;
  ...
  else if(ssa_lhs.id() == ID_member)
  {
    ...
    if(compound_type.id() == ID_struct)
    {
      ...
      member_name.set(
        ID_component_name, ssa_lhs_member_expr.get_component_name());
    ...
    else if(compound_type.id() == ID_union)
    {
      ...
      byte_extract_exprt new_lhs(
        byte_update_id(), struct_op, offset, ssa_rhs.type());
  ...
  else if(ssa_lhs.id() == ID_if)
  {
    ...
    ssa_lhs.id() == ID_byte_extract_little_endian ||
    ssa_lhs.id() == ID_byte_extract_big_endian)
  {
    ...
    const byte_extract_exprt &byte_extract_expr= to_byte_extract_expr(ssa_lhs);
    ...
    for(std::size_t i= 0; i < components.size(); i++)
    {
      exprt new_rhs=
        ssa_rhs.is_nil()
          ...
          : simplify_expr(
              member_exprt(
                ssa_rhs, components[i].get_name(), components[i].type()),
              state.var_map.ns);
    ...
    for(std::size_t i= 0; i < operands.size(); i++)
    {
      exprt new_rhs=
        ssa_rhs.is_nil()
          ...
          : simplify_expr(
              index_exprt(
                ssa_rhs, from_integer(i, index_type()), array_type.subtype()),
              state.var_map.ns);
      assign_rec(state, guard, operands[i], new_rhs);
    }
  }
    ...
    for(std::size_t i= 0; i < operands.size(); i++)
    {
      exprt new_rhs=
        ssa_rhs.is_nil()
          ...
          : simplify_expr(
              index_exprt(
                ssa_rhs, from_integer(i, index_type()), vector_type.subtype()),
              state.var_map.ns);
      assign_rec(state, guard, operands[i], new_rhs);
    }
  }
  else if(
    ssa_lhs.id() == ID_string_constant || ssa_lhs.id() == "NULL-object" ||
    ssa_lhs.id() == "zero_string" || ssa_lhs.id() == "is_zero_string" ||
...
void path_symext::return_from_function(path_symex_statet &state)
{
  ...
  else
  {
    ...
    if(
      thread.call_stack.back().return_rhs.is_not_nil() &&
      thread.call_stack.back().return_lhs.is_not_nil())
      assign(
        state,
        thread.call_stack.back().return_lhs,
...
#ifdef DEBUG
  std::cout << "path_symext::operator(): " << state.pc() << " "
            << instruction.type << std::endl;
#endif
