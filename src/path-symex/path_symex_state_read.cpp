...
#ifdef DEBUG
  std::cout << "expand_structs_and_arrays: " << from_expr(var_map.ns, "", src)
            << std::endl;
#endif
...
exprt path_symex_statet::instantiate_rec(const exprt &src, bool propagate) {
#ifdef DEBUG
  std::cout << "instantiate_rec: " << from_expr(var_map.ns, "", src)
            << std::endl;
#endif
  ...
  if (is_symbol_member_index(src)) {
    exprt tmp_symbol_member_index = read_symbol_member_index(src, propagate);
...
exprt path_symex_statet::read_symbol_member_index(const exprt &src,
  ...
  if (final.id() == ID_struct || final.id() == ID_array ||
      final.id() == ID_vector) {
    Forall_operands(it, final) *it =
        read_symbol_member_index(*it, propagate); // rec. call
    return final;
  }
...
#ifdef DEBUG
  std::cout << "read_symbol_member_index_rec " << identifier << " var_info "
            << var_info.full_identifier << std::endl;
#endif
