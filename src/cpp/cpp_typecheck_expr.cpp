...
bool cpp_typecheckt::operator_is_overloaded(exprt &expr)
{
  ...
  for(const operator_entryt *e= operators; e->id != irep_idt(); e++)
    if(expr.id() == e->id)
    {
      ...
      if(
        expr.op0().type().id() == ID_symbol &&
        follow(expr.op0().type()).id() == ID_struct)
      {
        const irep_idt &struct_identifier= expr.op0().type().get(ID_identifier);
...
void cpp_typecheckt::typecheck_expr_delete(exprt &expr)
{
  ...
  codet destructor_code=
    cpp_destructor(expr.source_location(), pointer_type.subtype(), new_object);
  ...
  const cpp_typecheck_fargst &fargs)
{
  ...
  if(
    followed_op0_type.id() == ID_incomplete_struct ||
  ...
  if(followed_op0_type.id() != ID_struct && followed_op0_type.id() != ID_union)
  {
    error().source_location= expr.find_source_location();
  ...
  {
    cpp_namet component_cpp_name= to_cpp_name(expr.find(ID_component_cpp_name));
  ...
  if(get_component(expr.source_location(), expr.op0(), component_name, member))
  {
...
void cpp_typecheckt::typecheck_cast_expr(exprt &expr)
{
  ...
  if(f_op.get_sub().size() != 2 || f_op.get_sub()[1].id() != ID_template_args)
  {
    error().source_location= expr.find_source_location();
  ...
  const cpp_typecheck_fargst &fargs)
{
  ...
  if(expr.get_sub().size() == 1 && expr.get_sub()[0].id() == ID_name)
  {
    ...
    else if(identifier == "__atomic_load" || identifier == "__atomic_store")
    {
      ...
      identifier == "__atomic_compare_exchange")
    {
      ...
      identifier == "__atomic_xor_fetch" || identifier == "__atomic_or_fetch" ||
      identifier == "__atomic_nand_fetch")
    {
      ...
      identifier == "__atomic_fetch_add" ||
      identifier == "__atomic_fetch_sub" ||
      identifier == "__atomic_fetch_and" ||
      identifier == "__atomic_fetch_xor" || identifier == "__atomic_fetch_or" ||
  ...
  if(expr.get_sub().size() >= 1 && expr.get_sub().front().id() == ID_name)
  {
    ...
    if(
      id == ID_const_cast || id == ID_dynamic_cast ||
  ...
  if(expr.type().id() == ID_constructor)
  {
    ...
    // will be available
    {
          ...
          it++)
      {
        ...
        if(
          !it->get_bool(ID_from_base) && type.id() == ID_code &&
...
void cpp_typecheckt::explicit_typecast_ambiguity(exprt &expr)
{
  ...
  if(
    expr.type().id() == ID_cpp_name && expr.op0().operands().size() == 1 &&
