...
bool cpp_typecheckt::find_parent(const symbolt &symb, const irep_idt &base_name,
                                 irep_idt &identifier) {
...
void cpp_typecheckt::typecheck_expr_trinary(if_exprt &expr) {
  ...
  if (expr.op1().type() == expr.op2().type()) {
    ...
    if (implicit_conversion_sequence(expr.op1(), expr.op2().type(), e1)) {
      expr.type() = e1.type();
      expr.op1().swap(e1);
    } else if (implicit_conversion_sequence(expr.op2(), expr.op1().type(),
                                            e2)) {
      expr.type() = e2.type();
      expr.op2().swap(e2);
...
void cpp_typecheckt::typecheck_expr_sizeof(exprt &expr) {
  ...
  if (expr.operands().empty()) {
    ...
    if (type.id() == ID_cpp_name) {
      ...
      exprt symbol_expr = resolve(to_cpp_name(static_cast<const irept &>(type)),
      ...
      if (type.subtype().id() == ID_cpp_name) {
        ...
        exprt symbol_expr =
            resolve(to_cpp_name(static_cast<const irept &>(type.subtype())),
                    cpp_typecheck_resolvet::BOTH, fargs);
...
void cpp_typecheckt::typecheck_function_expr(
    exprt &expr, const cpp_typecheck_fargst &fargs) {
  if (expr.id() == ID_cpp_name)
...
bool cpp_typecheckt::overloadable(const exprt &expr) {
  ...
  forall_operands(it, expr) {
    ...
    if (t.id() == ID_struct || t.id() == ID_union || t.id() == ID_c_enum)
      return true;
  }
...
struct operator_entryt {
  ...
  const char *op_name;
    ...
    {ID_lt, "<"},          {ID_le, "<="},         {ID_gt, ">"},
    {ID_ge, ">="},         {ID_shl, "<<"},        {ID_shr, ">>"},
    {ID_notequal, "!="},   {ID_dereference, "*"}, {ID_ptrmember, "->"},
    {irep_idt(), NULL}};
...
bool cpp_typecheckt::operator_is_overloaded(exprt &expr) {
  ...
  if (expr.id() == "explicit-typecast") {
    ...
    if (t0.id() == ID_struct) {
      ...
      for (struct_typet::componentst::const_iterator it = components.begin();
        ...
        if (!it->get_bool(ID_from_base) && it->get(ID_base_name) == op_name) {
  ...
  for (const operator_entryt *e = operators; e->id != irep_idt(); e++)
    if (expr.id() == e->id) {
      ...
      if (expr.op0().type().id() == ID_symbol &&
        ...
        exprt resolve_result =
            resolve(cpp_name, cpp_typecheck_resolvet::VAR, fargs, false);
      ...
      // 2nd option!
      {
        ...
        if (resolve_result.is_not_nil()) {
          ...
          // now do arguments
          forall_operands(it, expr) function_call.arguments().push_back(*it);
...
void cpp_typecheckt::typecheck_expr_address_of(exprt &expr) {
  ...
  if (expr.op0().id() == ID_address_of && expr.op0().get_bool(ID_C_implicit)) {
...
void cpp_typecheckt::typecheck_expr_throw(exprt &expr) {
  ...
  assert(expr.operands().size() == 1 || expr.operands().empty());
...
void cpp_typecheckt::typecheck_expr_new(exprt &expr) {
  ...
  exprt code = cpp_constructor(expr.find_source_location(), object_expr,
...
void cpp_typecheckt::typecheck_expr_explicit_typecast(exprt &expr) {
  ...
  if (expr.operands().empty()) {
    ...
    typecheck_type(expr.type());
    exprt new_expr = ::zero_initializer(
        expr.type(), expr.find_source_location(), *this, get_message_handler());
    ...
    if (expr.type().id() == ID_cpp_name) {
      ...
      exprt symbol_expr =
          resolve(to_cpp_name(static_cast<const irept &>(expr.type())),
                  cpp_typecheck_resolvet::TYPE, fargs,
                  false); // fail silently
...
void cpp_typecheckt::typecheck_expr_delete(exprt &expr) {
  ...
  codet destructor_code = cpp_destructor(expr.source_location(),
                                         pointer_type.subtype(), new_object);
...
void cpp_typecheckt::typecheck_expr_member(exprt &expr,
  ...
  const struct_union_typet &type = to_struct_union_type(followed_op0_type);
  ...
  if (expr.find(ID_component_cpp_name).is_not_nil()) {
    ...
    assert(symbol_expr.id() == ID_symbol || symbol_expr.id() == ID_member ||
  ...
  if (get_component(expr.source_location(), expr.op0(), component_name,
                    member)) {
...
void cpp_typecheckt::typecheck_cast_expr(exprt &expr) {
  ...
  const irep_idt &id = f_op.get_sub().front().get(ID_identifier);
  ...
  if (template_arg.id() != ID_type && template_arg.id() != "ambiguous") {
    error().source_location = expr.find_source_location();
  ...
  typet &type =
      static_cast<typet &>(template_arguments.get_sub().front().add(ID_type));
...
void cpp_typecheckt::typecheck_expr_cpp_name(
  ...
  if (expr.get_sub().size() >= 1 && expr.get_sub().front().id() == ID_name) {
    const irep_idt &id = expr.get_sub().front().get(ID_identifier);

    if (id == ID_const_cast || id == ID_dynamic_cast ||
        id == ID_reinterpret_cast || id == ID_static_cast) {
...
void cpp_typecheckt::typecheck_side_effect_function_call(
  ...
  if (expr.type().id() == ID_constructor) {
    ...
    get_component(expr.source_location(), new_object,
                  expr.function().get(ID_identifier), member);
    ...
    // will be available
    {
      ...
      for (struct_typet::componentst::const_iterator it = components.begin();
        ...
        if (!it->get_bool(ID_from_base) && type.id() == ID_code &&
  ...
  for (const auto &parameter : parameters) {
    if (parameter.get_bool("#call_by_value")) {
      ...
      if (arg_it->id() != ID_temporary_object) {
        ...
        exprt temporary;
        new_temporary(arg_it->source_location(), parameter.type().subtype(),
                      arg, temporary);
  ...
  if (!expr.function().type().get_bool("#is_static")) {
    ...
    if (expr.arguments().size() == func_type.parameters().size()) {
      ...
      if (!base_type_eq(expr.arguments().front().type(),
                        func_type.parameters().front().type(), *this)) {
...
void cpp_typecheckt::typecheck_expr_dereference(exprt &expr) {
  ...
  if (op_type.id() == ID_pointer && op_type.find("to-member").is_not_nil()) {
    error().source_location = expr.find_source_location();
...
void cpp_typecheckt::convert_pmop(exprt &expr) {
  ...
  typet t0 = expr.op0().type().id() == ID_pointer ? expr.op0().type().subtype()
                                                  : expr.op0().type();
...
void cpp_typecheckt::explicit_typecast_ambiguity(exprt &expr) {
  ...
  if (expr.type().id() == ID_cpp_name && expr.op0().operands().size() == 1 &&
      (op0_id == ID_unary_plus || op0_id == ID_unary_minus ||
       op0_id == ID_address_of || op0_id == ID_dereference)) {
    exprt resolve_result =
        resolve(to_cpp_name(expr.type()), cpp_typecheck_resolvet::BOTH,
