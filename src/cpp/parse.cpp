...
protected:
  ...
  bool
  rConstructorDecl(cpp_declaratort &, typet &, typet &trailing_return_type);
...
bool Parser::rDefinition(cpp_itemt &item)
{
  ...
  if(t == ';')
    ...
    t == TOK_USING && lex.LookAhead(1) == TOK_IDENTIFIER &&
    lex.LookAhead(2) == '=')
...
bool Parser::isTypeSpecifier()
{
  ...
  if(
    ...
    t == TOK_GCC_INT128 || t == TOK_PTR32 || t == TOK_PTR64 ||
...
bool Parser::optStorageSpec(cpp_storage_spect &storage_spec)
{
  ...
  if(
    ...
    t == TOK_MUTABLE || t == TOK_GCC_ASM || t == TOK_THREAD_LOCAL)
  {
    cpp_tokent tk;
    lex.get_token(tk);
...
bool Parser::rTypeNameOrFunctionType(typet &tname)
{
  ...
  //                                    true, false>
  if(
    lex.LookAhead(0) == TOK_IDENTIFIER && lex.LookAhead(1) == TOK_SCOPE &&
    ...
    lex.LookAhead(4) == '(')
  {
    ...
    lex.LookAhead(0) == TOK_IDENTIFIER && lex.LookAhead(1) == ')' &&
    lex.LookAhead(2) == '(')
  {
    lex.get_token(op);
    ...
    lex.LookAhead(0) == '*' && lex.LookAhead(1) == TOK_IDENTIFIER &&
    lex.LookAhead(2) == ')' && lex.LookAhead(3) == '(')
  {
    lex.get_token(op);
    lex.get_token(op);
...
bool Parser::moreVarName()
{
  if(lex.LookAhead(0) == TOK_SCOPE)
  {
    int t= lex.LookAhead(1);
    if(
      t == TOK_IDENTIFIER || t == '~' || t == TOK_OPERATOR || t == TOK_TEMPLATE)
      return true;
  }
