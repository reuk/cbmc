...
public:
  ...
  bool is_type() const {
    return kind == TYPEDEF || kind == TYPE_TEMPLATE_PARAMETER || kind == TAG ||
           kind == CLASS_TEMPLATE;
  }

  bool is_template() const {
    return kind == FUNCTION_TEMPLATE || kind == CLASS_TEMPLATE ||
  ...
  inline void print(std::ostream &out) const { print_rec(out, 0); }
  ...
  std::string full_name() const {
    return (parent == NULL ? "" : (parent->full_name() + "::")) + id2string(id);
  }
...
public:
  ...
  inline ~save_scopet() { scope_ptr = old_scope; }
...
void new_scopet::print_rec(std::ostream &out, unsigned indent) const {
  for (id_mapt::const_iterator it = id_map.begin(); it != id_map.end(); it++) {
...
public:
  explicit Parser(cpp_parsert &_cpp_parser)
      : lex(_cpp_parser.token_buffer), parser(_cpp_parser) {
...
protected:
  ...
  bool rIntegralDeclaration(cpp_declarationt &, cpp_storage_spect &,
                            cpp_member_spect &, typet &, typet &);
  bool rConstDeclaration(cpp_declarationt &, cpp_storage_spect &,
                         cpp_member_spect &, typet &);
  bool rOtherDeclaration(cpp_declarationt &, cpp_storage_spect &,
                         cpp_member_spect &, typet &);
...
bool Parser::rDefinition(cpp_itemt &item) {
  ...
  if (t == ';')
    ...
    return rNamespaceSpec(item.make_namespace_spec());
  else if (t == TOK_USING && lex.LookAhead(1) == TOK_IDENTIFIER &&
           lex.LookAhead(2) == '=')
...
bool Parser::isTypeSpecifier() {
  ...
  if (t == TOK_IDENTIFIER || t == TOK_SCOPE || t == TOK_CONSTEXPR ||
      ...
      t == TOK_PTR64 || t == TOK_GCC_FLOAT128 || t == TOK_VOID ||
      t == TOK_BOOL || t == TOK_CPROVER_BOOL || t == TOK_CLASS ||
...
bool Parser::rTemplateDecl(cpp_declarationt &decl) {
  ...
  case tdk_decl:
#ifdef DEBUG
    std::cout << std::string(__indent, ' ') << "BODY: " << body.pretty()
              << '\n';
    std::cout << std::string(__indent, ' ')
              << "TEMPLATE_TYPE: " << template_type.pretty() << '\n';
#endif
...
bool Parser::rTempArgDeclaration(cpp_declarationt &declaration) {
  ...
  if (t0 == TOK_TEMPLATE) {
    ...
    if (lex.get_token(tk1) != TOK_CLASS || lex.get_token(tk2) != TOK_IDENTIFIER)
      return false;
...
bool Parser::rDeclaration(cpp_declarationt &declaration) {
  ...
  indenter _i;
  std::cout << std::string(__indent, ' ')
            << "Parser::rDeclaration 0.1  token: " << lex.LookAhead(0)
            << std::endl;
#endif
...
#endif
    return rIntegralDeclaration(declaration, storage_spec, member_spec,
                                integral, cv_q);
  } else {
    int t = lex.LookAhead(0);
...
#ifdef DEBUG
      std::cout << std::string(__indent, ' ')
                << "Parser::rIntegralDeclaration 8 " << declaration.pretty()
                << '\n';
#endif
      lex.get_token(tk);
      return true;
...
bool Parser::rConstDeclaration(cpp_declarationt &declaration,
                               cpp_storage_spect &storage_spec,
                               cpp_member_spect &member_spec, typet &cv_q) {
#ifdef DEBUG
  indenter _i;
...
bool Parser::rOtherDeclaration(cpp_declarationt &declaration,
  ...
  if (is_operator && is_constructor) {
    ...
    forall_irep(it, type_name.get_sub()) if (it->id() == "~") {
      is_destructor = true;
      break;
    }
...
bool Parser::isConstructorDecl() {
  ...
  if (lex.LookAhead(0) != '(')
    ...
    else if (t == TOK_STDCALL || t == TOK_FASTCALL || t == TOK_CLRCALL ||
             t == TOK_CDECL)
      return false; // it's a declarator
...
bool Parser::optStorageSpec(cpp_storage_spect &storage_spec) {
  ...
  if (t == TOK_STATIC || t == TOK_EXTERN || t == TOK_AUTO ||
    ...
    case TOK_MUTABLE:
      ...
      break;
    case TOK_GCC_ASM:
      storage_spec.set_asm();
      break;
    case TOK_THREAD_LOCAL:
      storage_spec.set_thread_local();
      break;
    default:
      assert(false);
    }
...
bool Parser::optCvQualify(typet &cv) {
  for (;;) {
    int t = lex.LookAhead(0);
    if (t == TOK_CONSTEXPR || t == TOK_CONST || t == TOK_VOLATILE ||
...
bool Parser::optIntegralTypeOrClassSpec(typet &p) {
  ...
  for (;;) {
    ...
    case TOK_INT64:
      ...
      break;
    case TOK_GCC_INT128:
      type_id = ID_gcc_int128;
      break;
    case TOK_GCC_FLOAT128:
      type_id = ID_gcc_float128;
      break;
    case TOK_BOOL:
      type_id = ID_bool;
      break;
    case TOK_CPROVER_BOOL:
      type_id = ID_proper_bool;
      break;
    default:
      type_id = irep_idt();
    }
...
/*
  ...
  | declarator
    {'=' initialize.expr |
     ':' expression}
*/
bool Parser::rDeclaratorWithInit(cpp_declaratort &dw, bool should_be_declarator,
                                 bool is_statement) {
  if (lex.LookAhead(0) == ':') {
    ...
    if (!rDeclarator(declarator, kDeclarator, false, should_be_declarator,
                     is_statement))
      return false;
...
bool Parser::rDeclarator(cpp_declaratort &declarator, DeclKind kind,
                         bool recursive, bool should_be_declarator,
                         bool is_statement) {
  int t;
  ...
  if (t == '(') {
    ...
    if (!should_be_declarator) {
      if ((kind == kDeclarator || kind == kCastDeclarator) &&
          d_outer.is_nil()) {
        t = lex.LookAhead(0);
        if (t != '[' && t != '(')
          return false;
...
bool Parser::rMemberInit(exprt &init) {
  ...
  if (tk1.kind == '{' || (tk1.kind == '(' && lex.LookAhead(0) == '{')) {
#ifdef DEBUG
    ...
    if (rTypeNameOrFunctionType(a) &&
        ((lex.LookAhead(0) == '>' || lex.LookAhead(0) == ',' ||
         ...
         (lex.LookAhead(0) == TOK_ELLIPSIS &&
          (lex.LookAhead(1) == '>' || lex.LookAhead(1) == TOK_SHIFTRIGHT)))) {
#ifdef DEBUG
...
*/
bool Parser::rArgDeclListOrInit(exprt &arglist, bool &is_args,
                                bool maybe_init) {
  cpp_token_buffert::post pos = lex.Save();
  if (maybe_init) {
...
bool Parser::rEnumSpec(typet &spec) {
  ...
  if (lex.LookAhead(0) == '}') {
    ...
    spec.add(ID_body);
  } else if (!rEnumBody(spec.add(ID_body)))
    return false;
...
bool Parser::rExpression(exprt &exp, bool template_args) {
  ...
  if (t == '=' || t == TOK_MULTASSIGN || t == TOK_DIVASSIGN ||
...
bool Parser::rRelationalExpr(exprt &exp, bool template_args) {
  ...
  while (t = lex.LookAhead(0), (t == TOK_LE || t == TOK_GE || t == '<' ||
                                (t == '>' && !template_args))) {
    cpp_tokent tk;
    lex.get_token(tk);
...
bool Parser::rTypeNameOrFunctionType(typet &tname) {
  ...
  //                                    true, false>
  if (lex.LookAhead(0) == TOK_IDENTIFIER && lex.LookAhead(1) == TOK_SCOPE &&
    ...
    lex.get_token();
    lex.get_token();
  } else if (lex.LookAhead(0) == TOK_IDENTIFIER && lex.LookAhead(1) == ')' &&
             lex.LookAhead(2) == '(') {
    lex.get_token(op);
    type.set(ID_identifier, op.data.get(ID_C_base_name));
    lex.get_token();
    lex.get_token();
  } else if (lex.LookAhead(0) == '*' && lex.LookAhead(1) == TOK_IDENTIFIER &&
             lex.LookAhead(2) == ')' && lex.LookAhead(3) == '(') {
    lex.get_token(op);
    lex.get_token(op);
    type.set(ID_identifier, op.data.get(ID_C_base_name));
  ...
  for (;;) {
    ...
    case '(':
...

      {
        ...
        Forall_operands(it, e) fc.arguments().push_back(*it);
...
bool Parser::rMSC_if_existsExpr(exprt &expr) {
  ...
  expr = exprt(tk1.kind == TOK_MSC_IF_EXISTS ? ID_msc_if_exists
...
bool Parser::rMSC_if_existsStatement(codet &code) {
  ...
  code = codet(tk1.kind == TOK_MSC_IF_EXISTS ? ID_msc_if_exists
                                             : ID_msc_if_not_exists);
...
bool Parser::rFunctionBody(cpp_declaratort &declarator) {
  ...
  if (lex.LookAhead(0) == '{' && lex.LookAhead(1) == TOK_ASM_STRING) {
    cpp_tokent ob, tk, cb;
    lex.get_token(ob);
...
bool Parser::rStatement(codet &statement) {
  ...
  case TOK_USING: {
    if (lex.LookAhead(1) == TOK_IDENTIFIER && lex.LookAhead(2) == '=') {
      statement = codet(ID_decl);
      statement.operands().resize(1);
...
bool Parser::rOtherDeclStatement(codet &statement,
                                 cpp_storage_spect &storage_spec, typet &cv_q) {
  typet type_name;
  cpp_tokent tk;
...
bool Parser::MaybeTypeNameOrClassTemplate(cpp_tokent &) { return true; }
