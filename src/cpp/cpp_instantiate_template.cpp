...
void cpp_typecheckt::show_instantiation_stack(std::ostream &out) {
  for (instantiation_stackt::const_iterator s_it = instantiation_stack.begin();
       s_it != instantiation_stack.end(); s_it++) {
...
const symbolt &cpp_typecheckt::class_template_symbol(
  ...
  new_symbol.type.set("specialization_template_args",
...
void cpp_typecheckt::elaborate_class_template(const typet &type) {
  ...
  if (t_type.id() == ID_incomplete_struct &&
    ...
    instantiate_template(type.source_location(),
