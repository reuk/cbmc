...
cpp_typecheckt::template_suffix(const cpp_template_args_tct &template_args)
{
  ...
  const cpp_template_args_tct::argumentst &arguments= template_args.arguments();

  for(cpp_template_args_tct::argumentst::const_iterator it= arguments.begin();
      it != arguments.end();
      it++)
  {
    if(first)
  ...
  const cpp_template_args_tct &full_template_args)
{
  ...
  irep_idt identifier= id2string(template_scope->prefix) + "tag-" +
                       id2string(template_symbol.base_name) + id2string(suffix);
  ...
  id.is_scope= true;
  id.prefix= template_scope->prefix + id2string(s_ptr->base_name) +
             id2string(suffix) + "::";
...
void cpp_typecheckt::elaborate_class_template(const typet &type)
{
  ...
  if(
    t_type.id() == ID_incomplete_struct &&
  ...
  const typet &specialization)
{
  ...
  if(new_decl.type().id() == ID_struct)
  {
    ...
    for(unsigned i= 0; i < template_methods.operands().size(); i++)
    {
      ...
      cpp_declarationt method_decl= static_cast<const cpp_declarationt &>(
  ...
  if(is_template_method)
  {
    symbol_tablet::symbolst::iterator it= symbol_table.symbols.find(class_name);
