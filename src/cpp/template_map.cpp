...
void template_mapt::apply(typet &type) const {
  if (type.id() == ID_array) {
    ...
    type_mapt::const_iterator m_it = type_map.find(type.get(ID_identifier));
    ...
    Forall_subtypes(it, type) apply(*it);
  }
}
...
void template_mapt::apply(exprt &expr) const {
  ...
  if (expr.id() == ID_symbol) {
    expr_mapt::const_iterator m_it = expr_map.find(expr.get(ID_identifier));
  ...
  Forall_operands(it, expr) apply(*it);
}
...
void template_mapt::print(std::ostream &out) const {
  for (type_mapt::const_iterator it = type_map.begin(); it != type_map.end();
       it++)
    out << it->first << " = " << it->second.pretty() << std::endl;

  for (expr_mapt::const_iterator it = expr_map.begin(); it != expr_map.end();
       it++)
    out << it->first << " = " << it->second.pretty() << std::endl;
}
...
void template_mapt::build(const template_typet &template_type,
  ...
  if (instance.size() < template_parameters.size()) {
    ...
    for (std::size_t i = instance.size(); i < template_parameters.size(); i++) {
...
void template_mapt::build_unassigned(const template_typet &template_type) {
  ...
  for (template_typet::template_parameterst::const_iterator t_it =
           template_parameters.begin();
       t_it != template_parameters.end(); t_it++) {
...
template_mapt::build_template_args(const template_typet &template_type) const {
  ...
  for (std::size_t i = 0; i < template_parameters.size(); i++) {
    ...
    if (t.id() == ID_type) {
      ...
      template_args.arguments()[i] = lookup_expr(t.get(ID_identifier));
    }
  }
