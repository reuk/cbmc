...
public:
  ...
  bool has_tag() const { return find(ID_tag).is_not_nil(); }

  cpp_namet &tag() { return static_cast<cpp_namet &>(add(ID_tag)); }

  const irept &body() const { return find(ID_body); }

  irept &body() { return add(ID_body); }

  bool has_body() const { return find(ID_body).is_not_nil(); }
