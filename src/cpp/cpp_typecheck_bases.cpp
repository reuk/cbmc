...
void cpp_typecheckt::typecheck_compound_bases(struct_typet &type) {
  ...
  Forall_irep(base_it, bases_irep) {
    ...
    add_base_components(base_struct_type, class_access, type, bases, vbases,
                        virtual_base);
  }
  ...
  if (!vbases.empty()) {
    ...
    most_derived.set_name(cpp_scopes.current_scope().prefix +
                          "::" + "@most_derived");
...
void cpp_typecheckt::add_base_components(
    const struct_typet &from, const irep_idt &access, struct_typet &to,
    std::set<irep_idt> &bases, std::set<irep_idt> &vbases, bool is_virtual) {
  ...
  forall_irep(it, from.find(ID_bases).get_sub()) {
    ...
    // recursive call
    add_base_components(to_struct_type(symb.type), sub_access, to, bases,
                        vbases, is_virtual);
  }
