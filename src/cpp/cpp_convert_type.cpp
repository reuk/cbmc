...
public:
  unsigned unsigned_cnt, signed_cnt, char_cnt, int_cnt, short_cnt, long_cnt,
      const_cnt, restrict_cnt, constexpr_cnt, volatile_cnt, double_cnt,
      float_cnt, complex_cnt, cpp_bool_cnt, proper_bool_cnt, extern_cnt,
      wchar_t_cnt, char16_t_cnt, char32_t_cnt, int8_cnt, int16_cnt, int32_cnt,
...
void cpp_convert_typet::read_rec(const typet &type) {
  ...
  if (type.id() == ID_merged_type) {
    forall_subtypes(it, type) read_rec(*it);
...
void cpp_convert_typet::read_function_type(const typet &type) {
  ...
  typet &return_type = static_cast<typet &>(t.add(ID_return_type));
...
void cpp_convert_typet::write(typet &type) {
  ...
  if (!other.empty()) {
    if (double_cnt || float_cnt || signed_cnt || unsigned_cnt || int_cnt ||
        cpp_bool_cnt || proper_bool_cnt || short_cnt || char_cnt ||
        wchar_t_cnt || char16_t_cnt || char32_t_cnt || int8_cnt || int16_cnt ||
        int32_cnt || int64_cnt || float128_cnt || int128_cnt)
    ...
    if (signed_cnt || unsigned_cnt || int_cnt || cpp_bool_cnt ||
        proper_bool_cnt || short_cnt || char_cnt || wchar_t_cnt ||
        char16_t_cnt || char32_t_cnt || float_cnt || int8_cnt || int16_cnt ||
        int32_cnt || int64_cnt || ptr32_cnt || ptr64_cnt || float128_cnt ||
        int128_cnt)
    ...
    if (signed_cnt || unsigned_cnt || int_cnt || cpp_bool_cnt ||
        proper_bool_cnt || short_cnt || char_cnt || wchar_t_cnt || double_cnt ||
        char16_t_cnt || char32_t_cnt || int8_cnt || int16_cnt || int32_cnt ||
        int64_cnt || ptr32_cnt || ptr64_cnt || float128_cnt || int128_cnt)
    ...
    if (signed_cnt || unsigned_cnt || int_cnt || short_cnt || char_cnt ||
        wchar_t_cnt || proper_bool_cnt || char16_t_cnt || char32_t_cnt ||
        int8_cnt || int16_cnt || int32_cnt || int64_cnt || int128_cnt ||
        ptr32_cnt || ptr64_cnt)
    ...
    if (signed_cnt || unsigned_cnt || int_cnt || short_cnt || char_cnt ||
        wchar_t_cnt || char16_t_cnt || char32_t_cnt || int8_cnt || int16_cnt ||
        int32_cnt || int64_cnt || int128_cnt || ptr32_cnt || ptr64_cnt)
      throw "illegal type modifier for __CPROVER_bool";

    type.id(ID_bool);
  } else if (char_cnt) {
    if (int_cnt || short_cnt || wchar_t_cnt || long_cnt || char16_t_cnt ||
        char32_t_cnt || int8_cnt || int16_cnt || int32_cnt || int64_cnt ||
        int128_cnt || ptr32_cnt || ptr64_cnt)
    ...
    if (int_cnt || short_cnt || char_cnt || long_cnt || char16_t_cnt ||
        char32_t_cnt || int8_cnt || int16_cnt || int32_cnt || int64_cnt ||
        ptr32_cnt || ptr64_cnt)
    ...
    // This is a distinct type, and can't be made signed/unsigned.
    if (int_cnt || short_cnt || char_cnt || long_cnt || char32_t_cnt ||
        int8_cnt || int16_cnt || int32_cnt || int64_cnt || ptr32_cnt ||
    ...
    // This is a distinct type, and can't be made signed/unsigned.
    if (int_cnt || short_cnt || char_cnt || long_cnt || int8_cnt || int16_cnt ||
        int32_cnt || int64_cnt || ptr32_cnt || ptr64_cnt || signed_cnt ||
        unsigned_cnt)
...
void cpp_convert_plain_type(typet &type) {
  if (type.id() == ID_cpp_name || type.id() == ID_struct ||
      type.id() == ID_union || type.id() == ID_pointer ||
      type.id() == ID_array || type.id() == ID_code ||
      type.id() == ID_unsignedbv || type.id() == ID_signedbv ||
      type.id() == ID_bool || type.id() == ID_floatbv ||
      type.id() == ID_empty || type.id() == ID_symbol ||
