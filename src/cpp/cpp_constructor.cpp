  ...
  const exprt::operandst &operands)
{
  ...
  if(tmp_type.id() == ID_array)
  {
    ...
    else*/
    {
      ...
      for(mp_integer i= 0; i < s; ++i)
      {
        ...
        exprt i_code= cpp_constructor(source_location, index, tmp_operands);
  ...
  else if(cpp_is_pod(tmp_type))
  {
    ...
    for(exprt::operandst::iterator it= operands_tc.begin();
        it != operands_tc.end();
        it++)
    {
      typecheck_expr(*it);
  ...
  else if(tmp_type.id() == ID_struct)
  {
    ...
    exprt &initializer= static_cast<exprt &>(function_call.add(ID_initializer));
    ...
    assert(
      tmp_this.id() == ID_address_of && tmp_this.op0().id() == "new_object");
  ...
  exprt &temporary)
{
  ...
  codet new_code= cpp_constructor(source_location, new_object, ops);
