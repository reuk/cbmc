...
public:
  ...
  symbolt &convert(const typet &type, // already typechecked
                   const cpp_storage_spect &storage_spec,
                   const cpp_member_spect &member_spec,
                   cpp_declaratort &declarator);

  symbolt &convert(const cpp_declarationt &declaration,
                   cpp_declaratort &declarator) {
    return convert(declaration.type(), declaration.storage_spec(),
...
protected:
  ...
  symbolt &convert_new_symbol(const cpp_storage_spect &storage_spec,
                              const cpp_member_spect &member_spec,
                              cpp_declaratort &declarator);
  ...
  void combine_types(const source_locationt &source_location,
                     const typet &decl_type, symbolt &symbol);
};
