...
bool cpp_typecheckt::standard_conversion_lvalue_to_rvalue(
  ...
  if (expr.type().id() == ID_code || expr.type().id() == ID_incomplete_struct ||
...
bool cpp_typecheckt::standard_conversion_integral_conversion(
  ...
  if (expr.type().id() != ID_signedbv && expr.type().id() != ID_unsignedbv &&
      expr.type().id() != ID_bool && expr.type().id() != ID_c_enum_tag)
    return false;
...
bool cpp_typecheckt::standard_conversion_pointer(const exprt &expr,
  ...
  if (simplify_expr(expr, *this).is_zero() && expr.type().id() != ID_pointer) {
    new_expr = expr;
...
bool cpp_typecheckt::standard_conversion_pointer_to_member(const exprt &expr,
  ...
  if (expr.type().id() != ID_pointer || expr.type().find("to-member").is_nil())
    return false;
...
bool cpp_typecheckt::standard_conversion_boolean(const exprt &expr,
  ...
  if (expr.type().id() != ID_signedbv && expr.type().id() != ID_unsignedbv &&
      expr.type().id() != ID_pointer && expr.type().id() != ID_c_enum_tag)
    return false;
...
bool cpp_typecheckt::standard_conversion_sequence(const exprt &expr,
  ...
  if (follow(type).id() == ID_c_enum &&
    ...
    if (follow(type).find(ID_tag) == follow(curr_expr.type()).find(ID_tag))
      return true;
    else {
  ...
  if (follow(curr_expr.type()) != follow(type) ||
    ...
    if (type.id() == ID_signedbv || type.id() == ID_unsignedbv ||
      ...
      if (!standard_conversion_integral_promotion(curr_expr, new_expr) ||
          new_expr.type() != type) {
        if (!standard_conversion_integral_conversion(curr_expr, type,
                                                     new_expr)) {
          if (!standard_conversion_floating_integral_conversion(curr_expr, type,
                                                                new_expr))
            return false;
        }
      ...
      if (!standard_conversion_floating_point_promotion(curr_expr, new_expr) ||
          new_expr.type() != type) {
        if (!standard_conversion_floating_point_conversion(curr_expr, type,
                                                           new_expr) &&
            !standard_conversion_floating_integral_conversion(curr_expr, type,
                                                              new_expr))
          return false;
    ...
    if (cpp_is_pod(to)) {
      ...
      for (struct_typet::componentst::const_iterator it =
        ...
        if (arg1_struct.is_nil()) {
          exprt tmp_expr;
          if (standard_conversion_sequence(expr, arg1_type, tmp_expr,
                                           tmp_rank)) {
          ...
          exprt expr_ptmp;
          tmp_rank = 0;
          if (standard_conversion_sequence(expr_pfrom, pto, expr_ptmp,
                                           tmp_rank)) {
            // check if it's ambiguous
            if (found)
              return false;
  ...
  if (from.id() == ID_struct) {
    ...
    for (struct_typet::componentst::const_iterator it =
      ...
      typet this_type = static_cast<const typet &>(
          comp_type.find(ID_parameters).get_sub().front().find(ID_type));
      ...
      if (implicit_conversion_sequence(this_expr, this_type, tmp_expr,
                                       tmp_rank)) {
...
bool cpp_typecheckt::reference_related(const exprt &expr,
  ...
  if (from.id() == ID_struct && type.get_bool(ID_C_this) &&
...
bool cpp_typecheckt::reference_compatible(const exprt &expr, const typet &type,
                                          unsigned &rank) const {
  assert(is_reference(type));
  assert(!is_reference(expr.type()));
...
bool cpp_typecheckt::reference_binding(exprt expr, const typet &type,
  ...
  if (type.get_bool(ID_C_this) && !expr.get_bool(ID_C_lvalue)) {
  ...
  if (!type.subtype().get_bool(ID_C_constant) && !expr.get_bool(ID_C_lvalue))
    return false;
  ...
  if (user_defined_conversion_sequence(arg_expr, type.subtype(), new_expr,
                                       rank)) {
    address_of_exprt tmp;
    tmp.type() = pointer_typet();
    tmp.object() = new_expr;
...
bool cpp_typecheckt::const_typecast(const exprt &expr, const typet &type,
                                    exprt &new_expr) {
...
bool cpp_typecheckt::dynamic_typecast(const exprt &expr, const typet &type,
                                      exprt &new_expr) {
  exprt e(expr);
...
bool cpp_typecheckt::reinterpret_typecast(const exprt &expr, const typet &type,
                                          exprt &new_expr,
  ...
  if ((e.type().id() == ID_unsignedbv || e.type().id() == ID_signedbv ||
       e.type().id() == ID_bool) &&
  ...
  if (e.type().id() == ID_pointer && type.id() == ID_pointer &&
      !is_reference(type)) {
...
bool cpp_typecheckt::static_typecast(const exprt &expr, // source expression
                                     const typet &type, // destination type
                                     exprt &new_expr, bool check_constantness) {
  exprt e = expr;
