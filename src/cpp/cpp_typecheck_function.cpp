...
void cpp_typecheckt::convert_parameter(const irep_idt &mode,
...
void cpp_typecheckt::convert_function(symbolt &symbol) {
  ...
  if (function_scope.is_method && !function_scope.is_static_member) {
    ...
    function_scope.this_expr.set(ID_identifier,
                                 this_parameter_expr.get(ID_C_identifier));
  } else
...
irep_idt cpp_typecheckt::function_identifier(const typet &type) {
  ...
  if (it != parameters.end() && it->get_identifier() == ID_this) {
