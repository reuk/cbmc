...
void cpp_typecheckt::typecheck_try_catch(codet &code) {
  ...
  for (codet::operandst::iterator it = operands.begin(); it != operands.end();
       it++) {
    if (it == operands.begin()) {
      ...
      if (to_code(code.op0()).get_statement() == ID_ellipsis) {
        ...
        // get the declaration
        const code_declt &code_decl = to_code_decl(to_code(code.op0().op0()));
...
void cpp_typecheckt::typecheck_member_initializer(codet &code) {
  ...
  Forall_operands(it, code) typecheck_expr(*it);
  ...
  exprt symbol_expr = resolve(member, cpp_typecheck_resolvet::VAR, fargs);
  ...
  if (symbol_expr.type().id() == ID_code) {
    ...
    make_ptr_typecast(this_expr, code_type.parameters().front().type());

    function_call.arguments().push_back(this_expr);

    forall_operands(it, code) function_call.arguments().push_back(*it);
    ...
    if (symbol_expr.id() == ID_symbol && symbol_expr.type().id() != ID_code) {
      ...
      cpp_typecheck_fargst fargs;
      exprt dereference(ID_dereference,
                        cpp_scopes.current_scope().this_expr.type().subtype());
    ...
    if (symbol_expr.id() == ID_member &&
      ...
      if (is_reference(symbol_expr.type())) {
        ...
        Forall_operands(it, code) already_typechecked(*it);

        exprt call = cpp_constructor(code.source_location(), symbol_expr,
                                     code.operands());
