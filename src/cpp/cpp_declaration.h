...
public:
  ...
  bool is_empty() const { return type().is_nil() && !has_operands(); }

  bool is_constructor() const { return type().id() == ID_constructor; }

  bool is_static_assert() const { return get_bool(ID_is_static_assert); }

  bool is_destructor() const { return type().id() == ID_destructor; }

  bool is_template() const { return get_bool(ID_is_template); }
  ...
  irep_idt get_specialization_of() const { return get("specialization_of"); }

  void set_is_typedef() { set(ID_is_typedef, true); }

  bool is_typedef() const { return get_bool(ID_is_typedef); }
