...
cpp_declarator_convertert::cpp_declarator_convertert(
    ...
    : is_typedef(false), is_template(false), is_template_parameter(false),
      is_friend(false), linkage_spec(_cpp_typecheck.current_linkage_spec),
...
symbolt &cpp_declarator_convertert::convert(
    const typet &declaration_type, const cpp_storage_spect &storage_spec,
    const cpp_member_spect &member_spec, cpp_declaratort &declarator) {
  ...
  // run resolver on scope
  {
    ...
    cpp_typecheck_resolve.resolve_scope(declarator.name(), base_name,
                                        template_args);
  ...
  if (scope->id_class == cpp_idt::CLASS && !is_friend) {
    ...
    if (c_it == cpp_typecheck.symbol_table.symbols.end()) {
      ...
      if (final_type.id() == ID_code)
        cpp_typecheck.add_this_to_method_type(scope->identifier, final_type,
                                              method_qualifier);
    ...
    if (final_type.get(ID_return_type) == ID_constructor) {
      ...
      if (symbol_expr.id() != ID_type || symbol_expr.type().id() != ID_symbol) {
        cpp_typecheck.error().source_location = name.source_location();
        cpp_typecheck.error() << "error: expected type" << messaget::eom;
        throw 0;
      }
      ...
      cpp_typecheck.check_member_initializers(type.find(ID_bases),
    ...
    if (symbol.type.id() == "cpp-template-type") {
      ...
      scope->lookup_identifier(symbol.name, cpp_idt::TEMPLATE_PARAMETER,
                               id_set);
...
void cpp_declarator_convertert::combine_types(
    const source_locationt &source_location, const typet &decl_type,
    symbolt &symbol) {
  if (symbol.type.id() == decl_type.id() && decl_type.id() == ID_code) {
    ...
    return; // ok
  else if (symbol.type.id() == ID_array && symbol.type.find(ID_size).is_nil() &&
...
void cpp_declarator_convertert::handle_initializer(
    symbolt &symbol, cpp_declaratort &declarator) {
  exprt &value = declarator.value();

  // moves member initializers into 'value'
  cpp_typecheck.move_member_initializers(declarator.member_initializers(),
                                         symbol.type, value);
...
void cpp_declarator_convertert::get_final_identifier() {
  ...
  if (is_code) {
    if (linkage_spec == ID_C) {
      ...
      symbol_tablet::symbolst::const_iterator c_it =
...
symbolt &cpp_declarator_convertert::convert_new_symbol(
    const cpp_storage_spect &storage_spec, const cpp_member_spect &member_spec,
  ...
  if (symbol.type.get_bool(ID_C_constant) && symbol.value.is_not_nil())
  ...
  if (!is_code) {
    ...
    for (cpp_scopest::id_sett::const_iterator id_it = id_set.begin();
