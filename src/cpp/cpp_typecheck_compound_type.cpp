...
bool cpp_typecheckt::has_const(const typet &type) {
  if (type.id() == ID_const)
    ...
    forall_subtypes(it, type) if (has_const(*it)) return true;
...
bool cpp_typecheckt::has_volatile(const typet &type) {
  if (type.id() == ID_volatile)
    ...
    forall_subtypes(it, type) if (has_volatile(*it)) return true;
...
cpp_scopet &cpp_typecheckt::tag_scope(const irep_idt &base_name, bool has_body,
...
void cpp_typecheckt::typecheck_compound_type(struct_union_typet &type) {
  ...
  if (previous_symbol != symbol_table.symbols.end()) {
    ...
    symbol.type.set(ID_tag, cpp_scopes.current_scope().prefix +
                                id2string(symbol.base_name));
...
void cpp_typecheckt::typecheck_compound_declarator(
    const symbolt &symbol, const cpp_declarationt &declaration,
    cpp_declaratort &declarator, struct_typet::componentst &components,
    const irep_idt &access, bool is_static, bool is_typedef, bool is_mutable) {
  ...
  if (is_method) {
    ...
    if (!is_virtual) {
      typecheck_member_function(symbol.name, component, initializers,
    ...
    } else // virtual
    {
      ...
      while (!virtual_bases.empty()) {
        ...
        for (auto &arg : args) {
          ...
          symbolt arg_symb;
          arg_symb.name =
              id2string(func_symb.name) + "::" + id2string(base_name);
        ...
        late_cast.op0() = namespacet(symbol_table)
                              .lookup(args[0].get(ID_C_identifier))
                              .symbol_expr();
          ...
          for (unsigned i = 1; i < args.size(); i++) {
            expr_call.arguments().push_back(
                namespacet(symbol_table)
                    .lookup(args[i].get(ID_C_identifier))
                    .symbol_expr());
          }
  ...
  if (is_static && !is_method) // static non-method member
  {
    ...
    if (value.is_not_nil()) {
      if (cpp_is_pod(new_symbol->type)) {
        ...
        exprt::operandst ops;
        ops.push_back(value);
        codet defcode = cpp_constructor(source_locationt(), symexpr, ops);
...
void cpp_typecheckt::put_compound_into_scope(
  ...
  if (compound.type().id() == ID_code) {
    ...
    id.id_class =
        compound.get_bool("is_type") ? cpp_idt::TYPEDEF : cpp_idt::SYMBOL;
    id.identifier = name;
    ...
    cpp_idt &id = cpp_scopes.current_scope().insert(base_name);
    id.id_class =
        compound.get_bool(ID_is_type) ? cpp_idt::TYPEDEF : cpp_idt::SYMBOL;
    id.identifier = name;
    id.class_identifier = cpp_scopes.current_scope().identifier;
    id.is_member = true;
    id.is_method = false;
...
void cpp_typecheckt::typecheck_friend_declaration(
    symbolt &symbol, cpp_declarationt &declaration) {
...
void cpp_typecheckt::typecheck_compound_body(symbolt &symbol) {
  ...
  Forall_operands(it, body) {
    if (it->id() == ID_cpp_declaration) {
      ...
      if (declaration.declarators().empty() &&
        ...
        if (final_type.id() != ID_union && final_type.id() != ID_struct) {
          error().source_location = declaration.type().source_location();
      ...
      for (auto &declarator : declaration.declarators()) {
        ...
        typecheck_compound_declarator(symbol, declaration, declarator,
                                      components, access, is_static, is_typedef,
                                      is_mutable);
      }
  ...
  if ((found_ctor || !cpp_is_pod(symbol.type)) && !found_dtor) {
    ...
    typecheck_compound_declarator(symbol, dtor, dtor.declarators()[0],
                                  components, ID_public, false, false, false);
  }
    ...
    if (it->id() == ID_cpp_declaration) {
      ...
      for (auto &declarator : declaration.declarators()) {
        ...
        if (declarator.value().is_not_nil()) // body?
        {
          ...
          check_member_initializers(type.add(ID_bases), type.components(),
  ...
  if (!cpp_is_pod(symbol.type)) {
    ...
    if (!find_cpctor(symbol)) {
      ...
      typecheck_compound_declarator(symbol, cpctor, cpctor.declarators()[0],
                                    components, ID_public, false, false, false);
    }
    ...
    if (!find_assignop(symbol)) {
      ...
      typecheck_compound_declarator(symbol, assignop, assignop.declarators()[0],
                                    components, ID_public, false, false, false);
    }
  }
...
void cpp_typecheckt::typecheck_member_function(
    const irep_idt &compound_identifier, struct_typet::componentt &component,
    irept &initializers, const typet &method_qualifier, exprt &value) {
  symbolt symbol;
  ...
  if (component.get_bool(ID_is_static)) {
    ...
    add_this_to_method_type(compound_identifier, type, method_qualifier);
  }
...
void cpp_typecheckt::add_this_to_method_type(const irep_idt &compound_symbol,
                                             typet &type,
                                             const typet &method_qualifier) {
  code_typet::parameterst &parameters = to_code_type(type).parameters();

  parameters.insert(parameters.begin(), code_typet::parametert());
...
void cpp_typecheckt::convert_anon_struct_union_member(
    const cpp_declarationt &declaration, const irep_idt &access,
...
bool cpp_typecheckt::get_component(const source_locationt &source_location,
  ...
  assert(followed_type.id() == ID_struct || followed_type.id() == ID_union);

  struct_union_typet final_type = to_struct_union_type(followed_type);
  ...
  for (const auto &component : components) {
    ...
    if (component.get_name() == component_name) {
      ...
      if (component_type.id() == ID_union || component_type.id() == ID_struct) {
        // recursive call!
...
bool cpp_typecheckt::check_component_access(
  ...
  // check friendship
  const irept::subt &friends = struct_union_type.find("#friends").get_sub();
...
void cpp_typecheckt::get_virtual_bases(const struct_typet &type,
  ...
  if (std::find(vbases.begin(), vbases.end(), type.get(ID_name)) !=
      vbases.end())
    return;
