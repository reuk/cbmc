...
void cpp_typecheckt::convert_initializer(symbolt &symbol) {
  ...
  if (is_reference(symbol.type)) {
    ...
    if (symbol.type.id() == ID_pointer &&
      ...
      for (code_typet::parameterst::const_iterator ait =
               code_type.parameters().begin();
           ait != code_type.parameters().end(); ait++) {
      ...
      exprt resolved_expr =
          resolve(to_cpp_name(static_cast<irept &>(symbol.value.op0())),
...
void cpp_typecheckt::zero_initializer(const exprt &object, const typet &type,
  ...
  } else if (final_type.id() == ID_array && !cpp_is_pod(final_type.subtype())) {
