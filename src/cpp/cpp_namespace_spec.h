...
public:
  ...
  const irep_idt &get_namespace() const { return get(ID_namespace); }

  void set_namespace(const irep_idt &_namespace) {
    set(ID_namespace, _namespace);
  }

  cpp_namet &alias() { return static_cast<cpp_namet &>(add("alias")); }
  ...
  void set_is_inline(bool value) { set(ID_is_inline, value); }
