...
void cpp_typecheckt::convert_anonymous_union(cpp_declarationt &declaration,
                                             codet &code) {
  ...
  const irept::subt &components =
      union_symbol.type.add(ID_components).get_sub();
  ...
  symbol_table.symbols[union_symbol.name].type.set("#unnamed_object",
                                                   symbol.base_name);
...
void cpp_typecheckt::convert_non_template_declaration(
  ...
  for (auto &d : declaration.declarators()) {
    ...
    symbolt &symbol = cpp_declarator_converter.convert(
        declaration_type, declaration.storage_spec(), declaration.member_spec(),
        declarator);
    ...
    if (declaration.find(ID_C_template).is_not_nil()) {
      ...
      symbol.type.set(ID_C_template_arguments,
    ...
    if (symbol.is_lvalue && declarator.init_args().has_operands()) {
      symbol.value = cpp_constructor(symbol.location, cpp_symbol_expr(symbol),
