...
static void copy_parent(const source_locationt &source_location,
                        ...
                        const irep_idt &arg_name, exprt &block) {
...
static void copy_member(const source_locationt &source_location,
  ...
  op1.add(ID_component_cpp_name)
      .get_sub()
      .back()
      .set(ID_identifier, member_base_name);
  op1.add(ID_component_cpp_name)
      .get_sub()
      .back()
...
static void copy_array(const source_locationt &source_location,
                       const irep_idt &member_base_name, mp_integer i,
                       const irep_idt &arg_name, exprt &block) {
...
void cpp_typecheckt::default_cpctor(const symbolt &symbol,
  ...
  forall_irep(parent_it, bases.get_sub()) {
    ...
    if (cpp_is_pod(parsymb.type))
      ...
      mem_init.copy_to_operands(static_cast<const exprt &>(
          static_cast<const irept &>(cpp_parameter)));
  ...
  for (struct_typet::componentst::const_iterator mem_it = components.begin();
    ...
    if (mem_it->get_bool("from_base") || mem_it->get_bool(ID_is_type) ||
        mem_it->get_bool(ID_is_static) || mem_it->type().id() == ID_code)
      continue;
...
void cpp_typecheckt::default_assignop(const symbolt &symbol,
  ...
  args_decl_type_sub.back().get_sub().back().set(ID_identifier,
                                                 symbol.base_name);
  args_decl_type_sub.back().get_sub().back().set(ID_C_source_location,
                                                 source_location);
...
void cpp_typecheckt::default_assignop_value(const symbolt &symbol,
  ...
  forall_irep(parent_it, bases.get_sub()) {
    ...
    const symbolt &symb = lookup(parent_it->find(ID_type).get(ID_identifier));
  ...
  forall_irep(mem_it, components.get_sub()) {
    if (mem_it->get_bool(ID_from_base) || mem_it->get_bool(ID_is_type) ||
        mem_it->get_bool(ID_is_static) || mem_it->get_bool("is_vtptr") ||
...
void cpp_typecheckt::check_member_initializers(
    const irept &bases, const struct_typet::componentst &components,
  ...
  forall_irep(init_it, initializers.get_sub()) {
    ...
    for (struct_typet::componentst::const_iterator c_it = components.begin();
      ...
      // Data member
      if (!c_it->get_bool(ID_from_base) && !c_it->get_bool(ID_is_static) &&
      ...
      // Parent constructor
      if (c_it->get_bool(ID_from_base) && !c_it->get_bool(ID_is_type) &&
          !c_it->get_bool(ID_is_static) && c_it->get(ID_type) == ID_code &&
...
void cpp_typecheckt::full_member_initialization(
    const struct_union_typet &struct_union_type, irept &initializers) {
      ...
      forall_irep(m_it, initializers.get_sub()) {
        ...
        if (!has_template_args) {
          ...
          for (struct_typet::componentst::const_iterator c_it =
               ...
               c_it != components.end(); c_it++) {
            if (c_it->get(ID_base_name) == base_name &&
                c_it->get(ID_type) != ID_code && !c_it->get_bool(ID_is_type)) {
              is_data = true;
              break;
            }
          }
  ...
  for (struct_typet::componentst::const_iterator mem_it = components.begin();
    ...
    if (mem_it->get_bool(ID_from_base) || mem_it->type().id() == ID_code ||
        mem_it->get_bool(ID_is_type) || mem_it->get_bool(ID_is_static))
      continue;
    ...
    // initialized
    if (!found && mem_it->find(ID_type).id() == ID_pointer &&
