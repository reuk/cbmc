  ...
  cpp_declarationt &cpctor) const
{
  ...
  forall_irep(parent_it, bases.get_sub())
  {
    ...
    const symbolt &parsymb= lookup(parent_it->find(ID_type).get(ID_identifier));
      ...
      mem_it++)
  {
    ...
    if(
      mem_it->get_bool("from_base") || mem_it->get_bool(ID_is_type) ||
  ...
  cpp_declaratort &declarator)
{
  ...
  {
    if(
      mem_it->get_bool(ID_from_base) || mem_it->get_bool(ID_is_type) ||
  ...
  const irept &initializers)
{
  ...
  forall_irep(init_it, initializers.get_sub())
  {
        ...
        c_it++)
    {
      ...
      // Data member
      if(
        !c_it->get_bool(ID_from_base) && !c_it->get_bool(ID_is_static) &&
      ...
      // Parent constructor
      if(
        c_it->get_bool(ID_from_base) && !c_it->get_bool(ID_is_type) &&
  ...
  irept &initializers)
{
    ...
    forall_irep(parent_it, bases.get_sub())
    {
      ...
      forall_irep(m_it, initializers.get_sub())
      {
        ...
        if(!has_template_args)
        {
          ...
          {
            if(
              c_it->get(ID_base_name) == base_name &&
      ...
      mem_it++)
  {
    ...
    if(
      mem_it->get_bool(ID_from_base) || mem_it->type().id() == ID_code ||
    ...
    // initialized
    if(
      !found && mem_it->find(ID_type).id() == ID_pointer &&
