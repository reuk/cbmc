...
class cpp_scopest {
public:
  cpp_scopest() { current_scope_ptr = &root_scope; }

  typedef std::set<cpp_scopet *> scope_sett;
  typedef std::set<cpp_idt *> id_sett;

  cpp_scopet &current_scope() { return *current_scope_ptr; }
  ...
  cpp_idt &put_into_scope(const symbolt &symbol, cpp_scopet &scope,
  ...
  cpp_scopet &get_root_scope() { return root_scope; }

  void go_to_root_scope() { current_scope_ptr = &root_scope; }
  ...
  void go_to_global_scope() { current_scope_ptr = &get_global_scope(); }

  cpp_scopet &get_global_scope() { return current_scope().get_global_scope(); }
...
public:
  ...
  void restore() { cpp_scopes.current_scope_ptr = saved_scope; }
