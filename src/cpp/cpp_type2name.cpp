...
static void irep2name(const irept &irep, std::string &result) {
  ...
  forall_named_irep(it, irep.get_comments()) if (it->first == ID_C_constant ||
...
std::string cpp_type2name(const typet &type) {
  ...
  if (type.id() == ID_empty || type.id() == ID_void)
    ...
    for (code_typet::parameterst::const_iterator arg_it = parameters.begin();
         arg_it != parameters.end(); arg_it++) {
