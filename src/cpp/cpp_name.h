...
public:
  ...
  cpp_namet(const irep_idt &_base_name,
  ...
  bool is_typename() const { return get_bool(ID_typename); }

  bool is_qualified() const {
    forall_irep(it, get_sub()) if (it->id() == "::") return true;
    return false;
  }
  ...
  bool has_template_args() const {
    forall_irep(it, get_sub()) if (it->id() == ID_template_args) return true;
