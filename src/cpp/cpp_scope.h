...
public:
  ...
  void lookup(const irep_idt &base_name, lookup_kindt kind,
              cpp_idt::id_classt id_class, id_sett &id_set);

  void lookup_identifier(const irep_idt &identifier,
                         cpp_idt::id_classt id_class, id_sett &id_set);
  ...
  bool is_root_scope() const { return id_class == ROOT_SCOPE; }

  bool is_global_scope() const {
    return id_class == ROOT_SCOPE || id_class == NAMESPACE;
  }

  bool is_template_scope() const { return id_class == TEMPLATE_SCOPE; }
