...
void cpp_typecheckt::salvage_default_arguments(const template_typet &old_type,
  ...
  for (std::size_t i = 0; i < new_parameters.size(); i++) {
    if (i < old_parameters.size() && old_parameters[i].has_default_argument() &&
...
void cpp_typecheckt::typecheck_class_template(cpp_declarationt &declaration) {
  ...
  if (previous_symbol != symbol_table.symbols.end()) {
    ...
    if (has_body) {
      ...
      salvage_default_arguments(previous_declaration.template_type(),
      ...
      salvage_default_arguments(declaration.template_type(),
...
void cpp_typecheckt::typecheck_function_template(
  ...
  if (previous_symbol != symbol_table.symbols.end()) {
    bool previous_has_value = to_cpp_declaration(previous_symbol->second.type)
                                  .declarators()[0]
                                  .find(ID_value)
                                  .is_not_nil();
...
void cpp_typecheckt::typecheck_class_template_member(
  ...
  if (cpp_name.get_sub().size() == 4 && cpp_name.get_sub()[0].id() == ID_name &&
...
std::string cpp_typecheckt::class_template_identifier(
  ...
  for (template_typet::template_parameterst::const_iterator it =
       ...
       it != template_type.template_parameters().end(); it++) {
    if (counter != 0)
      identifier += ',';
  ...
  if (!partial_specialization_args.arguments().empty()) {
    ...
    for (cpp_template_args_non_tct::argumentst::const_iterator
         ...
         it != partial_specialization_args.arguments().end(); it++, counter++) {
      if (counter != 0)
        identifier += ',';
...
void cpp_typecheckt::convert_template_function_or_member_specialization(
  ...
  if (cpp_name.get_sub().back().id() == ID_template_args) {
    ...
    cpp_template_args_tct template_args = typecheck_template_args(
        declaration.source_location(), template_symbol,
        to_cpp_template_args_non_tc(cpp_name.get_sub()[1]));
    ...
    instantiate_template(cpp_name.source_location(), template_symbol,
                         template_args, template_args, specialization);
  } else {
...
cpp_scopet &
cpp_typecheckt::typecheck_template_parameters(template_typet &type) {
  cpp_save_scopet cpp_saved_scope(cpp_scopes);
...
void cpp_typecheckt::convert_template_declaration(
  ...
  if (declaration.is_class_template()) {
    ...
    if ((static_cast<const cpp_namet &>(type.find(ID_tag)))
            .has_template_args()) {
