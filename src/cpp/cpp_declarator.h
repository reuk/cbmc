...
public:
  ...
  cpp_namet &name()
  {
    return static_cast<cpp_namet &>(add(ID_name));
  }
  ...
  {
    return add(ID_method_qualifier);
  }
  const irept &method_qualifier() const
  {
    return find(ID_method_qualifier);
  }

  irept &member_initializers()
  {
    return add(ID_member_initializers);
  }
  ...
  irept &throw_decl()
  {
    return add("throw_decl");
  }
  const irept &throw_decl() const
  {
    return find("throw_decl");
  }
