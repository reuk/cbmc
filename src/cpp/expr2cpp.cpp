...
protected:
  ...
  std::string
  convert_constant(const constant_exprt &src, unsigned &precedence) override;
...
std::string
expr2cppt::convert_constant(const constant_exprt &src, unsigned &precedence)
{
  ...
  const std::string &declarator)
{
  ...
  {
    const irep_idt &identifier= to_symbol_type(src).get_identifier();
  ...
  else if(src.id() == ID_struct || src.id() == ID_incomplete_struct)
  {
    std::string dest= q;
  ...
  else if(src.id() == ID_verilog_signedbv || src.id() == ID_verilog_unsignedbv)
...
std::string expr2cppt::convert_cpp_this(const exprt &src, unsigned precedence)
{
  return "this";
}
...
std::string expr2cppt::convert_cpp_new(const exprt &src, unsigned precedence)
{
  std::string dest;
...
std::string expr2cppt::convert(const exprt &src, unsigned &precedence)
{
  ...
  if(src.id() == ID_extractbit)
    ...
    src.id() == ID_side_effect && (src.get(ID_statement) == ID_cpp_new ||
...
std::string expr2cppt::convert_code(const codet &src, unsigned indent)
{
  ...
  if(statement == ID_cpp_delete || statement == ID_cpp_delete_array)
...
std::string expr2cppt::convert_extractbit(const exprt &src, unsigned precedence)
{
...
std::string
expr2cppt::convert_extractbits(const exprt &src, unsigned precedence)
{
