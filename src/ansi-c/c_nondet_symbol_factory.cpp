...
public:
  symbol_factoryt(std::vector<symbolt const *> &_symbols_created,
                  symbol_tablet &_symbol_table, const source_locationt &loc,
                  const bool _assume_non_null)
      : symbols_created(_symbols_created), symbol_table(_symbol_table),
        loc(loc), assume_non_null(_assume_non_null), ns(_symbol_table) {}

  exprt allocate_object(code_blockt &assignments, const exprt &target_expr,
                        const typet &allocate_type, const bool static_lifetime);
...
exprt symbol_factoryt::allocate_object(code_blockt &assignments,
  ...
  const symbolt &aux_symbol =
      c_new_tmp_symbol(symbol_table, loc, allocate_type, static_lifetime);
...
void symbol_factoryt::gen_nondet_init(code_blockt &assignments,
  ...
  // TODO(OJones): Add support for structs and arrays
    ...
    //   <expr> = NONDET(type);
    exprt rhs = type.id() == ID_c_bool ? c_get_nondet_bool(type)
...
exprt c_nondet_symbol_factory(code_blockt &init_code,
                              symbol_tablet &symbol_table,
                              const irep_idt base_name, const typet &type,
                              const source_locationt &loc, bool allow_null) {
  irep_idt identifier =
      id2string(goto_functionst::entry_point()) + "::" + id2string(base_name);
  ...
  symbol_factoryt state(symbols_created, symbol_table, loc, !allow_null);
  ...
  for (symbolt const *symbol_ptr : symbols_created) {
    ...
    input_code.op0() =
        address_of_exprt(index_exprt(string_constantt(symbol_ptr->base_name),
