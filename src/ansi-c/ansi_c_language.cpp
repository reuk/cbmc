...
bool ansi_c_languaget::typecheck(symbol_tablet &symbol_table,
  ...
  if (ansi_c_typecheck(parse_tree, new_symbol_table, module,
                       get_message_handler())) {
    return true;
  }
...
void ansi_c_languaget::show_parse(std::ostream &out) { parse_tree.output(out); }
...
languaget *new_ansi_c_language() { return new ansi_c_languaget; }
...
bool ansi_c_languaget::from_expr(const exprt &expr, std::string &code,
                                 const namespacet &ns) {
  code = expr2c(expr, ns);
  return false;
}
...
bool ansi_c_languaget::from_type(const typet &type, std::string &code,
                                 const namespacet &ns) {
  code = type2c(type, ns);
  return false;
}
...
bool ansi_c_languaget::type_to_name(const typet &type, std::string &name,
                                    const namespacet &ns) {
...
bool ansi_c_languaget::to_expr(const std::string &code,
  ...
  std::istringstream i_preprocessed("void __my_expression = (void) (\n" + code +
                                    "\n);");
  ...
  if (expr.id() == ID_typecast && expr.type().id() == ID_empty &&
