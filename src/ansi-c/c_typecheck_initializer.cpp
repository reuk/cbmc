...
void c_typecheck_baset::do_initializer(exprt &initializer, const typet &type,
                                       bool force_constant) {
...
exprt c_typecheck_baset::do_initializer_rec(const exprt &value,
  ...
  if (value.id() == ID_array && value.get_bool(ID_C_string_constant) &&
      full_type.id() == ID_array &&
      (full_type.subtype().id() == ID_signedbv ||
       full_type.subtype().id() == ID_unsignedbv) &&
      full_type.subtype().get(ID_width) ==
          value.type().subtype().get(ID_width)) {
    exprt tmp = value;

    // adjust char type
    tmp.type().subtype() = full_type.subtype();

    Forall_operands(it, tmp) it->type() = full_type.subtype();

    if (full_type.id() == ID_array && to_array_type(full_type).is_complete()) {
      // check size
      ...
      if (mp_integer(tmp.operands().size()) > array_size) {
        ...
        tmp.type() = type;
        exprt zero =
            zero_initializer(full_type.subtype(), value.source_location(),
  ...
  if (value.id() == ID_string_constant && full_type.id() == ID_array &&
      (full_type.subtype().id() == ID_signedbv ||
       full_type.subtype().id() == ID_unsignedbv) &&
  ...
  if (full_type.id() == ID_array && to_array_type(full_type).size().is_nil()) {
    err_location(value);
    error() << "type `" << to_string(full_type)
...
void c_typecheck_baset::designator_enter(const typet &type,
  ...
  if (full_type.id() == ID_struct) {
    ...
    for (struct_typet::componentst::const_iterator it =
         ...
         it != struct_type.components().end(); ++it) {
      if (it->type().id() != ID_code && !it->get_is_padding()) {
...
void c_typecheck_baset::do_designated_initializer(exprt &result,
                                                  designatort &designator,
                                                  const exprt &value,
                                                  bool force_constant) {
  ...
  if (value.id() == ID_designated_initializer) {
    ...
    designator =
        make_designator(designator.front().type,
                        static_cast<const exprt &>(value.find(ID_designator)));

    assert(!designator.empty());

    return do_designated_initializer(result, designator, value.op0(),
                                     force_constant);
  }
  ...
  for (size_t i = 0; i < designator.size(); i++) {
    ...
    if (full_type.id() == ID_array || full_type.id() == ID_vector) {
  ...
  while (true) {
    ...
    assert(full_type.id() == ID_struct || full_type.id() == ID_union ||
           full_type.id() == ID_array || full_type.id() == ID_vector);
...
void c_typecheck_baset::increment_designator(designatort &designator) {
  ...
  while (true) {
    ...
    if (full_type.id() == ID_struct && entry.index < entry.size) {
...
designatort c_typecheck_baset::make_designator(const typet &src_type,
                                               const exprt &src) {
      ...
      if (struct_union_type.has_component(component_name)) {
        ...
        do {
          ...
          for (struct_union_typet::componentst::const_iterator
               ...
               c_it != components.end(); c_it++, number++) {
            if (c_it->get_name() == component_name) {
              ...
              entry.type = tmp_type;
                       ...
                       has_component_rec(c_it->type(), component_name, *this)) {
              entry.index = number;
              entry.size = components.size();
...
exprt c_typecheck_baset::do_initializer_list(const exprt &value,
  ...
  forall_operands(it, value) {
    do_designated_initializer(result, current_designator, *it, force_constant);
