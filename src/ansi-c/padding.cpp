...
mp_integer alignment(const typet &type, const namespacet &ns)
{
  ...
  if(given_alignment.is_nil() || to_integer(given_alignment, a_int))
    a_int= 0;
...
void add_padding(struct_typet &type, const namespacet &ns)
{
  ...
  if(type.get_bool(ID_C_packed) && type.find(ID_C_alignment).is_nil())
    return; // done
      ...
      it++)
  {
    ...
    const bool packed=
      it_type.get_bool(ID_C_packed) || ns.follow(it_type).get_bool(ID_C_packed);
