...
static exprt make_member_expr(const exprt &struct_union,
  ...
  if (result.get_bool(ID_C_constant) || type.get_bool(ID_C_constant) ||
...
exprt get_component_rec(const exprt &struct_union,
                        const irep_idt &component_name, const namespacet &ns) {
