...
static std::string type_max(const typet &src) {
  if (src.id() == ID_signedbv)
    return integer2string(power(2, to_signedbv_type(src).get_width() - 1) - 1);
  else if (src.id() == ID_unsignedbv)
    return integer2string(power(2, to_unsignedbv_type(src).get_width() - 1) -
                          1);
  else
    assert(false);
}
...
static void error_parse_line(const std::string &line, bool warning_only,
                             messaget &message) {
  ...
  messaget::mstreamt &m = warning_only ? message.warning() : message.error();
...
static void error_parse(std::istream &errors, bool warning_only,
                        messaget &message) {
  std::string line;
...
bool c_preprocess(std::istream &instream, std::ostream &outstream,
...
static bool is_dot_i_file(const std::string &path) {
  ...
  if (ext == NULL)
    return false;
  if (std::string(ext) == ".i" || std::string(ext) == ".ii")
    return true;
  return false;
}
...
bool c_preprocess_codewarrior(const std::string &, std::ostream &,
                              message_handlert &);
bool c_preprocess_arm(const std::string &, std::ostream &, message_handlert &);
bool c_preprocess_gcc_clang(const std::string &, std::ostream &,
                            message_handlert &,
                            configt::ansi_ct::preprocessort);
bool c_preprocess_none(const std::string &, std::ostream &, message_handlert &);
bool c_preprocess_visual_studio(const std::string &, std::ostream &,
                                message_handlert &);
...
bool c_preprocess(const std::string &path, std::ostream &outstream,
  ...
  case configt::ansi_ct::preprocessort::GCC:
    return c_preprocess_gcc_clang(path, outstream, message_handler,
...
bool c_preprocess_visual_studio(const std::string &file,
  ...
  if (!instream) {
    ...
    message.error() << "CL Preprocessing failed (open failed)" << messaget::eom;
    return true;
  }
...
bool c_preprocess_codewarrior(const std::string &file, std::ostream &outstream,
                              message_handlert &message_handler) {
  // check extension
  if (is_dot_i_file(file))
    return c_preprocess_none(file, outstream, message_handler);
  ...
  if (stream_i) {
    ...
    unlink(tmpi.c_str());
    unlink(stderr_file.c_str());
    message.error() << "Preprocessing failed (fopen failed)" << messaget::eom;
    return true;
  }
...
bool c_preprocess_gcc_clang(const std::string &file, std::ostream &outstream,
                            message_handlert &message_handler,
...
bool c_preprocess_arm(const std::string &file, std::ostream &outstream,
                      message_handlert &message_handler) {
  // check extension
  if (is_dot_i_file(file))
    return c_preprocess_none(file, outstream, message_handler);
  ...
  //  command+="
  //  -D__sizeof_long="+std::to_string(config.ansi_c.long_int_width/8);
...
bool c_preprocess_none(const std::string &file, std::ostream &outstream,
                       message_handlert &message_handler) {
#ifdef _MSC_VER
...
const char c_test_program[] = "#include <stdlib.h>\n"
                              "\n"
                              "int main() { }\n";
