...
bool c_implicit_typecast(exprt &expr, const typet &dest_type,
                         const namespacet &ns) {
...
bool c_implicit_typecast_arithmetic(exprt &expr1, exprt &expr2,
                                    const namespacet &ns) {
  c_typecastt c_typecast(ns);
...
bool check_c_implicit_typecast(const typet &src_type, const typet &dest_type) {
  ...
  if (src_type_id == ID_natural) {
    if (dest_type.id() == ID_bool || dest_type.id() == ID_c_bool ||
    ...
    if (dest_type.id() == ID_c_bool || dest_type.id() == ID_integer ||
        ...
        dest_type.id() == ID_complex)
      return false;
  } else if (src_type_id == ID_unsignedbv || src_type_id == ID_signedbv ||
             src_type_id == ID_c_enum || src_type_id == ID_c_enum_tag ||
             src_type_id == ID_incomplete_c_enum || src_type_id == ID_c_bool) {
    if (dest_type.id() == ID_unsignedbv || dest_type.id() == ID_bool ||
...
c_typecastt::c_typet c_typecastt::minimum_promotion(const typet &type) const {
  ...
  if (max_type == UINT && type.id() == ID_c_bit_field &&
...
void c_typecastt::implicit_typecast_followed(exprt &expr, const typet &src_type,
                                             const typet &orig_dest_type,
                                             const typet &dest_type) {
  // do transparent union
  if (dest_type.id() == ID_union &&
      dest_type.get_bool(ID_C_transparent_union) && src_type.id() != ID_union) {
