...
void ansi_c_parsert::add_declarator(exprt &declaration, irept &declarator) {
  ...
  if (!base_name.empty()) {
    ...
    ansi_c_id_classt id_class = is_typedef ? ANSI_C_TYPEDEF : ANSI_C_SYMBOL;

    scopet &scope = force_root_scope ? root_scope() : current_scope();

    // set the final name
    irep_idt prefixed_name =
        force_root_scope ? base_name
...
ansi_c_id_classt ansi_c_parsert::get_class(const typet &type) {
  if (type.id() == ID_typedef)
    ...
    forall_subtypes(it, type) if (get_class(*it) ==
                                  ANSI_C_TYPEDEF) return ANSI_C_TYPEDEF;
