...
void c_typecheck_baset::typecheck_type(typet &type)
{
  ...
  if(type.id() == ID_code)
    ...
    type.id() == ID_custom_unsignedbv || type.id() == ID_custom_signedbv ||
    type.id() == ID_custom_floatbv || type.id() == ID_custom_fixedbv)
      ...
      underlying_type.id() == ID_unsignedbv)
    {
      ...
      else if(mode == "__DI__") // 64 bits
      {
        ...
        else
        {
          ...
          result= is_signed ? signed_long_long_int_type()
                            : unsigned_long_long_int_type();
        }
      }
      else if(mode == "__TI__") // 128 bits
        result=
          is_signed ? gcc_signed_int128_type() : gcc_unsigned_int128_type();
  ...
  if(
    type.get_bool(ID_C_restricted) && type.id() != ID_pointer &&
    type.id() != ID_array)
  {
    error().source_location= type.source_location();
...
void c_typecheck_baset::typecheck_custom_type(typet &type)
{
  ...
  exprt size_expr= static_cast<const exprt &>(type.find(ID_size));
  ...
  else if(type.id() == ID_custom_fixedbv)
  {
    ...
    exprt f_expr= static_cast<const exprt &>(type.find(ID_f));
...
void c_typecheck_baset::typecheck_code_type(code_typet &type)
{
  ...
  else // we do have parameters
  {
    ...
    for(auto &param : type.parameters())
    {
      ...
      {
        ansi_c_declarationt &declaration= to_ansi_c_declaration(param);
    ...
    if(parameters.size() == 1 && follow(parameters[0].type()).id() == ID_empty)
    {
...
void c_typecheck_baset::typecheck_array_type(array_typet &type)
{
  ...
  if(size.is_not_nil())
  {
    ...
    if(tmp_size.is_constant())
    {
      ...
      {
        error().source_location= source_location;
        error() << "failed to convert constant: " << tmp_size.pretty() << eom;
        throw 0;
      }
    ...
    else if(tmp_size.id() == ID_infinity)
    {
      ...
      tmp_size.id() == ID_symbol && tmp_size.type().get_bool(ID_C_constant))
    {
...
void c_typecheck_baset::typecheck_vector_type(vector_typet &type)
{
  ...
  if(
    ...
    subtype.id() != ID_floatbv && subtype.id() != ID_fixedbv)
  {
    error().source_location= source_location;
    error() << "cannot make a vector of subtype " << to_string(subtype) << eom;
    throw 0;
  }
  ...
  {
    error().source_location= source_location;
    error() << "failed to convert constant: " << size.pretty() << eom;
    throw 0;
  }
  ...
  {
    error().source_location= source_location;
    error() << "type had size 0: `" << to_string(type.subtype()) << "'" << eom;
    throw 0;
  }
...
void c_typecheck_baset::typecheck_compound_type(struct_union_typet &type)
{
  ...
  else
  {
    ...
    if(s_it == symbol_table.symbols.end())
    {
      ...
      compound_symbol.type= type;
      compound_symbol.location= type.source_location();
      compound_symbol.pretty_name=
        id2string(type.id()) + " " + id2string(base_name);
    ...
    else
    {
      ...
      if(
        s_it->second.type.id() == ID_incomplete_struct ||
...
void c_typecheck_baset::typecheck_compound_body(struct_union_typet &type)
{
  ...
  for(auto &decl : old_components)
  {
    ...
    else
    {
      ...
      for(const auto &declarator : declaration.declarators())
      {
        ...
        new_component.add_source_location()= declarator.source_location();
        ...
        if(
          !is_complete_type(new_component.type()) &&
  ...
  if(type.id() == ID_struct || type.id() == ID_union)
  {
        ...
        it++)
    {
      ...
      if(c_type.id() == ID_array && to_array_type(c_type).is_incomplete())
      {
  ...
  const mp_integer &max_value) const
{
  ...
  else
  {
    ...
    if(
      max_value < (mp_integer(1) << (config.ansi_c.int_width - 1)) &&
      min_value >= -(mp_integer(1) << (config.ansi_c.int_width - 1)))
      return signed_int_type();
    else if(
      max_value < (mp_integer(1) << config.ansi_c.int_width) && min_value >= 0)
      return unsigned_int_type();
    else if(
      max_value < (mp_integer(1) << config.ansi_c.long_int_width) &&
      min_value >= 0)
      return unsigned_long_int_type();
    else if(
      max_value < (mp_integer(1) << config.ansi_c.long_long_int_width) &&
      min_value >= 0)
      return unsigned_long_long_int_type();
    else if(
      max_value < (mp_integer(1) << (config.ansi_c.long_int_width - 1)) &&
  ...
  else
  {
    if(min_value < 0)
    {
      ...
      if(is_packed)
      {
        ...
        if(
          max_value < (mp_integer(1) << (config.ansi_c.char_width - 1)) &&
          min_value >= -(mp_integer(1) << (config.ansi_c.char_width - 1)))
          return signed_char_type();
        else if(
          max_value < (mp_integer(1) << (config.ansi_c.short_int_width - 1)) &&
...
void c_typecheck_baset::typecheck_c_enum_type(typet &type)
{
  ...
  for(auto &op : as_expr.operands())
  {
    ...
    typet constant_type= enum_constant_type(min_value, max_value);
  ...
  typet underlying_type= enum_underlying_type(min_value, max_value, is_packed);
...
void c_typecheck_baset::typecheck_c_bit_field_type(c_bit_field_typet &type)
{
  ...
  else if(subtype.id() == ID_c_enum_tag)
  {
    ...
    // to check.
    const typet &c_enum_type= follow_tag(to_c_enum_tag_type(subtype));
  ...
  {
    error().source_location= type.source_location();
    error() << "bit field with non-integer type: " << to_string(subtype) << eom;
    throw 0;
  }
  ...
  {
    error().source_location= type.source_location();
    error() << "bit field (" << i << " bits) larger than type (" << sub_width
            << " bits)" << eom;
    throw 0;
  }
}
...
void c_typecheck_baset::typecheck_typeof_type(typet &type)
{
  ...
  else
  {
    ...
    if(expr.id() == ID_address_of && expr.get_bool(ID_C_implicit))
    {
...
void c_typecheck_baset::typecheck_symbol_type(typet &type)
{
  ...
  {
    error().source_location= type.source_location();
    error() << "type symbol `" << identifier << "' not found" << eom;
    throw 0;
  }
