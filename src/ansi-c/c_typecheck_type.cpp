...
void c_typecheck_baset::typecheck_type(typet &type) {
  ...
  if (type.id() == ID_code)
    ...
    typecheck_vector_type(to_vector_type(type));
           ...
           type.id() == ID_custom_signedbv || type.id() == ID_custom_floatbv ||
    ...
    if (underlying_type.id() == ID_signedbv ||
      ...
      if (mode == "__QI__") // 8 bits
        ...
        result =
            is_signed ? signed_short_int_type() : unsigned_short_int_type();
      ...
      else if (mode == "__DI__") // 64 bits
      {
        if (config.ansi_c.long_int_width == 64)
          result =
              is_signed ? signed_long_int_type() : unsigned_long_int_type();
        else {
        ...
        result =
            vector_typet(is_signed ? signed_int_type() : unsigned_int_type(),
  ...
  if (type.get_bool(ID_C_restricted) && type.id() != ID_pointer &&
      type.id() != ID_array) {
    error().source_location = type.source_location();
      ...
      if (param.id() == ID_declaration) {
        ...
        if (identifier == irep_idt()) {
          ...
          parameter.add_source_location() =
              declaration.type().source_location();
        } else {
...
typet c_typecheck_baset::enum_constant_type(const mp_integer &min_value,
  ...
  if (config.ansi_c.mode == configt::ansi_ct::flavourt::VISUAL_STUDIO) {
    ...
    if (max_value < (mp_integer(1) << (config.ansi_c.int_width - 1)) &&
      ...
      return unsigned_long_long_int_type();
                 ...
                 (mp_integer(1) << (config.ansi_c.long_int_width - 1)) &&
             min_value >=
                 -(mp_integer(1) << (config.ansi_c.long_int_width - 1)))
...
typet c_typecheck_baset::enum_underlying_type(const mp_integer &min_value,
      ...
      if (is_packed) {
        ...
        if (max_value < (mp_integer(1) << (config.ansi_c.char_width - 1)) &&
          ...
          return signed_char_type();
                     ...
                     (mp_integer(1) << (config.ansi_c.short_int_width - 1)) &&
                 min_value >=
                     -(mp_integer(1) << (config.ansi_c.short_int_width - 1)))
...
void c_typecheck_baset::typecheck_c_bit_field_type(c_bit_field_typet &type) {
  ...
  if (subtype.id() == ID_bool) {
    ...
    sub_width = 1;
  } else if (subtype.id() == ID_signedbv || subtype.id() == ID_unsignedbv ||
