...
std::string
expr2ct::convert_nondet_symbol(const exprt &src, unsigned &precedence)
{
...
expr2ct::convert_code_ifthenelse(const code_ifthenelset &src, unsigned indent)
{
  ...
  if(src.then_case().is_nil())
  {
    ...
    dest+= convert_code(
      ...
      to_code(src.then_case()).get_statement() == ID_block ? indent
                                                           : indent + 2);
  dest+= "\n";
  ...
  if(!src.else_case().is_nil())
  {
    ...
    dest+= convert_code(
      ...
      to_code(src.else_case()).get_statement() == ID_block ? indent
                                                           : indent + 2);
  }
