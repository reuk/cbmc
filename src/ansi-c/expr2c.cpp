...
irep_idt expr2ct::id_shorthand(const irep_idt &identifier) const {
  ...
  if (!ns.lookup(identifier, symbol) && !symbol->base_name.empty() &&
...
std::string expr2ct::convert_rec(const typet &src,
  ...
  if (src.id() == ID_bool) {
  ...
  } else if (src.id() == ID_natural || src.id() == ID_integer ||
             src.id() == ID_rational) {
    ...
    for (union_typet::componentst::const_iterator it =
         ...
         it != union_type.components().end(); it++) {
      dest += ' ';
      dest +=
...
std::string expr2ct::convert_binary(const exprt &src, const std::string &symbol,
                                    unsigned precedence,
...
std::string expr2ct::convert_unary(const exprt &src, const std::string &symbol,
                                   unsigned precedence) {
  if (src.operands().size() != 1)
    return convert_norep(src, precedence);
...
std::string expr2ct::convert_complex(const exprt &src, unsigned precedence) {
  if (src.operands().size() == 2 && src.op0().is_zero() &&
...
std::string expr2ct::convert_symbol(const exprt &src, unsigned &precedence) {
  ...
  if (src.operands().size() == 1 &&
    ...
    std::unordered_map<irep_idt, irep_idt, irep_id_hash>::const_iterator entry =
        shorthands.find(id);
...
std::string expr2ct::convert_constant(const constant_exprt &src,
  ...
  if (type.id() == ID_integer || type.id() == ID_natural ||
    ...
    } else if (type == char_type() && type != signed_int_type() &&
...
std::string expr2ct::convert_array(const exprt &src, unsigned &precedence) {
  ...
  forall_operands(it, src) if (!it->is_constant()) all_constant = false;

  if (src.get_bool(ID_C_string_constant) && all_constant &&
...
std::string expr2ct::convert_side_effect_expr_function_call(
    const side_effect_expr_function_callt &src, unsigned &precedence) {
  std::string dest;
...
std::string expr2ct::convert_code_while(const code_whilet &src,
  ...
  if (src.body().is_nil())
    ...
    dest += ")\n";
    dest += convert_code(src.body(), src.body().get_statement() == ID_block
                                         ? indent
                                         : indent + 2);
  }
...
std::string expr2ct::convert_code_ifthenelse(const code_ifthenelset &src,
  ...
  if (src.then_case().is_nil()) {
    ...
    dest += convert_code(to_code(src.then_case()),
                         to_code(src.then_case()).get_statement() == ID_block
                             ? indent
                             : indent + 2);
  dest += "\n";
  ...
  if (!src.else_case().is_nil()) {
    ...
    dest += indent_str(indent);
    dest += "else\n";
    dest += convert_code(to_code(src.else_case()),
                         to_code(src.else_case()).get_statement() == ID_block
                             ? indent
                             : indent + 2);
  }
...
std::string expr2ct::convert_code_fence(const codet &src, unsigned indent) {
  ...
  irep_idt att[] = {ID_WRfence, ID_RRfence, ID_RWfence, ID_WWfence, ID_RRcumul,
                    ID_RWcumul, ID_WWcumul, ID_WRcumul, irep_idt()};
...
std::string expr2ct::convert(const exprt &src, unsigned &precedence) {
  ...
  if (src.id() == ID_plus)
  ...
  else if (src.id() == ID_lt || src.id() == ID_gt || src.id() == ID_le ||
           src.id() == ID_ge)
    ...
    return convert_cond(src, precedence);
           ...
           src.id() == ID_overflow_minus || src.id() == ID_overflow_mult ||
