...
exprt c_sizeoft::sizeof_rec(const typet &type) {
  ...
  if (type.id() == ID_signedbv || type.id() == ID_unsignedbv ||
    ...
    if (size_expr.is_nil()) {
      ...
      if (!to_integer(tmp_dest, a) && !to_integer(size_expr, b)) {
    ...
    for (const auto &comp : components) {
      ...
      if (sub_type.id() == ID_c_bit_field) {
        ...
        tmp = from_integer(width / 8, size_type());
      } else {
      ...
      max_size =
          if_exprt(binary_relation_exprt(max_size, ID_lt, tmp), tmp, max_size);
    ...
    // simply multiply
    const exprt &size_expr = to_vector_type(type).size();
...
exprt c_offsetof(const struct_typet &src, const irep_idt &component_name,
                 const namespacet &ns) {
  c_sizeoft c_sizeof_inst(ns);
