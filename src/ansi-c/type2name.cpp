...
static std::string type2name(const typet &type, const namespacet &ns,
...
static std::string type2name_symbol(const typet &type, const namespacet &ns,
                                    symbol_numbert &symbol_number) {
  ...
  std::pair<symbol_numbert::iterator, bool> entry = symbol_number.insert(
      std::make_pair(identifier, std::make_pair(symbol_number.size(), true)));
...
static std::string type2name(const typet &type, const namespacet &ns,
  ...
  } else if (type.id() == ID_symbol || type.id() == ID_c_enum_tag ||
