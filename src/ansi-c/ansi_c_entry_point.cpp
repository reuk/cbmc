...
exprt::operandst
build_function_environment(const code_typet::parameterst &parameters,
  ...
  for (std::size_t param_number = 0; param_number < parameters.size();
    ...
    const irep_idt base_name =
        p.get_base_name().empty() ? ("argument#" + std::to_string(param_number))
                                  : p.get_base_name();

    main_arguments[param_number] =
        c_nondet_symbol_factory(init_code, symbol_table, base_name, p.type(),
                                p.source_location(), true);
  }
...
void record_function_outputs(const symbolt &function, code_blockt &init_code,
    ...
    if (parameters.empty()) {
...

      {
        ...
        input.op0() = address_of_exprt(index_exprt(
            string_constantt("argc"), from_integer(0, index_type())));
...

      {
        ...
        index_expr.copy_to_operands(argv_symbol.symbol_expr(),
      ...
      if (parameters.size() == 3) {
        ...
        index_expr.copy_to_operands(envp_symbol.symbol_expr(),
