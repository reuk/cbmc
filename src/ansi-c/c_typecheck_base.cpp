...
void c_typecheck_baset::typecheck_symbol(symbolt &symbol)
{
  ...
  if(
    symbol.is_type &&
    (final_type.id() == ID_struct || final_type.id() == ID_incomplete_struct))
  {
    symbol.pretty_name= "struct " + id2string(symbol.base_name);
  }
  else if(
    symbol.is_type &&
    (final_type.id() == ID_union || final_type.id() == ID_incomplete_union))
  {
    symbol.pretty_name= "union " + id2string(symbol.base_name);
  }
  else if(
    symbol.is_type &&
    (final_type.id() == ID_c_enum || final_type.id() == ID_incomplete_c_enum))
  {
...
void c_typecheck_baset::typecheck_new_symbol(symbolt &symbol)
{
  ...
  else
  {
    if(
      ...
      to_array_type(symbol.type).size().is_nil() && !symbol.is_type)
    {
  ...
  symbolt &new_symbol)
{
  ...
  if(
    final_old.id() == ID_incomplete_struct ||
  ...
  else
  {
    ...
    if(
      final_new.id() == ID_incomplete_struct ||
  ...
  symbolt &new_symbol)
{
  ...
  if(follow(new_symbol.type).id() != ID_code && !new_symbol.is_macro)
  ...
  if(old_symbol.type.id() == ID_KnR)
  {
    ...
    {
      error().source_location= new_symbol.location;
      error() << "function type not allowed for K&R function parameter" << eom;
      throw 0;
    }
    ...
    if(new_symbol.value.is_not_nil())
    {
      if(old_symbol.value.is_not_nil())
      {
           ...
           config.ansi_c.mode == configt::ansi_ct::flavourt::ARM))
        {
          ...
          for(code_typet::parameterst::const_iterator p_it= old_p.begin();
              p_it != old_p.end();
              p_it++)
          {
  ...
  if(final_old != final_new)
  {
      ...
      final_old.subtype() == final_new.subtype())
    {
      ...
      if(old_symbol.type.id() == ID_symbol)
      {
        ...
        if(s_it == symbol_table.symbols.end())
        {
          ...
          error() << "typecheck_redefinition_non_type: "
                  << "failed to find symbol `" << identifier << "'" << eom;
          throw 0;
        }
      ...
      (final_old.id() == ID_incomplete_c_enum || final_old.id() == ID_c_enum) &&
      (final_new.id() == ID_incomplete_c_enum || final_new.id() == ID_c_enum))
    {
      // this is ok for now
    }
    else if(
  ...
  if(new_symbol.value.is_not_nil())
  {
    ...
    if(old_symbol.value.is_not_nil())
    {
      ...
      {
        if(
          new_symbol.is_macro && (final_new.id() == ID_incomplete_c_enum ||
...
void c_typecheck_baset::typecheck_function_body(symbolt &symbol)
{
  ...
  for(code_typet::parameterst::iterator p_it= parameters.begin();
      p_it != parameters.end();
      p_it++)
  {
    // may be anonymous
  ...
  // check the labels
  for(std::map<irep_idt, source_locationt>::const_iterator it=
        labels_used.begin();
      it != labels_used.end();
      it++)
  {
  ...
  symbolt &symbol)
{
  ...
  if(
    !asm_label.empty() && !symbol.is_type &&
  ...
  {
    if(!asm_label_map.insert(std::make_pair(orig_name, asm_label)).second)
    {
      if(asm_label_map[orig_name] != asm_label)
      {
        error().source_location= symbol.location;
        error() << "error: replacing asm renaming " << asm_label_map[orig_name]
                << " by " << asm_label << eom;
        throw 0;
      }
    }
  ...
  {
    asm_label_mapt::const_iterator entry= asm_label_map.find(symbol.name);
    ...
    symbol.value.is_not_nil() && !symbol.is_macro)
  {
        ...
        ++p_it)
    {
      ...
      if(!asm_label_map.insert(std::make_pair(p_id, p_new_id)).second)
...
void c_typecheck_baset::typecheck_declaration(ansi_c_declarationt &declaration)
{
  ...
  else
  {
    ...
    for(ansi_c_declarationt::declaratorst::iterator d_it=
          declaration.declarators().begin();
        ...
        d_it++)
    {
      ...
      if(!full_spec.alias.empty())
      {
        ...
        {
          error().source_location= symbol.location;
          error() << "alias attribute cannot be used with a body" << eom;
          throw 0;
        }
