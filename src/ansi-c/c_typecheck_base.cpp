...
void c_typecheck_baset::typecheck_new_symbol(symbolt &symbol) {
  ...
  if (symbol.type.id() == ID_code) {
    if (symbol.value.is_not_nil() && !symbol.is_macro)
      ...
      for (code_typet::parameterst::iterator it =
...
void c_typecheck_baset::typecheck_redefinition_non_type(symbolt &old_symbol,
  ...
  if (final_new.id() == ID_code) {
    bool inlined = (new_symbol.type.get_bool(ID_C_inlined) ||
...
void c_typecheck_baset::apply_asm_label(const irep_idt &asm_label,
  ...
  if (!asm_label.empty() && !symbol.is_type &&
...
void c_typecheck_baset::typecheck_declaration(
  ...
  if (declaration.get_is_static_assert()) {
    ...
    for (ansi_c_declarationt::declaratorst::iterator d_it =
         ...
         d_it != declaration.declarators().end(); d_it++) {
