...
void c_typecheck_baset::add_rounding_mode(exprt &expr) {
  ...
  if (expr.id() == ID_div || expr.id() == ID_mult || expr.id() == ID_plus ||
      expr.id() == ID_minus) {
    if (expr.type().id() == ID_floatbv && expr.operands().size() == 2) {
...
bool c_typecheck_baset::gcc_types_compatible_p(const typet &type1,
  ...
  if (type1.id() == ID_c_enum) {
    ...
    return gcc_types_compatible_p(type1.subtype(),
                                  type2.subtype()); // ignore size
    ...
    if (!gcc_types_compatible_p(c_type1.return_type(), c_type2.return_type()))
      return false;
    ...
    for (std::size_t i = 0; i < c_type1.parameters().size(); i++)
      if (!gcc_types_compatible_p(c_type1.parameters()[i].type(),
...
void c_typecheck_baset::typecheck_expr_main(exprt &expr) {
  if (expr.id() == ID_side_effect)
  ...
  else if (expr.id() == ID_unary_plus || expr.id() == ID_unary_minus ||
           expr.id() == ID_bitnot)
    ...
    typecheck_expr_alignof(expr);
           ...
           expr.id() == ID_mult || expr.id() == ID_div || expr.id() == ID_mod ||
    ...
    Forall_irep(it,
                generic_associations) if (it->get(ID_type_arg) != ID_default) {
    ...
    typecheck_expr(expr);
  } else if (expr.id() == ID_gcc_asm_input || expr.id() == ID_gcc_asm_output ||
...
void c_typecheck_baset::typecheck_expr_operands(exprt &expr) {
  if (expr.id() == ID_side_effect &&
    ...
    ansi_c_declarationt &declaration = to_ansi_c_declaration(expr.op0());
...
void c_typecheck_baset::typecheck_expr_symbol(exprt &expr) {
  ...
  asm_label_mapt::const_iterator entry = asm_label_map.find(identifier);
...
// preserve enum key
    ...
    // preserve location
    expr.add_source_location() = source_location;
  } else if (has_prefix(id2string(identifier),
                        CPROVER_PREFIX "constant_infinity")) {
    expr = infinity_exprt(symbol.type);

    // put it back
    expr.add_source_location() = source_location;
  } else if (identifier == "__func__" || identifier == "__FUNCTION__" ||
...
void c_typecheck_baset::typecheck_expr_typecast(exprt &expr) {
  ...
  if (expr_type.id() == ID_union &&
    ...
    for (union_typet::componentst::const_iterator it = components.begin();
  ...
  if (is_numeric_type(op_type) || op_type.id() == ID_pointer) {
    ...
    const vector_typet &op_vector_type = to_vector_type(op_type);
...
void c_typecheck_baset::typecheck_expr_index(exprt &expr) {
  ...
  if (final_array_type.id() == ID_array || final_array_type.id() == ID_vector) {
...
void c_typecheck_baset::typecheck_expr_rel(binary_relation_exprt &expr) {
  ...
  if (follow(o_type0).id() == ID_vector || follow(o_type1).id() == ID_vector) {
  ...
  if (type0 == type1) {
    ...
    // pointer and zero
    if (type0.id() == ID_pointer && simplify_expr(op1, *this).is_zero()) {
      op1 = constant_exprt(ID_NULL, type0);
      return;
    }

    if (type1.id() == ID_pointer && simplify_expr(op0, *this).is_zero()) {
...
void c_typecheck_baset::typecheck_expr_rel_vector(binary_relation_exprt &expr) {
  exprt &op0 = expr.op0();
  exprt &op1 = expr.op1();
...
void c_typecheck_baset::typecheck_expr_ptrmember(exprt &expr) {
  ...
  if (final_op0_type.id() != ID_pointer && final_op0_type.id() != ID_array) {
    err_location(expr);
...
void c_typecheck_baset::typecheck_expr_trinary(if_exprt &expr) {
  ...
  if (operands[1].type().id() == ID_pointer &&
    ...
    if (operands[1].type().subtype().id() == ID_empty && tmp1.is_constant() &&
  ...
  if (follow(operands[1].type()) == follow(operands[2].type())) {
    ...
    if (operands[1].get_bool(ID_C_lvalue) && operands[2].get_bool(ID_C_lvalue))
      expr.set(ID_C_lvalue, true);
...
void c_typecheck_baset::typecheck_expr_address_of(exprt &expr) {
  ...
  if (op.id() == ID_address_of && op.get_bool(ID_C_implicit) &&
      op.operands().size() == 1 && op.op0().type().id() == ID_code) {
  ...
  if (op.id() == ID_struct || op.id() == ID_union || op.id() == ID_array ||
...
void c_typecheck_baset::typecheck_expr_side_effect(side_effect_exprt &expr) {
  ...
  if (statement == ID_preincrement || statement == ID_predecrement ||
...
void c_typecheck_baset::typecheck_side_effect_function_call(
  ...
  if (f_op.id() == ID_symbol) {
    ...
    if (symbol_table.symbols.find(identifier) == symbol_table.symbols.end()) {
      ...
      if (identifier == "malloc" || identifier == "realloc" ||
          identifier == "reallocf" || identifier == "valloc")
...
exprt c_typecheck_baset::do_special_functions(
  ...
  if (identifier == CPROVER_PREFIX "same_object") {
    ...
    exprt get_may_expr = binary_predicate_exprt(expr.arguments()[0], "get_may",
                                                expr.arguments()[1]);
    ...
    constant_exprt inf_expr =
        ieee_floatt::plus_infinity(ieee_float_spect::double_precision())
            .to_expr();
    ...
    constant_exprt inff_expr =
        ieee_floatt::plus_infinity(ieee_float_spect::single_precision())
            .to_expr();
    ...
    if (!base_type_eq(equality_expr.lhs().type(), equality_expr.rhs().type(),
                      *this)) {
      err_location(f_op);
    ...
    if (tmp1.id() == ID_typecast && tmp1.operands().size() == 1 &&
        tmp1.op0().id() == ID_address_of && tmp1.op0().operands().size() == 1 &&
    ...
    const irep_idt &id = identifier == CPROVER_PREFIX "float_debug1"
                             ? "float_debug1"
                             : "float_debug2";
...
void c_typecheck_baset::typecheck_function_call_arguments(
  ...
  if (code_type.get_bool(ID_C_incomplete)) {
    ...
    error() << "wrong number of function arguments: "
            << "expected " << parameter_types.size() << ", but got "
            << arguments.size() << eom;
    throw 0;
  }
  ...
  for (unsigned i = 0; i < arguments.size(); i++) {
    ...
    if (op.is_nil()) {
      ...
      const code_typet::parametert &parameter_type = parameter_types[i];

      const typet &op_type = parameter_type.type();

      if (op_type.id() == ID_bool && op.id() == ID_side_effect &&
...
void c_typecheck_baset::typecheck_expr_binary_arithmetic(exprt &expr) {
  ...
  if (o_type0.id() == ID_vector && o_type1.id() == ID_vector) {
    if (gcc_vector_types_compatible(to_vector_type(o_type0),
                                    to_vector_type(o_type1)) &&
  ...
  if (expr.id() == ID_plus || expr.id() == ID_minus || expr.id() == ID_mult ||
      expr.id() == ID_div) {
    if (type0.id() == ID_pointer || type1.id() == ID_pointer) {
  ...
  } else if (expr.id() == ID_bitand || expr.id() == ID_bitxor ||
             expr.id() == ID_bitor) {
    if (type0 == type1) {
      if (is_number(type0)) {
        expr.type() = type0;
...
void c_typecheck_baset::typecheck_expr_shifts(shift_exprt &expr) {
  ...
  if (o_type0.id() == ID_vector && is_number(follow(o_type0.subtype())) &&
      is_number(o_type1)) {
...
void c_typecheck_baset::typecheck_expr_pointer_arithmetic(exprt &expr) {
  ...
  if (expr.id() == ID_minus || (expr.id() == ID_side_effect &&
                                expr.get(ID_statement) == ID_assign_minus)) {
    if (type0.id() == ID_pointer && type1.id() == ID_pointer) {
    ...
    if (type0.id() == ID_pointer &&
        (type1.id() == ID_bool || type1.id() == ID_c_bool ||
         type1.id() == ID_unsignedbv || type1.id() == ID_signedbv ||
         type1.id() == ID_c_bit_field || type1.id() == ID_c_enum_tag)) {
      typecheck_arithmetic_pointer(op0);
      make_index_type(op1);
      expr.type() = type0;
      return;
             ...
             (expr.id() == ID_side_effect &&
              expr.get(ID_statement) == ID_assign_plus)) {
    exprt *p_op, *int_op;
    ...
    if (int_op_type.id() == ID_bool || int_op_type.id() == ID_c_bool ||
        int_op_type.id() == ID_unsignedbv || int_op_type.id() == ID_signedbv ||
...
void c_typecheck_baset::typecheck_side_effect_assignment(
...

  {
    ...
    if (type0.id() == ID_array || type0.id() == ID_incomplete_array) {
      err_location(expr);
  ...
  if (statement == ID_assign) {
  ...
  } else if (statement == ID_assign_bitxor || statement == ID_assign_bitand ||
             statement == ID_assign_bitor) {
    // these are more restrictive
    if (o_type0.id() == ID_bool || o_type0.id() == ID_c_bool) {
      implicit_typecast_arithmetic(op1);
      if (op1.type().id() == ID_bool || op1.type().id() == ID_c_bool ||
          op1.type().id() == ID_c_enum_tag ||
          op1.type().id() == ID_unsignedbv || op1.type().id() == ID_signedbv)
        return;
    } else if (o_type0.id() == ID_c_enum_tag || o_type0.id() == ID_unsignedbv ||
    ...
    if (o_type0.id() == ID_pointer &&
    ...
    } else if (o_type0.id() == ID_bool || o_type0.id() == ID_c_bool) {
      implicit_typecast_arithmetic(op1);
      if (op1.type().id() == ID_bool || op1.type().id() == ID_c_bool ||
          op1.type().id() == ID_c_enum_tag ||
          op1.type().id() == ID_unsignedbv || op1.type().id() == ID_signedbv)
      ...
      if (is_number(op1.type()) || op1.type().id() == ID_bool ||
