  ...
  const __CPROVER_jsa_abstract_heapt *const rhs)
{
  ...
  for(i= 0; i < __CPROVER_JSA_MAX_ABSTRACT_NODES; ++i)
  {
    ...
    if(
      lhs_node.list != rhs_node.list || lhs_node.next != rhs_node.next ||
  ...
  for(i= 0; i < __CPROVER_JSA_MAX_ABSTRACT_RANGES; ++i)
  {
    ...
    if(
      lhs_range.max != rhs_range.max || lhs_range.min != rhs_range.min ||
  ...
  for(i= 0; i < lhs->iterator_count; ++i)
  {
    ...
    if(
      lhs_it.index != rhs_it.index || lhs_it.list != rhs_it.list ||
...
#ifdef __CPROVER_JSA_DEFINE_TRANSFORMERS
{
  ...
  __CPROVER_jsa_assume(
    __CPROVER_jsa_null == __CPROVER_jsa__internal_get_head_node(h, list));
}
#else
  ;
#endif
...
#ifdef __CPROVER_JSA_DEFINE_TRANSFORMERS
{
      ...
      ++cnode)
  {
    ...
    if(__CPROVER_jsa_null == node_list)
    {
      ...
      __CPROVER_jsa_assume(
        __CPROVER_jsa_null == h->concrete_nodes[cnode].value);
    }
    else
    {
  ...
  for(__CPROVER_jsa__internal_index_t listc= 0; listc < __CPROVER_JSA_MAX_LISTS;
      ++listc)
  {
...
#ifdef __CPROVER_JSA_DEFINE_TRANSFORMERS
{
  ...
  else
  {
    ...
    __CPROVER_jsa_assume(
      list == __CPROVER_jsa__internal_get_list(heap, last_node));
    __CPROVER_jsa_assume(
      __CPROVER_jsa_null == __CPROVER_jsa__internal_get_next(heap, last_node));
#else
