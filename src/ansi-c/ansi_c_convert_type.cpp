...
void ansi_c_convert_typet::read_rec(const typet &type) {
  if (type.id() == ID_merged_type) {
    ...
    if (type.has_subtype() && type.subtype().id() == ID_string_constant)
      c_storage_spec.asm_label = type.subtype().get(ID_value);
  } else if (type.id() == ID_section && type.has_subtype() &&
    ...
    const exprt &size_expr = static_cast<const exprt &>(type.find(ID_size));
    const exprt &fsize_expr = static_cast<const exprt &>(type.find(ID_f));
    ...
    destructor = true;
  else if (type.id() == ID_alias && type.has_subtype() &&
           type.subtype().id() == ID_string_constant) {
...
void ansi_c_convert_typet::write(typet &type) {
  ...
  if (!other.empty()) {
    if (double_cnt || float_cnt || signed_cnt || unsigned_cnt || int_cnt ||
        c_bool_cnt || proper_bool_cnt || short_cnt || char_cnt || complex_cnt ||
        long_cnt || int8_cnt || int16_cnt || int32_cnt || int64_cnt ||
    ...
    if (constructor || destructor) {
      if (constructor && destructor) {
        error().source_location = source_location;
        error() << "combining constructor and destructor not supported" << eom;
        throw 0;
      }
    ...
    if (signed_cnt || unsigned_cnt || int_cnt || c_bool_cnt ||
        proper_bool_cnt || int8_cnt || int16_cnt || int32_cnt || int64_cnt ||
    ...
    if (signed_cnt || unsigned_cnt || int_cnt || short_cnt || int8_cnt ||
        int16_cnt || int32_cnt || int64_cnt || gcc_float128_cnt || bv_cnt ||
        proper_bool_cnt || char_cnt || long_cnt) {
      error().source_location = source_location;
    ...
    if (int_cnt || short_cnt || long_cnt || int8_cnt || int16_cnt ||
        int32_cnt || int64_cnt || gcc_float128_cnt || bv_cnt ||
        proper_bool_cnt) {
      error().source_location = source_location;
    ...
    if (int8_cnt || int16_cnt || int32_cnt || int64_cnt) {
      ...
      if (int8_cnt)
        ...
        type = is_signed ? signed_long_long_int_type()
