...
void ansi_c_convert_typet::write(typet &type)
{
  ...
  else if(proper_bool_cnt)
  {
    ...
    complex_cnt && !char_cnt && !signed_cnt && !unsigned_cnt && !short_cnt &&
    !gcc_int128_cnt)
  {
  ...
  {
    if(
      int_cnt || short_cnt || long_cnt || int8_cnt || int16_cnt || int32_cnt ||
      int64_cnt || gcc_float128_cnt || bv_cnt || proper_bool_cnt)
    {
      error().source_location= source_location;
