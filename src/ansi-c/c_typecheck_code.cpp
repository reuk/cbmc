...
void c_typecheck_baset::typecheck_code(codet &code) {
  ...
  } else if (statement == ID_assume || statement == ID_assert) {
...
void c_typecheck_baset::typecheck_asm(codet &code) {
  ...
  if (flavor == ID_gcc) {
    ...
    for (unsigned i = 1; i < code.operands().size(); i++) {
      ...
      Forall_operands(it, list) typecheck_expr(*it);
    }
...
void c_typecheck_baset::typecheck_decl(codet &code) {
  ...
  for (ansi_c_declarationt::declaratorst::const_iterator d_it =
    ...
    // or a typedef.
    if (!symbol.is_type && !symbol.is_extern &&
...
void c_typecheck_baset::typecheck_for(codet &code) {
  ...
  if (!config.ansi_c.for_has_scope || code.op0().is_nil()) {
    ...
    if (to_code(code.op3()).get_statement() == ID_block) {
      code_block.set(ID_C_end_location,
                     to_code_block(to_code(code.op3())).end_location());
    } else {
      code_block.set(ID_C_end_location, code.op3().source_location());
    }
...
void c_typecheck_baset::typecheck_return(codet &code) {
  if (code.operands().empty()) {
    if (follow(return_type).id() != ID_empty) {
      ...
      exprt zero = zero_initializer(return_type, code.source_location(), *this,
                                    get_message_handler());
