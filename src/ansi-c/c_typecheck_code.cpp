...
void c_typecheck_baset::typecheck_code(codet &code)
{
  ...
  else if(statement == ID_static_assert)
  {
    ...
    statement == ID_CPROVER_try_catch || statement == ID_CPROVER_try_finally)
  {
    assert(code.operands().size() == 2);
    typecheck_code(to_code(code.op0()));
    typecheck_code(to_code(code.op1()));
...
void c_typecheck_baset::typecheck_decl(codet &code)
{
      ...
      d_it++)
  {
    ...
    // or a typedef.
    if(!symbol.is_type && !symbol.is_extern && !is_complete_type(symbol.type))
    {
...
void c_typecheck_baset::typecheck_for(codet &code)
{
  ...
  else
  {
    ...
    {
      code_block.set(
        ID_C_end_location, to_code_block(to_code(code.op3())).end_location());
    }
    else
    {
