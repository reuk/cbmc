...
void ansi_c_internal_additions(std::string &code) {
  ...
  if (config.ansi_c.mode == configt::ansi_ct::flavourt::GCC ||
    ...
    if (config.ansi_c.arch == "i386" || config.ansi_c.arch == "x86_64" ||
    ...
    } else if (config.ansi_c.arch == "arm64" || config.ansi_c.arch == "armel" ||
...
void ansi_c_architecture_strings(std::string &code) {
  ...
  code +=
      architecture_string(config.ansi_c.use_fixed_for_float,
  ...
  code += architecture_string(static_cast<int>(config.ansi_c.endianness),
                              "endianness"); // NOLINT(whitespace/line_length)
  code += architecture_string(id2string(config.ansi_c.arch), "arch");
  code += architecture_string(configt::ansi_ct::os_to_string(config.ansi_c.os),
