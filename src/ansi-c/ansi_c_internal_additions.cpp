...
const char gcc_builtin_headers_ia32[]= "# 1 \"gcc_builtin_headers_ia32.h\"\n"
...
const char gcc_builtin_headers_arm[]= "# 1 \"gcc_builtin_headers_arm.h\"\n"
#include "gcc_builtin_headers_arm.inc"
  ; // NOLINT(whitespace/semicolon)

const char gcc_builtin_headers_mips[]= "# 1 \"gcc_builtin_headers_mips.h\"\n"
#include "gcc_builtin_headers_mips.inc"
  ; // NOLINT(whitespace/semicolon)

const char gcc_builtin_headers_power[]= "# 1 \"gcc_builtin_headers_power.h\"\n"
#include "gcc_builtin_headers_power.inc"
  ; // NOLINT(whitespace/semicolon)

const char arm_builtin_headers[]= "# 1 \"arm_builtin_headers.h\"\n"
#include "arm_builtin_headers.inc"
  ; // NOLINT(whitespace/semicolon)

const char cw_builtin_headers[]= "# 1 \"cw_builtin_headers.h\"\n"
#include "cw_builtin_headers.inc"
  ; // NOLINT(whitespace/semicolon)

const char clang_builtin_headers[]= "# 1 \"clang_builtin_headers.h\"\n"
...
{
  return std::string("const char *__CPROVER_architecture_") + std::string(s) +
         "=\"" + value + "\";\n";
}
...
{
  return std::string("const int __CPROVER_architecture_") + std::string(s) +
         "=" + std::to_string(value) + ";\n";
}
...
void ansi_c_internal_additions(std::string &code)
{
  code+=
    ...
    "__CPROVER_memory[__CPROVER_constant_infinity_uint];\n"
    ...
    // arrays
    // NOLINTNEXTLINE(whitespace/line_length)
    "__CPROVER_bool __CPROVER_array_equal(const void *array1, const void "
    "*array2);\n"
    ...
    config.ansi_c.mode == configt::ansi_ct::flavourt::ARM)
  {
      ...
      config.ansi_c.arch == "x32")
    {
      ...
      config.ansi_c.arch == "arm64" || config.ansi_c.arch == "armel" ||
      config.ansi_c.arch == "armhf" || config.ansi_c.arch == "arm")
    {
    ...
    else if(config.ansi_c.arch == "alpha")
    {
      ...
      config.ansi_c.arch == "mips64el" || config.ansi_c.arch == "mipsn32el" ||
      config.ansi_c.arch == "mipsel" || config.ansi_c.arch == "mips64" ||
      config.ansi_c.arch == "mipsn32" || config.ansi_c.arch == "mips")
    {
...
void ansi_c_architecture_strings(std::string &code)
{
  ...
  code+= architecture_string(
    ...
    "long_long_int_width"); // NOLINT(whitespace/line_length)
  ...
  code+= architecture_string(
    ...
    "long_double_width"); // NOLINT(whitespace/line_length)
  code+= architecture_string(config.ansi_c.wchar_t_width, "wchar_t_width");
  code+=
    architecture_string(config.ansi_c.char_is_unsigned, "char_is_unsigned");
  code+= architecture_string(
    config.ansi_c.wchar_t_is_unsigned,
    "wchar_t_is_unsigned"); // NOLINT(whitespace/line_length)
  code+= architecture_string(
    config.ansi_c.use_fixed_for_float,
    "fixed_for_float"); // NOLINT(whitespace/line_length)
  code+= architecture_string(config.ansi_c.alignment, "alignment");
  code+= architecture_string(
    config.ansi_c.memory_operand_size,
    "memory_operand_size"); // NOLINT(whitespace/line_length)
  code+= architecture_string(
    static_cast<int>(config.ansi_c.endianness),
    "endianness"); // NOLINT(whitespace/line_length)
  code+= architecture_string(id2string(config.ansi_c.arch), "arch");
  code+= architecture_string(
    configt::ansi_ct::os_to_string(config.ansi_c.os),
    "os"); // NOLINT(whitespace/line_length)
