...
public:
  change_impactt(const goto_modelt &model_old, const goto_modelt &model_new,
...
protected:
  ...
  void change_impact(const goto_programt &old_goto_program,
  ...
  void output_change_impact(const irep_idt &function,
  ...
  void output_instruction(char prefix, const goto_programt &goto_program,
                          const namespacet &ns, const irep_idt &function,
...
change_impactt::change_impactt(const goto_modelt &model_old,
                               ...
                               impact_modet _impact_mode, bool _compact_output)
    : impact_mode(_impact_mode), compact_output(_compact_output),
      old_goto_functions(model_old.goto_functions),
      ns_old(model_old.symbol_table),
      new_goto_functions(model_new.goto_functions),
      ns_new(model_new.symbol_table), unified_diff(model_old, model_new),
      old_dep_graph(ns_old), new_dep_graph(ns_new) {
...
void change_impactt::change_impact(const irep_idt &function) {
  ...
  change_impact(old_goto_program, new_goto_program, diff,
                old_change_impact[function], new_change_impact[function]);
}
...
void change_impactt::change_impact(
    ...
    goto_program_change_impactt &old_impact,
    goto_program_change_impactt &new_impact) {
  goto_programt::const_targett o_it = old_goto_program.instructions.begin();
  goto_programt::const_targett n_it = new_goto_program.instructions.begin();
  ...
  for (const auto &d : diff) {
    ...
    case unified_difft::differencet::NEW:
      ...
      assert(n_it == d.first);
      {
        ...
        if (impact_mode == FORWARD || impact_mode == BOTH)
          propogate_dep_forward(d_node, new_dep_graph, new_change_impact,
                                false);
      }
      new_impact[n_it] |= NEW;
      ++n_it;
...
void change_impactt::propogate_dep_forward(
    const dependence_grapht::nodet &d_node, const dependence_grapht &dep_graph,
...
void change_impactt::operator()() {
  ...
  for (goto_functions_change_impactt::const_iterator nc_it =
           new_change_impact.begin();
       nc_it != new_change_impact.end(); ++nc_it) {
    for (; oc_it != old_change_impact.end() && oc_it->first < nc_it->first;
         ++oc_it)
      output_change_impact(oc_it->first, oc_it->second, old_goto_functions,
                           ns_old);

    if (oc_it == old_change_impact.end() || nc_it->first < oc_it->first)
      output_change_impact(nc_it->first, nc_it->second, new_goto_functions,
                           ns_new);
    else {
      assert(oc_it->first == nc_it->first);

      output_change_impact(nc_it->first, oc_it->second, old_goto_functions,
                           ns_old, nc_it->second, new_goto_functions, ns_new);
...
void change_impactt::output_change_impact(
    const irep_idt &function, const goto_program_change_impactt &c_i,
    const goto_functionst &goto_functions, const namespacet &ns) const {
  ...
  forall_goto_program_instructions(target, goto_program) {
    goto_program_change_impactt::const_iterator c_entry = c_i.find(target);
    const unsigned mod_flags = c_entry == c_i.end() ? SAME : c_entry->second;
...
void change_impactt::output_change_impact(
    const irep_idt &function, const goto_program_change_impactt &o_c_i,
    const goto_functionst &o_goto_functions, const namespacet &o_ns,
    const goto_program_change_impactt &n_c_i,
    const goto_functionst &n_goto_functions, const namespacet &n_ns) const {
  ...
  goto_programt::const_targett o_target = old_goto_program.instructions.begin();
  forall_goto_program_instructions(target, goto_program) {
    ...
    goto_program_change_impactt::const_iterator c_entry = n_c_i.find(target);
    const unsigned mod_flags = c_entry == n_c_i.end() ? SAME : c_entry->second;
    ...
    if (mod_flags == SAME) {
      ...
      assert(old_mod_flags == SAME || old_mod_flags & DEL_DATA_DEP ||
  ...
  for (; o_target != old_goto_program.instructions.end(); ++o_target) {
    goto_program_change_impactt::const_iterator o_c_entry =
        o_c_i.find(o_target);
    const unsigned old_mod_flags =
        o_c_entry == o_c_i.end() ? SAME : o_c_entry->second;
...
void change_impactt::output_instruction(
    char prefix, const goto_programt &goto_program, const namespacet &ns,
    const irep_idt &function, goto_programt::const_targett &target) const {
  if (compact_output) {
    if (prefix == ' ')
      return;
    ...
    if (!file.empty() && !line.empty())
      std::cout << prefix << " " << id2string(file) << " " << id2string(line)
                << std::endl;
  } else {
    std::cout << prefix;
