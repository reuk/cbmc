...
void unified_difft::get_diff(const irep_idt &function,
  ...
  differences_mapt::const_iterator entry = differences_map.find(function);
  ...
  get_diff(function, old_goto_program, new_goto_program, entry->second, dest);
}
...
void unified_difft::output_diff(const irep_idt &identifier,
  ...
  get_diff(identifier, old_goto_program, new_goto_program, differences, diff);
...
void unified_difft::unified_diff(const irep_idt &identifier,
  ...
  if (old_goto_program.instructions.empty() ||
    ...
    if (new_goto_program.instructions.empty())
      differences.resize(old_goto_program.instructions.size(),
                         differencet::DELETED);
    else
      differences.resize(new_goto_program.instructions.size(),
                         differencet::NEW);
  } else
...
bool unified_difft::operator()() {
  ...
  for (function_mapt::const_iterator itn = new_funcs.begin();
    ...
    if (ito == old_funcs.end() || itn->first < ito->first)
      ...
      unified_diff(itn->first, ito->second->second.body,
