...
::goto_diff_parse_optionst::goto_diff_parse_optionst(
    int argc, const char **argv, const std::string &extra_options)
...
int goto_diff_parse_optionst::doit() {
  ...
  if (cmdline.isset("change-impact") || cmdline.isset("forward-impact") ||
...
int goto_diff_parse_optionst::get_goto_program(const optionst &options,
                                               goto_diff_languagest &languages,
                                               goto_modelt &goto_model) {
  status() << "Reading program from `" << cmdline.args[0] << "'" << eom;

  if (is_goto_binary(cmdline.args[0])) {
    if (read_goto_binary(cmdline.args[0], goto_model.symbol_table,
...
bool goto_diff_parse_optionst::process_goto_program(const optionst &options,
                                                    goto_modelt &goto_model) {
