...
public:
  ...
  void add_declarator(const symbol_exprt &expr) { add(ID_declarator, expr); }
  ...
  const irep_idt &throws_value() const { return find(ID_throw).get(ID_value); }

  const irep_idt &throws_label() const { return find(ID_throw).get(ID_label); }

  void add_value(const code_blockt &code) { add(ID_value, code); }

  const code_blockt &value() const {
    return static_cast<const code_blockt &>(find(ID_value));
  }

  code_blockt &value() { return static_cast<code_blockt &>(add(ID_value)); }
...
public:
  ...
  void swap(jsil_parse_treet &other) { items.swap(other.items); }
