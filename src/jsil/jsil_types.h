...
{
public:
  explicit jsil_builtin_code_typet(code_typet &code) : code_typet(code)
  {
    set("jsil_builtin_proceduret", true);
  }
};

inline jsil_builtin_code_typet &to_jsil_builtin_code_type(code_typet &code)
{
...
{
  return type.id() == ID_code && type.get_bool("jsil_builtin_proceduret");
}

class jsil_spec_code_typet : public code_typet
{
public:
  explicit jsil_spec_code_typet(code_typet &code) : code_typet(code)
  {
    set("jsil_spec_proceduret", true);
  }
};

inline jsil_spec_code_typet &to_jsil_spec_code_type(code_typet &code)
{
...
{
  return type.id() == ID_code && type.get_bool("jsil_spec_proceduret");
}
