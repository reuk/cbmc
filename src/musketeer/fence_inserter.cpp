  ...
  if(model == Power || model == Unknown)
  {
        ...
        ++e_i)
    {
          ...
          ++e_c_it)
      {
        ...
        assert(
          invisible_var.map_to_e.find(*e_c_it) != invisible_var.map_to_e.end());
        const_graph_visitor.CT(
          invisible_var.map_to_e.find(*e_c_it)->second, ct_set);
...
void inline fence_insertert::mip_set_var(ilpt &ilp, unsigned &i)
{
  ...
  for(; i <= unique * fence_options; i+= fence_options)
  {
    ...
    if(model == Power || model == Unknown)
    {
      ...
      glp_set_obj_coef(
        ilp.lp, i + 1, (has_cost ? fence_cost(Fence) : 0) * freq_sum);
...
// Note: uncomment for br and cf fences
      ...
      if(model == Power)
      {
        ...
        glp_set_obj_coef(
          ilp.lp, i + 2 /*4*/, (has_cost ? fence_cost(Lwfence) : 0) * freq_sum);
    ...
    else
    {
      ...
      glp_set_col_bnds(ilp.lp, i, GLP_LO, 0.0, 0.0);
      glp_set_obj_coef(
        ilp.lp, i, (has_cost ? fence_cost(Fence) : 0) * freq_sum);
      glp_set_col_kind(ilp.lp, i, GLP_BV);
    }
  }
#else
      ...
      ++e_i)
  {
        ...
        ++e_nc_it)
    {
      ...
      for(col= 1; col <= unique * fence_options; ++col)
      {
        ...
        {
          if(
            col == var_fence_to_col(Dp, *e_nc_it) ||
            (pt_set.find(col_to_var(col)) != pt_set.end() &&
             (col_to_fence(col) == Lwfence || col_to_fence(col) == Fence
#if 0
        ...
        ++e_i)
    {
          ...
          ++e_c_it)
      {
        ...
        for(col= 1; col <= unique * fence_options; ++col)
        {
          ...
          ilp.jmat[i]= col;
          if(
            (ct_set.find(col_to_var(col)) != ct_set.end() &&
             col_to_fence(col) == Fence) ||
...
void fence_insertert::print_to_file_4()
{
      ...
      ++it)
  {
    ...
    {
      instrumenter.message.warning()
        << "Couldn't retrieve types of variables " << first.variable << " and "
        << second.variable << " due to " << s << messaget::eom;
    }
  }
...
inline unsigned
fence_insertert::var_fence_to_col(fence_typet f, unsigned var) const
{
  switch(f)
  {
  case Fence:
