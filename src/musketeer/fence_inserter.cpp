...
void fence_insertert::preprocess() {
  ...
  for (std::list<std::set<event_idt>>::const_iterator e_i =
           powr_constraints.begin();
       e_i != powr_constraints.end(); ++e_i) {
    /* for all e */
    for (std::set<event_idt>::const_iterator e_c_it = e_i->begin();
         e_c_it != e_i->end(); ++e_c_it) {
  ...
  for (std::list<std::set<event_idt>>::const_iterator e_i =
       ...
       e_i != poww_constraints.end(); ++e_i) {
    /* for all e */
    for (std::set<event_idt>::const_iterator e_nc_it = e_i->begin();
         e_nc_it != e_i->end(); ++e_nc_it) {
      std::set<unsigned> pt_set;
...
void inline fence_insertert::mip_set_var(ilpt &ilp, unsigned &i) {
#ifdef HAVE_GLPK
  ...
  for (; i <= unique * fence_options; i += fence_options) {
    ...
    if (with_freq) {
      ...
      for (std::set<event_grapht::critical_cyclet>::const_iterator C_j =
        ...
        for (it = C_j->begin(); it != C_j->end() && col_to_var(i) != *it;
             ++it) {
        }
    ...
    if (model == Power || model == Unknown) {
      ...
      glp_set_obj_coef(ilp.lp, i + 1,
                       (has_cost ? fence_cost(Fence) : 0) * freq_sum);
...
void inline fence_insertert::mip_set_cst(ilpt &ilp, unsigned &i) {
  ...
  for (std::list<std::set<event_idt>>::const_iterator c_wr_it =
           powr_constraints.begin();
       c_wr_it != powr_constraints.end(); ++c_wr_it) {
    /* for all e */
    for (std::size_t j = 1; j <= c_wr_it->size(); ++j) {
      std::string name =
          "C_" + std::to_string(i) + "_c_wr_" + std::to_string(j);
  ...
  for (std::list<std::set<event_idt>>::const_iterator c_ww_it =
           poww_constraints.begin();
       c_ww_it != poww_constraints.end(); ++c_ww_it) {
    /* for all e */
    for (std::size_t j = 1; j <= c_ww_it->size(); ++j) {
      std::string name =
          "C_" + std::to_string(i) + "_c_ww_" + std::to_string(j);
      glp_set_row_name(ilp.lp, i, name.c_str());
      glp_set_row_bnds(ilp.lp, i, GLP_LO, 1.0, 0.0); /* >= 1*/
      ++i;
    }
  ...
  for (std::list<std::set<event_idt>>::const_iterator c_rw_it =
           porw_constraints.begin();
       c_rw_it != porw_constraints.end(); ++c_rw_it) {
    /* for all e */
    for (std::size_t j = 1; j <= c_rw_it->size(); ++j) {
      std::string name =
          "C_" + std::to_string(i) + "_c_rw_" + std::to_string(j);
      glp_set_row_name(ilp.lp, i, name.c_str());
      glp_set_row_bnds(ilp.lp, i, GLP_LO, 1.0, 0.0); /* >= 1*/
      ++i;
    }
  ...
  for (std::list<std::set<event_idt>>::const_iterator c_rr_it =
           porr_constraints.begin();
       c_rr_it != porr_constraints.end(); ++c_rr_it) {
    /* for all e */
    for (std::size_t j = 1; j <= c_rr_it->size(); ++j) {
      std::string name =
          "C_" + std::to_string(i) + "_c_rr_" + std::to_string(j);
      glp_set_row_name(ilp.lp, i, name.c_str());
      glp_set_row_bnds(ilp.lp, i, GLP_LO, 1.0, 0.0); /* >= 1*/
      ++i;
    }
  ...
  if (model == Power || model == Unknown) {
    for (std::list<std::set<event_idt>>::const_iterator c_it =
             com_constraints.begin();
         c_it != com_constraints.end(); ++c_it) {
      /* for all e */
...
void fence_insertert::solve() {
  ...
  instrumenter.message.statistics()
      << "po^+ edges considered:" << unique
      << " cycles:" << instrumenter.set_of_cycles.size() << messaget::eom;
  ...
  instrumenter.message.statistics()
      << "# of variables: " << const_unique * fence_options << messaget::eom;
...
// assert(i-1==mat_size);
  ...
  case GLP_OPT:
    instrumenter.message.result() << "Optimal solution found" << messaget::eom;
    break;
  case GLP_UNDEF:
  ...
  /* loads results (x_i) */
  instrumenter.message.statistics()
      << "minimal cost: " << glp_mip_obj_val(ilp.lp) << messaget::eom;
...
void fence_insertert::import_freq() { /* TODO */
}
...
void fence_insertert::print_to_file() {
  ...
  for (std::set<std::string>::const_iterator it = non_redundant_display.begin();
       it != non_redundant_display.end(); ++it)
    results << *it;
  results.close();
}
  ...
  for (std::map<edget, fence_typet>::const_iterator it = fenced_edges.begin();
    ...
    try {
      s << to_string(it->second) << "|" << first.source_location.get_file()
        ...
        << second.source_location.get_line() << "|" << second.variable
        << std::endl;
      non_redundant_display.insert(s.str());
    } catch (std::string s) {
...
inline unsigned fence_insertert::col_to_var(unsigned u) const {
  return (u - u % fence_options) / fence_options +
         (u % fence_options != 0 ? 1 : 0);
}
...
inline fence_insertert::fence_typet
fence_insertert::col_to_fence(unsigned u) const {
...
inline unsigned fence_insertert::var_fence_to_col(fence_typet f,
                                                  unsigned var) const {
  switch (f) {
  case Fence:
    return var * fence_options;
  case Dp:
    return (var - 1) * fence_options + 1;
  case Lwfence:
    return (var - 1) * fence_options + 2;
  case Branching:
    return (var - 1) * fence_options + 3;
  case Ctlfence:
    return (var - 1) * fence_options + 4;
  }
  assert(0);
}
