...
void goto_fence_inserter_parse_optionst::instrument_goto_program(
  ...
  if (cmdline.isset("mm") || cmdline.isset("all-shared") ||
    ...
    if (cmdline.isset("remove-function-pointers")) {
      ...
      remove_function_pointers(get_message_handler(), symbol_table,
                               goto_functions, cmdline.isset("pointer-check"));
    }
...
// return;
    ...
    if (cmdline.isset("all-shared-aeg")) {
      ...
      fence_pensieve(value_set_analysis, symbol_table, goto_functions,
                     unwind_loops, max_po_trans,
                     !cmdline.isset("no-po-rendering"),
                     cmdline.isset("render-cluster-file"),
                     cmdline.isset("render-cluster-function"),
                     cmdline.isset("naive"), get_message_handler());
      // simple analysis, coupled with script to insert;
      // does not transform the goto-binary
      return;
      ...
      /*if(model!=Unknown)*/
      fence_weak_memory(
          model, value_set_analysis, symbol_table, goto_functions,
          cmdline.isset("scc"), inst_strategy, unwind_loops,
          !cmdline.isset("cfg-kill"), cmdline.isset("no-dependencies"), loops,
          max_var, max_po_trans, !cmdline.isset("no-po-rendering"),
          cmdline.isset("render-cluster-file"),
          cmdline.isset("render-cluster-function"), cmdline.isset("cav11"),
          cmdline.isset("hide-internals"), cmdline.isset("print-graph"),
          infer_mode, get_message_handler(), cmdline.isset("ignore-arrays"));
    }
  }
...
void goto_fence_inserter_parse_optionst::help() {
  std::cout << "\n"
               ...
               " --mm <tso,pso,rmo,power>     detects all the fences to insert "
               "for a weak\n"
               "                              memory model\n"
               "\n"
               "Alternative methods:\n"
               "\n"
               // NOLINTNEXTLINE(whitespace/line_length)
               " --all-shared                 detects and fences all the "
               "accesses to shared\n"
               "                              variables (context insensitive)\n"
               // NOLINTNEXTLINE(whitespace/line_length)
               " --all-shared-aeg             detects all the accesses to "
               "shared variables\n"
               "                              (context sensitive)\n"
               // NOLINTNEXTLINE(whitespace/line_length)
               " --volatile                   detects all the accesses to "
               "volatile variables\n"
               " --pensieve                   detects all the pairs to be "
               "delayed with\n"
               "                              Pensieve's criteria (context "
               "sensitive)\n"
               // NOLINTNEXTLINE(whitespace/line_length)
               " --naive                      detects all the pairs to be "
               "delayed in a naive\n"
               "                              approach (context sensitive)\n"
               "\n"
               "Options:\n"
               "\n"
               " --remove-function-pointers   removes soundly function "
               "pointers based on\n"
               "                              their signatures\n"
               // NOLINTNEXTLINE(whitespace/line_length)
               " --async                      replaces all the pthread_creates "
               "by CPROVER_ASYNC\n"
               " --const-function-pointer-propagation\n"
               // NOLINTNEXTLINE(whitespace/line_length)
               "                              propagates the constant pointers "
               "to functions\n"
               // NOLINTNEXTLINE(whitespace/line_length)
               " --scc                        detects cycles in parallel (one "
               "thread/SCC)\n"
               // NOLINTNEXTLINE(whitespace/line_length)
               " --force-loop-duplication     duplicates the bodies of all the "
               "loops, and not\n"
               "                              only those with arrays accesses\n"
               " --no-loop-duplication        constructs back-edges for all "
               "the loops\n"
               // NOLINTNEXTLINE(whitespace/line_length)
               " --no-dependencies            ignores existing dependencies in "
               "the program\n"
               " --print-graph                prints the AEG into graph.dot\n"
               " --max-po-var <n>             limits the number of variables "
               "per cycle\n"
               " --max-po-trans <n>           limits the size of pos^+ in "
               "terms of pos\n"
               // NOLINTNEXTLINE(whitespace/line_length)
               " --ignore-arrays              ignores cycles with multiple "
               "accesses to the\n"
               "                              same array\n"
               "\n";
}
