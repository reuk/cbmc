...
protected:
  ...
  bool resolve(const irep_idt &symb, symbol_exprt &goto_function,
  ...
  symbol_exprt get(const irep_idt &symb) { return pointer_to_fun[symb]; }
...
public:
  ...
  void propagate() { propagate(goto_functionst::entry_point()); }
};
...
void const_function_pointer_propagationt::dup_caller_and_inline_callee(
  ...
  for (unsigned current_call = callsite_stack.size();
       current_call >= stack_scope; --current_call) {
...
void const_function_pointer_propagationt::arg_stackt::add_args(
  ...
  for (code_function_callt::argumentst::const_iterator arg_it = arg.begin();
       arg_it != arg.end(); ++arg_it, ++cor_arg_it) {
    ...
    if (arg_it->id() == ID_address_of) {
      ...
      for (std::unordered_map<irep_idt, unsigned, irep_id_hash>::const_iterator
           ...
           it != cfpp.fun_id_to_invok.end(); ++it)
...
void propagate_const_function_pointers(symbol_tablet &symbol_table,
  ...
  const_function_pointer_propagationt propagation(symbol_table, goto_functions,
                                                  message);
