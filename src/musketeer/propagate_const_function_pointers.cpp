...
protected:
  ...
  bool
  add(const irep_idt &symb, const symbol_exprt &goto_function, unsigned scope)
  {
  ...
  void dup_caller_and_inline_callee(
  ...
  unsigned stack_scope)
{
  ...
  goto_programt::const_targetst::const_iterator callsite=
    callsite_stack.begin();

  std::string last_suffix= "";

  for(unsigned current_call= callsite_stack.size(); current_call >= stack_scope;
      --current_call)
  {
    message.debug() << "current_call=" << current_call
                    << " stack_scope=" << stack_scope
                    << " callsite_stack.size()=" << callsite_stack.size()
                    << messaget::eom;
    ...
    goto_functionst::goto_functiont *pfunction_dup=
    ...
    if(current_call > stack_scope)
    {
      ...
      message.debug() << "duplicate function: " << function_id << messaget::eom;
      ...
      for(goto_programt::targett it= function_dup.body.instructions.begin();
          it != function_dup.body.instructions.end();
          ++it)
        it->function= function_new_id;

      assert(
        goto_functions.function_map[function_new_id].body.instructions.size() >
        0);

      /* removes in definition the argument leading to the const_function */
      code_typet::parameterst &args= function_dup.type.parameters();
      for(code_typet::parameterst::iterator it= args.begin(); it != args.end();
          ++it)
      {
        if(
          pointer_to_fun.find(it->get_identifier()) != pointer_to_fun.end() &&
      ...
      message.debug() << "new function "
                      << goto_functions.function_map[function_new_id]
                           .body.instructions.front()
                           .function
                      << " created." << messaget::eom;
    }

    if(current_call < callsite_stack.size())
    {
      message.debug() << "we then modify the previous callers" << messaget::eom;
          ...
          ++arg_it)
      {
        if(arg_it->id() == ID_symbol)
        {
          ...
          if(
            symb_arg.get_identifier() == const_function.get_identifier() ||
    ...
    else
    {
      ...
      message.debug() << "callsite targetted: " << (*callsite)->source_location
                      << " function: " << const_function.get_identifier()
                      << messaget::eom;
  ...
  goto_programt::instructionst::iterator it)
{
  ...
  /* retrieve the corresponding parameters expressions in the
     function definition */
  assert(
    cfpp.goto_functions.function_map.find(const_function.get_identifier()) !=
    cfpp.goto_functions.function_map.end());
      ...
      ++arg_it, ++cor_arg_it)
  {
    ...
    {
      cfpp.message.debug()
        << "fun: " << const_function.get_identifier() << " - arg: (symb) "
        << cfpp.symbol_table.lookup(to_symbol_expr(*arg_it).get_identifier())
             .base_name
        << messaget::eom;
      ...
      {
        cfpp.add(
          cor_arg_it->get_base_name(),
          cfpp.get(arg_symbol.base_name),
...
void const_function_pointer_propagationt::propagate(const irep_idt &function_id)
{
  if(
  ...
  Forall_goto_program_instructions(it, function.body)
  {
    ...
    else if(it->is_function_call())
    {
      ...
      {
        message.debug() << "Propagates through "
                        << to_symbol_expr(fun).get_identifier()
                        << messaget::eom;
  ...
  message_handlert &message_handler)
{
  ...
  const_function_pointer_propagationt propagation(
    symbol_table, goto_functions, message);
