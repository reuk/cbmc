...
void const_graph_visitort::graph_explore(event_grapht &egraph, event_idt next,
  ...
  if (next == end) {
    ...
    for (wmm_grapht::edgest::const_iterator next_it =
         ...
         next_it != egraph.po_out(next).end(); ++next_it) {
...
void const_graph_visitort::const_graph_explore(event_grapht &egraph,
                                               event_idt next, event_idt end,
                                               std::list<event_idt> &old_path) {
  if (next == end) {
    /* inserts all the pos collected from old_path in edges */
    std::list<event_idt>::const_iterator it = old_path.begin();
...
void const_graph_visitort::graph_explore_BC(event_grapht &egraph,
  ...
  fence_inserter.instrumenter.message.debug()
      << "(BC) explore " << old_path.front() << " --...--> " << next
      << messaget::eom;
  ...
  for (wmm_grapht::edgest::const_iterator it = egraph.po_out(next).begin();
       it != egraph.po_out(next).end(); ++it)
...
void const_graph_visitort::const_graph_explore_BC(
    event_grapht &egraph, event_idt next, std::list<event_idt> &old_path) {
...
void const_graph_visitort::graph_explore_AC(event_grapht &egraph,
  ...
  fence_inserter.instrumenter.message.debug()
      << "(AC) explore " << old_path.front() << " --...--> " << next
      << messaget::eom;
  ...
  /* if all the incoming pos were already visited, add */
  bool no_other_pos = true;
  for (wmm_grapht::edgest::const_iterator it = egraph.po_in(next).begin();
       it != egraph.po_in(next).end(); ++it)
    if (visited_nodes.find(it->first) == visited_nodes.end()) {
      no_other_pos = false;
      break;
    }
  ...
  if (egraph.po_in(next).size() == 0 || no_other_pos) {
    ...
    for (wmm_grapht::edgest::const_iterator next_it =
         ...
         next_it != egraph.po_in(next).end(); ++next_it) {
      old_path.push_back /*front*/ (next_it->first);
...
void const_graph_visitort::PT(const edget &e, std::set<unsigned> &edges) {
  ...
  if (list_po_out.find(e.second) == list_po_out.end()) {
    ...
    for (wmm_grapht::edgest::const_iterator next_it =
             egraph.po_out(e.first).begin();
         next_it != egraph.po_out(e.first).end(); ++next_it) {
...
void const_graph_visitort::CT(const edget &edge, std::set<unsigned> &edges) {
  ...
  const event_idt first =
      (test_first.operation == abstract_eventt::Write ? edge.first
                                                      : edge.second);
  const event_idt second =
      (test_second.operation == abstract_eventt::Read ? edge.second
                                                      : edge.first);
  ...
  if (!egraph.po_in(first).empty()) {
    for (wmm_grapht::edgest::const_iterator next_it =
             egraph.po_in(first).begin();
         next_it != egraph.po_in(first).end(); ++next_it) {
      std::list<event_idt> new_path;
  ...
  if (!egraph.po_out(second).empty()) {
    for (wmm_grapht::edgest::const_iterator next_it =
             egraph.po_out(second).begin();
         next_it != egraph.po_out(second).end(); ++next_it) {
      std::list<event_idt> new_path;
