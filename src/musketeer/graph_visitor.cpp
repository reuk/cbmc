  ...
  std::set<unsigned> &edges)
{
  ...
  {
    for(wmm_grapht::edgest::const_iterator next_it= egraph.po_out(next).begin();
  ...
  bool porw)
{
  ...
  if(egraph.po_out(next).size() == 0 || no_other_pos)
  {
    ...
    for(; next_it != old_path.end() && it != old_path.end(); ++it, ++next_it)
    {
      ...
      if(
        !porw || (e1.operation == abstract_eventt::Read &&
  ...
  bool porw)
{
  ...
  {
    for(wmm_grapht::edgest::const_iterator next_it= egraph.po_in(next).begin();
...
void const_graph_visitort::CT(const edget &edge, std::set<unsigned> &edges)
{
  ...
  {
    for(wmm_grapht::edgest::const_iterator next_it= egraph.po_in(first).begin();
