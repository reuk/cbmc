...
#define SYMEX_OPTIONS                                                          \
  "(function):"                                                                \
  "D:I:"                                                                       \
  "(depth):(context-bound):(branch-bound):(unwind):" OPT_GOTO_CHECK            \
  ...
  "(drop-unused-functions)" OPT_SHOW_GOTO_FUNCTIONS                            \
  "(property):(trace)(show-trace)(stop-on-fail)(eager-infeasibility)"          \
  "(no-simplify)(no-unwinding-assertions)(no-propagation)"
// the last line is for CBMC-regression testing only

class symex_parse_optionst : public parse_options_baset, public language_uit
{
public:
  virtual int doit();
  virtual void help();
