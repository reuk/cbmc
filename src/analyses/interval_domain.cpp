...
void interval_domaint::output(std::ostream &out, const ai_baset &ai,
...
void interval_domaint::transform(locationt from, locationt to, ai_baset &ai,
                                 const namespacet &ns) {
...
bool interval_domaint::merge(const interval_domaint &b, locationt from,
                             locationt to) {
  if (b.bottom)
    return false;
  if (bottom) {
...
void interval_domaint::assume_rec(const exprt &lhs, irep_idt id,
                                  const exprt &rhs) {
...
void interval_domaint::assume_rec(const exprt &cond, bool negation) {
  if (cond.id() == ID_lt || cond.id() == ID_le || cond.id() == ID_gt ||
    ...
    if (!negation)
      forall_operands(it, cond) assume_rec(*it, false);
  } else if (cond.id() == ID_or) {
    if (negation)
      forall_operands(it, cond) assume_rec(*it, true);
  }
}
