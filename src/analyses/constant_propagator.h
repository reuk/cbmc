...
private:
  void assign(valuest &dest, const symbol_exprt &lhs, exprt rhs,
              const namespacet &ns) const;

  void assign_rec(valuest &values, const exprt &lhs, const exprt &rhs,
                  const namespacet &ns);
...
public:
  constant_propagator_ait(goto_functionst &goto_functions,
                          const namespacet &ns) {
    operator()(goto_functions, ns);
    replace(goto_functions, ns);
  }

  constant_propagator_ait(goto_functionst::goto_functiont &goto_function,
                          const namespacet &ns) {
