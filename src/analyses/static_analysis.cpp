...
exprt static_analysis_baset::get_guard(locationt from, locationt to) {
  if (!from->is_goto())
    return true_exprt();
...
void static_analysis_baset::generate_states(const goto_programt &goto_program) {
  forall_goto_program_instructions(i_it, goto_program) generate_state(i_it);
}
...
void static_analysis_baset::update(const goto_functionst &goto_functions) {
  forall_goto_functions(f_it, goto_functions) update(f_it->second.body);
}
...
bool static_analysis_baset::visit(locationt l, working_sett &working_set,
  ...
  for (const auto &to_l : goto_program.get_successors(l)) {
    ...
    if (l->is_function_call()) {
      ...
      do_function_call_rec(l, to_l, code.function(), code.arguments(),
                           new_values, goto_functions);
    } else
      new_values.transform(ns, l, to_l);

    statet &other = get_state(to_l);

    bool have_new_values = merge(other, new_values, to_l);
...
void static_analysis_baset::do_function_call(
    locationt l_call, locationt l_return, const goto_functionst &goto_functions,
...

  {
    ...
    std::unique_ptr<statet> tmp_state(make_temporary_state(end_of_function));
...
void static_analysis_baset::do_function_call_rec(
    locationt l_call, locationt l_return, const exprt &function,
    const exprt::operandst &arguments, statet &new_state,
    const goto_functionst &goto_functions) {
  ...
  if (function.id() == ID_symbol) {
    ...
    do_function_call(l_call, l_return, goto_functions, it, arguments,
                     new_state);
    ...
    do_function_call_rec(l_call, l_return, function.op1(), arguments, new_state,
                         goto_functions);

    do_function_call_rec(l_call, l_return, function.op2(), arguments, *n2,
                         goto_functions);
    ...
    for (const auto &value : values) {
      if (value.id() == ID_object_descriptor) {
        ...
        std::unique_ptr<statet> n2(make_temporary_state(new_state));
        do_function_call_rec(l_call, l_return, o.object(), arguments, *n2,
                             goto_functions);
        merge(new_state, *n2, l_return);
      }
    }
...
void static_analysis_baset::sequential_fixedpoint(
  ...
  forall_goto_functions(
      it, goto_functions) if (functions_done.insert(it->first).second)
...
void static_analysis_baset::concurrent_fixedpoint(
  ...
  typedef std::list<
      std::pair<goto_programt const *, goto_programt::const_targett>>
      thread_wlt;
