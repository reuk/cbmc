...
public:
  goto_checkt(const namespacet &_ns, const optionst &_options)
      : ns(_ns), local_bitvector_analysis(0) {
...
protected:
  ...
  void pointer_validity_check(const dereference_exprt &expr,
                              const guardt &guard, const exprt &access_lb,
                              const exprt &access_ub, const irep_idt &mode);
  ...
  void add_guarded_claim(const exprt &expr, const std::string &comment,
                         ...
                         const source_locationt &, const exprt &src_expr,
                         const guardt &guard);
...
void goto_checkt::invalidate(const exprt &lhs) {
  if (lhs.id() == ID_index)
    ...
    for (assertionst::iterator it = assertions.begin();
         it != assertions.end();) // no it++
    {
      assertionst::iterator next = it;
      next++;

      if (has_symbol(*it, find_symbols_set) || has_dereference(*it))
        assertions.erase(it);
...
void goto_checkt::div_by_zero_check(const div_exprt &expr,
  ...
  add_guarded_claim(inequality, "division by zero", "division-by-zero",
                    expr.find_source_location(), expr, guard);
}
...
void goto_checkt::undefined_shift_check(const shift_exprt &expr,
  ...
  const typet &distance_type = ns.follow(expr.distance().type());

  if (distance_type.id() == ID_signedbv) {
    binary_relation_exprt inequality(expr.distance(), ID_ge,
                                     from_integer(0, distance_type));

    add_guarded_claim(inequality, "shift distance is negative",
                      "undefined-shift", expr.find_source_location(), expr,
                      guard);
  }
  ...
  if (op_type.id() == ID_unsignedbv || op_type.id() == ID_signedbv) {
    ...
    binary_relation_exprt inequality(expr.distance(), ID_lt, width_expr);

    add_guarded_claim(inequality, "shift distance too large", "undefined-shift",
                      expr.find_source_location(), expr, guard);
  }
}
...
void goto_checkt::conversion_check(const exprt &expr, const guardt &guard) {
...
void goto_checkt::integer_overflow_check(const exprt &expr,
  ...
  if (expr.id() == ID_div) {
    ...
    if (type.id() == ID_signedbv) {
      equal_exprt int_min_eq(expr.op0(),
                             to_signedbv_type(type).smallest_expr());

      equal_exprt minus_one_eq(expr.op1(), from_integer(-1, type));

      add_guarded_claim(not_exprt(and_exprt(int_min_eq, minus_one_eq)),
    ...
    if (type.id() == ID_signedbv) {
      ...
      add_guarded_claim(not_exprt(int_min_eq),
...
void goto_checkt::float_overflow_check(const exprt &expr, const guardt &guard) {
  ...
  if (expr.id() == ID_typecast) {
    ...
    if (ns.follow(expr.op0().type()).id() == ID_floatbv) {
      ...
      add_guarded_claim(not_exprt(new_inf),
                        "arithmetic overflow on floating-point typecast",
                        "overflow", expr.find_source_location(), expr, guard);
    }
...
void goto_checkt::nan_check(const exprt &expr, const guardt &guard) {
  ...
  if (expr.id() != ID_plus && expr.id() != ID_mult && expr.id() != ID_div &&
      expr.id() != ID_minus)
    return;
  ...
  if (expr.id() == ID_div) {
    ...
    isnan = or_exprt(and_exprt(equal_exprt(expr.op0(), minus_inf),
                               equal_exprt(expr.op1(), plus_inf)),
                     and_exprt(equal_exprt(expr.op0(), plus_inf),
    ...
    isnan = or_exprt(and_exprt(equal_exprt(expr.op0(), plus_inf),
                               equal_exprt(expr.op1(), plus_inf)),
                     and_exprt(equal_exprt(expr.op0(), minus_inf),
                               equal_exprt(expr.op1(), minus_inf)));
  } else
    assert(false);
...
void goto_checkt::pointer_validity_check(const dereference_exprt &expr,
  ...
  if (mode == ID_java) {
    if (flags.is_unknown() || flags.is_null()) {
      ...
      add_guarded_claim(not_eq_null, "reference is null", "pointer dereference",
                        expr.find_source_location(), expr, guard);
    }
  } else {
    if (flags.is_unknown() || flags.is_null()) {
      add_guarded_claim(
          not_exprt(null_pointer(pointer)), "dereference failure: pointer NULL",
          "pointer dereference", expr.find_source_location(), expr, guard);
    }

    if (flags.is_unknown())
      add_guarded_claim(not_exprt(invalid_pointer(pointer)),
                        "dereference failure: pointer invalid",
                        "pointer dereference", expr.find_source_location(),
                        expr, guard);
    ...
    if (flags.is_unknown() || flags.is_dynamic_heap())
      add_guarded_claim(not_exprt(deallocated(pointer, ns)),
                        "dereference failure: deallocated dynamic object",
                        "pointer dereference", expr.find_source_location(),
                        expr, guard);

    if (flags.is_unknown() || flags.is_dynamic_local())
      add_guarded_claim(not_exprt(dead_object(pointer, ns)),
                        "dereference failure: dead object",
                        "pointer dereference", expr.find_source_location(),
                        expr, guard);

    if (flags.is_unknown() || flags.is_dynamic_heap()) {
      exprt dynamic_bounds = or_exprt(
...
void goto_checkt::bounds_check(const index_exprt &expr, const guardt &guard) {
  if (!enable_bounds_check)
    return;

  if (expr.find("bounds_check").is_not_nil() && !expr.get_bool("bounds_check"))
    return;
  ...
  if (index.type().id() != ID_unsignedbv) {
    ...
    if (index.id() == ID_typecast && index.operands().size() == 1 &&
  ...
  if (ode.root_object().id() == ID_dereference) {
    ...
    if_exprt size(dynamic_object(pointer),
    ...
    or_exprt precond(and_exprt(dynamic_object(pointer),
                               not_exprt(malloc_object(pointer, ns))),
                     inequality);

    add_guarded_claim(precond, name + " dynamic object upper bound",
                      "array bounds", expr.find_source_location(), expr, guard);

    exprt type_size = size_of_expr(ode.root_object().type(), ns);
    if (type_size.is_not_nil())
      type_matches_size =
          equal_exprt(size, typecast_exprt(type_size, size.type()));
  }
  ...
  if (size.is_nil()) {
    ...
    add_guarded_claim(implies_exprt(type_matches_size, inequality),
...
void goto_checkt::check_rec(const exprt &expr, guardt &guard, bool address,
  ...
  if (address) {
    if (expr.id() == ID_dereference) {
      ...
      forall_operands(it, expr) check_rec(*it, guard, true, mode);
    }
    return;
  }
    ...
    for (const auto &op : expr.operands()) {
      if (!op.is_boolean())
        throw "`" + expr.id_string() +
            "' takes Boolean operands only, but got " + op.pretty();
  ...
  forall_operands(it, expr) check_rec(*it, guard, false, mode);
  ...
  else if (expr.id() == ID_le || expr.id() == ID_lt || expr.id() == ID_ge ||
           expr.id() == ID_gt)
    pointer_rel_check(expr, guard);
  else if (expr.id() == ID_dereference)
    pointer_validity_check(to_dereference_expr(expr), guard, nil_exprt(),
                           size_of_expr(expr.type(), ns), mode);
}
...
void goto_checkt::goto_check(goto_functiont &goto_function,
  ...
  Forall_goto_program_instructions(it, goto_program) {
    ...
    for (const auto &label : error_labels) {
      if (std::find(i.labels.begin(), i.labels.end(), label) !=
          i.labels.end()) {
        goto_program_instruction_typet type =
            enable_assert_to_assume ? ASSUME : ASSERT;
    ...
    if (i.is_other()) {
      ...
      if (statement == ID_expression) {
        ...
        forall_operands(it, i.code) check(*it, mode);
      }
    } else if (i.is_assign()) {
      ...
      if (mode == ID_java && enable_pointer_check &&
        ...
        if (flags.is_unknown() || flags.is_null()) {
          ...
          add_guarded_claim(not_eq_null, "this is null on method invokation",
                            "pointer dereference", i.source_location, pointer,
                            guardt());
        }
      }

      forall_operands(it, code_function_call) check(*it, mode);
      ...
      if (i.code.get_statement() == ID_expression &&
        ...
        if (pointer.type().subtype().get(ID_identifier) !=
          ...
          add_guarded_claim(not_eq_null, "throwing null", "pointer dereference",
                            i.source_location, pointer, guardt());
        }
      }
      ...
      if (enable_pointer_check) {
        ...
        if (local_bitvector_analysis->dirty(variable)) {
          ...
          exprt rhs = if_exprt(side_effect_expr_nondett(bool_typet()),
                               address_of_expr, lhs, lhs.type());
          t->source_location = i.source_location;
      ...
      if (i.function == goto_functionst::entry_point() &&
        ...
        equal_exprt eq(leak_expr,
                       null_pointer_exprt(to_pointer_type(leak.type)));
        add_guarded_claim(eq, "dynamically allocated memory never freed",
                          "memory-leak", source_location, eq, guardt());
      }
    }
...
void goto_check(const namespacet &ns, const optionst &options,
