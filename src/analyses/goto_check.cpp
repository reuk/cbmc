...
void goto_checkt::div_by_zero_check(const div_exprt &expr, const guardt &guard)
{
...
void goto_checkt::mod_by_zero_check(const mod_exprt &expr, const guardt &guard)
{
  if(!enable_div_by_zero_check)
    return;
...
void goto_checkt::integer_overflow_check(const exprt &expr, const guardt &guard)
{
...
void goto_checkt::nan_check(const exprt &expr, const guardt &guard)
{
  ...
  else if(expr.id() == ID_plus)
  {
    ...
    isnan= or_exprt(
      and_exprt(
        equal_exprt(expr.op0(), minus_inf), equal_exprt(expr.op1(), plus_inf)),
      and_exprt(
        equal_exprt(expr.op0(), plus_inf), equal_exprt(expr.op1(), minus_inf)));
  }
  else if(expr.id() == ID_minus)
  {
    ...
    isnan= or_exprt(
      and_exprt(
        equal_exprt(expr.op0(), plus_inf), equal_exprt(expr.op1(), plus_inf)),
      and_exprt(
...
void goto_checkt::pointer_overflow_check(const exprt &expr, const guardt &guard)
{
...
void goto_checkt::bounds_check(const index_exprt &expr, const guardt &guard)
{
  ...
  if(index.type().id() != ID_unsignedbv)
  {
    ...
    if(
      index.id() == ID_typecast && index.operands().size() == 1 &&
  ...
  const irep_idt &mode)
{
  ...
  else if(expr.id() == ID_if)
  {
    ...
    {
      std::string msg=
        "first argument of if must be boolean, but got " + expr.op0().pretty();
      throw msg;
    }
  ...
  const irep_idt &mode)
{
  ...
  Forall_goto_program_instructions(it, goto_program)
  {
    ...
    {
      if(
        i.code.get_statement() == ID_expression &&
        i.code.operands().size() == 1 && i.code.op0().operands().size() == 1)
      {
        // must not throw NULL
        ...
        if(
          pointer.type().subtype().get(ID_identifier) !=
