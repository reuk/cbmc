...
bool local_bitvector_analysist::loc_infot::merge(const loc_infot &src) {
  ...
  std::size_t max_index = std::max(src.points_to.size(), points_to.size());
...
bool local_bitvector_analysist::is_tracked(const irep_idt &identifier) {
  ...
  if (it == locals.locals_map.end() || it->second.id() != ID_pointer ||
      dirty(identifier))
    return false;
...
void local_bitvector_analysist::assign_lhs(const exprt &lhs, const exprt &rhs,
  ...
  if (lhs.id() == ID_symbol) {
    ...
    assign_lhs(to_member_expr(lhs).struct_op(), rhs, loc_info_src,
               loc_info_dest);
...
local_bitvector_analysist::get(const goto_programt::const_targett t,
                               const exprt &rhs) {
...
local_bitvector_analysist::flagst
local_bitvector_analysist::get_rec(const exprt &rhs,
                                   const loc_infot &loc_info_src) {
...
void local_bitvector_analysist::build(const goto_functiont &goto_function) {
  ...
  for (const auto &local : locals.locals_map)
    if (is_tracked(local.first))
      loc_infos[0].points_to[pointers.number(local.first)] =
          flagst::mk_unknown();
  ...
  while (!work_queue.empty()) {
    ...
    case ASSIGN: {
      ...
      assign_lhs(code_assign.lhs(), code_assign.rhs(), loc_info_src,
                 loc_info_dest);
    } break;

    case DECL: {
      const code_declt &code_decl = to_code_decl(instruction.code);
      assign_lhs(code_decl.symbol(), exprt(ID_uninitialized), loc_info_src,
                 loc_info_dest);
    } break;

    case DEAD: {
      const code_deadt &code_dead = to_code_dead(instruction.code);
      assign_lhs(code_dead.symbol(), exprt(ID_uninitialized), loc_info_src,
                 loc_info_dest);
    } break;
    ...
    case FUNCTION_CALL: {
      ...
      if (code_function_call.lhs().is_not_nil())
        assign_lhs(code_function_call.lhs(), nil_exprt(), loc_info_src,
                   loc_info_dest);
    } break;
...
void local_bitvector_analysist::output(std::ostream &out,
  ...
  forall_goto_program_instructions(i_it, goto_function.body) {
    ...
    for (points_tot::const_iterator p_it = loc_info.points_to.begin();
         p_it != loc_info.points_to.end(); p_it++) {
