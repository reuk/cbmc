...
public:
  ...
  explicit local_bitvector_analysist(const goto_functiont &_goto_function)
      : dirty(_goto_function), locals(_goto_function),
        cfg(_goto_function.body) {
    build(_goto_function);
  }

  void output(std::ostream &out, const goto_functiont &goto_function,
  ...
  struct flagst {
    ...
    static flagst mk_unknown() { return flagst(B_unknown); }

    bool is_unknown() const { return (bits & B_unknown) != 0; }

    static flagst mk_uninitialized() { return flagst(B_uninitialized); }

    bool is_uninitialized() const { return (bits & B_uninitialized) != 0; }

    static flagst mk_uses_offset() { return flagst(B_uses_offset); }

    bool is_uses_offset() const { return (bits & B_uses_offset) != 0; }

    static flagst mk_dynamic_local() { return flagst(B_dynamic_local); }

    bool is_dynamic_local() const { return (bits & B_dynamic_local) != 0; }

    static flagst mk_dynamic_heap() { return flagst(B_dynamic_heap); }

    bool is_dynamic_heap() const { return (bits & B_dynamic_heap) != 0; }

    static flagst mk_null() { return flagst(B_null); }

    bool is_null() const { return (bits & B_null) != 0; }

    static flagst mk_static_lifetime() { return flagst(B_static_lifetime); }

    bool is_static_lifetime() const { return (bits & B_static_lifetime) != 0; }

    static flagst mk_integer_address() { return flagst(B_integer_address); }

    bool is_integer_address() const { return (bits & B_integer_address) != 0; }
...
protected:
  ...
  void assign_lhs(const exprt &lhs, const exprt &rhs,
                  const loc_infot &loc_info_src, loc_infot &loc_info_dest);
