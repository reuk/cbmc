...
template <class P, class T> class natural_loops_templatet {
public:
  ...
  void operator()(P &program) { compute(program); }
  ...
  explicit natural_loops_templatet(P &program) { compute(program); }
...
void natural_loops_templatet<P, T>::compute(P &program) {
  ...
  for (T m_it = program.instructions.begin();
       m_it != program.instructions.end(); ++m_it) {
...
#ifdef DEBUG
          std::cout << "Computing loop for " << m_it->location_number << " -> "
...
void natural_loops_templatet<P, T>::compute_natural_loop(T m, T n) {
  ...
  while (!stack.empty()) {
    ...
    const nodet &node = cfg_dominators.cfg[cfg_dominators.cfg.entry_map[p]];
