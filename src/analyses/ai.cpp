...
void ai_baset::output(const namespacet &ns, const goto_programt &goto_program,
...
xmlt ai_baset::output_xml(const namespacet &ns,
  ...
  forall_goto_functions(f_it, goto_functions) {
    ...
    function.set_attribute("body_available",
...
xmlt ai_baset::output_xml(const namespacet &ns,
  ...
  forall_goto_program_instructions(i_it, goto_program) {
    xmlt location;
    location.set_attribute("location_number",
                           std::to_string(i_it->location_number));
    location.set_attribute("source_location",
...
void ai_baset::initialize(const goto_programt &goto_program) {
  ...
  forall_goto_program_instructions(i_it, goto_program) get_state(i_it)
      .make_bottom();
}
...
void ai_baset::initialize(const goto_functionst &goto_functions) {
  forall_goto_functions(it, goto_functions) initialize(it->second);
}
...
bool ai_baset::visit(locationt l, working_sett &working_set,
                     const goto_programt &goto_program,
                     const goto_functionst &goto_functions,
                     const namespacet &ns) {
  bool new_data = false;
  ...
  for (const auto &to_l : goto_program.get_successors(l)) {
    ...
    if (l->is_function_call() && !goto_functions.function_map.empty()) {
      ...
      if (do_function_call_rec(l, to_l, code.function(), code.arguments(),
                               goto_functions, ns))
...
bool ai_baset::do_function_call_rec(locationt l_call, locationt l_return,
  ...
  if (function.id() == ID_symbol) {
    ...
    bool new_data1 = do_function_call_rec(l_call, l_return, function.op1(),
                                          arguments, goto_functions, ns);

    bool new_data2 = do_function_call_rec(l_call, l_return, function.op2(),
                                          arguments, goto_functions, ns);
