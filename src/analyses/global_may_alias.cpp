...
void global_may_alias_domaint::assign_lhs_aliases(
    const exprt &lhs, const std::set<irep_idt> &alias_set) {
  if (lhs.id() == ID_symbol) {
...
void global_may_alias_domaint::get_rhs_aliases_address_of(
    const exprt &rhs, std::set<irep_idt> &alias_set) {
  if (rhs.id() == ID_symbol) {
    irep_idt identifier = to_symbol_expr(rhs).get_identifier();
...
void global_may_alias_domaint::transform(locationt from, locationt to,
...
void global_may_alias_domaint::output(std::ostream &out, const ai_baset &ai,
