...
public:
  ...
  virtual void transform(locationt from, locationt to, ai_baset &ai,
                         const namespacet &ns) = 0;

  virtual void output(std::ostream &out, const ai_baset &ai,
...
public:
  ...
  void operator()(const goto_functionst::goto_functiont &goto_function,
                  const namespacet &ns) {
    goto_functionst goto_functions;
  ...
  virtual void output(const namespacet &ns,
  ...
  void output(const namespacet &ns, const goto_programt &goto_program,
              std::ostream &out) const {
  ...
  virtual jsont output_json(const namespacet &ns,
  ...
  jsont output_json(const namespacet &ns,
                    const goto_programt &goto_program) const {
    return output_json(ns, goto_program, "");
  }

  jsont
  output_json(const namespacet &ns,
              const goto_functionst::goto_functiont &goto_function) const {
    return output_json(ns, goto_function.body, "");
  }

  virtual xmlt output_xml(const namespacet &ns,
                          const goto_functionst &goto_functions) const;
  ...
  xmlt output_xml(const namespacet &ns,
                  const goto_programt &goto_program) const {
...
protected:
  ...
  virtual void output(const namespacet &ns, const goto_programt &goto_program,
  ...
  // function calls
  bool do_function_call_rec(locationt l_call, locationt l_return,
  ...
  bool
  do_function_call(locationt l_call, locationt l_return,
                   const goto_functionst &goto_functions,
                   const goto_functionst::function_mapt::const_iterator f_it,
                   const exprt::operandst &arguments, const namespacet &ns);

  // abstract methods

  virtual bool merge(const statet &src, locationt from, locationt to) = 0;
  // for concurrent fixedpoint
  virtual bool merge_shared(const statet &src, locationt from, locationt to,
                            const namespacet &ns) = 0;
...
template <typename domainT> class ait : public ai_baset {
public:
  // constructor
  ait() : ai_baset() {}
...
protected:
  ...
  bool merge(const statet &src, locationt from, locationt to) override {
    ...
    return static_cast<domainT &>(dest).merge(static_cast<const domainT &>(src),
                                              from, to);
  }
...
private:
  ...
  bool merge_shared(const statet &src, goto_programt::const_targett from,
...
template <typename domainT> class concurrency_aware_ait : public ait<domainT> {
public:
