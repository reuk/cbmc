...
void local_may_aliast::assign_lhs(const exprt &lhs, const exprt &rhs,
  ...
  if (lhs.id() == ID_symbol) {
    if (lhs.type().id() == ID_pointer) {
      ...
      for (object_sett::const_iterator p_it = rhs_set.begin();
           p_it != rhs_set.end(); p_it++) {
...
std::set<exprt> local_may_aliast::get(const goto_programt::const_targett t,
  ...
  for (object_sett::const_iterator it = result_tmp.begin();
...
bool local_may_aliast::aliases(const goto_programt::const_targett t,
  ...
  std::set_intersection(tmp1.begin(), tmp1.end(), tmp2.begin(), tmp2.end(),
...
void local_may_aliast::get_rec(object_sett &dest, const exprt &rhs,
...
void local_may_aliast::build(const goto_functiont &goto_function) {
  ...
  while (!work_queue.empty()) {
    ...
    for (local_cfgt::successorst::const_iterator it = node.successors.begin();
