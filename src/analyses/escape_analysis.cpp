...
void escape_domaint::transform(locationt from, locationt to, ai_baset &ai,
                               const namespacet &ns) {
...
void escape_domaint::output(std::ostream &out, const ai_baset &ai,
  ...
  for (aliasest::const_iterator a_it1 = aliases.begin(); a_it1 != aliases.end();
       a_it1++) {
    bool first = true;

    for (aliasest::const_iterator a_it2 = aliases.begin();
         a_it2 != aliases.end(); a_it2++) {
...
bool escape_domaint::merge(const escape_domaint &b, locationt from,
                           locationt to) {
  ...
  // do union
  for (aliasest::const_iterator it = b.aliases.begin(); it != b.aliases.end();
       it++) {
...
void escape_analysist::insert_cleanup(
    ...
    goto_programt::targett location, const exprt &lhs,
    const std::set<irep_idt> &cleanup_functions, bool is_object,
    const namespacet &ns) {
...
void escape_analysist::instrument(goto_functionst &goto_functions,
                                  const namespacet &ns) {
  Forall_goto_functions(f_it, goto_functions) {
    Forall_goto_program_instructions(i_it, f_it->second.body) {
      ...
      case ASSIGN: {
        ...
        insert_cleanup(f_it->second, i_it, code_assign.lhs(), cleanup_functions,
                       false, ns);
      } break;
      ...
      case DEAD: {
        ...
        if (m_it != d.cleanup_map.end()) {
          cleanup_functions1.insert(m_it->second.cleanup_functions.begin(),
                                    m_it->second.cleanup_functions.end());
        }
        ...
        insert_cleanup(f_it->second, i_it, code_dead.symbol(),
                       cleanup_functions1, true, ns);
        insert_cleanup(f_it->second, i_it, code_dead.symbol(),
                       cleanup_functions2, false, ns);
    ...
    Forall_goto_program_instructions(i_it, f_it->second.body) get_state(i_it);
  }
}
