...
public:
  ...
  const objectst &get_r_set() const { return r_range_set; }

  const objectst &get_w_set() const { return w_range_set; }
  ...
  virtual void get_objects_rec(goto_programt::const_targett _target,
                               get_modet mode, const exprt &expr) {
...
protected:
  ...
  virtual void get_objects_complex(get_modet mode, const exprt &expr,
                                   const range_spect &range_start,
                                   const range_spect &size);

  virtual void get_objects_if(get_modet mode, const if_exprt &if_expr,
                              const range_spect &range_start,
                              const range_spect &size);

  // overload to include expressions read/written
  // through dereferencing
  virtual void get_objects_dereference(get_modet mode,
                                       const dereference_exprt &deref,
                                       const range_spect &range_start,
                                       const range_spect &size);

  virtual void get_objects_byte_extract(get_modet mode,
                                        const byte_extract_exprt &be,
                                        const range_spect &range_start,
                                        const range_spect &size);

  virtual void get_objects_shift(get_modet mode, const shift_exprt &shift,
                                 const range_spect &range_start,
                                 const range_spect &size);

  virtual void get_objects_member(get_modet mode, const member_exprt &expr,
                                  const range_spect &range_start,
                                  const range_spect &size);

  virtual void get_objects_index(get_modet mode, const index_exprt &expr,
                                 const range_spect &range_start,
                                 const range_spect &size);

  virtual void get_objects_array(get_modet mode, const array_exprt &expr,
                                 const range_spect &range_start,
                                 const range_spect &size);

  virtual void get_objects_struct(get_modet mode, const struct_exprt &expr,
                                  const range_spect &range_start,
                                  const range_spect &size);

  virtual void get_objects_typecast(get_modet mode, const typecast_exprt &tc,
                                    const range_spect &range_start,
                                    const range_spect &size);
  ...
  virtual void add(get_modet mode, const irep_idt &identifier,
                   const range_spect &range_start,
...
public:
  rw_guarded_range_set_value_sett(const namespacet &_ns,
