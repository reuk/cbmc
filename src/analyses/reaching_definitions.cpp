...
void rd_range_domaint::transform(locationt from, locationt to, ai_baset &ai,
                                 const namespacet &ns) {
...
void rd_range_domaint::transform_dead(const namespacet &ns, locationt from) {
...
void rd_range_domaint::transform_start_thread(
    const namespacet &ns, reaching_definitions_analysist &rd) {
  for (valuest::iterator it = values.begin(); it != values.end();) // no ++it
  {
    const irep_idt &identifier = it->first;

    if (!ns.lookup(identifier).is_shared() && !rd.get_is_dirty()(identifier)) {
      export_cache.erase(identifier);
...
void rd_range_domaint::transform_function_call(
    const namespacet &ns, locationt from, locationt to,
    reaching_definitions_analysist &rd) {
...
void rd_range_domaint::transform_end_function(
  ...
  for (const auto &new_value : new_values) {
    ...
    if (!rd.get_is_threaded()(call) || (!ns.lookup(identifier).is_shared() &&
                                        !rd.get_is_dirty()(identifier))) {
...
void rd_range_domaint::transform_assign(const namespacet &ns, locationt from,
                                        locationt to,
                                        reaching_definitions_analysist &rd) {
  rw_range_set_value_sett rw_set(ns, rd.get_value_sets());
  goto_rw(to, rw_set);
  ...
  forall_rw_range_set_w_objects(it, rw_set) {
    ...
    if (is_must_alias &&
        (!rd.get_is_threaded()(from) ||
         (!symbol_ptr->is_shared() && !rd.get_is_dirty()(identifier))))
      for (const auto &range : ranges)
        kill(identifier, range.first, range.second);
...
void rd_range_domaint::kill(const irep_idt &identifier,
       ...
       it != entry->second.end();) // no ++it
  {
    ...
    if (v.bit_begin >= range_end)
      ...
      ++it;
    else if (v.bit_begin >= range_start && v.bit_end != -1 &&
...
bool rd_range_domaint::gen(locationt from, const irep_idt &identifier,
                           const range_spect &range_start,
                           const range_spect &range_end) {
...
bool rd_range_domaint::merge(const rd_range_domaint &other, locationt from,
                             locationt to) {
...
rd_range_domaint::get(const irep_idt &identifier) const {
