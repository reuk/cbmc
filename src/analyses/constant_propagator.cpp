...
exprt concatenate_array_id(const exprt &array, const exprt &index,
                           const typet &type) {
...
exprt concatenate_array_id(const exprt &array, const mp_integer &index,
                           const typet &type) {
...
void constant_propagator_domaint::assign_rec(valuest &values, const exprt &lhs,
                                             const exprt &rhs,
                                             const namespacet &ns) {
  ...
  if (lhs.id() == ID_symbol && rhs.id() == ID_if) {
    ...
    if (values.is_constant(cond.op0()) && values.is_constant(cond.op1())) {
...
void constant_propagator_domaint::transform(locationt from, locationt to,
                                            ai_baset &ai,
                                            const namespacet &ns) {
#ifdef DEBUG
...
bool constant_propagator_domaint::two_way_propagate_rec(const exprt &expr,
  ...
  if (expr.id() == ID_and) {
    ...
    do {
      ...
      forall_operands(it, expr) if (two_way_propagate_rec(*it, ns)) change =
          true;
    } while (change);
...
void constant_propagator_domaint::assign(valuest &dest, const symbol_exprt &lhs,
                                         exprt rhs,
...
bool constant_propagator_domaint::valuest::is_array_constant(
  ...
  exprt new_expr = concatenate_array_id(expr.op0(), expr.op1(), expr.type());
...
bool constant_propagator_domaint::valuest::is_constant(
  ...
  forall_operands(it, expr) if (!is_constant(*it)) return false;
...
bool constant_propagator_domaint::valuest::set_to_top(const irep_idt &id) {
  ...
  replace_symbolt::expr_mapt::iterator r_it = replace_const.expr_map.find(id);
...
void constant_propagator_domaint::output(std::ostream &out, const ai_baset &ai,
                                         const namespacet &ns) const {
...
bool constant_propagator_domaint::valuest::merge(const valuest &src) {
  ...
  for (replace_symbolt::expr_mapt::const_iterator it =
           replace_const.expr_map.begin();
       it != replace_const.expr_map.end();) // no it++
  {
    const replace_symbolt::expr_mapt::const_iterator b_it =
        src.replace_const.expr_map.find(it->first);
...
bool constant_propagator_domaint::merge(
    const constant_propagator_domaint &other, locationt from, locationt to) {
...
void constant_propagator_ait::replace(goto_functionst &goto_functions,
                                      const namespacet &ns) {
  Forall_goto_functions(f_it, goto_functions) replace(f_it->second, ns);
}
...
void constant_propagator_ait::replace(
    goto_functionst::goto_functiont &goto_function, const namespacet &ns) {
  Forall_goto_program_instructions(it, goto_function.body) {
    ...
    if (it->is_goto() || it->is_assume() || it->is_assert()) {
      ...
      exprt::operandst &args = to_code_function_call(it->code).arguments();

      for (exprt::operandst::iterator o_it = args.begin(); o_it != args.end();
           ++o_it) {
...
void constant_propagator_ait::replace_types_rec(
    const replace_symbolt &replace_const, exprt &expr) {
  replace_const(expr.type());

  Forall_operands(it, expr) replace_types_rec(replace_const, *it);
}
