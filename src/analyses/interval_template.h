...
public:
  ...
  const T &get_lower() const { return lower; }

  const T &get_upper() const { return upper; }

  bool empty() const { return upper_set && lower_set && lower > upper; }
  ...
  bool singleton() const { return upper_set && lower_set && lower == upper; }
  ...
  void join(const interval_template<T> &i) { approx_union_with(i); }

  // Intersection or conjunction
  void meet(const interval_template<T> &i) { intersect_with(i); }
...
template <class T> interval_template<T> upper_interval(const T &u) {
...
template <class T> interval_template<T> lower_interval(const T &l) {
  interval_template<T> i;
  i.lower_set = true;
  i.lower = l;
  return i;
