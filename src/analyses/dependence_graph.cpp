...
void dep_graph_domaint::control_dependencies(goto_programt::const_targett from,
                                             goto_programt::const_targett to,
  ...
  for (depst::iterator it = control_deps.begin();
       it != control_deps.end();) // no ++it
  {
    ...
    const cfg_post_dominatorst::cfgt::nodet &m = pd.cfg[e->second];
...
static bool may_be_def_use_pair(const mp_integer &w_start,
  ...
  if ((w_end != -1 && w_end <= r_start) || // we < rs
    ...
    return true;
  else if (w_end == -1 || (r_end != -1 && w_end > r_start)) // ws <= rs < we,re
    return true;
  else
    return false;
}
    ...
    for (const auto &w_range : w_ranges) {
      ...
      for (const auto &wr : w_range.second)
        for (const auto &r_range : r_ranges)
          if (!found && may_be_def_use_pair(wr.first, wr.second, r_range.first,
                                            r_range.second)) {
...
void dep_graph_domaint::transform(goto_programt::const_targett from,
                                  goto_programt::const_targett to, ai_baset &ai,
                                  const namespacet &ns) {
...
void dep_graph_domaint::output(std::ostream &out, const ai_baset &ai,
                               const namespacet &ns) const {
  if (!control_deps.empty()) {
    out << "Control dependencies: ";
    for (depst::const_iterator it = control_deps.begin();
  ...
  if (!data_deps.empty()) {
    ...
    for (depst::const_iterator it = data_deps.begin(); it != data_deps.end();
         ++it) {
