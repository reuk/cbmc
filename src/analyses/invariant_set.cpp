...
bool inv_object_storet::is_constant(const exprt &expr) const {
  return expr.is_constant() || is_constant_address(expr);
}
...
bool invariant_sett::get_object(const exprt &expr, unsigned &n) const {
...
void invariant_sett::output(const irep_idt &identifier,
  ...
  for (unsigned i = 0; i < eq_set.size(); i++)
    if (eq_set.is_root(i) && eq_set.count(i) >= 2) {
      bool first = true;
  ...
  for (const auto &bounds : bounds_map) {
    out << to_string(bounds.first, identifier) << " in " << bounds.second
        << std::endl;
  }

  for (const auto &le : le_set) {
    out << to_string(le.first, identifier)
        << " <= " << to_string(le.second, identifier) << std::endl;
  }

  for (const auto &ne : ne_set) {
    out << to_string(ne.first, identifier)
        << " != " << to_string(ne.second, identifier) << std::endl;
  }
}
...
void invariant_sett::strengthen_rec(const exprt &expr) {
  ...
  if (expr.is_true()) {
    ...
    forall_operands(it, expr) strengthen_rec(*it);
    ...
    if (get_object(expr.op0(), p.first) || get_object(expr.op1(), p.second))
      return;
...
tvt invariant_sett::implies_rec(const exprt &expr) const {
  ...
  if (expr.is_true())
    ...
    forall_operands(it, expr) if (implies_rec(*it) !=
                                  tvt(true)) return tvt::unknown();

    return tvt(true);
  } else if (expr.id() == ID_or) {
    forall_operands(it,
                    expr) if (implies_rec(*it) == tvt(true)) return tvt(true);
  } else if (expr.id() == ID_le || expr.id() == ID_lt ||
...
void invariant_sett::nnf(exprt &expr, bool negate) {
  ...
  if (expr.is_true()) {
    ...
    Forall_operands(it, expr) nnf(*it, negate);
  } else if (expr.id() == ID_or) {
    if (negate)
      expr.id(ID_and);
...
void invariant_sett::simplify(exprt &expr) const {
  ...
  Forall_operands(it, expr) simplify(*it);
...
std::string inv_object_storet::to_string(unsigned a,
...
bool invariant_sett::make_union(const invariant_sett &other) {
  ...
  if (old_eq_roots != eq_set.count_roots() || old_ne_set != ne_set.size() ||
...
bool invariant_sett::make_union_bounds_map(const bounds_mapt &other) {
  ...
  for (bounds_mapt::iterator it = bounds_map.begin();
       it != bounds_map.end();) // no it++
  {
...
void invariant_sett::modifies(const exprt &lhs) {
  if (lhs.id() == ID_symbol || lhs.id() == ID_member) {
    ...
    modifies(lhs.op0());
  } else if (lhs.id() == "NULL-object" || lhs.id() == "is_zero_string" ||
...
void invariant_sett::assignment(const exprt &lhs, const exprt &rhs) {
  equal_exprt equality;
  equality.lhs() = lhs;
  equality.rhs() = rhs;
...
void invariant_sett::apply_code(const codet &code) {
  ...
  if (statement == ID_block) {
    forall_operands(it, code) apply_code(to_code(*it));
