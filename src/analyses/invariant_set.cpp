...
void invariant_sett::output(const irep_idt &identifier, std::ostream &out) const
{
  if(is_false)
  {
    out << "FALSE" << std::endl;
...
tvt invariant_sett::implies_rec(const exprt &expr) const
{
  ...
  else if(expr.id() == ID_or)
  {
    ...
    expr.id() == ID_le || expr.id() == ID_lt || expr.id() == ID_equal ||
    expr.id() == ID_notequal)
  {
    assert(expr.operands().size() == 2);
...
std::string
inv_object_storet::to_string(unsigned a, const irep_idt &identifier) const
{
...
std::string
invariant_sett::to_string(unsigned a, const irep_idt &identifier) const
{
  assert(object_store != NULL);
...
bool invariant_sett::make_union(const invariant_sett &other)
{
  ...
  if(
    old_eq_roots != eq_set.count_roots() || old_ne_set != ne_set.size() ||
...
void invariant_sett::modifies(const exprt &lhs)
{
    ...
    lhs.id() == ID_byte_extract_big_endian)
  {
    ...
    lhs.id() == "NULL-object" || lhs.id() == "is_zero_string" ||
...
void invariant_sett::apply_code(const codet &code)
{
  ...
  else if(statement == "lock" || statement == "unlock" || statement == ID_asm)
  {
    // ignore for now
  }
  else
