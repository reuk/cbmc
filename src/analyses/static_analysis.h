...
public:
  ...
  virtual void transform(const namespacet &ns, locationt from,
                         locationt to) = 0;
...
public:
  ...
  virtual void output(const goto_functionst &goto_functions,
  ...
  void insert(locationt l) { generate_state(l); }
...
protected:
  ...
  // function calls
  void do_function_call_rec(locationt l_call, locationt l_return,
  ...
  virtual void get_reference_set(locationt l, const exprt &expr,
                                 std::list<exprt> &dest) = 0;
};

// T is expected to be derived from domain_baset
template <typename T> class static_analysist : public static_analysis_baset {
public:
  // constructor
...
protected:
  ...
  virtual void generate_state(locationt l) { state_map[l].initialize(ns, l); }
...
private:
  ...
  virtual bool merge_shared(statet &a, const statet &b,
...
public:
  ...
  virtual bool merge_shared(static_analysis_baset::statet &a,
                            const static_analysis_baset::statet &b,
                            goto_programt::const_targett to) {
    return static_cast<T &>(a).merge_shared(this->ns, static_cast<const T &>(b),
                                            to);
  }
