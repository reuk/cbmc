...
void custom_bitvector_domaint::set_bit(const exprt &lhs, unsigned bit_nr,
                                       modet mode) {
...
custom_bitvector_domaint::get_rhs(const exprt &rhs) const {
  if (rhs.id() == ID_symbol || rhs.id() == ID_dereference) {
...
void custom_bitvector_domaint::transform(locationt from, locationt to,
    ...
    if (function.id() == ID_symbol) {
      ...
      if (identifier == "__CPROVER_set_must" ||
        ...
        if (code_function_call.arguments().size() == 2) {
          unsigned bit_nr = cba.get_bit_nr(code_function_call.arguments()[1]);
  ...
  case OTHER: {
    ...
    if (statement == "set_may" || statement == "set_must" ||
        statement == "clear_may" || statement == "clear_must") {
      assert(instruction.code.operands().size() == 2);

      unsigned bit_nr = cba.get_bit_nr(instruction.code.op1());
      ...
      {
        if (mode == CLEAR_MAY) {
          for (bitst::iterator b_it = may_bits.begin(); b_it != may_bits.end();
               b_it++)
...
void custom_bitvector_domaint::output(std::ostream &out, const ai_baset &ai,
...
bool custom_bitvector_domaint::merge(const custom_bitvector_domaint &b,
  ...
  // now do MUST
  for (auto &bit : must_bits) {
    bitst::const_iterator b_it = b.must_bits.find(bit.first);
...
void custom_bitvector_domaint::erase_blank_vectors(bitst &bits) {
  for (bitst::iterator a_it = bits.begin(); a_it != bits.end();) // no a_it++
  {
    if (a_it->second == 0)
...
bool custom_bitvector_domaint::has_get_must_or_may(const exprt &src) {
  ...
  forall_operands(it, src) if (has_get_must_or_may(*it)) return true;
...
exprt custom_bitvector_domaint::eval(
  ...
  if (src.id() == "get_must" || src.id() == "get_may") {
    if (src.operands().size() == 2) {
      unsigned bit_nr = custom_bitvector_analysis.get_bit_nr(src.op1());
    ...
    exprt tmp = src;
    Forall_operands(it, tmp) *it = eval(*it, custom_bitvector_analysis);
...
void custom_bitvector_analysist::check(const namespacet &ns,
                                       const goto_functionst &goto_functions,
                                       bool use_xml, std::ostream &out) {
  ...
  if (!use_xml)
    out << "SUMMARY: " << pass << " pass, " << fail << " fail, " << unknown
        << " unknown\n";
}
