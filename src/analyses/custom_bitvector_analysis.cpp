...
std::set<exprt>
custom_bitvector_analysist::aliases(const exprt &src, locationt loc)
{
  if(src.id() == ID_symbol)
  {
  ...
  else if(src.id() == ID_dereference)
  {
    ...
    std::set<exprt> pointer_set= local_may_alias_factory(loc).get(loc, pointer);
  ...
  const namespacet &ns)
{
  ...
  case OTHER:
  {
    ...
    if(
      statement == "set_may" || statement == "set_must" ||
      statement == "clear_may" || statement == "clear_must")
    {
        ...
        to_constant_expr(lhs).get_value() == ID_NULL) // NULL means all
      {
        ...
        {
          for(bitst::iterator b_it= must_bits.begin(); b_it != must_bits.end();
              b_it++)
            clear_bit(b_it->second, bit_nr);
