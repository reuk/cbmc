...
public:
  explicit is_name_equalt(const irep_idt &name) : name(name) {}
...
public:
  ...
  java_bytecode_vtable_factoryt(symbol_tablet &symbol_table,
                                const std::string &module)
      : symbol_table(symbol_table), module(module), ns(symbol_table),
        has_error(false) {}
  ...
  void create_vtable_symbol(symbolt &result, const class_typet &class_type) {
    ...
    const std::string &base_class_name =
        id2string(class_type.get(ID_base_name));
  ...
  void add_vtable_entry(struct_exprt &vtable_value,
                        const class_typet &interface,
                        const class_typet &implementor,
  ...
  void extract_types(std::vector<class_typet> &result, const irept::subt &types,
    ...
    for (irept::subt::const_iterator it = types.begin(); it != types.end();
         ++it) {
  ...
  bool is_virtual(const class_typet::methodt &method) {
    return method.get_bool(ID_is_virtual) && !method.get_bool(ID_constructor);
  }
...
bool java_bytecode_vtable(symbol_tablet &symbol_table,
  ...
  std::transform(
      symbols.begin(), symbols.end(), std::back_inserter(names),
      [](const std::pair<irep_idt, symbolt> &entry) { return entry.first; });
...
static void create_vtable_type(const irep_idt &vt_name,
  ...
  vt_symb_type.base_name =
      vtnamest::get_type_base(id2string(class_symbol.base_name));
...
static void add_vtable_pointer_member(const irep_idt &vt_name,
...
bool has_vtable_info(const symbol_tablet &symbol_table,
  ...
  return symbol_table.has_symbol(
         ...
         to_struct_union_type(class_symbol.type)
             .has_component(ID_vtable_pointer);
}
