...
public:
  java_bytecode_convert_methodt(symbol_tablet &_symbol_table,
                                message_handlert &_message_handler,
                                size_t _max_array_length,
                                safe_pointer<ci_lazy_methodst> _lazy_methods)
      : messaget(_message_handler), symbol_table(_symbol_table),
        max_array_length(_max_array_length), lazy_methods(_lazy_methods) {}
...
protected:
  ...
  struct converted_instructiont {
    converted_instructiont(const instructionst::const_iterator &it,
                           const codet &_code)
...
protected:
  void find_initialisers(local_variable_table_with_holest &vars,
  ...
  void find_initialisers_for_slot(
      ...
      const address_mapt &amap, const java_cfg_dominatorst &doms);
  ...
  static void replace_goto_target(codet &repl, const irep_idt &old_label,
                                  const irep_idt &new_label);

  code_blockt &get_block_for_pcrange(block_tree_nodet &tree,
  ...
  code_blockt &get_or_create_block_for_pcrange(
      ...
      const address_mapt &amap, bool allow_merge = true);
