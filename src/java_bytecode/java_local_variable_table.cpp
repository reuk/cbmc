/// \file Java local variable table processing
...
/// See above
///
/// parameters: `start`: Variable to find the predecessors of
///   `predecessor_map`: Map from local variables to sets of predecessors
/// Outputs: `result`: populated with all transitive predecessors of `start`
...
/// See above
///
/// parameters: `inst`: Java bytecode instruction `slotidx`: local variable
///   slot number
///
/// \returns Returns true if `inst` is any form of store instruction targeting
///   slot `slotidx`
...
/// See above
///
/// parameters: `from`, `to`: bounds of a gap in `var`'s live range, inclusive
///   and exclusive respectively
///
/// \returns Adds a hole to `var`, unless it would be of zero size.
...
/// See above
///
/// parameters: `firstvar`-`varlimit`: range of local variable table entries to
///   consider
///
/// \returns `live_variable_at_address` is populated with a sequence of local
...
/// control flow edges may exist from bytecode addresses that fall under a
/// table entry which differs, but which has the same variable name and type
/// descriptor. This indicates a split live range, and will be recorded in the
/// predecessor map.
///
/// parameters: `firstvar`-`varlimit`: range of local variable table entries to
///   consider `live_variable_at_address`: map from bytecode address to table
///   entry (drawn from firstvar-varlimit) live at that address `amap`: map
///   from bytecode address to instructions
///
/// \returns Populates `predecessor_map` with a graph from local variable table
...
///   for. `dominator_analysis`: Already-initialised dominator tree
///
/// \returns Returns the bytecode address of the closest common dominator of
///   all given variable table entries. In the worst case the function entry
///   point should always satisfy this criterion.
...
///   declared
///
/// \returns Adds holes to `merge_into`, indicating where gaps in the
///   variable's live range fall. For example, if the declaration happens at
///   address 10 and the entries in `merge_into` have live ranges [(20-30),
///   (40-50)] then holes will be added at (10-20) and (30-40).
...
///
/// \returns Populates `merge_into` as a combined variable table entry, with
...
/// address, which may be moved back so that the declaration dominates all
/// uses.
///
/// parameters: `firstvar`-`varlimit`: sequence of variable table entries, all
///   of which should concern the same slot index. `amap`: Map from bytecode
///   address to instruction
///
/// \returns Side-effect: merges variable table entries which flow into one
...
/// time.
///
/// parameters: `it1` and `it2`, which are iterators into the same vector, of
///   which `itend` is the end() iterator.
///
/// \returns Moves `it1` and `it2` to delimit a sequence of variable table
///   entries with slot index equal to it2->var.index on entering this
///   function, or to both equal itend if it2==itend on entry.
...
///
/// parameters: `vars`: Local variable table `amap`: Map from bytecode index to
///   instruction `dominator_analysis`: Already computed dominator tree for the
///   Java function described by `amap`
///
/// \returns Combines entries in `vars` which flow together
...
///
/// \returns Removes zero-size entries from `vars_with_holes`
...
/// See `find_initialisers_for_slot` above for more detail.
///
/// parameters: `m`: Java method `amap`: Map from bytecode indices to
///   instructions in `m`
///
/// \returns Populates `this->vars_with_holes` equal to
...
///
/// \returns Returns the list entry covering this address (taking live range
