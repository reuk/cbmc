...
struct procedure_local_cfg_baset<
  ...
  unsigned> : public grapht<cfg_base_nodet<T, unsigned>>
{
  ...
  void operator()(const method_with_amapt &args)
  {
    ...
    for(const auto &table_entry : method.exception_table)
    {
      ...
      for(; findit->first < table_entry.end_pc; ++findit)
      {
        ...
        if(
          thisinst.successors.size() == 1 &&
          thisinst.successors.back() == succit->first)
        {
          this->add_edge(
            entry_map.at(findit->first), entry_map.at(table_entry.handler_pc));
        }
      }
    }
  }
...
struct is_predecessor_oft
{
  ...
  bool
  operator()(local_variable_with_holest *a, local_variable_with_holest *b) const
  {
...
static void
maybe_add_hole(local_variable_with_holest &var, unsigned from, unsigned to)
{
  assert(to >= from);
  if(to != from)
  ...
  unsigned expanded_live_range_start)
{
  ...
  maybe_add_hole(
    merge_into, expanded_live_range_start, sorted_by_startpc[0]->var.start_pc);
  ...
  const java_cfg_dominatorst &dominator_analysis)
{
  ...
  for(auto merge_into : topsorted_vars)
  {
    ...
    merge_variable_table_entries(
      *merge_into, merge_vars, dominator_analysis, status());
  }
}
...
cleanup_var_table(std::vector<local_variable_with_holest> &vars_with_holes)
{
  ...
  for(size_t i= 0; i < (vars_with_holes.size() - toremove); ++i)
  {
    ...
    if(v.var.length == 0)
    {
      ...
      ++toremove;
      if(
        i != vars_with_holes.size() - toremove) // Already where it needs to be?
