...
struct procedure_local_cfg_baset<
    T, java_bytecode_convert_methodt::method_with_amapt, unsigned>
    ...
    for (const auto &table_entry : method.exception_table) {
      ...
      for (; findit->first < table_entry.end_pc; ++findit) {
        ...
        if (thisinst.successors.size() == 1 &&
          ...
          this->add_edge(entry_map.at(findit->first),
...
static void
gather_transitive_predecessors(local_variable_with_holest *start,
...
static bool
is_store_to_slot(const java_bytecode_convert_methodt::instructiont &inst,
                 unsigned slotidx) {
...
static void maybe_add_hole(local_variable_with_holest &var, unsigned from,
                           unsigned to) {
  assert(to >= from);
  if (to != from)
...
static void populate_predecessor_map(
    ...
    const address_mapt &amap, predecessor_mapt &predecessor_map,
  ...
  for (auto it = firstvar, itend = varlimit; it != itend; ++it) {
    ...
    for (; amapit->first >= it->var.start_pc; --amapit) {
      for (auto pred : amapit->second.predecessors) {
        auto pred_var = (pred < live_variable_at_address.size()
        ...
        if (pred_var == &*it) {
          ...
          --inst_before_this;
          if (amapit->first != it->var.start_pc ||
            ...
            msg.warning() << "Local variable table: ignoring flow from "
                          << "out of range for " << it->var.name << ' ' << pred
                          << " -> " << amapit->first << messaget::eom;
            continue;
          }
          if (!is_store_to_slot(*(inst_before_this->second.source),
                                it->var.index)) {
          ...
          if (pred_var->var.name != it->var.name ||
            ...
            msg.warning() << "Local variable table: ignoring flow from "
                          << "clashing variable for " << it->var.name << ' '
                          << pred << " -> " << amapit->first << messaget::eom;
            continue;
          }
...
static unsigned
get_common_dominator(const std::set<local_variable_with_holest *> &merge_vars,
...
static void merge_variable_table_entries(
    local_variable_with_holest &merge_into,
    const std::set<local_variable_with_holest *> &merge_vars,
    const java_cfg_dominatorst &dominator_analysis, std::ostream &debug_out) {
...
void java_bytecode_convert_methodt::find_initialisers_for_slot(
    local_variable_table_with_holest::iterator firstvar,
    local_variable_table_with_holest::iterator varlimit,
    const address_mapt &amap, const java_cfg_dominatorst &dominator_analysis) {
  ...
  populate_predecessor_map(firstvar, varlimit, live_variable_at_address, amap,
                           predecessor_map, get_message_handler());
  ...
  for (auto merge_into : topsorted_vars) {
    ...
    merge_variable_table_entries(*merge_into, merge_vars, dominator_analysis,
                                 status());
  }
}
...
static void
walk_to_next_index(local_variable_table_with_holest::iterator &it1,
...
void java_bytecode_convert_methodt::find_initialisers(
    local_variable_table_with_holest &vars, const address_mapt &amap,
    const java_cfg_dominatorst &dominator_analysis) {
...
static void
cleanup_var_table(std::vector<local_variable_with_holest> &vars_with_holes) {
  size_t toremove = 0;
  for (size_t i = 0; i < (vars_with_holes.size() - toremove); ++i) {
    ...
    if (v.var.length == 0) {
      ...
      ++toremove;
      if (i !=
          vars_with_holes.size() - toremove) // Already where it needs to be?
...
void java_bytecode_convert_methodt::setup_local_variables(
    const methodt &m, const address_mapt &amap) {
...
java_bytecode_convert_methodt::find_variable_for_slot(size_t address,
                                                      variablest &var_list) {
  for (const variablet &var : var_list) {
    ...
    if (address >= start_pc && address < (start_pc + length)) {
      ...
      for (auto &hole : var.holes)
        if (address >= hole.start_pc &&
            address < (hole.start_pc + hole.length)) {
          found_hole = true;
          break;
        }
      if (found_hole)
