...
void java_bytecode_typecheckt::typecheck_expr_java_new(
...
void java_bytecode_typecheckt::typecheck_expr_java_string_literal(exprt &expr) {
  ...
  std::string escaped_symbol_name = "java::java.lang.String.Literal.";
  ...
  new_symbol.is_static_lifetime =
      true; // These are basically const global data.
  ...
  jlo_init.copy_to_operands(constant_exprt("java::java.lang.String",
...
void java_bytecode_typecheckt::typecheck_expr_member(member_exprt &expr) {
  ...
  while (1) {
    ...
    struct_typet &struct_type = to_struct_type(base_type);
    ...
    if (struct_type.get_bool(ID_incomplete_class)) {
      ...
      add_to_components.push_back(
          struct_typet::componentt(component_name, expr.type()));
  ...
  warning() << "failed to find field `" << component_name
            << "` in class hierarchy" << eom;
