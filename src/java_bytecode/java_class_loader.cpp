...
java_bytecode_parse_treet &java_class_loadert::
operator()(const irep_idt &class_name) {
  ...
  while (!queue.empty()) {
    ...
    for (java_bytecode_parse_treet::class_refst::const_iterator it =
             parse_tree.class_refs.begin();
         it != parse_tree.class_refs.end(); it++)
      queue.push(*it);
  }
...
java_class_loadert::get_parse_tree(java_class_loader_limitt &class_loader_limit,
  ...
  for (const auto &jf : jar_files) {
    ...
    if (jm_it != jm.entries.end()) {
      debug() << "Getting class `" << class_name << "' from JAR " << jf << eom;
  ...
  for (const auto &cp : config.java.classpath) {
    ...
    if (has_suffix(cp, ".jar")) {
      ...
      if (jm_it != jm.entries.end()) {
        debug() << "Getting class `" << class_name << "' from JAR " << cp
                << eom;
...
void java_class_loadert::load_entire_jar(
    java_class_loader_limitt &class_loader_limit, const std::string &file) {
...
void java_class_loadert::read_jar_file(
    java_class_loader_limitt &class_loader_limit, const irep_idt &file) {
  // done already?
