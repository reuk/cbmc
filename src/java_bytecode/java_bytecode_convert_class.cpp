...
public:
  java_bytecode_convert_classt(symbol_tablet &_symbol_table,
...
void java_bytecode_convert_classt::convert(const classt &c) {
  ...
  if (c.is_enum)
    class_type.set(ID_java_enum_static_unwind,
  ...
  for (const auto &method : c.methods) {
    ...
    java_bytecode_convert_method_lazy(*class_symbol, method_identifier, method,
                                      symbol_table);
    if (lazy_methods_mode == LAZY_METHODS_MODE_EAGER) {
      ...
      java_bytecode_convert_method(*class_symbol, method, symbol_table,
                                   get_message_handler(), max_array_length);
    } else {
...
void java_bytecode_convert_classt::convert(symbolt &class_symbol,
  ...
  if (f.is_static) {
    ...
    new_symbol.value = zero_initializer(field_type, class_symbol.location, ns,
                                        get_message_handler());
...
void java_bytecode_convert_classt::add_array_types() {
  ...
  for (const char l : letters) {
    ...
    struct_typet::componentt comp0("@java.lang.Object",
    ...
    struct_typet::componentt comp2("data",
                                   pointer_typet(java_type_from_char(l)));
...
bool java_bytecode_convert_class(const java_bytecode_parse_treet &parse_tree,
  ...
  java_bytecode_convert_classt java_bytecode_convert_class(
      ...
      lazy_methods_mode, string_refinement_enabled);
