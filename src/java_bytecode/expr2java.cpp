  ...
  unsigned indent)
{
  ...
  const code_typet &code_type= to_code_type(src.function().type());
...
std::string expr2javat::convert_struct(const exprt &src, unsigned &precedence)
{
...
std::string
expr2javat::convert_constant(const constant_exprt &src, unsigned &precedence)
{
...
std::string expr2javat::convert_java_this(const exprt &src, unsigned precedence)
{
  return "this";
}
...
std::string
expr2javat::convert_java_instanceof(const exprt &src, unsigned precedence)
{
...
std::string expr2javat::convert_java_new(const exprt &src, unsigned precedence)
{
  std::string dest;
...
std::string
expr2javat::convert_code_java_delete(const exprt &src, unsigned indent)
{
...
std::string expr2javat::convert(const exprt &src, unsigned &precedence)
{
  if(src.id() == "java-this")
  ...
  if(src.id() == ID_java_instanceof)
    ...
    src.id() == ID_side_effect && (src.get(ID_statement) == ID_java_new ||
                                   src.get(ID_statement) == ID_java_new_array))
    return convert_java_new(src, precedence= 15);
  else if(src.id() == ID_side_effect && src.get(ID_statement) == ID_throw)
  ...
  {
    return "VIRTUAL_FUNCTION(" + id2string(src.get(ID_C_class)) + "." +
...
std::string expr2javat::convert_code(const codet &src, unsigned indent)
{
