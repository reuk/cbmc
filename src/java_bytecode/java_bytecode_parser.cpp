...
public:
  java_bytecode_parsert() { get_bytecodes(); }
...
protected:
  ...
  exprt &constant(u2 index) { return pool_entry(index).expr; }
  ...
  u1 read_u1() { return (u1)read_bytes(1); }

  inline u2 read_u2() { return (u2)read_bytes(2); }

  u4 read_u4() { return (u4)read_bytes(4); }

  u8 read_u8() { return read_bytes(8); }
};
...
void java_bytecode_parsert::rClassFile() {
  ...
  parsed_class.name = constant(this_class).type().get(ID_C_base_name);

  if (super_class != 0)
    parsed_class.extends = constant(super_class).type().get(ID_C_base_name);
...
void java_bytecode_parsert::rconstant_pool() {
  ...
  for (constant_poolt::iterator it = constant_pool.begin();
       it != constant_pool.end(); it++) {
    ...
    default:
      error() << "unknown constant pool entry (" << it->tag << ")" << eom;
      throw 0;
    }
  }
  ...
  for (constant_poolt::iterator it = constant_pool.begin();
    ...
    case CONSTANT_Fieldref: {
      ...
      symbol_typet class_symbol = java_classname(id2string(class_name_entry.s));
    ...
    case CONSTANT_InterfaceMethodref: {
      ...
      irep_idt component_name = id2string(name_entry.s) + ":" +
                                id2string(pool_entry(nameandtype_entry.ref2).s);

      irep_idt class_name = class_symbol.get_identifier();
...
void java_bytecode_parsert::rfields(classt &parsed_class) {
  ...
  for (std::size_t i = 0; i < fields_count; i++) {
    ...
    size_t flags = (field.is_public ? 1 : 0) + (field.is_protected ? 1 : 0) +
  ...
  for (address = 0; address < code_length; address++) {
    ...
    case 'o': // two byte branch offset, signed
    {
      ...
      instruction.args.push_back(
          from_integer(address + offset, integer_typet()));
    }
      address += 2;
      break;
    ...
    case 'L': // lookupswitch
    {
      ...
      instruction.args.push_back(
          from_integer(base_offset + default_value, integer_typet()));
      address += 4;
      ...
      for (std::size_t i = 0; i < npairs; i++) {
        ...
        instruction.args.push_back(
            from_integer(base_offset + offset, integer_typet()));
        address += 8;
      }
    } break;
...
void java_bytecode_parsert::rmethod_attribute(methodt &method) {
  ...
  if (attribute_name == "Code") {
    ...
    for (methodt::instructionst::iterator it = method.instructions.begin();
         it != method.instructions.end(); it++) {
      ...
      it->source_location.set_function(
...
void java_bytecode_parsert::rcode_attribute(methodt &method) {
  ...
  if (attribute_name == "LineNumberTable") {
    ...
    typedef std::map<unsigned, methodt::instructionst::iterator>
        instruction_mapt;
    ...
    for (std::size_t i = 0; i < line_number_table_length; i++) {
      ...
      instruction_mapt::const_iterator it = instruction_map.find(start_pc);
    ...
    for (size_t i = 0; i < stack_map_entries; i++) {
      ...
      if (0 <= frame_type && frame_type <= 63) {
        ...
        method.stack_map_table[i].type =
            methodt::stack_map_table_entryt::APPEND;
...
void java_bytecode_parsert::relement_value_pair(
  ...
  case 's': {
    ...
    element_value_pair.value =
        string_constantt(pool_entry(const_value_index).s);
  } break;
...
void java_bytecode_parsert::rmethod(classt &parsed_class) {
  ...
  size_t flags = (method.is_public ? 1 : 0) + (method.is_protected ? 1 : 0) +
