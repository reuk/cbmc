...
static bool should_init_symbol(const symbolt &sym) {
  if (sym.type.id() != ID_code && sym.is_lvalue && sym.is_state_var &&
      sym.is_static_lifetime && sym.mode == ID_java)
    return true;
...
bool java_static_lifetime_init(symbol_tablet &symbol_table,
      ...
      if (sym.value.is_nil() && sym.type != empty_typet()) {
        ...
        if (allow_null) {
          ...
          if (has_suffix(namestr, suffix))
            allow_null = false;
          if (allow_null &&
              has_prefix(namestr, "java::java.lang.String.Literal"))
            allow_null = false;
        }
        auto newsym =
            object_factory(sym.type, code_block, allow_null, symbol_table,
                           max_nondet_array_length, source_location);
  ...
  for (symbol_tablet::symbolst::const_iterator it =
    ...
    if (it->second.base_name == "<clinit>" && it->second.type.id() == ID_code &&
  ...
  for (std::size_t param_number = 0; param_number < parameters.size();
    ...
    if (!is_main) {
      ...
      bool has_correct_type =
          ...
          (!to_code_type(function.type).has_this()) && parameters.size() == 1 &&
          parameters[0].type().full_eq(string_array_type);
      is_main = (named_main && has_correct_type);
    }

    bool allow_null =
        (!is_main) && (!is_this) && !assume_init_pointers_not_null;

    main_arguments[param_number] = object_factory(
        parameters[param_number].type(), init_code, allow_null, symbol_table,
        max_nondet_array_length, function.location);
...
void java_record_outputs(const symbolt &function,
                         ...
                         code_blockt &init_code, symbol_tablet &symbol_table) {
  const code_typet::parameterst &parameters =
      to_code_type(function.type).parameters();
...
main_function_resultt get_main_symbol(symbol_tablet &symbol_table,
  ...
  if (config.main != "") {
    ...
    if (symbol.type.id() != ID_code) {
      message.error() << "main symbol `" << config.main << "' not a function"
                      << messaget::eom;
      res.main_function = symbol;
      res.error_found = true;
      res.stop_convert = true;
    ...
    if (symbol.value.is_nil() && !allow_no_body) {
      message.error() << "main method `" << main_class << "' has no body"
                      << messaget::eom;
      res.main_function = symbol;
      res.error_found = true;
      res.stop_convert = true;
    ...
    for (symbol_tablet::symbolst::const_iterator s_it =
             symbol_table.symbols.begin();
         s_it != symbol_table.symbols.end(); s_it++) {
    ...
    if (matches.size() >= 2) {
      message.error() << "main method in `" << main_class << "' is ambiguous"
                      << messaget::eom;
...
bool java_entry_point(symbol_tablet &symbol_table, const irep_idt &main_class,
                      message_handlert &message_handler,
                      bool assume_init_pointers_not_null,
  ...
  exprt::operandst main_arguments = java_build_arguments(
      symbol, init_code, symbol_table, assume_init_pointers_not_null,
      max_nondet_array_length, message_handler);
