...
static symbolt &
new_tmp_symbol(symbol_tablet &symbol_table, const source_locationt &loc,
               const typet &type,
               const std::string &prefix = "tmp_object_factory") {
  return get_fresh_aux_symbol(type, "", prefix, loc, ID_java, symbol_table);
}
...
public:
  java_object_factoryt(code_blockt &_init_code, bool _assume_non_null,
                       size_t _max_nondet_array_length,
                       symbol_tablet &_symbol_table)
      : init_code(_init_code), assume_non_null(_assume_non_null),
        max_nondet_array_length(_max_nondet_array_length),
        symbol_table(_symbol_table), ns(_symbol_table) {}

  exprt allocate_object(const exprt &, const typet &, const source_locationt &,
                        bool create_dynamic_objects);

  void gen_nondet_array_init(const exprt &expr, const source_locationt &);

  void gen_nondet_init(const exprt &expr, bool is_sub,
                       irep_idt class_identifier, const source_locationt &loc,
                       bool create_dynamic_objects, bool override = false,
...
exprt java_object_factoryt::allocate_object(const exprt &target_expr,
  ...
  if (!create_dynamic_objects) {
    ...
    if (allocate_type.id() != ID_empty) {
      ...
      symbolt &malloc_sym = new_tmp_symbol(
          symbol_table, loc, pointer_typet(allocate_type), "malloc_site");
...
void java_object_factoryt::gen_nondet_init(const exprt &expr, bool is_sub,
  ...
  if (type.id() == ID_pointer) {
    ...
    if (!assume_non_null) {
      ...
      std::string fresh_label = "post_synthetic_malloc_" +
                                std::to_string(++synthetic_constructor_count);
    ...
    if (java_is_array_type(subtype))
      ...
      gen_nondet_init(init_expr, false, "", loc, create_dynamic_objects);
    }
    ...
    for (const auto &component : components) {
      ...
      if (name == "@class_identifier") {
        ...
        gen_nondet_init(me, _is_sub, class_identifier, loc,
                        create_dynamic_objects);
      }
    }
...
void java_object_factoryt::gen_nondet_array_init(const exprt &expr,
                                                 const source_locationt &loc) {
  ...
  typet allocate_type;
  symbolt &length_sym =
      new_tmp_symbol(symbol_table, loc, java_int_type(), "nondet_array_length");
  ...
  binary_relation_exprt assume2(length_sym_expr, ID_le, max_length_expr);
  ...
  exprt init_array_expr =
      member_exprt(dereference_exprt(expr, expr.type().subtype()), "data",
  ...
  symbolt &array_init_symbol = new_tmp_symbol(
      symbol_table, loc, init_array_expr.type(), "array_data_init");
  ...
  symbolt &counter = new_tmp_symbol(symbol_table, loc, length_sym_expr.type(),
                                    "array_init_iter");
  ...
  gen_nondet_init(arraycellref, false, irep_idt(), loc, true, true,
                  element_type);
...
exprt object_factory(const typet &type, code_blockt &init_code, bool allow_null,
                     symbol_tablet &symbol_table,
                     size_t max_nondet_array_length,
                     const source_locationt &loc) {
  if (type.id() == ID_pointer) {
    ...
    java_object_factoryt state(init_code, !allow_null, max_nondet_array_length,
                               symbol_table);
    state.gen_nondet_init(object, false, "", loc, false);
