  ...
  else if(type.id() == ID_struct)
  {
    ...
    for(const auto &component : components)
    {
      ...
      else
      {
        ...
        gen_nondet_init(
          me, _is_sub, class_identifier, loc, create_dynamic_objects);
      }
    }
  ...
  const source_locationt &loc)
{
  ...
  binary_relation_exprt assume1(
    length_sym_expr, ID_ge, from_integer(0, java_int_type()));
  ...
  symbolt &counter= new_tmp_symbol(
    symbol_table, loc, length_sym_expr.type(), "array_init_iter");
  ...
  gen_nondet_init(
    arraycellref, false, irep_idt(), loc, true, true, element_type);
  ...
  const source_locationt &loc)
{
  if(type.id() == ID_pointer)
  {
    ...
    java_object_factoryt state(
      init_code, !allow_null, max_nondet_array_length, symbol_table);
