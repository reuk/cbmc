...
symbol_exprt
java_bytecode_convert_methodt::tmp_variable(const std::string &prefix,
                                            const typet &type) {
...
const exprt java_bytecode_convert_methodt::variable(
    const exprt &arg, char type_char, size_t address,
  ...
  if (var.symbol_expr.get_identifier().empty()) {
    ...
    irep_idt identifier =
        id2string(current_method) + "::" + id2string(base_name);
...
void java_bytecode_convert_method_lazy(
    ...
    const java_bytecode_parse_treet::methodt &m, symbol_tablet &symbol_table) {
...
void java_bytecode_convert_methodt::convert(const symbolt &class_symbol,
                                            const methodt &m) {
  const irep_idt method_identifier = id2string(class_symbol.name) + "." +
                                     id2string(m.name) + ":" + m.signature;
...
const bytecode_infot &
java_bytecode_convert_methodt::get_bytecode_info(const irep_idt &statement) {
  for (const bytecode_infot *p = bytecode_info; p->mnemonic != 0; p++)
    if (statement == p->mnemonic)
      return *p;

  error() << "failed to find bytecode mnemonic `" << statement << '\'' << eom;
  throw 0;
}
...
static member_exprt to_member(const exprt &pointer, const exprt &fieldref) {
  ...
  exprt pointer2 = typecast_exprt(pointer, pointer_typet(class_type));

  const dereference_exprt obj_deref(pointer2, class_type);

  return member_exprt(obj_deref, fieldref.get(ID_component_name),
                      fieldref.type());
}

codet java_bytecode_convert_methodt::get_array_bounds_check(
    const exprt &arraystruct, const exprt &idx,
  ...
  bounds_checks.operands().back().add_source_location().set_comment(
      "Array index < 0");
  bounds_checks.operands().back().add_source_location().set_property_class(
...
void java_bytecode_convert_methodt::replace_goto_target(
    codet &repl, const irep_idt &old_label, const irep_idt &new_label) {
...
Function: java_bytecode_convert_methodt::get_block_for_pcrange
 ...
 Outputs: Returns the code_blockt most closely enclosing the given address
range.
 ...
 Purpose: 'tree' describes a tree of code_blockt objects; this_block is the
          ...
          sibling and is used to determine when the range spans out of its
bounds.

\*******************************************************************/

code_blockt &java_bytecode_convert_methodt::get_block_for_pcrange(
    block_tree_nodet &tree, code_blockt &this_block, unsigned address_start,
    unsigned address_limit, unsigned next_block_start_address) {
  address_mapt dummy;
  return get_or_create_block_for_pcrange(
      tree, this_block, address_start, address_limit, next_block_start_address,
      dummy, false);
}
...
code_blockt &java_bytecode_convert_methodt::get_or_create_block_for_pcrange(
    block_tree_nodet &tree, code_blockt &this_block, unsigned address_start,
    unsigned address_limit, unsigned next_block_start_address,
    const address_mapt &amap, bool allow_merge) {
  ...
  const auto afterstart =
      std::upper_bound(tree.branch_addresses.begin(),
                       tree.branch_addresses.end(), address_start);
  ...
  auto findlim = std::lower_bound(tree.branch_addresses.begin(),
                                  tree.branch_addresses.end(), address_limit);
  unsigned findlim_block_start_address = findlim == tree.branch_addresses.end()
  ...
  if (single_child) {
    ...
    return get_or_create_block_for_pcrange(
        tree.branch[child_offset], child_block, address_start, address_limit,
        findlim_block_start_address, amap, allow_merge);
  }
  ...
  for (; checkit != amap.end() &&
         (checkit->first) < (findlim_block_start_address);
       ++checkit) {
  ...
  return to_code_block(
      to_code_label(to_code(this_block_children[child_offset])).code());
}

static void gather_symbol_live_ranges(
    unsigned pc, const exprt &e,
  ...
  if (e.id() == ID_symbol) {
    ...
    forall_operands(it, e) gather_symbol_live_ranges(pc, *it, result);
  }
}
...
codet java_bytecode_convert_methodt::convert_instructions(
    const methodt &method, const code_typet &method_type) {
  ...
  for (instructionst::const_iterator i_it = instructions.begin();
    ...
    if (i_it->statement != "goto" && i_it->statement != "return" &&
        !(i_it->statement == patternt("?return")) &&
        i_it->statement != "athrow" && i_it->statement != "jsr" &&
        i_it->statement != "jsr_w" && i_it->statement != "ret") {
      instructionst::const_iterator next = i_it;
      if (++next != instructions.end())
        a_entry.first->second.successors.push_back(next->address);
    }

    if (i_it->statement == "athrow" || i_it->statement == "invokestatic" ||
    ...
    if (i_it->statement == "goto" || i_it->statement == patternt("if_?cmp??") ||
        i_it->statement == patternt("if??") || i_it->statement == "ifnonnull" ||
        i_it->statement == "ifnull" || i_it->statement == "jsr" ||
      ...
      if (i_it->statement == "jsr" || i_it->statement == "jsr_w") {
        instructionst::const_iterator next = i_it + 1;
        assert(next != instructions.end() &&
  ...
  for (const auto retinst : ret_instructions) {
    ...
    a_entry.successors.insert(a_entry.successors.end(), jsr_ret_targets.begin(),
                              jsr_ret_targets.end());
  }
  ...
  while (!working_set.empty()) {
    ...
    assert(stack.empty() || a_it->second.predecessors.size() <= 1 ||
    ...
    if (statement.size() >= 2 && statement[statement.size() - 2] == '_' &&
    ...
    for (; it != method.exception_table.end(); ++it) {
      if (cur_pc == it->handler_pc) {
        exprt exc_var = variable(arg0, statement[0], i_it->address, NO_CAST);
        ...
        code_labelt newlabel(label(std::to_string(cur_pc)), code_blockt());
    ...
    if (statement == "aconst_null") {
      ...
      const exprt not_equal_null(binary_relation_exprt(lhs, ID_notequal, rhs));
      code_assertt check(not_equal_null);
      check.add_source_location().set_comment("Throw null");
      check.add_source_location().set_property_class("null-pointer-exception");
      ...
      if (return_type.id() != ID_empty) {
        results.resize(1);
        results[0] =
            zero_initializer(return_type, i_it->source_location,
                             namespacet(symbol_table), get_message_handler());
      }
    } else if (statement == "invokeinterface" || statement == "invokespecial" ||
               statement == "invokevirtual" || statement == "invokestatic") {
      const bool use_this(statement != "invokestatic");
      const bool is_virtual(statement == "invokevirtual" ||
      ...
      if (use_this) {
        if (parameters.empty() || !parameters[0].get_this()) {
          ...
          if (statement == "invokespecial") {
            if (as_string(arg0.get(ID_identifier)).find("<init>") !=
                std::string::npos) {
              if (lazy_methods)
      ...
      exprt pointer = typecast_exprt(op[0], java_array_type(type_char));
      ...
      if (arg0.id() == ID_java_string_literal) {
        ...
        symbol_exprt symbol_expr(id2string(class_id) + "@class_model",
                                 java_lang_Class);
      ...
      code_gotot code_goto(label(number));
      c = code_goto;
      results[0] = from_integer(std::next(i_it)->address, unsignedbv_typet(64));
      ...
      for (size_t idx = 0, idxlim = jsr_ret_targets.size(); idx != idxlim;
           ++idx) {
      ...
      if (is_double || is_float) {
        const ieee_float_spect spec(is_float
      ...
      code_assign.rhs() =
          plus_exprt(variable(arg0, 'i', i_it->address, CAST_AS_NEEDED),
      ...
      results[0] = if_exprt(binary_relation_exprt(op[0], ID_equal, op[1]),
                            from_integer(0, t), greater);
    } else if (statement == patternt("?cmp?")) {
      assert(op.size() == 2 && results.size() == 1);
      ...
      results[0] =
          if_exprt(or_exprt(nan_op0, nan_op1), nan_result,
                   if_exprt(ieee_float_equal_exprt(op[0], op[1]),
                            from_integer(0, result_type),
                            if_exprt(binary_relation_exprt(op[0], ID_lt, op[1]),
                                     minus_one, one)));
    ...
    } else if (statement == patternt("?2?")) // i2c etc.
    {
      ...
      results[0] = tmp;
    } else if (statement == "newarray" || statement == "anewarray") {
      ...
      check.add_source_location().set_property_class(
      ...
      exprt pointer = typecast_exprt(op[0], java_array_type(statement[0]));
      ...
      bool is_label = true;
      for (instructiont::argst::const_iterator a_it = i_it->args.begin();
           a_it != i_it->args.end(); a_it++, is_label = !is_label) {
        if (is_label) {
      ...
      results[0] = binary_predicate_exprt(op[0], ID_java_instanceof, arg0);
    ...
    for (const auto &exception_row : method.exception_table) {
      ...
      if (cur_pc < exception_row.end_pc && !working_set.empty() &&
    ...
    for (const unsigned address : a_it->second.successors) {
      ...
      if (!stack.empty() && a_it2->second.predecessors.size() > 1) {
        ...
        if (a_it2->second.stack.empty()) {
          for (stackt::iterator s_it = stack.begin(); s_it != stack.end();
               ++s_it) {
        ...
        if (results.empty()) {
          ...
          if (last_statement.get_statement() == ID_goto) {
            ...
            forall_operands(o_it, more_code) c.copy_to_operands(*o_it);
        }
      }
  ...
  for (const auto &aentry : address_map)
    gather_symbol_live_ranges(aentry.first, aentry.second.code,
  ...
  for (const auto vp : vars_to_process) {
    ...
    get_or_create_block_for_pcrange(
        root, root_block, v.start_pc, v.start_pc + v.length,
        std::numeric_limits<unsigned>::max(), address_map);
  }
  for (const auto vp : vars_to_process) {
    const auto &v = *vp;
    if (v.is_parameter)
    ...
    if (v.symbol_expr.get_identifier() == irep_idt())
      continue;
    auto &block = get_block_for_pcrange(root, root_block, v.start_pc,
                                        v.start_pc + v.length,
...
void java_bytecode_convert_method(
    ...
    symbol_tablet &symbol_table, message_handlert &message_handler,
    size_t max_array_length, safe_pointer<ci_lazy_methodst> lazy_methods) {
  java_bytecode_convert_methodt java_bytecode_convert_method(
      symbol_table, message_handler, max_array_length, lazy_methods);
