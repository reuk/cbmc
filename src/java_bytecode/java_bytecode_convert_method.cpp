/// \file JAVA Bytecode Language Conversion
...
///   `method_identifier`: fully qualified method name, including type
///   signature (e.g. "x.y.z.f:(I)") `m`: parsed method object to convert
...
/// Find all goto statements in 'repl' that target 'old_label' and redirect
/// them to 'new_label'.
///
/// parameters: 'repl', a block of code in which to perform replacement, and an
///   old_label that should be replaced throughout by new_label.
///
/// \returns None (side-effects on repl)
...
///   corresponding actual code_blockt. 'address_start' and 'address_limit',
///   the Java bytecode offsets searched for. 'next_block_start_address', the
///   bytecode offset of tree/this_block's successor sibling, or UINT_MAX if
///   none exists.
///
/// \returns Returns the code_blockt most closely enclosing the given address
///   range.
...
/// surrounding tree node, leaving the tree of shape (1-10), ^( (11-20),
/// (21-30) )^, and return a reference to the new branch highlighted with ^^.
/// 'tree' and 'this_block' trees are always maintained with equal shapes.
/// ('this_block' may additionally contain code_declt children which are
/// ignored for this purpose)
///
/// parameters: See above, plus the bytecode address map 'amap' and
///   'allow_merge' which is always true except when called from
///   get_block_for_pcrange
///
/// \returns See above, plus potential side-effects on 'tree' and 'this_block'
