...
static size_t
count_slots(const size_t value, const code_typet::parametert &param)
{
  ...
  bool allow_merge)
{
  ...
  const auto afterstart= std::upper_bound(
    tree.branch_addresses.begin(), tree.branch_addresses.end(), address_start);
  ...
  auto findlim= std::lower_bound(
    tree.branch_addresses.begin(), tree.branch_addresses.end(), address_limit);
  ...
  if(
    findstart == tree.branch_addresses.begin() &&
  ...
  newnode.branch_addresses.insert(
    newnode.branch_addresses.begin(), branchaddriter, branchaddrlim);
  ...
  const code_typet &method_type)
{
      ...
      i_it++)
  {
    ...
    if(
      i_it->statement != "goto" && i_it->statement != "return" &&
    ...
    if(
      i_it->statement == "athrow" || i_it->statement == "invokestatic" ||
    ...
    if(
      i_it->statement == "goto" || i_it->statement == patternt("if_?cmp??") ||
      ...
      i_it->statement == "jsr_w")
    {
      ...
      if(i_it->statement == "jsr" || i_it->statement == "jsr_w")
      {
        ...
        assert(
          next != instructions.end() && "jsr without valid return address?");
        targets.insert(next->address);
        jsr_ret_targets.push_back(next->address);
      }
    }
    else if(
      i_it->statement == "tableswitch" || i_it->statement == "lookupswitch")
    {
      bool is_label= true;
      for(const auto &arg : i_it->args)
      {
        if(is_label)
        {
          const unsigned target=
            safe_string2unsigned(id2string(to_constant_expr(arg).get_value()));
          targets.insert(target);
          a_entry.first->second.successors.push_back(target);
        }
        is_label= !is_label;
  ...
  while(!working_set.empty())
  {
    ...
    assert(
      stack.empty() || a_it->second.predecessors.size() <= 1 ||
    ...
    if(
      statement.size() >= 2 && statement[statement.size() - 2] == '_' &&
    ...
    else if(statement == "invokedynamic")
    {
      ...
      statement == "invokeinterface" || statement == "invokespecial" ||
      statement == "invokevirtual" || statement == "invokestatic")
    {
      ...
      if(use_this)
      {
        if(parameters.empty() || !parameters[0].get_this())
        {
          ...
          {
            if(
              as_string(arg0.get(ID_identifier)).find("<init>") !=
              std::string::npos)
            {
              if(lazy_methods)
    ...
    else if(statement == patternt("?load"))
    {
      ...
      statement == "ldc" || statement == "ldc_w" || statement == "ldc2" ||
      statement == "ldc2_w")
    {
      ...
      else if(arg0.id() == ID_type)
      {
        ...
        symbol_exprt symbol_expr(
          id2string(class_id) + "@class_model", java_lang_Class);
    ...
    else if(statement == patternt("?cmp?"))
    {
      ...
      results[0]= if_exprt(
        ...
        if_exprt(
          ...
          if_exprt(
            binary_relation_exprt(op[0], ID_lt, op[1]), minus_one, one)));
    }
    ...
    for(const unsigned address : a_it->second.successors)
    {
      ...
      if(!stack.empty() && a_it2->second.predecessors.size() > 1)
      {
        ...
        {
          for(stackt::iterator s_it= stack.begin(); s_it != stack.end(); ++s_it)
          {
  ...
  for(const auto &aentry : address_map)
    gather_symbol_live_ranges(
      aentry.first, aentry.second.code, temporary_variable_live_ranges);
