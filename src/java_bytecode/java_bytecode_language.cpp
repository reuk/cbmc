...
void java_bytecode_languaget::get_language_options(const cmdlinet &cmd) {
  ...
  if (cmd.isset("java-cp-include-files")) {
    ...
    if (java_cp_include_files[0] == '@') {
      ...
      if (parse_json(java_cp_include_files.substr(1), get_message_handler(),
                     json_cp_config))
...
bool java_bytecode_languaget::parse(std::istream &instream,
  ...
  if (has_suffix(path, ".class")) {
    ...
    java_class_loader_limitt class_loader_limit(get_message_handler(),
                                                java_cp_include_files);
...
static irep_idt get_virtual_method_target(
    const std::set<irep_idt> &needed_classes, const irep_idt &call_basename,
    const irep_idt &classname, const symbol_tablet &symbol_table) {
...
static void get_virtual_method_targets(
    const code_function_callt &c, const std::set<irep_idt> &needed_classes,
    std::vector<irep_idt> &needed_methods, symbol_tablet &symbol_table,
  ...
  while (1) {
    auto parent_method = get_virtual_method_target(
        needed_classes, call_basename, parent_class_id, symbol_table);
  ...
  if (needed_methods.size() == old_size) {
    ...
    std::string stubname =
        id2string(call_class) + "." + id2string(call_basename);
    symbolt symbol;
...
static void
gather_virtual_callsites(const exprt &e,
  ...
  if (c.get_statement() == ID_function_call &&
    ...
    forall_operands(it, e) gather_virtual_callsites(*it, result);
}
...
static void gather_needed_globals(const exprt &e,
  ...
  if (e.id() == ID_symbol) {
    ...
    forall_operands(opit, e) gather_needed_globals(*opit, symbol_table, needed);
}
...
static void gather_field_types(const typet &class_type, const namespacet &ns,
...
bool java_bytecode_languaget::typecheck(symbol_tablet &symbol_table,
                                        const std::string &module) {
  // first convert all
  for (java_class_loadert::class_mapt::const_iterator c_it =
           java_class_loader.class_map.begin();
       c_it != java_class_loader.class_map.end(); c_it++) {
    ...
    if (java_bytecode_convert_class(
            ...
            max_user_array_length, lazy_methods, lazy_methods_mode,
  ...
  // now typecheck all
  if (java_bytecode_typecheck(symbol_table, get_message_handler(),
                              string_refinement_enabled))
    return true;
...
bool java_bytecode_languaget::do_ci_lazy_method_conversion(
    symbol_tablet &symbol_table, lazy_methodst &lazy_methods) {
  class_hierarchyt ch;
  ch(symbol_table);
...

  {
    ...
    ci_lazy_methodst initial_lazy_methods(needed_clinits, needed_classes,
                                          symbol_table);
    initialize_needed_classes(method_worklist2, namespacet(symbol_table), ch,
                              initial_lazy_methods);
    method_worklist2.insert(method_worklist2.end(), needed_clinits.begin(),
                            needed_clinits.end());
  }
  ...
  do {
    ...
    while (method_worklist2.size() != 0) {
      ...
      for (const auto &mname : method_worklist1) {
        ...
        ci_lazy_methodst lazy_methods(method_worklist2, needed_classes,
                                      symbol_table);
        java_bytecode_convert_method(
            *parsed_method.first, *parsed_method.second, symbol_table,
            get_message_handler(), max_user_array_length,
            safe_pointer<ci_lazy_methodst>::create_non_null(&lazy_methods));
        gather_virtual_callsites(symbol_table.lookup(mname).value,
                                 virtual_callsites);
    ...
    debug() << "CI lazy methods: add virtual method targets ("
            << virtual_callsites.size() << " callsites)" << eom;
...
void java_bytecode_languaget::convert_lazy_method(const irep_idt &id,
  ...
  java_bytecode_convert_method(*lazy_method_entry.first,
                               *lazy_method_entry.second, symtab,
                               get_message_handler(), max_user_array_length);
}
...
bool java_bytecode_languaget::final(symbol_tablet &symbol_table) {
  ...
  */
  ...
  return (java_entry_point(symbol_table, main_class, get_message_handler(),
...
languaget *new_java_bytecode_language() { return new java_bytecode_languaget; }
...
bool java_bytecode_languaget::from_expr(const exprt &expr, std::string &code,
                                        const namespacet &ns) {
...
bool java_bytecode_languaget::from_type(const typet &type, std::string &code,
                                        const namespacet &ns) {
...
bool java_bytecode_languaget::to_expr(const std::string &code,
                                      const std::string &module, exprt &expr,
                                      const namespacet &ns) {
#if 0
  expr.make_nil();
