...
/// exist.
///
/// parameters: `needed_classes`: set of classes that can be instantiated. Any
///   potential callee not in this set will be ignored. `call_basename`:
...
///
/// \returns Returns the fully qualified name of `classname`'s definition of
...
/// instantiated.
///
/// parameters: `c`: function call whose potential target functions should be
///   determined. `needed_classes`: set of classes that can be instantiated.
///   Any potential callee not in this set will be ignored. `symbol_table`:
///   global symtab `class_hierarchy`: global class hierarchy
///
/// \returns Populates `needed_methods` with all possible `c` callees, taking
...
///
/// \returns Populates `result` with pointers to each function call within e
...
/// See output
///
/// parameters: `e`: expression tree to search `symbol_table`: global symtab
///
/// \returns Populates `needed` with global variable symbols referenced from
///   `e` or its children.
...
/// See output
///
/// parameters: `class_type`: root of class tree to search `ns`: global
///   namespace
///
/// \returns Populates `needed_classes` with all Java reference types reachable
///   starting at `class_type`. For example if `class_type` is
///   `symbol_typet("java::A")` and A has a B field, then `B` (but not `A`)
///   will be added to `needed_classes`.
...
/// See output
///
/// parameters: `entry_points`: list of fully-qualified function names that we
///   should assume are reachable `ns`: global namespace `ch`: global class
///   hierarchy
///
/// \returns Populates `needed_classes` with all Java reference types whose
...
/// methods may be reachable from the main entry point. In brief, static
/// methods are reachable if we find a callsite in another reachable site,
/// while virtual methods are reachable if we find a virtual callsite targeting
/// a compatible type *and* a constructor callsite indicating an object of that
/// type may be instantiated (or evidence that an object of that type exists
/// before the main function is entered, such as being passed as a parameter).
///
/// parameters: `symbol_table`: global symbol table `lazy_methods`: map from
///   method names to relevant symbol and parsed-method objects.
///
/// \returns Elaborates lazily-converted methods that may be reachable starting
...
/// parameters: None
///
/// \returns Populates `methods` with the complete list of lazy methods that
///   are available to convert (those which are valid parameters for
///   `convert_lazy_method`)
...
///
/// parameters: `id`: method ID to convert `symtab`: global symbol table
///
/// \returns Amends the symbol table entry for function `id`, which should be a
