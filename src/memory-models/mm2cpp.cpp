...
void mm2cppt::check_acyclic(const exprt &expr, unsigned indent) {
  if (expr.id() == ID_symbol) {
    ...
    if (v_it != let_values.end())
      ...
      throw expr.source_location().as_string() + ": unknown identifier `" +
...
void mm2cppt::instruction2cpp(const codet &code, unsigned indent) {
  ...
  if (statement == ID_block) {
    forall_operands(it, code) { instruction2cpp(to_code(*it), indent + 2); }
...
void mm2cpp(const irep_idt &model_name, const irept &instruction,
            std::ostream &out) {
  mm2cppt mm2cpp(out);
