...
public:
  ...
  bool operator()(const std::string &taint_file_name, const symbol_tablet &,
                  goto_functionst &, bool show_full,
...
void taint_analysist::instrument(
    const namespacet &ns, goto_functionst::goto_functiont &goto_function) {
    ...
    case FUNCTION_CALL: {
      ...
      if (function.id() == ID_symbol) {
        ...
        if (class_id.empty()) {
          ...
          std::string suffix = std::string(id2string(identifier),
                                           class_id.size(), std::string::npos);
        ...
        for (const auto &rule : taint.rules) {
          ...
          if (match) {
            ...
            case taint_parse_treet::rulet::SINK: {
              ...
              t->source_location = instruction.source_location;
              t->source_location.set_property_class("taint rule " +
                                                    id2string(rule.id));
...
bool taint_analysist::operator()(const std::string &taint_file_name,
  ...
  try {
    ...
    status() << "Reading taint file `" << taint_file_name << "'" << eom;
    ...
    status() << "Got " << taint.rules.size() << " taint definitions" << eom;
    ...
    forall_goto_functions(f_it, goto_functions) {
      ...
      forall_goto_program_instructions(i_it, f_it->second.body) {
        ...
        if (use_json) {
          json_objectt json;
          json["bugClass"] = json_stringt(
              id2string(i_it->source_location.get_property_class()));
          json["file"] =
    ...
    if (use_json) {
      ...
      if (!json_out) {
        error() << "Failed to open json output `" << json_file_name << "'"
                << eom;
        return true;
      }

      status() << "Analysis result is written to `" << json_file_name << "'"
               << eom;
...
bool taint_analysis(goto_modelt &goto_model, const std::string &taint_file_name,
                    message_handlert &message_handler, bool show_full,
                    const std::string &json_file_name) {
  taint_analysist taint_analysis;
  taint_analysis.set_message_handler(message_handler);
  return taint_analysis(taint_file_name, goto_model.symbol_table,
                        goto_model.goto_functions, show_full, json_file_name);
}
