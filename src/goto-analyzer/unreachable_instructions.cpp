...
static void unreachable_instructions(const goto_programt &goto_program,
  ...
  for (cfg_dominatorst::cfgt::entry_mapt::const_iterator it =
           dominators.cfg.entry_map.begin();
       it != dominators.cfg.entry_map.end(); ++it) {
    const cfg_dominatorst::cfgt::nodet &n = dominators.cfg[it->second];
    if (n.dominators.empty())
      dest.insert(std::make_pair(it->first->location_number, it->first));
  }
}
...
static void all_unreachable(const goto_programt &goto_program,
                            dead_mapt &dest) {
  forall_goto_program_instructions(it, goto_program) if (!it->is_end_function())
...
static void output_dead_plain(const namespacet &ns,
                              ...
                              const dead_mapt &dead_map, std::ostream &os) {
...
static void add_to_json(const namespacet &ns, const goto_programt &goto_program,
                        const dead_mapt &dead_map, json_arrayt &dest) {
...
void unreachable_instructions(const goto_modelt &goto_model, const bool json,
                              std::ostream &os) {
...
static void json_output_function(const irep_idt &function,
  ...
  entry["file name"] = json_stringt(
      concat_dir_file(id2string(first_location.get_working_directory()),
...
static void list_functions(const goto_modelt &goto_model, const bool json,
                           std::ostream &os, bool unreachable) {
  json_arrayt json_result;
  ...
  forall_goto_functions(f_it, goto_model.goto_functions) {
    ...
    // f_it->first may be a link-time renamed version, use the
    // base_name instead; do not list inlined functions
    if (unreachable == (called.find(decl.base_name) != called.end() ||
                        f_it->second.is_inlined()))
      continue;
    ...
    if (!json) {
      os << concat_dir_file(id2string(first_location.get_working_directory()),
                            id2string(first_location.get_file()))
         << " " << decl.base_name << " " << first_location.get_line() << " "
         << last_location.get_line() << "\n";
    } else
      json_output_function(decl.base_name, first_location, last_location,
                           json_result);
  }
...
void unreachable_functions(const goto_modelt &goto_model, const bool json,
                           std::ostream &os) {
...
void reachable_functions(const goto_modelt &goto_model, const bool json,
                         std::ostream &os) {
