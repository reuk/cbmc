...
bool taint_parser(const std::string &file_name, taint_parse_treet &dest,
  ...
  if (!json.is_array()) {
    messaget message(message_handler);
    message.error() << "expecting an array in the taint file, but got " << json
                    << messaget::eom;
    return true;
  }
  ...
  for (jsont::arrayt::const_iterator it = json.array.begin();
    ...
    if (!it->is_object()) {
      ...
      message.error() << "expecting an array of objects "
                      << "in the taint file, but got " << *it << messaget::eom;
      return true;
    }
    ...
    if (function.empty()) {
      messaget message(message_handler);
      message.error() << "taint rule must have \"function\"" << messaget::eom;
      return true;
    } else
...
void taint_parse_treet::rulet::output(std::ostream &out) const {
  ...
  switch (where) {
  case THIS:
    out << "this in " << function_identifier;
    break;
  case PARAMETER:
    out << "parameter " << parameter_number << " of " << function_identifier;
    break;
  case RETURN_VALUE:
    out << "return value of " << function_identifier;
    break;
  }
