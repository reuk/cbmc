...
void prop_convt::set_assumptions(const bvt &) { assert(false); }
...
void prop_convt::set_frozen(const literalt) { assert(false); }
...
literalt prop_conv_solvert::convert_bool(const exprt &expr) {
  ...
  if (expr.is_constant()) {
    ...
    unsigned i = 0;
    forall_operands(it, expr) op_bv[i++] = convert(*it);
    ...
    forall_expr(it, op) bv.push_back(convert(*it));
...
void prop_conv_solvert::set_to(const exprt &expr, bool value) {
  ...
  forall_operands(it, expr) if (it->type().id() != ID_bool) {
    boolean = false;
    break;
  }
  ...
  if (boolean) {
    if (expr.id() == ID_not) {
      ...
      if (value) {
        ...
        if (expr.id() == ID_and) {
          forall_operands(it, expr) set_to_true(*it);
          ...
          if (expr.operands().size() > 0) {
            ...
            forall_operands(it, expr) bv.push_back(convert(*it));
        ...
        {
          forall_operands(it, expr) set_to_false(*it);
          return;
        }
      }
    }
