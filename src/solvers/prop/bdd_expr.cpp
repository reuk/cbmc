...
mini_bddt bdd_exprt::from_expr_rec(const exprt &expr) {
  ...
  if (expr.is_constant())
    ...
    return n_op0 | op1;
  } else if (expr.id() == ID_equal && expr.operands().size() == 2 &&
    ...
    if (entry.second) {
      ...
      node_map.insert(std::make_pair(entry.first->second.var(), expr));
    }
...
void bdd_exprt::from_expr(const exprt &expr) { root = from_expr_rec(expr); }
