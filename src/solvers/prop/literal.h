...
public:
  ...
  // constructors
  literalt() { set(unused_var_no(), false); }

  literalt(var_not v, bool sign) { set(v, sign); }

  bool operator==(const literalt other) const { return l == other.l; }
  ...
  var_not var_no() const { return l >> 1; }

  bool sign() const { return l & 1; }

  void set(var_not _l) { l = _l; }

  void set(var_not v, bool sign) { l = (v << 1) | ((var_not)sign); }

  var_not get() const { return l; }

  void invert() { l ^= (var_not)1; }
  ...
  void swap(literalt &x) { std::swap(x.l, l); }

  // constants
  void make_true() { set(const_var_no(), true); }

  void make_false() { set(const_var_no(), false); }

  bool is_true() const { return is_constant() && sign(); }

  bool is_false() const { return is_constant() && !sign(); }

  bool is_constant() const { return var_no() == const_var_no(); }

  static inline var_not const_var_no() { return (var_not(-1) << 1) >> 1; }

  static inline var_not unused_var_no() { return (var_not(-2) << 1) >> 1; }
