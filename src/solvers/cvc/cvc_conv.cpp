...
void cvc_convt::convert_constant_expr(const exprt &expr) {
  if (expr.type().id() == ID_unsignedbv || expr.type().id() == ID_signedbv ||
    ...
    if (value.size() == 8 || value.size() == 16 || value.size() == 32 ||
        value.size() == 64) {
    ...
    if (value == "NULL") {
      out << "(# object:=" << pointer_logic.get_null_object()
          << ", offset:=" << bin_zero(config.ansi_c.pointer_width) << " #)";
    } else
...
void cvc_convt::convert_plus_expr(const exprt &expr) {
  if (expr.operands().size() >= 2) {
    if (expr.type().id() == ID_unsignedbv || expr.type().id() == ID_signedbv) {
      ...
      convert_expr(*p);
      out << " IN P WITH .offset:=BVPLUS(" << config.ansi_c.pointer_width
          << ", P.offset, ";
      convert_expr(*i);
      out << "))";
    } else
...
std::string cvc_convt::array_index_type() {
  return std::string("BITVECTOR(") + std::to_string(32) + ")";
}
...
void cvc_convt::convert_address_of_rec(const exprt &expr) {
  if (expr.id() == ID_symbol || expr.id() == ID_constant ||
    ...
    out << "(# object:=" << pointer_logic.add_object(expr)
        << ", offset:=" << bin_zero(config.ansi_c.pointer_width) << " #)";
...
void cvc_convt::convert_array_value(const exprt &expr) { convert_as_bv(expr); }
...
void cvc_convt::convert_expr(const exprt &expr) {
  ...
  if (expr.id() == ID_implies) {
    ...
    if (expr.operands().size() == 2) {
      ...
      assert(false);
  } else if (expr.id() == ID_address_of || expr.id() == "reference_to") {
    assert(expr.operands().size() == 1);
