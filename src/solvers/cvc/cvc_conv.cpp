...
void cvc_convt::convert_constant_expr(const exprt &expr)
{
    ...
    expr.type().id() == ID_bv)
  {
    ...
    if(
      value.size() == 8 || value.size() == 16 || value.size() == 32 ||
      value.size() == 64)
    {
...
void cvc_convt::convert_typecast_expr(const exprt &expr)
{
  ...
  {
    if(
      op.type().id() == ID_signedbv || op.type().id() == ID_unsignedbv ||
  ...
  else if(expr.type().id() == ID_signedbv || expr.type().id() == ID_unsignedbv)
  {
