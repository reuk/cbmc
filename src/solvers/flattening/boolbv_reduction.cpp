...
literalt boolbvt::convert_reduction(const unary_exprt &expr)
{
  ...
  for(std::size_t i= 1; i < op_bv.size(); i++)
  {
    ...
    {
    case O_OR:
      l= prop.lor(l, op_bv[i]);
      break;
    case O_AND:
      l= prop.land(l, op_bv[i]);
      break;
    case O_XOR:
      l= prop.lxor(l, op_bv[i]);
      break;
    }
  }

  if(
    id == ID_reduction_nor || id == ID_reduction_nand ||
...
bvt boolbvt::convert_bv_reduction(const unary_exprt &expr)
{
  ...
  if(
    op_type.id() != ID_verilog_signedbv ||
    op_type.id() != ID_verilog_unsignedbv)
  {
      ...
      expr.type().get_int(ID_width) == 1)
    {
      ...
      for(std::size_t i= 2; i < op_bv.size(); i+= 2)
      {
        ...
        {
        case O_OR:
          l0= prop.lor(l0, op_bv[i]);
          l1= prop.lor(l1, op_bv[i + 1]);
          break;
        case O_AND:
          l0= prop.land(l0, op_bv[i]);
          l1= prop.lor(l1, op_bv[i + 1]);
          break;
        case O_XOR:
          l0= prop.lxor(l0, op_bv[i]);
          l1= prop.lor(l1, op_bv[i + 1]);
          break;
        }
      }
