...
bvt bv_utilst::extension(const bvt &bv, std::size_t new_size,
                         representationt rep) {
  ...
  literalt extend_with =
      (rep == SIGNED && !bv.empty()) ? bv[old_size - 1] : const_literal(false);
...
Function: bv_utilst::full_adder
 ...
 Outputs: return value is the literal for the sum, carry_out gets the output
carry

 Purpose: Generates the encoding of a full adder.  The optimal encoding is the
default.
...
literalt bv_utilst::full_adder(const literalt a, const literalt b,
...
literalt bv_utilst::carry(literalt a, literalt b, literalt c) {
  ...
  if (prop.has_set_to() && prop.cnf_handled_well()) {
    ...
    unsigned const_count = a.is_constant() + b.is_constant() + c.is_constant();

    // propagation is possible if two or three inputs are constant
    if (const_count >= 2)
      return prop.lor(prop.lor(prop.land(a, b), prop.land(a, c)),
                      prop.land(b, c));
...
void bv_utilst::adder(bvt &sum, const bvt &op, literalt carry_in,
                      literalt &carry_out) {
...
literalt bv_utilst::carry_out(const bvt &op0, const bvt &op1,
                              literalt carry_in) {
...
bvt bv_utilst::add_sub_no_overflow(const bvt &op0, const bvt &op1,
                                   bool subtract, representationt rep) {
  bvt sum = op0;
...
literalt bv_utilst::overflow_add(const bvt &op0, const bvt &op1,
                                 representationt rep) {
  if (rep == SIGNED) {
    ...
    literalt sign_the_same =
        prop.lequal(op0[op0.size() - 1], op1[op1.size() - 1]);

    bvt result = add(op0, op1);
    return prop.land(sign_the_same,
                     prop.lxor(result[result.size() - 1], old_sign));
...
literalt bv_utilst::overflow_sub(const bvt &op0, const bvt &op1,
                                 representationt rep) {
  if (rep == SIGNED) {
    ...
    return prop.lselect(op1_is_int_min, !op0_is_negative,
...
void bv_utilst::adder_no_overflow(bvt &sum, const bvt &op, bool subtract,
                                  representationt rep) {
...
bvt bv_utilst::shift(const bvt &src, const shiftt s, std::size_t dist) {
  ...
  for (std::size_t i = 0; i < src.size(); i++) {
    ...
    case ARIGHT:
      l = (i + dist < src.size() ? src[i + dist]
                                 : src[src.size() - 1]); // sign bit
      break;
...
bvt bv_utilst::inverted(const bvt &bv) {
  bvt result = bv;
  Forall_literals(it, result) *it = !*it;
  return result;
}
...
bvt bv_utilst::cond_negate_no_overflow(const bvt &bv, literalt cond) {
  prop.l_set_to(prop.limplies(cond, !overflow_negate(bv)), true);
...
bvt bv_utilst::multiplier_no_overflow(const bvt &op0, const bvt &op1,
                                      representationt rep) {
  switch (rep) {
  case SIGNED:
...
void bv_utilst::signed_divider(const bvt &op0, const bvt &op1, bvt &res,
                               bvt &rem) {
  if (op0.empty() || op1.empty())
    return;
...
void bv_utilst::divider(const bvt &op0, const bvt &op1, bvt &result,
                        bvt &remainer, representationt rep) {
...
void bv_utilst::unsigned_divider(const bvt &op0, const bvt &op1, bvt &res,
                                 bvt &rem) {
...
literalt bv_utilst::lt_or_le(bool or_equal, const bvt &bv0, const bvt &bv1,
                             representationt rep) {
...
// When comparing signs we are comparing the top bit
    ...
    prop.lcnf(!compareBelow[0], !bv0[0], !bv1[0],
              (or_equal) ? result : !result);
...
literalt bv_utilst::rel(const bvt &bv0, irep_idt id, const bvt &bv1,
                        representationt rep) {
  if (id == ID_equal)
...
bool bv_utilst::is_constant(const bvt &bv) {
  forall_literals(it, bv) if (!it->is_constant()) return false;
