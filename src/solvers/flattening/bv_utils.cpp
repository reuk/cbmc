...
///
/// \returns return value is the literal for the sum, carry_out gets the output
///   carry
...
///
/// parameters: A bit-vector of a variable that is to be registered.
///
/// \returns None.
...
/// The obvious recursive comparison, the interesting thing is that it is
/// cached so the literals are shared between constants.
///
/// parameters: Bit-vectors for a variable and a const to compare, note that to
///   avoid significant amounts of copying these are mutable and consumed.
///
/// \returns The literal that is true if and only if all the bits in var and
///   const are equal.
...
///
/// parameters: Bit-vectors for a variable and a const to compare.
///
/// \returns The literal that is true if and only if they are equal.
...
///
/// parameters: Bit-vectors for the two things to compare.
///
/// \returns The literal that is true if and only if they are equal.
...
///
/// parameters: bvts for each input and whether they are signed and whether a
///   model of < or <= is required.
///
/// \returns A literalt that models the value of the comparison.
