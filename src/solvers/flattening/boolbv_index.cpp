  ...
  if (array_op_type.id() == ID_array) {
    ...
    if (is_unbounded_array(array_type)) {
      ...
      if (array.id() == ID_symbol)
        map.get_map_entry(to_symbol_expr(array).get_identifier(), array_type);
...
#define UNIFORM_ARRAY_HACK
    ...
    if (is_uniform && prop.has_set_to()) {
      ...
      std::string identifier = "__CPROVER_internal_uniform_array_" +
      ...
      binary_relation_exprt lower_bound(from_integer(0, index.type()), ID_le,
                                        index);
      binary_relation_exprt upper_bound(index, ID_lt,
                                        from_integer(array_size, index.type()));

      if (lower_bound.lhs().is_nil() || upper_bound.rhs().is_nil())
...
// TODO : Compile the whole array into a single relation
      ...
      std::string identifier = "__CPROVER_internal_actual_array_" +
...
bvt boolbvt::convert_index(const exprt &array, const mp_integer &index) {
  ...
  if (offset >= 0 && offset + width <= mp_integer(tmp.size())) {
    // in bounds
