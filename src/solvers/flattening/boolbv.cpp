...
bool boolbvt::literal(const exprt &expr, const std::size_t bit, literalt &dest)
  const
{
...
bvt boolbvt::convert_bitvector(const exprt &expr)
{
  ...
  else if(expr.id() == ID_unary_minus || expr.id() == "no-overflow-unary-minus")
...
bool boolbvt::boolbv_set_equality_to_true(const equal_exprt &expr)
{
  ...
  if(
    expr.lhs().id() == ID_symbol && type == ns.follow(expr.rhs().type()) &&
    type.id() != ID_bool)
  {
