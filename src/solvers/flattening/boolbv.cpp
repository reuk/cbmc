...
bool boolbvt::literal(const exprt &expr, const std::size_t bit,
                      literalt &dest) const {
  if (expr.type().id() == ID_bool) {
    ...
    if (expr.id() == ID_symbol || expr.id() == ID_nondet_symbol) {
      ...
      boolbv_mapt::mappingt::const_iterator it_m = map.mapping.find(identifier);
...
bvt boolbvt::convert_bitvector(const exprt &expr) {
  ...
  if (expr.id() == ID_index)
  ...
  else if (expr.id() == ID_mult || expr.id() == "no-overflow-mult")
  ...
  else if (expr.id() == ID_float_debug1 || expr.id() == ID_float_debug2) {
    ...
    bvt bv = expr.id() == ID_float_debug1 ? float_utils.debug1(bv0, bv1)
...
bvt boolbvt::convert_lambda(const exprt &expr) {
  ...
  const exprt &array_size = to_array_type(expr.type()).size();
...
bvt boolbvt::convert_symbol(const exprt &expr) {
  ...
  if (width == 0) {
    ...
    forall_literals(it, bv) if (it->var_no() >= prop.no_variables() &&
                                !it->is_constant()) {
...
literalt boolbvt::convert_rest(const exprt &expr) {
  ...
  else if (expr.id() == ID_le || expr.id() == ID_ge || expr.id() == ID_lt ||
           expr.id() == ID_gt)
...
bool boolbvt::boolbv_set_equality_to_true(const equal_exprt &expr) {
  ...
  if (expr.lhs().id() == ID_symbol && type == ns.follow(expr.rhs().type()) &&
      type.id() != ID_bool) {
...
void boolbvt::set_to(const exprt &expr, bool value) {
  if (expr.type().id() != ID_bool) {
    error() << "boolbvt::set_to got non-boolean operand: " << expr.pretty()
            << eom;
    throw 0;
  }
...
void boolbvt::print_assignment(std::ostream &out) const {
  for (boolbv_mapt::mappingt::const_iterator it = map.mapping.begin();
    ...
    out << it->first << "=" << it->second.get_value(prop) << '\n';
  }
}
