...
exprt pointer_logict::pointer_expr(std::size_t object, const typet &type) const
{
...
exprt pointer_logict::pointer_expr(const pointert &pointer, const typet &type)
  const
{
  if(pointer.object == null_object) // NULL?
  {
    ...
    else
    {
      ...
      return plus_exprt(null, from_integer(pointer.offset, integer_typet()));
    }
  }
  ...
  const exprt &src) const
{
  ...
  {
    mp_integer size= pointer_offset_size(src.type().subtype(), ns);
