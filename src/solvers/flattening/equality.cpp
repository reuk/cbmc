...
void equalityt::add_equality_constraints() {
  for (typemapt::const_iterator it = typemap.begin(); it != typemap.end(); it++)
...
void equalityt::add_equality_constraints(const typestructt &typestruct) {
  ...
  for (equalitiest::const_iterator it = typestruct.equalities.begin();
       it != typestruct.equalities.end(); it++) {
