...
exprt flatten_byte_update(const byte_update_exprt &src, const namespacet &ns,
  ...
  if (t.id() == ID_array) {
    ...
    // array of bitvectors?
    if (subtype.id() == ID_unsignedbv || subtype.id() == ID_signedbv ||
      ...
      } else // sub_size!=1
      {
        ...
        for (mp_integer i = 0; i < num_elements; ++i) {
          ...
          if (is_first_cell)
            ...
            overwrite_offset = minus_exprt(minus_exprt(cell_offset, src.op1()),
                                           stored_value_offset);
          } else
            overwrite_offset = zero_offset;

          byte_update_exprt byte_update_expr(src.id(), old_cell_value,
                                             overwrite_offset, new_value);
      ...
      throw "flatten_byte_update can only do arrays of scalars right now, "
          ...
          subtype.id_string();
             ...
             t.id() == ID_floatbv || t.id() == ID_c_bool ||
             t.id() == ID_pointer) {
    ...
    binary_predicate_exprt offset_ge_zero(offset_times_eight, ID_ge,
...
bool has_byte_operator(const exprt &src) {
  ...
  forall_operands(it, src) if (has_byte_operator(*it)) return true;
