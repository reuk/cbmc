  ...
  bvt &dest)
{
  ...
  case IS_RANGE:
    if(
      src_bvtype == IS_UNSIGNED || src_bvtype == IS_SIGNED ||
  ...
  case IS_FIXED:
    ...
    {
      // bool to fixed
      std::size_t fraction_bits= to_fixedbv_type(dest_type).get_fraction_bits();
  ...
  case IS_VERILOG_UNSIGNED:
    if(
      src_bvtype == IS_UNSIGNED || src_bvtype == IS_C_BOOL ||
      src_type.id() == ID_bool)
    {
