...
bool boolbvt::type_conversion(const typet &src_type, const bvt &src,
  ...
  if (src_bvtype == IS_C_BIT_FIELD)
    return type_conversion(
        c_bit_field_replacement_type(to_c_bit_field_type(src_type), ns), src,
        dest_type, dest);

  if (dest_bvtype == IS_C_BIT_FIELD)
    return type_conversion(
        src_type, src,
        c_bit_field_replacement_type(to_c_bit_field_type(dest_type), ns), dest);
  ...
  if (dest_type.id() == ID_complex) {
    if (src_type == dest_type.subtype()) {
      forall_literals(it, src) dest.push_back(*it);
      ...
      type_conversion(ns.follow(src_type.subtype()), lower,
                      ns.follow(dest_type.subtype()), lower_res);
      type_conversion(ns.follow(src_type.subtype()), upper,
                      ns.follow(dest_type.subtype()), upper_res);
  ...
  case IS_RANGE:
    ...
    } else if (src_bvtype == IS_RANGE) // range to range
    {
      ...
      if (dest_from == src_from) {
        ...
        dest = bv_utils.add(bv_utils.zero_extension(src, dest_width),
  ...
  case IS_FLOAT: // to float
  {
    ...
    default:
      if (src_type.id() == ID_bool) {
        ...
        Forall_literals(it, dest) *it = prop.land(*it, src[0]);
  ...
  case IS_FIXED:
    if (src_bvtype == IS_FIXED) {
      ...
      dest = src;
      return false;
    } else if (src_bvtype == IS_UNSIGNED || src_bvtype == IS_SIGNED ||
               src_bvtype == IS_C_BOOL || src_bvtype == IS_C_ENUM) {
      // integer to fixed
  ...
  case IS_C_ENUM:
    ...
    case IS_C_BOOL: {
      ...
      bool sign_extension = src_bvtype == IS_SIGNED || src_bvtype == IS_C_ENUM;
  ...
  case IS_VERILOG_UNSIGNED:
    if (src_bvtype == IS_UNSIGNED || src_bvtype == IS_C_BOOL ||
        src_type.id() == ID_bool) {
