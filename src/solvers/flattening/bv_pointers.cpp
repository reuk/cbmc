...
literalt bv_pointerst::convert_rest(const exprt &expr) {
  ...
  if (expr.id() == ID_invalid_pointer) {
    if (operands.size() == 1 && is_ptr(operands[0].type())) {
      ...
      if (!bv.empty()) {
        ...
        for (std::size_t i = 0; i < object_bits; i++) {
          equal_invalid_bv[i] =
              prop.lequal(bv[offset_bits + i], invalid_bv[offset_bits + i]);
          equal_null_bv[i] =
              prop.lequal(bv[offset_bits + i], null_bv[offset_bits + i]);
        }
  ...
  } else if (expr.id() == ID_lt || expr.id() == ID_le || expr.id() == ID_gt ||
             expr.id() == ID_ge) {
    if (operands.size() == 2 && is_ptr(operands[0].type()) &&
...
bv_pointerst::bv_pointerst(const namespacet &_ns, propt &_prop)
    : boolbvt(_ns, _prop), pointer_logic(_ns) {
...
bool bv_pointerst::convert_address_of_rec(const exprt &expr, bvt &bv) {
  if (expr.id() == ID_symbol) {
    ...
    if (struct_op_type.id() == ID_struct) {
      mp_integer offset = member_offset(to_struct_type(struct_op_type),
    ...
    return false;
  } else if (expr.id() == ID_constant || expr.id() == ID_string_constant ||
             expr.id() == ID_array) { // constant
    add_addr(expr, bv);
    return false;
...
bvt bv_pointerst::convert_pointer_type(const exprt &expr) {
  ...
  if (expr.id() == ID_symbol) {
    ...
    Forall_literals(it, bv) *it = prop.new_variable();
    ...
    exprt neg_op1 = unary_exprt(ID_unary_minus, expr.op1(), expr.op1().type());
    ...
    return bv;
  } else if (expr.id() == ID_lshr || expr.id() == ID_shl) {
...
bvt bv_pointerst::convert_bitvector(const exprt &expr) {
  ...
  if (expr.id() == ID_minus && expr.operands().size() == 2 &&
    ...
    return bv;
  } else if (expr.id() == ID_pointer_offset && expr.operands().size() == 1 &&
    ...
    return bv;
  } else if (expr.id() == ID_pointer_object && expr.operands().size() == 1 &&
             is_ptr(expr.op0().type())) {
    bvt op0 = convert_bv(expr.op0());
  ...
  } else if (expr.id() == ID_typecast && expr.operands().size() == 1 &&
             expr.op0().type().id() == ID_pointer) {
    // pointer to int
...
exprt bv_pointerst::bv_get_rec(const bvt &bv, const std::vector<bool> &unknown,
                               std::size_t offset, const typet &type) const {
  if (!is_ptr(type))
  ...
  result.copy_to_operands(pointer_logic.pointer_expr(pointer, type));
...
void bv_pointerst::offset_arithmetic(bvt &bv, const mp_integer &factor,
                                     const exprt &index) {
