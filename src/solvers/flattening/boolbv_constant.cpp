...
bvt boolbvt::convert_constant(const constant_exprt &expr)
{
  ...
  else if(expr_type.id() == ID_range)
  {
    ...
    expr_type.id() == ID_c_bit_field || expr_type.id() == ID_incomplete_c_enum)
  {
    ...
    {
      error().source_location= expr.find_source_location();
      error() << "wrong value length in constant: " << expr.pretty() << eom;
      throw 0;
    }
  ...
  else if(expr_type.id() == ID_enumeration)
  {
    ...
    expr_type.id() == ID_verilog_signedbv ||
    expr_type.id() == ID_verilog_unsignedbv)
  {
    ...
    for(std::size_t i= 0; i < binary.size(); i++)
    {
      ...
      default:
        error().source_location= expr.find_source_location();
        error() << "unknown character in Verilog constant:" << expr.pretty()
                << eom;
        throw 0;
      }
    }
