...
bvt boolbvt::convert_floatbv_typecast(const floatbv_typecast_exprt &expr) {
  ...
  if (src_type.id() == ID_floatbv && dest_type.id() == ID_floatbv) {
    ...
    return float_utils.conversion(bv0,
  ...
  if (type.id() == ID_floatbv) {
    ...
    if (subtype.id() == ID_floatbv) {
      ...
      for (std::size_t i = 0; i < size; i++) {
        ...
        tmp_bv0.assign(bv0.begin() + i * sub_width,
                       bv0.begin() + (i + 1) * sub_width);
        tmp_bv1.assign(bv1.begin() + i * sub_width,
                       bv1.begin() + (i + 1) * sub_width);
