...
bvt boolbvt::convert_with(const exprt &expr) {
  ...
  for (std::size_t op_no = 1; op_no < ops.size(); op_no += 2) {
    ...
    convert_with(expr.op0().type(), ops[op_no], ops[op_no + 1], prev_bv, bv);
  }
...
void boolbvt::convert_with(const typet &type, const exprt &op1,
  ...
  else if (type.id() == ID_bv || type.id() == ID_unsignedbv ||
           type.id() == ID_signedbv)
    return convert_with_bv(type, op1, op2, prev_bv, next_bv);
  else if (type.id() == ID_struct)
    return convert_with_struct(to_struct_type(type), op1, op2, prev_bv,
                               next_bv);
...
void boolbvt::convert_with_array(const array_typet &type, const exprt &op1,
                                 const exprt &op2, const bvt &prev_bv,
                                 bvt &next_bv) {
  ...
  for (struct_typet::componentst::const_iterator it = components.begin();
    ...
    if (it->get_name() == component_name) {
      ...
      if (sub_width != op2_bv.size()) {
        error().source_location = type.source_location();
        error() << "convert_with_struct: unexpected operand op2 width" << eom;
        throw 0;
      }
...
void boolbvt::convert_with_union(const union_typet &type, const exprt &op1,
  ...
  if (config.ansi_c.endianness ==
    ...
    assert(config.ansi_c.endianness ==
