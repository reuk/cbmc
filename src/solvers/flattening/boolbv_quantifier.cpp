...
exprt get_quantifier_var_min(const exprt &var_expr,
  ...
  assert(quantifier_expr.id() == ID_or || quantifier_expr.id() == ID_and);
  exprt res;
  res.make_false();
...
bool instantiate_quantifier(exprt &expr, const namespacet &ns) {
  ...
  exprt body_expr = re;
  if (var_expr.is_false() || min_i.is_false() || max_i.is_false() ||
      body_expr.is_false())
    return false;
...
void boolbvt::post_process_quantifiers() {
  ...
  for (auto it = quantifier_list.begin(); it != quantifier_list.end(); ++it) {
