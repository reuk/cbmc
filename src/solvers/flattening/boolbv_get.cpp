...
exprt boolbvt::get(const exprt &expr) const {
  if (expr.id() == ID_symbol || expr.id() == ID_nondet_symbol) {
    ...
    boolbv_mapt::mappingt::const_iterator it = map.mapping.find(identifier);
...
exprt boolbvt::bv_get_rec(const bvt &bv, const std::vector<bool> &unknown,
  ...
  if (bvtype == IS_UNKNOWN) {
    if (type.id() == ID_array) {
      ...
      if (sub_width != 0) {
        ...
        for (std::size_t new_offset = 0; new_offset < width;
             new_offset += sub_width) {
          op.push_back(bv_get_rec(bv, unknown, offset + new_offset, subtype));
        }
      ...
      return bv_get_rec(bv, unknown, offset,
      ...
      value.set_component_name(components[component_nr].get_name());
...
exprt boolbvt::bv_get_unbounded_array(const exprt &expr) const {
...

  {
    ...
    for (index_sett::const_iterator it1 = index_set.begin();
         it1 != index_set.end(); it1++) {
      index_exprt index;
  ...
  if (size_mpint > 100 || size.id() == ID_infinity) {
    ...
    for (valuest::const_iterator it = values.begin(); it != values.end();
         it++) {
...
mp_integer boolbvt::get_value(const bvt &bv, std::size_t offset,
                              std::size_t width) {
  mp_integer value = 0;
  mp_integer weight = 1;
