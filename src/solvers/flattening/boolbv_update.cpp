...
bvt boolbvt::convert_update(const exprt &expr) {
  ...
  // start the recursion
  convert_update_rec(expr.op1().operands(), 0, expr.type(), 0, expr.op2(), bv);
...
void boolbvt::convert_update_rec(const exprt::operandst &designators,
                                 std::size_t d, const typet &type,
                                 std::size_t offset, const exprt &new_value,
                                 bvt &bv) {
  if (type.id() == ID_symbol)
    convert_update_rec(designators, d, ns.follow(type), offset, new_value, bv);
  ...
  if (designator.id() == ID_index_designator) {
    ...
    for (std::size_t i = 0; i != integer2size_t(size); ++i) {
      ...
      convert_update_rec(designators, d + 1, subtype, new_offset, new_value,
                         tmp_bv);
    ...
    if (type.id() == ID_struct) {
      ...
      // recursive call
      convert_update_rec(designators, d + 1, new_type, new_offset, new_value,
                         bv);
      ...
      // recursive call
      convert_update_rec(designators, d + 1, new_type, offset, new_value, bv);
    } else
