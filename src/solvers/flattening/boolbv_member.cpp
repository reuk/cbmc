...
bvt boolbvt::convert_member(const member_exprt &expr) {
  ...
  if (struct_op_type.id() == ID_union) {
    return convert_bv(byte_extract_exprt(byte_extract_id(), struct_op,
      ...
      if (it->get_name() == component_name) {
        if (!base_type_eq(subtype, expr.type(), ns)) {
          ...
          error() << "member: component type does not match: "
                  << subtype.pretty() << " vs. " << expr.type().pretty() << eom;
          throw 0;
        }
    ...
    error().source_location = expr.find_source_location();
    error() << "component " << component_name << " not found in structure"
            << eom;
    throw 0;
  } else {
    error().source_location = expr.find_source_location();
