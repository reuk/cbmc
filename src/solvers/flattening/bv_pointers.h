...
protected:
  ...
  exprt bv_get_rec(const bvt &bv, const std::vector<bool> &unknown,
                   std::size_t offset, const typet &type) const override;
  ...
  void offset_arithmetic(bvt &bv, const mp_integer &factor,
                         const bvt &index_bv);
