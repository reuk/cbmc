...
void bv_minimizet::add_objective(prop_minimizet &prop_minimize,
                                 const exprt &objective) {
  // build bit-wise objective function

  const typet &type = objective.type();

  if (type.id() == ID_bool || type.id() == ID_unsignedbv ||
...
void bv_minimizet::operator()(const minimization_listt &symbols) {
  ...
  for (minimization_listt::const_iterator l_it = symbols.begin();
       l_it != symbols.end(); l_it++) {
