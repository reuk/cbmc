...
bvt bv_refinementt::convert_floatbv_op(const exprt &expr) {
  ...
  if (ns.follow(expr.type()).id() != ID_floatbv || expr.operands().size() != 3)
    return SUB::convert_floatbv_op(expr);
...
void bv_refinementt::check_SAT(approximationt &a) {
  ...
  if (type.id() == ID_floatbv) {
    ...
    debug() << "S1: " << o0 << " " << a.expr.id() << " " << o1 << " != " << rr
            << eom;
    debug() << "S2: " << integer2binary(a.op0_value, spec.width()) << " "
            << a.expr.id() << " " << integer2binary(a.op1_value, spec.width())
    ...
    if ((a.expr.id() == ID_div || a.expr.id() == ID_mod) && o1 == 0)
      return;
    ...
    if (a.over_state == 0) {
      ...
      bvt r;
      if (a.expr.id() == ID_mult) {
        r = bv_utils.multiplier(a.op0_bv, a.op1_bv,
                                a.expr.type().id() == ID_signedbv
                                    ? bv_utilst::SIGNED
                                    : bv_utilst::UNSIGNED);
      } else if (a.expr.id() == ID_div) {
        r = bv_utils.divider(a.op0_bv, a.op1_bv,
                             a.expr.type().id() == ID_signedbv
                                 ? bv_utilst::SIGNED
                                 : bv_utilst::UNSIGNED);
      } else if (a.expr.id() == ID_mod) {
        r = bv_utils.remainder(a.op0_bv, a.op1_bv,
                               a.expr.type().id() == ID_signedbv
                                   ? bv_utilst::SIGNED
                                   : bv_utilst::UNSIGNED);
      } else
  ...
  status() << "Found spurious `" << a.as_string() << "' (state " << a.over_state
           << ")" << eom;
...
void bv_refinementt::check_UNSAT(approximationt &a) {
  ...
  status() << "Found assumption for `" << a.as_string() << "' in proof (state "
           << a.under_state << ")" << eom;
...
bv_refinementt::add_approximation(const exprt &expr, bvt &bv) {
