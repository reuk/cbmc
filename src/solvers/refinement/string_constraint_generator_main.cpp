...
constant_exprt
string_constraint_generatort::constant_char(int i, const typet &char_type) {
...
symbol_exprt
string_constraint_generatort::fresh_univ_index(const irep_idt &prefix,
                                               const typet &type) {
...
symbol_exprt
string_constraint_generatort::fresh_exist_index(const irep_idt &prefix,
                                                const typet &type) {
...
symbol_exprt
string_constraint_generatort::fresh_boolean(const irep_idt &prefix) {
...
string_exprt
string_constraint_generatort::fresh_string(const refined_string_typet &type) {
  symbol_exprt length = fresh_symbol("string_length", type.get_index_type());
  symbol_exprt content =
      fresh_symbol("string_content", type.get_content_type());
...
string_constraint_generatort::add_axioms_for_if(const if_exprt &expr) {
  ...
  axioms.push_back(implies_exprt(not_exprt(expr.cond()),
                                 res.axiom_for_has_same_length_as(f)));
...
exprt string_constraint_generatort::add_axioms_for_function_application(
  ...
  const irep_idt &id = is_ssa_expr(name)
...
string_exprt string_constraint_generatort::add_axioms_for_copy(
  ...
  symbol_exprt idx =
      fresh_univ_index("QA_index_copy", ref_type.get_index_type());
...
string_exprt string_constraint_generatort::add_axioms_for_java_char_array(
  ...
  string_exprt res =
      fresh_string(refined_string_typet(java_int_type(), java_char_type()));
...
string_exprt string_constraint_generatort::add_axioms_from_char_array(
    const exprt &length, const exprt &data, const exprt &offset,
    const exprt &count) {
...
exprt string_constraint_generatort::axiom_for_is_positive_index(
    const exprt &x) {
...
exprt string_constraint_generatort::add_axioms_for_char_literal(
  ...
  assert(args.size() ==
         1); // there should be exactly 1 argument to char literal

  const exprt &arg = args[0];
  // for C programs argument to char literal should be one string constant
  // of size 1.
  if (arg.operands().size() == 1 && arg.op0().operands().size() == 1 &&
