...
string_refinementt::string_refinementt(const namespacet &_ns, propt &_prop,
    ...
    : supert(_ns, _prop), use_counter_example(false),
...
void string_refinementt::initial_index_set(const string_constraintt &axiom) {
  ...
  while (!to_process.empty()) {
    ...
    if (cur.id() == ID_index) {
      ...
      forall_operands(it, cur) to_process.push_back(*it);
  }
}
...
Function: string_refinementt::instantiate
 ...
 Outputs: substitute `qvar` the universaly quantified variable of `axiom`, by
          ...
          $\forall q. s[q+x]='a' && t[q]='b'$, `instantiate(axom,s,v)` would
return
...
exprt string_refinementt::instantiate(const string_constraintt &axiom,
  ...
  exprt bounds =
      and_exprt(axiom.univ_within_bounds(),
...
void string_refinementt::instantiate_not_contains(
  ...
  for (auto it0 : index_set0)
    for (auto it1 : index_set1) {
      ...
      binary_relation_exprt c2(to_string_expr(s0).length(), ID_gt,
                               plus_exprt(val, witness));
