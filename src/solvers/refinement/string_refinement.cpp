/// \file String support via creating string constraints and progressively
...
/// Parameterized Array and Interval Automaton"        by Guodong Li and
/// Indradeep Ghosh.
...
/// string symbols that we maintain, and convert it; otherwise use the method
/// of the parent class
///
/// parameters: an expression
///
...
///
/// parameters: an equality expression
///
/// \returns a Boolean flag to signal a proble
...
///
/// \returns result of the decision procedure
...
///
/// \returns a map where each leaf of the input is mapped to the number of
///   times it is added. For instance, expression $x + x - y$ would give the
///   map x -> 2, y -> -1.
...
///
/// \returns a expression for the sum of each element in the map a number of
...
///
/// \returns an equivalent expression in a cannonical form
...
/// parameters: a symbol qvar, an expression val, an expression f containing +
///   and − operations in which qvar should appear exactly once.
///
/// \returns an expression corresponding of $f^{−1}(val)$ where $f$ is seen
///   as a function of $qvar$, i.e. the value that is necessary for qvar for f
///   to be equal to val. For instance, if `f` corresponds to the expression $q
///   + x$, `compute_inverse_function(q,v,f)` returns an expression for $v -
///   x$.
...
///
/// \returns substitute `qvar` the universaly quantified variable of `axiom`,
///   by an index `val`, in `axiom`, so that the index used for `str` equals
///   `val`. For instance, if `axiom` corresponds to $\forall q. s[q+x]='a' &&
