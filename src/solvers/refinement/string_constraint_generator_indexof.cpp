...
exprt string_constraint_generatort::add_axioms_for_index_of(
  ...
  implies_exprt a3(contains,
                   and_exprt(binary_relation_exprt(from_index, ID_le, index),
                             equal_exprt(str[index], c)));
  axioms.push_back(a3);

  symbol_exprt n = fresh_univ_index("QA_index_of", index_type);
  string_constraintt a4(n, from_index, index, contains,
...
exprt string_constraint_generatort::add_axioms_for_index_of_string(
    const string_exprt &str, const string_exprt &substring,
  ...
  implies_exprt a1(contains,
                   and_exprt(str.axiom_for_is_longer_than(
                                 plus_exprt(substring.length(), offset)),
  ...
  string_constraintt a3(
      qvar, substring.length(), contains,
...
exprt string_constraint_generatort::add_axioms_for_last_index_of(
  ...
  implies_exprt a3(contains,
                   and_exprt(binary_relation_exprt(from_index, ID_ge, index),
                             equal_exprt(str[index], c)));
  axioms.push_back(a3);

  symbol_exprt n = fresh_univ_index("QA_last_index_of", index_type);
  string_constraintt a4(n, plus_exprt(index, index1), from_index_plus_one,
                        contains, not_exprt(equal_exprt(str[n], c)));
  axioms.push_back(a4);
...
exprt string_constraint_generatort::add_axioms_for_last_index_of(
  ...
  if (args.size() == 2)
    from_index =
        minus_exprt(str.length(), from_integer(1, str.length().type()));
  else if (args.size() == 3)
    from_index = args[2];
  else
    assert(false);
