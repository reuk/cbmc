...
public:
  ...
  const exprt &premise() const { return op0(); }

  const exprt &body() const { return op1(); }

  const symbol_exprt &univ_var() const { return to_symbol_expr(op2()); }

  const exprt &upper_bound() const { return op3(); }

  const exprt &lower_bound() const { return operands()[4]; }

private:
  string_constraintt();

public:
  string_constraintt(const symbol_exprt &_univ_var, const exprt &bound_inf,
                     const exprt &bound_sup, const exprt &prem,
                     const exprt &body)
  ...
  string_constraintt(const symbol_exprt &_univ_var, const exprt &bound_sup,
                     const exprt &prem, const exprt &body)
      : string_constraintt(_univ_var, from_integer(0, _univ_var.type()),
                           bound_sup, prem, body) {}
  ...
  exprt univ_within_bounds() const {
    return and_exprt(binary_relation_exprt(lower_bound(), ID_le, univ_var()),
                     binary_relation_exprt(upper_bound(), ID_gt, univ_var()));
  }
};

extern inline const string_constraintt &
to_string_constraint(const exprt &expr) {
...
public:
  ...
  string_not_contains_constraintt(exprt univ_lower_bound, exprt univ_bound_sup,
  ...
  const exprt &univ_lower_bound() const { return op0(); }

  const exprt &univ_upper_bound() const { return op1(); }

  const exprt &premise() const { return op2(); }

  const exprt &exists_lower_bound() const { return op3(); }

  const exprt &exists_upper_bound() const { return operands()[4]; }

  const string_exprt &s0() const { return to_string_expr(operands()[5]); }

  const string_exprt &s1() const { return to_string_expr(operands()[6]); }
};
