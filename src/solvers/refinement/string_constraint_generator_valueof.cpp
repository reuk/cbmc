...
string_exprt
string_constraint_generatort::add_axioms_from_float(const exprt &f,
  ...
  ieee_float_spect fspec = double_precision
  ...
  string_constraintt a2(qvar, nan_string.length(), isnan,
  ...
  implies_exprt a5(isneg,
                   equal_exprt(sign_string[0], constant_char('-', char_type)));
  axioms.push_back(a5);
  ...
  implies_exprt a6(isinf,
                   magnitude.axiom_for_has_same_length_as(infinity_string));
  axioms.push_back(a6);
...
string_exprt string_constraint_generatort::add_axioms_from_bool(
  ...
  string_constraintt a2(qvar, true_string.length(), eq,
                        equal_exprt(res[qvar], true_string[qvar]));
  axioms.push_back(a2);

  implies_exprt a3(not_exprt(eq),
                   res.axiom_for_has_same_length_as(false_string));
  axioms.push_back(a3);

  symbol_exprt qvar1 = fresh_univ_index("QA_equal_false", index_type);
  string_constraintt a4(qvar, false_string.length(), not_exprt(eq),
...
static mp_integer smallest_by_digit(int nb) { return power(10, nb - 1); }
...
string_exprt string_constraint_generatort::add_axioms_from_int_hex(
  ...
  size_t max_size = 8;
  axioms.push_back(and_exprt(res.axiom_for_is_strictly_longer_than(0),
...
string_exprt string_constraint_generatort::add_axioms_for_value_of(
  ...
  if (args.size() == 3) {
    ...
    symbol_exprt idx =
        fresh_univ_index("QA_index_value_of", ref_type.get_index_type());
