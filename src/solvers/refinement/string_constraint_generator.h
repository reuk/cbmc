...
public:
  ...
  exprt get_witness_of(const string_not_contains_constraintt &c,
  ...
  void set_string_symbol_equal_to_expr(const symbol_exprt &sym,
                                       const exprt &str);
...
private:
  ...
  exprt add_axioms_for_is_prefix(const string_exprt &prefix,
                                 const string_exprt &str, const exprt &offset);
  exprt add_axioms_for_is_prefix(const function_application_exprt &f,
                                 bool swap_arguments = false);
  exprt add_axioms_for_is_suffix(const function_application_exprt &f,
                                 bool swap_arguments = false);
  ...
  string_exprt add_axioms_for_concat(const string_exprt &s1,
  ...
  string_exprt
  add_axioms_for_concat_double(const function_application_exprt &f);
  ...
  string_exprt add_axioms_for_delete(const string_exprt &str,
                                     const exprt &start, const exprt &end);
  ...
  string_exprt add_axioms_for_insert(const string_exprt &s1,
                                     const string_exprt &s2,
                                     const exprt &offset);
  ...
  string_exprt
  add_axioms_for_insert_char_array(const function_application_exprt &f);
  ...
  string_exprt add_axioms_from_char_array(const exprt &length,
                                          const exprt &data,
                                          const exprt &offset,
                                          const exprt &count);
  exprt add_axioms_for_index_of(const string_exprt &str, const exprt &c,
  ...
  exprt add_axioms_for_index_of_string(const string_exprt &str,
  ...
  // TODO: the specifications are only partial
  exprt add_axioms_for_last_index_of_string(const string_exprt &str,
                                            const string_exprt &substring,
                                            const exprt &from_index);

  // Add axioms corresponding to the String.lastIndexOf:(CI) java function
  exprt add_axioms_for_last_index_of(const string_exprt &str, const exprt &c,
                                     const exprt &from_index);
  ...
  string_exprt add_axioms_from_float(const exprt &f,
  ...
  string_exprt add_axioms_for_substring(const string_exprt &str,
                                        const exprt &start, const exprt &end);
  string_exprt add_axioms_for_substring(const function_application_exprt &expr);

  string_exprt
  add_axioms_for_to_lower_case(const function_application_exprt &expr);
  string_exprt
  add_axioms_for_to_upper_case(const function_application_exprt &expr);
  ...
  static const function_application_exprt::argumentst &
  args(const function_application_exprt &expr, size_t nb) {
  ...
  static exprt character_equals_ignore_case(exprt char1, exprt char2,
                                            exprt char_a, exprt char_A,
                                            exprt char_Z);
