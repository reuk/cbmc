...
exprt float_bvt::is_equal(const exprt &src0, const exprt &src1,
...
exprt float_bvt::from_signed_integer(const exprt &src, const exprt &rm,
                                     const ieee_float_spect &spec) {
...
exprt float_bvt::from_unsigned_integer(const exprt &src, const exprt &rm,
                                       const ieee_float_spect &spec) {
  unbiased_floatt result;
...
exprt float_bvt::to_signed_integer(const exprt &src, std::size_t dest_width,
                                   const exprt &rm,
                                   const ieee_float_spect &spec) {
...
exprt float_bvt::to_unsigned_integer(const exprt &src, std::size_t dest_width,
                                     const exprt &rm,
                                     const ieee_float_spect &spec) {
...
exprt float_bvt::to_integer(const exprt &src, std::size_t dest_width,
                            bool is_signed, const exprt &rm,
                            const ieee_float_spect &spec) {
...
exprt float_bvt::conversion(const exprt &src, const exprt &rm,
  ...
  if (dest_spec.e >= src_spec.e && dest_spec.f >= src_spec.f &&
    ...
    result.fraction = concatenation_exprt(
        unpacked_src.fraction, from_integer(0, unsignedbv_typet(padding)),
...
exprt float_bvt::isnormal(const exprt &src, const ieee_float_spect &spec) {
  return and_exprt(not_exprt(exponent_all_zeros(src, spec)),
...
exprt float_bvt::add_sub(bool subtract, const exprt &op0, const exprt &op1,
                         const exprt &rm, const ieee_float_spect &spec) {
  ...
  exprt fraction2_stickied = bitor_exprt(
      fraction2_shifted,
      concatenation_exprt(from_integer(0, unsignedbv_typet(spec.f + 3)),
                          sticky_bit, fraction2_shifted.type()));
  ...
  result.fraction =
      if_exprt(subtract_lit, minus_exprt(fraction1_ext, fraction2_ext),
  ...
  // NaN?
  result.NaN =
      or_exprt(and_exprt(and_exprt(unpacked1.infinity, unpacked2.infinity),
                         notequal_exprt(unpacked1.sign, unpacked2.sign)),
               or_exprt(unpacked1.NaN, unpacked2.NaN));

  // infinity?
  result.infinity = and_exprt(not_exprt(result.NaN),
  ...
  exprt zero_sign = if_exprt(rounding_mode_bits.round_to_minus_inf,
                             or_exprt(unpacked1.sign, unpacked2.sign),
                             and_exprt(unpacked1.sign, unpacked2.sign));

  result.sign = if_exprt(result.infinity, infinity_sign,
...
exprt float_bvt::limit_distance(const exprt &dist, mp_integer limit) {
  ...
  exprt upper_bits = extractbits_exprt(dist, dist_width - 1, nb_bits,
  ...
  return if_exprt(upper_bits_zero, lower_bits,
...
exprt float_bvt::mul(const exprt &src1, const exprt &src2, const exprt &rm,
                     const ieee_float_spect &spec) {
  // unpack
...
exprt float_bvt::div(const exprt &src1, const exprt &src2, const exprt &rm,
                     const ieee_float_spect &spec) {
  // unpack
  const unbiased_floatt unpacked1 = unpack(src1, spec);
  const unbiased_floatt unpacked2 = unpack(src2, spec);
  ...
  result.fraction = concatenation_exprt(result.fraction, have_remainder,
                                        unsignedbv_typet(div_width + 1));
  ...
  result.infinity = or_exprt(
      ...
      and_exprt(unpacked1.infinity, and_exprt(not_exprt(unpacked2.NaN),
...
exprt float_bvt::relation(const exprt &src1, relt rel, const exprt &src2,
                          const ieee_float_spect &spec) {
  if (rel == GT)
  ...
  if (rel == LT || rel == LE) {
    ...
    exprt signs_different = notequal_exprt(sign_bit(src1), sign_bit(src2));
    ...
    exprt less_than2 =
        notequal_exprt(less_than1, and_exprt(sign_bit(src1), sign_bit(src2)));

    exprt less_than3 = if_exprt(signs_different, sign_bit(src1), less_than2);
...
exprt float_bvt::get_exponent(const exprt &src, const ieee_float_spect &spec) {
  return extractbits_exprt(src, spec.f + spec.e - 1, spec.f,
...
void float_bvt::denormalization_shift(exprt &fraction, exprt &exponent,
  ...
  // use sign bit
  exprt denormal =
      and_exprt(not_exprt(sign_exprt(distance)),
...
exprt float_bvt::rounder(const unbiased_floatt &src, const exprt &rm,
                         const ieee_float_spect &spec) {
...
exprt float_bvt::fraction_rounding_decision(
    const std::size_t dest_bits, const exprt sign, const exprt &fraction,
  ...
  if (extra_bits >= 2) {
    ...
    exprt tail = extractbits_exprt(fraction, extra_bits - 2, 0,
  ...
  return if_exprt(
      rounding_mode_bits.round_to_even, round_to_even,
      if_exprt(
          rounding_mode_bits.round_to_plus_inf, round_to_plus_inf,
...
void float_bvt::round_fraction(unbiased_floatt &result,
  ...
  if (result_fraction_size < fraction_size) {
    ...
    result.fraction = concatenation_exprt(result.fraction,
  ...
  } else // fraction gets smaller -- rounding
  {
    ...
    result.fraction =
        extractbits_exprt(result.fraction, result_fraction_size - 1, extra_bits,
                          unsignedbv_typet(fraction_size));
    ...
    result.exponent = plus_exprt(
        result.exponent, if_exprt(or_exprt(overflow, subnormal_to_normal),
...
void float_bvt::round_exponent(unbiased_floatt &result,
  ...
  } else // exponent gets smaller -- chop off top bits
  {
    ...
    result.exponent = extractbits_exprt(result.exponent, spec.e - 1, 0,
                                        signedbv_typet(spec.e));
    ...
    exprt exponent_too_large =
        and_exprt(binary_relation_exprt(old_exponent, ID_ge, max_exponent),
                  notequal_exprt(result.fraction,
                                 from_integer(0, result.fraction.type())));
    ...
    result.fraction = if_exprt(
        set_to_max, to_unsignedbv_type(result.fraction.type()).largest_expr(),
        result.fraction);

    result.infinity = or_exprt(result.infinity,
                               and_exprt(exponent_too_large, overflow_to_inf));
#else
...
float_bvt::biased_floatt float_bvt::bias(const unbiased_floatt &src,
  ...
  // (includes zero)
  result.exponent = if_exprt(denormal, from_integer(0, result.exponent.type()),
                             result.exponent);
...
float_bvt::unbiased_floatt float_bvt::unpack(const exprt &src,
  ...
  result.fraction = concatenation_exprt(hidden_bit, result.fraction,
  ...
  result.exponent =
      if_exprt(denormal, from_integer(-spec.bias() + 1, signedbv_typet(spec.e)),
...
exprt float_bvt::sticky_right_shift(const exprt &op, const exprt &dist,
                                    exprt &sticky) {
