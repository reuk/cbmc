  ...
  const ieee_float_spect &dest_spec)
{
  ...
  int sourceSmallestDenormalExponent= sourceSmallestNormalExponent - src_spec.f;
  ...
  const rounding_mode_bitst &rounding_mode_bits)
{
  ...
  // round down
  exprt round_to_minus_inf= and_exprt(sign, or_exprt(rounding_bit, sticky_bit));
  ...
  const ieee_float_spect &spec)
{
  ...
  else // exponent gets smaller -- chop off top bits
  {
    ...
    exprt set_to_max= and_exprt(exponent_too_large, not_exprt(overflow_to_inf));
