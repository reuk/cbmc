...
bvt float_utilst::from_signed_integer(const bvt &src) {
  ...
  result.exponent = bv_utils.build_constant(
      src.size() - 1, address_bits(src.size() - 1).to_long() + 1);
...
bvt float_utilst::to_integer(const bvt &src, std::size_t dest_width,
                             bool is_signed) {
  ...
  if (rounding_mode_bits.round_to_zero.is_true()) {
    ...
    if (dest_width > fraction.size()) {
      ...
      fraction.insert(fraction.begin(), lsb_extension.begin(),
                      lsb_extension.end());
    }
...
bvt float_utilst::conversion(const bvt &src,
  ...
  if (dest_spec.e >= spec.e && dest_spec.f >= spec.f &&
...
bvt float_utilst::add_sub(const bvt &src1, const bvt &src2, bool subtract) {
  ...
  const bvt fraction2_ext = bv_utils.zero_extension(
      fraction2_stickied, fraction2_stickied.size() + 2);
  ...
  // NaN?
  result.NaN =
      prop.lor(prop.land(prop.land(unpacked1.infinity, unpacked2.infinity),
  ...
  result.zero = prop.land(!prop.lor(result.infinity, result.NaN),
...
#if 1
  literalt zero_sign = prop.lselect(rounding_mode_bits.round_to_minus_inf,
                                    prop.lor(unpacked1.sign, unpacked2.sign),
                                    prop.land(unpacked1.sign, unpacked2.sign));

  result.sign =
      prop.lselect(result.infinity, infinity_sign,
...
bvt float_utilst::mul(const bvt &src1, const bvt &src2) {
  ...
  const bvt fraction1 = bv_utils.zero_extension(unpacked1.fraction,
                                                unpacked1.fraction.size() * 2);
  const bvt fraction2 = bv_utils.zero_extension(unpacked2.fraction,
                                                unpacked2.fraction.size() * 2);
  ...
  const bvt exponent1 = bv_utils.sign_extension(unpacked1.exponent,
                                                unpacked1.exponent.size() + 2);
  const bvt exponent2 = bv_utils.sign_extension(unpacked2.exponent,
                                                unpacked2.exponent.size() + 2);
...
bvt float_utilst::div(const bvt &src1, const bvt &src2) {
  ...
  result.infinity = prop.lor(
      ...
      prop.land(unpacked1.infinity,
                prop.land(!unpacked2.NaN, !unpacked2.zero)));
...
void float_utilst::normalization_shift(bvt &fraction, bvt &exponent) {
  ...
  for (int d = depth - 1; d >= 0; d--) {
    ...
    const bvt shifted = bv_utils.shift(fraction, bv_utilst::LEFT, distance);
...
void float_utilst::denormalization_shift(bvt &fraction, bvt &exponent) {
  ...
  fraction = bv_utils.select(
      denormal, bv_utils.shift(fraction, bv_utilst::LRIGHT, distance),
      fraction);
#endif
...
literalt float_utilst::fraction_rounding_decision(const std::size_t dest_bits,
  ...
  return prop.lselect(
      ...
      prop.lselect(
          ...
          prop.lselect(
              rounding_mode_bits.round_to_minus_inf, round_to_minus_inf,
...
void float_utilst::round_fraction(unbiased_floatt &result) {
  ...
  } else // fraction gets smaller -- rounding
  {
    ...
    result.fraction = bv_utils.extract(result.fraction, extra_bits,
...
void float_utilst::round_exponent(unbiased_floatt &result) {
  ...
  } else // exponent gets smaller -- chop off top bits
  {
    ...
    literalt exponent_too_large =
        prop.land(!bv_utils.signed_less_than(old_exponent, max_exponent),
    ...
    result.fraction = bv_utils.select(
        set_to_max, bv_utils.inverted(bv_utils.zeros(result.fraction.size())),
        result.fraction);
...
float_utilst::unbiased_floatt float_utilst::unpack(const bvt &src) {
  ...
  result.exponent = bv_utils.select(
      denormal, bv_utils.build_constant(-spec.bias() + 1, spec.e),
...
bvt float_utilst::sticky_right_shift(const bvt &op, const bvt &dist,
                                     literalt &sticky) {
  std::size_t d = 1;
  bvt result = op;
...
bvt float_utilst::debug1(const bvt &src1, const bvt &src2) { return src1; }
...
bvt float_utilst::debug2(const bvt &op0, const bvt &op1) { return op0; }
