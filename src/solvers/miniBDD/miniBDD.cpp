...
void mini_bdd_mgrt::DumpDot(std::ostream &out, bool suppress_zero) const {
  ...
  for (unsigned v = 0; v < var_table.size(); v++) {
    ...
    forall_nodes(u) if (u->var == (v + 1) && u->reference_counter != 0) out
        << '"' << u->node_number << "\"; ";
...
void mini_bdd_mgrt::DumpTikZ(std::ostream &out, bool suppress_zero,
...
mini_bddt mini_bdd_applyt::APP(const mini_bddt &x, const mini_bddt &y) {
  ...
  if (x.is_constant() && y.is_constant())
    ...
    u = mgr->mk(x.var(), APP(x.low(), y.low()), APP(x.high(), y.high()));
  else if (x.var() < y.var())
    u = mgr->mk(x.var(), APP(x.low(), y), APP(x.high(), y));
  else /* x.var() > y.var() */
    u = mgr->mk(y.var(), APP(x, y.low()), APP(x, y.high()));
...
bool equal_fkt(bool x, bool y) { return x == y; }

mini_bddt mini_bddt::operator==(const mini_bddt &other) const {
  return mini_bdd_applyt(equal_fkt)(*this, other);
}

bool xor_fkt(bool x, bool y) { return x ^ y; }

mini_bddt mini_bddt::operator^(const mini_bddt &other) const {
  return mini_bdd_applyt(xor_fkt)(*this, other);
}

mini_bddt mini_bddt::operator!() const { return node->mgr->True() ^ *this; }

bool and_fkt(bool x, bool y) { return x && y; }

mini_bddt mini_bddt::operator&(const mini_bddt &other) const {
  return mini_bdd_applyt(and_fkt)(*this, other);
}

bool or_fkt(bool x, bool y) { return x || y; }
...
mini_bddt mini_bdd_mgrt::mk(unsigned var, const mini_bddt &low,
...
operator<(const mini_bdd_mgrt::reverse_keyt &other) const {
