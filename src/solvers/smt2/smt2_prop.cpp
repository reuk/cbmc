...
smt2_propt::smt2_propt(const std::string &benchmark, const std::string &source,
                       const std::string &logic, bool _core_enabled,
                       std::ostream &_out)
    : out(_out), core_enabled(_core_enabled) {
  out << "; SMT 2"
      << "\n";
...
void smt2_propt::finalize() {
  ...
  for (smt2_identifierst::const_iterator it = smt2_identifiers.begin();
       it != smt2_identifiers.end(); it++)
    out << "(get-value (" << *it << "))"
        << "\n";
...
literalt smt2_propt::land(const bvt &bv) {
  ...
  forall_literals(it, bv) out << " " << smt2_literal(*it);
...
literalt smt2_propt::lnand(literalt a, literalt b) { return !land(a, b); }
...
literalt smt2_propt::lnor(literalt a, literalt b) { return !lor(a, b); }
...
literalt smt2_propt::lequal(literalt a, literalt b) { return !lxor(a, b); }
...
literalt smt2_propt::limplies(literalt a, literalt b) { return lor(!a, b); }
...
literalt smt2_propt::lselect(literalt a, literalt b, literalt c) {
  ...
  out << "; lselect"
      << "\n";
  out << " (if_then_else " << smt2_literal(a) << " " << smt2_literal(b) << " "
      << smt2_literal(c) << "))"
      << "\n";
...
propt::resultt smt2_propt::prop_solve() { return P_ERROR; }
