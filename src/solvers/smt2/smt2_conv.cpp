...
void smt2_convt::define_object_size(const irep_idt &id, const exprt &expr) {
  ...
  for (const auto &o : pointer_logic.objects) {
    ...
    if (o.id() != ID_symbol || size_expr.is_nil() ||
...
constant_exprt smt2_convt::parse_literal(const irept &src, const typet &type) {
  ...
  if (src.id() != irep_idt()) {
  ...
  } else if (src.get_sub().size() == 3 && src.get_sub()[0].id() == "_" &&
    ...
    value = string2integer(src.get_sub()[1].id_string().substr(2));
             ...
             src.get_sub()[0].id() ==
                 "fp") // (fp signbv exponentbv significandbv)
  {
    if (type.id() == ID_floatbv) {
      ...
      std::string bits = id2string(s1.get_value()) + id2string(s2.get_value()) +
                         id2string(s3.get_value());
      value = binary2integer(bits, false);
    } else
      value = 0;
  } else if (src.get_sub().size() == 4 && src.get_sub()[0].id() == "_" &&
  ...
  if (type.id() == ID_signedbv || type.id() == ID_unsignedbv ||
      type.id() == ID_bv || type.id() == ID_c_enum || type.id() == ID_c_bool) {
    return from_integer(value, type);
  } else if (type.id() == ID_c_enum_tag) {
    return from_integer(value, ns.follow_tag(to_c_enum_tag_type(type)));
    ...
    UNEXPECTEDCASE("smt2_convt::parse_literal can't do type " +
                   type.id_string());
}
...
exprt smt2_convt::parse_struct(const irept &src, const struct_typet &type) {
...
void smt2_convt::convert_byte_update(const byte_update_exprt &expr) {
  ...
  exprt distance =
      mult_exprt(expr.offset(), from_integer(8, expr.offset().type()));
...
void smt2_convt::convert_expr(const exprt &expr) {
  ...
  if (expr.id() == ID_symbol) {
  ...
  } else if (expr.id() == ID_concatenation || expr.id() == ID_bitand ||
    ...
    if (expr.type().id() == ID_vector) {
      if (use_datatypes) {
        ...
        const std::string smt_typename = datatype_map.find(expr.type())->second;
    ...
    if (type.id() == ID_unsignedbv || type.id() == ID_signedbv ||
      ...
      if (expr.op1().type().id() == ID_integer) {
        ...
        UNEXPECTEDCASE("unsupported op1 type for " + expr.id_string() + ": " +
                       type.id_string());

      out << ")"; // bv*sh
    } else
      UNEXPECTEDCASE("unsupported type for " + expr.id_string() + ": " +
                     type.id_string());
    ...
    if (op_type.id() == ID_signedbv) {
      ...
      UNEXPECTEDCASE("overflow-* check on unknown type: " +
                     op_type.id_string());
    ...
    UNEXPECTEDCASE("smt2_convt::convert_expr: `" + expr.id_string() +
...
void smt2_convt::convert_typecast(const typecast_exprt &expr) {
  ...
  if (dest_type.id() == ID_bool) {
    ...
    } else if (src_type.id() == ID_floatbv) // from floatbv to int
    {
      if (dest_type.id() == ID_bv) {
        ...
        UNEXPECTEDCASE("typecast unexpected " + src_type.id_string() + " -> " +
                       dest_type.id_string());
  ...
  } else if (dest_type.id() == ID_fixedbv) // to fixedbv
  {
    ...
    UNEXPECTEDCASE("TODO typecast8 " + src_type.id_string() + " -> " +
                   dest_type.id_string());
}
...
void smt2_convt::convert_floatbv_typecast(const floatbv_typecast_exprt &expr) {
  ...
  if (dest_type.id() == ID_floatbv) {
    if (src_type.id() == ID_floatbv) {
      ...
      UNEXPECTEDCASE("TODO typecast11 " + src_type.id_string() + " -> " +
                     dest_type.id_string());
  } else if (dest_type.id() == ID_signedbv) {
    if (use_FPA_theory) {
    ...
    UNEXPECTEDCASE("TODO typecast12 " + src_type.id_string() + " -> " +
                   dest_type.id_string());
  }
}
...
void smt2_convt::convert_constant(const constant_exprt &expr) {
  ...
  if (expr_type.id() == ID_unsignedbv || expr_type.id() == ID_signedbv ||
...
/* Should be sufficient, but not currently supported by mathsat */
      ...
      mp_integer v = binary2integer(id2string(expr.get_value()), false);
...
void smt2_convt::convert_is_dynamic_object(const exprt &expr) {
  ...
  if (dynamic_objects.empty())
    ...
    if (dynamic_objects.size() == 1) {
      out << "(= (_ bv" << dynamic_objects.front() << " " << BV_ADDR_BITS
          << ") ?obj)";
    } else {
      out << "(or";

      for (const auto &object : dynamic_objects)
        out << " (= (_ bv" << object << " " << BV_ADDR_BITS << ") ?obj)";
    ...
    if (expr.type().id() == ID_unsignedbv || expr.type().id() == ID_signedbv ||
      ...
      for (mp_integer i = 0; i != size; ++i) {
        ...
        forall_operands(it, expr) tmp.copy_to_operands(
            index_exprt(*it, from_integer(size - i - 1, index_type),
                        vector_type.subtype()));
...
void smt2_convt::convert_rounding_mode_FPA(const exprt &expr) {
  ...
  if (expr.id() == ID_constant) {
    ...
    if (value == 0)
      ...
      INVALIDEXPR("Unknown constant rounding mode with value " +
...
void smt2_convt::convert_minus(const minus_exprt &expr) {
  ...
  if (expr.type().id() == ID_unsignedbv || expr.type().id() == ID_signedbv ||
    ...
    for (mp_integer i = 0; i != size; ++i) {
      ...
      forall_operands(it, expr) tmp.copy_to_operands(index_exprt(
...
void smt2_convt::convert_with(const with_exprt &expr) {
  ...
  if (expr_type.id() == ID_array) {
    ...
    UNEXPECTEDCASE("with expects struct, union, or array type, but got " +
                   expr.type().id_string());
}
...
void smt2_convt::convert_index(const index_exprt &expr) {
  ...
  if (array_op_type.id() == ID_array) {
    ...
    UNEXPECTEDCASE("index with unsupported array type: " +
...
void smt2_convt::convert_member(const member_exprt &expr) {
  ...
  if (struct_op_type.id() == ID_struct) {
    ...
    UNEXPECTEDCASE("convert_member on an unexpected type " +
...
void smt2_convt::convert_overflow(const exprt &expr) { UNREACHABLE; }
...
// use a quantifier instead of the lambda
    ...
    if (op.type().id() == ID_pointer || op.type().id() == ID_reference) {
        ...
        object_sizes[expr] = id;
  ...
  } else if (!use_FPA_theory && expr.operands().size() >= 1 &&
             (expr.id() == ID_floatbv_plus || expr.id() == ID_floatbv_minus ||
              ...
              ((expr.id() == ID_lt || expr.id() == ID_gt ||
                ...
                expr.id() == ID_isnan || expr.id() == ID_isnormal ||
                expr.id() == ID_isfinite || expr.id() == ID_isinf ||
                expr.id() == ID_sign || expr.id() == ID_unary_minus ||
                expr.id() == ID_typecast || expr.id() == ID_abs) &&
...
void smt2_convt::convert_type(const typet &type) {
  ...
  } else if (type.id() == ID_bv || type.id() == ID_fixedbv ||
...
exprt smt2_convt::letify_rec(exprt &expr, std::vector<exprt> &let_order,
...
void smt2_convt::collect_bindings(exprt &expr, seen_expressionst &map,
                                  std::vector<exprt> &let_order) {
...
exprt smt2_convt::substitute_let(exprt &expr, const seen_expressionst &map) {
  if (expr.operands().empty())
    return expr;
