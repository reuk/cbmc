...
decision_proceduret::resultt smt2_dect::dec_solve() {
  ...
  switch (solver) {
  case BOOLECTOR:
    command = "boolector --smt2 " + smt2_temp_file.temp_out_filename +
  ...
  case CVC4:
    ...
    command = "cvc4 -L smt2 " + smt2_temp_file.temp_out_filename + " > " +
              smt2_temp_file.temp_result_filename;
    break;
  ...
  case OPENSMT:
    command = "opensmt " + smt2_temp_file.temp_out_filename + " > " +
              smt2_temp_file.temp_result_filename;
    break;
  ...
  case Z3:
    command = "z3 -smt2 " + smt2_temp_file.temp_out_filename + " > " +
              smt2_temp_file.temp_result_filename;
    break;
...
decision_proceduret::resultt smt2_dect::read_result(std::istream &in) {
  ...
  while (in) {
    ...
    if (parsed.id() == "sat")
      ...
      res = D_UNSATISFIABLE;
    else if (parsed.id() == "" && parsed.get_sub().size() == 1 &&
      ...
      values[s0.id()] = s1;
    } else if (parsed.id() == "" && parsed.get_sub().size() == 2 &&
