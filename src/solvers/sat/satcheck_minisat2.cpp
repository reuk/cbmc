...
template <typename T> tvt satcheck_minisat2_baset<T>::l_get(literalt a) const {
  if (a.is_true())
    return tvt(true);
  else if (a.is_false())
    return tvt(false);
...
template <typename T> void satcheck_minisat2_baset<T>::add_variables() {
...
template <typename T> void satcheck_minisat2_baset<T>::lcnf(const bvt &bv) {
  add_variables();
...
template <typename T> propt::resultt satcheck_minisat2_baset<T>::prop_solve() {
  assert(status != ERROR);
...
void satcheck_minisat2_baset<T>::set_assumptions(const bvt &bv) {
  ...
  forall_literals(it, assumptions) if (it->is_true()) {
    assumptions.clear();
    break;
  }
}
