...
void convert(const bvt &bv, Glucose::vec<Glucose::Lit> &dest) {
  ...
  forall_literals(it, bv) if (!it->is_false())
...
template <typename T> tvt satcheck_glucose_baset<T>::l_get(literalt a) const {
  if (a.is_true())
    return tvt(true);
  else if (a.is_false())
    return tvt(false);
...
template <typename T> void satcheck_glucose_baset<T>::add_variables() {
...
template <typename T> void satcheck_glucose_baset<T>::lcnf(const bvt &bv) {
  add_variables();
...
template <typename T> propt::resultt satcheck_glucose_baset<T>::prop_solve() {
  assert(status != ERROR);
  ...
  if (!solver->okay()) {
    ...
    forall_literals(it, assumptions) if (it->is_false()) has_false = true;
...
template <> satcheck_glucose_baset<Glucose::Solver>::~satcheck_glucose_baset() {
  delete solver;
}
