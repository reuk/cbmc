...
exprt smt1_convt::ce_value(const typet &type, const std::string &index,
                           const std::string &value, bool in_struct) const {
  ...
  if (type.id() == ID_signedbv || type.id() == ID_unsignedbv ||
    ...
    p.object =
        integer2unsigned(binary2integer(value.substr(0, BV_ADDR_BITS), false));
    ...
    if (in_struct) {
      ...
      if (!to_integer(to_array_type(type).size(), size)) {
        ...
        for (std::size_t i = 0; i != size_int; i++) {
          ...
          array_list.operands()[i] = ce_value(subtype, "", sub_value, true);
        }
...
void smt1_convt::convert_address_of_rec(const exprt &expr,
  ...
  if (expr.id() == ID_symbol || expr.id() == ID_constant ||
...
void smt1_convt::convert_byte_update(const exprt &expr, bool bool_as_bv) {
  assert(expr.operands().size() == 3);
...
void smt1_convt::convert_expr(const exprt &expr, bool bool_as_bv) {
  if (expr.id() == ID_symbol) {
  ...
  } else if (expr.id() == ID_ashr || expr.id() == ID_lshr ||
             expr.id() == ID_shl) {
    const typet &type = expr.type();
    ...
    if (type.id() == ID_unsignedbv || type.id() == ID_signedbv ||
        type.id() == ID_bv || type.id() == ID_struct || type.id() == ID_union) {
      if (expr.id() == ID_ashr)
        out << "(bvashr ";
    ...
    out << "(extract[" << (op_width - 1) << ":" << op_width - 1 - BV_ADDR_BITS
        << "] ";
    convert_expr(expr.op0(), true);
    out << ")";
    ...
    out << "(= (extract[" << (op_width - 1) << ":" << op_width - BV_ADDR_BITS
        << "] ";
    convert_expr(expr.op0(), true);
    out << ") bv" << pointer_logic.get_invalid_object() << "[" << BV_ADDR_BITS
        << "])";
    ...
    if (type.id() == ID_signedbv || type.id() == ID_fixedbv) {
      ...
      out << "(bvand ";
      convert_expr(op0, true);
      out << " bv" << (power(2, result_width - 1) - 1) << "[" << result_width
          << "])";
    } else
    ...
    if (op_type.id() == ID_unsignedbv || op_type.id() == ID_signedbv ||
        op_type.id() == ID_bv || op_type.id() == ID_fixedbv ||
        op_type.id() == ID_struct || op_type.id() == ID_union ||
    ...
    forall_expr(it, operands) out << "(store ";
    ...
    throw "smt1_convt::convert_expr: `" + expr.id_string() + "' is unsupported";
}
...
void smt1_convt::convert_typecast(const typecast_exprt &expr, bool bool_as_bv) {
  ...
  if (dest_type.id() == ID_bool) {
    ...
    if (src_type.id() == ID_signedbv || src_type.id() == ID_unsignedbv ||
        src_type.id() == ID_fixedbv || src_type.id() == ID_c_bit_field ||
        src_type.id() == ID_pointer) {
      out << "(not (= ";
      convert_expr(src, true);
      out << " ";
      convert_expr(from_integer(0, src_type), true);
      out << "))";
    } else {
  ...
  } else if (dest_type.id() == ID_signedbv || dest_type.id() == ID_unsignedbv ||
    ...
    } else if (src_type.id() == ID_fixedbv) // from fixedbv to integer
    {
      ...
      if (to_width > from_integer_bits) {
        ...
        out << "(extract[" << (from_width - 1) << ":" << from_fraction_bits
            << "] ";
        convert_expr(src, true);
        out << "))";
      } else {
  ...
  } else if (dest_type.id() == ID_fixedbv) // to fixedbv
  {
    ...
    if (src_type.id() == ID_unsignedbv || src_type.id() == ID_signedbv ||
      ...
      if (from_width == to_integer_bits)
        ...
        if (dest_type.id() == ID_unsignedbv) {
          out << " (zero_extend[" << (to_integer_bits - from_width) << "] ";
          convert_expr(src, true);
          out << ")";
        } else {
          out << " (sign_extend[" << (to_integer_bits - from_width) << "] ";
          convert_expr(src, true);
          out << ")";
        }
      }
      ...
      if (to_integer_bits <= from_integer_bits) {
        out << "(extract[" << (from_fraction_bits + to_integer_bits - 1) << ":"
            << from_fraction_bits << "] ";
        convert_expr(src, true);
        out << ")";
      } else {
        assert(to_integer_bits > from_integer_bits);
        out << "(sign_extend[" << (to_integer_bits - from_integer_bits)
            << "] (extract[" << (from_width - 1) << ":" << from_fraction_bits
            << "] ";
        convert_expr(src, true);
        out << "))";
      }
    ...
    if (src_type.id() == ID_pointer) {
      ...
      convert_expr(src, true);
    } else if (src_type.id() == ID_unsignedbv || src_type.id() == ID_signedbv) {
      std::size_t from_width = boolbv_width(src_type);

      if (from_width == to_width)
        convert_expr(src, true); // pass through
      else if (from_width < to_width) {
        out << "(zero_extend[" << (to_width - from_width) << "] ";
        convert_expr(src, true);
        out << ")"; // zero_extend
      } else        // from_width>to_width
      {
        out << "(extract[" << to_width << ":0] ";
        convert_expr(src, true);
        out << ")"; // extract
      }
    } else
...
void smt1_convt::convert_constant(const constant_exprt &expr, bool bool_as_bv) {
  if (expr.type().id() == ID_unsignedbv || expr.type().id() == ID_signedbv ||
      expr.type().id() == ID_bv || expr.type().id() == ID_c_enum ||
      expr.type().id() == ID_c_enum_tag || expr.type().id() == ID_c_bool ||
    ...
    if (value == ID_NULL) {
      ...
      out << "(concat"
          << " bv" << pointer_logic.get_null_object() << "[" << BV_ADDR_BITS
          << "]"
  ...
  } else if (expr.type().id() == ID_integer || expr.type().id() == ID_natural) {
    std::string value = expr.get_string(ID_value);
...
void smt1_convt::convert_is_dynamic_object(const exprt &expr, bool bool_as_bv) {
  ...
  if (dynamic_objects.empty())
    ...
    out << "(let (?obj (extract[" << (op_width - 1) << ":"
        << op_width - BV_ADDR_BITS << "] ";
    convert_expr(expr.op0(), true);
    out << ")) ";

    if (dynamic_objects.size() == 1) {
      out << "(= bv" << dynamic_objects.front() << "[" << BV_ADDR_BITS
          << "] ?obj)";
    } else {
      out << "(or";

      for (const auto &obj : dynamic_objects)
        out << " (= bv" << obj << "[" << BV_ADDR_BITS << "] ?obj)";
...
void smt1_convt::convert_relation(const exprt &expr, bool bool_as_bv) {
  ...
  if (op_type.id() == ID_unsignedbv || op_type.id() == ID_pointer) {
    if (expr.id() == ID_le)
      out << "bvule";
  ...
  if (expr.type().id() == ID_unsignedbv || expr.type().id() == ID_signedbv ||
    ...
    if (expr.operands().size() == 2) {
      ...
      if (element_size >= 2) {
        out << "(bvmul ";
        convert_expr(i, true);
        out << " bv" << element_size << "[" << boolbv_width(expr.type())
            << "])";
      } else
        convert_expr(i, true);
      ...
      Forall_operands(it, integer_sum) if (it->type() != integer_type)
...
void smt1_convt::convert_minus(const minus_exprt &expr) {
  ...
  if (expr.type().id() == ID_unsignedbv || expr.type().id() == ID_signedbv ||
    ...
    convert_expr(binary_exprt(expr.op0(), "+",
...
void smt1_convt::convert_mult(const mult_exprt &expr) {
  ...
  if (expr.type().id() == ID_unsignedbv || expr.type().id() == ID_signedbv) {
    ...
    out << "(extract[" << spec.width + fraction_bits - 1 << ":" << fraction_bits
        << "] ";
...
void smt1_convt::convert_with(const exprt &expr) {
  ...
  if (expr_type.id() == ID_array) {
    ...
    if (array.id() == ID_member) {
      ...
      if (struct_op_type.id() == ID_struct)
        offset = boolbv_width.get_member(to_struct_type(struct_op_type), name)
                     .offset;
      else if (struct_op_type.id() == ID_union)
        offset = 0;
      else
    ...
    std::size_t offset =
        boolbv_width.get_member(struct_type, index.get(ID_component_name))
            .offset;
    ...
    if (total_width == member_width)
      ...
      assert(total_width > member_width);
      out << "(concat ";
      out << "(extract[" << (total_width - 1) << ":" << member_width << "] ";
      convert_expr(expr.op0(), true);
      out << ") "; // extract
      convert_expr(value, true);
      out << ")"; // concat
    }
  } else if (expr_type.id() == ID_bv || expr_type.id() == ID_unsignedbv ||
...
void smt1_convt::convert_index(const index_exprt &expr, bool bool_as_bv) {
  ...
  if (expr.array().id() == ID_member || expr.array().id() == ID_index) {
...
void smt1_convt::convert_member(const member_exprt &expr, bool bool_as_bv) {
  ...
  if (struct_op_type.id() == ID_struct) {
    std::size_t offset =
        boolbv_width.get_member(to_struct_type(struct_op_type), name).offset;
  ...
  if (expr.id() == ID_symbol || expr.id() == ID_nondet_symbol) {
    ...
    if (id.type.is_nil()) {
      ...
      if (id.type.id() == ID_bool) {
        out << ":extrapreds((" << convert_identifier(identifier) << "))"
            << "\n";
      } else {
        out << ":extrafuns((" << convert_identifier(identifier) << " ";
        convert_type(type);
        out << "))"
            << "\n";
      }
...
void smt1_convt::convert_type(const typet &type) {
  if (type.id() == ID_array) {
    ...
    out << "BitVec[" << width << "]";
  } else if (type.id() == ID_pointer || type.id() == ID_reference) {
    std::size_t width = boolbv_width(type);
    ...
    out << "BitVec[" << width << "]";
  } else if (type.id() == ID_bv || type.id() == ID_floatbv ||
             type.id() == ID_fixedbv || type.id() == ID_unsignedbv ||
             type.id() == ID_signedbv || type.id() == ID_c_enum ||
             type.id() == ID_c_bool || type.id() == ID_vector) {
...
void smt1_convt::find_symbols_rec(const typet &type,
...
void smt1_convt::convert_nary(const exprt &expr, const irep_idt op_string,
                              bool bool_as_bv) {
