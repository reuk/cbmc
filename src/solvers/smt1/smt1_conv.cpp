...
void smt1_convt::convert_expr(const exprt &expr, bool bool_as_bv)
{
  ...
  else if(expr.id() == "implicit_address_of" || expr.id() == "reference_to")
  {
    // old stuff
    assert(false);
  }
...
void smt1_convt::convert_plus(const plus_exprt &expr)
{
  ...
  else if(expr.type().id() == ID_rational || expr.type().id() == ID_integer)
  {
...
void smt1_convt::convert_with(const exprt &expr)
{
  ...
  else if(expr_type.id() == ID_struct)
  {
    ...
    std::size_t offset=
      boolbv_width.get_member(struct_type, index.get(ID_component_name)).offset;
...
void smt1_convt::find_symbols(const exprt &expr)
{
  ...
  else if(expr.id() == ID_array_of)
  {
    if(array_of_map.find(expr) == array_of_map.end())
    {
      ...
      if(type.find(ID_size) != get_nil_irep() && expr.op0().id() == ID_constant)
      {
...
void smt1_convt::convert_type(const typet &type)
{
  ...
  else if(type.id() == ID_pointer || type.id() == ID_reference)
  {
    ...
    type.id() == ID_bv || type.id() == ID_floatbv || type.id() == ID_fixedbv ||
    type.id() == ID_unsignedbv || type.id() == ID_signedbv ||
    type.id() == ID_c_enum || type.id() == ID_c_bool || type.id() == ID_vector)
  {
