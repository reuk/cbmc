...
decision_proceduret::resultt smt1_dect::dec_solve()
{
  ...
  case CVC4:
    command= "cvc4 -L smt1 " + temp_out_filename + " > " + temp_result_filename;
    break;

  case MATHSAT:
    command=
      "mathsat -model -input=smt"
      " < " +
      temp_out_filename + " > " + temp_result_filename;
    break;
...
std::string smt1_dect::mathsat_value(const std::string &src)
{
  ...
  if(
    std::string(src, 0, 2) == "bv" && pos != std::string::npos &&
    src[src.size() - 1] == ']')
  {
    unsigned width=
...
decision_proceduret::resultt smt1_dect::read_result_cvc3(std::istream &in)
{
  ...
  while(std::getline(in, line))
  {
    if(line == "sat")
      ...
      line.find("Current scope level") != std::string::npos ||
    ...
    else
    {
      ...
      if(pos != std::string::npos)
      {
        ...
        if(line[pos + 1] == '=')
        {
          ...
          if(
            (var.length() >= 4 && var.substr(0, 4) == "cvc3") ||
            (val.length() >= 4 && val.substr(0, 4) == "cvc3") || var == val)
            continue;
          else if(
            (var.substr(0, 9) == "array_of'") ||
