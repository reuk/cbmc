...
decision_proceduret::resultt smt1_dect::dec_solve() {
  ...
  temp_result_filename = get_temporary_file("smt1_dec_result_", "");
  ...
  case BOOLECTOR:
    ...
    command = "boolector --smt " + temp_out_filename + " --model --output " +
              temp_result_filename;
    break;

  case CVC3:
    command = "cvc3 +model -lang smtlib -output-lang smtlib " +
              temp_out_filename + " > " + temp_result_filename;
    break;

  case CVC4:
    command =
        "cvc4 -L smt1 " + temp_out_filename + " > " + temp_result_filename;
    break;
  ...
  case OPENSMT:
    command = "opensmt " + temp_out_filename + " > " + temp_result_filename;
    break;
  ...
  case Z3:
    command = "z3 -smt " + temp_out_filename + " > " + temp_result_filename;
    break;
...
smt1_dect::read_result_boolector(std::istream &in) {
  std::string line;
  ...
  if (line == "sat") {
    ...
    for (identifier_mapt::iterator it = identifier_map.begin();
         it != identifier_map.end(); it++) {
      it->second.value.make_nil();
      std::string conv_id = convert_identifier(it->first);
      const valuet &v = values[conv_id];

      for (valuet::index_value_mapt::const_iterator i_it =
               v.index_value_map.begin();
           i_it != v.index_value_map.end(); i_it++)
...
std::string smt1_dect::mathsat_value(const std::string &src) {
  ...
  if (std::string(src, 0, 2) == "bv" && pos != std::string::npos &&
      src[src.size() - 1] == ']') {
    unsigned width =
...
bool smt1_dect::string_to_expr_z3(const typet &type, const std::string &value,
                                  exprt &e) const {
  if (value.substr(0, 2) == "bv") {
    ...
    std::string binary = integer2binary(
        string2integer(v, 10), integer2unsigned(string2integer(w, 10)));
  ...
  } else if (value.substr(0, 6) == "(const") // const arrays
  {
    ...
    while (fit != array_of_map.end() && fit->second != id)
      fit++;
      ...
      if (pos != std::string::npos) {
        ...
        if (line[pos + 1] == '=') {
          ...
          if ((var.length() >= 4 && var.substr(0, 4) == "cvc3") ||
              (val.length() >= 4 && val.substr(0, 4) == "cvc3") || var == val)
            continue;
