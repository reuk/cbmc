...
void dplib_convt::convert_address_of_rec(const exprt &expr) {
  if (expr.id() == ID_symbol || expr.id() == ID_constant ||
    ...
    dplib_prop.out << "(# object:=" << pointer_logic.add_object(expr)
    ...
    if (index.is_zero()) {
      ...
      dplib_prop.out << " IN P WITH .offset:=BVPLUS("
                     << config.ansi_c.pointer_width << ", P.offset, ";
    ...
    mp_integer offset =
        member_offset(ns, to_struct_type(struct_op.type()), component_name);
...
void dplib_convt::convert_dplib_expr(const exprt &expr) {
  if (expr.id() == ID_symbol) {
    ...
    if (expr.type().id() == ID_unsignedbv || expr.type().id() == ID_signedbv ||
      ...
      if (value == "NULL") {
        dplib_prop.out << "(# object:=" << pointer_logic.get_null_object()
                       << ", offset:=" << bin_zero(config.ansi_c.pointer_width)
                       << " #)";
      } else
      ...
      dplib_prop.out << "\n  ENDIF";
               ...
               expr.type().id() == ID_natural || expr.type().id() == ID_range) {
...
void dplib_convt::convert_dplib_type(const typet &type) {
  if (type.id() == ID_array) {
    ...
    dplib_prop.out << "ARRAY " << array_index_type() << " OF ";
