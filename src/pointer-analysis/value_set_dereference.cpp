...
exprt value_set_dereferencet::dereference(const exprt &pointer,
  ...
  if (values.empty()) {
    ...
    may_fail = false;
    for (std::list<valuet>::const_iterator it = values.begin();
         it != values.end(); it++)
      if (it->value.is_nil())
        may_fail = true;
  }
  ...
  if (may_fail) {
    ...
    if (dereference_callback.has_failed_symbol(pointer, failed_symbol)) {
      // yes!
...
bool value_set_dereferencet::dereference_type_compare(
    const typet &object_type, const typet &dereference_type) const {
  ...
  if (ot_base.id() == ID_struct && dt_base.id() == ID_struct) {
    if (to_struct_type(dt_base).is_prefix_of(to_struct_type(ot_base)))
      return true; // ok, dt is a prefix of ot
  }

  // we are generous about code pointers
  if (dereference_type.id() == ID_code && object_type.id() == ID_code)
    return true;
...
void value_set_dereferencet::invalid_pointer(const exprt &pointer,
                                             const guardt &guard) {
  ...
  dereference_callback.dereference_failure("pointer dereference",
                                           "invalid pointer", tmp_guard);
}
...
value_set_dereferencet::build_reference_to(const exprt &what, const modet mode,
                                           const exprt &pointer_expr,
                                           const guardt &guard) {
  const typet &dereference_type = ns.follow(pointer_expr.type()).subtype();
  ...
  if (root_object.id() == "NULL-object") {
    if (options.get_bool_option("pointer-check")) {
      ...
      if (o.offset().is_zero()) {
        ...
        dereference_callback.dereference_failure(
            "pointer dereference", "NULL plus offset pointer", tmp_guard);
      }
    }
    ...
    if (options.get_bool_option("pointer-check")) {
      // if(!dynamic_object.valid().is_true())
      {
        ...
        dereference_callback.dereference_failure(
            "pointer dereference", "dynamic object deallocated", tmp_guard);
      }
      ...
      if (options.get_bool_option("bounds-check")) {
        if (!o.offset().is_zero()) {
          ...
          tmp_guard.add(
              dynamic_object_lower_bound(pointer_expr, ns, nil_exprt()));
          dereference_callback.dereference_failure(
              "pointer dereference", "dynamic object lower bound", tmp_guard);
        }

        {
          ...
          tmp_guard.add(is_malloc_object);
          tmp_guard.add(
              dynamic_object_upper_bound(pointer_expr, dereference_type, ns,
    ...
    if (base_type_eq(ns.follow(memory_symbol.type).subtype(), dereference_type,
                     ns)) {
    ...
    if (dereference_type_compare(object_type, dereference_type) &&
      ...
      if (object_type != ns.follow(dereference_type))
        result.value.make_typecast(dereference_type);
               ...
               dereference_type_compare(root_object_type.subtype(),
                                        dereference_type)) {
      ...
      // are we doing a byte?
      mp_integer element_size = pointer_offset_size(dereference_type, ns);
      ...
      if (element_size == 1) {
        ...
        adjusted_offset =
            binary_exprt(offset, ID_div, element_size_expr, offset.type());
      ...
      if (ns.follow(result.value.type()) != ns.follow(dereference_type))
        result.value.make_typecast(dereference_type);
    } else if (get_subexpression_at_offset(root_object_subexpression,
                                           o.offset(), dereference_type, ns)) {
      ...
      if (memory_model(result.value, dereference_type, tmp_guard, offset)) {
        ...
        if (options.get_bool_option("pointer-check")) {
          ...
          dereference_callback.dereference_failure("pointer dereference", msg,
                                                   tmp_guard);
        }
...
void value_set_dereferencet::valid_check(const exprt &object,
                                         const guardt &guard,
                                         const modet mode) {
  if (!options.get_bool_option("pointer-check"))
    return;
  ...
  if (symbol_expr.id() == ID_string_constant) {
    ...
    const irep_idt identifier =
        is_ssa_expr(symbol_expr) ? to_ssa_expr(symbol_expr).get_object_name()
...
void value_set_dereferencet::bounds_check(const index_exprt &expr,
                                          const guardt &guard) {
  if (!options.get_bool_option("pointer-check"))
    return;
  ...
  {
    mp_integer i;
    if (!to_integer(expr.index(), i) && i >= 0) {
    } else {
      exprt zero = from_integer(0, expr.index().type());

      if (zero.is_nil())
        throw "no zero constant of index type " + expr.index().type().pretty();

      binary_relation_exprt inequality(expr.index(), ID_lt, zero);

      guardt tmp_guard(guard);
      tmp_guard.add(inequality);
      dereference_callback.dereference_failure(
          "array bounds", name + " lower bound", tmp_guard);
    }
  }
  ...
  if (size_expr.id() == ID_infinity) {
    ...
    if (c_implicit_typecast(inequality.op0(), inequality.op1().type(), ns))
      throw "index address of wrong type";

    guardt tmp_guard(guard);
    tmp_guard.add(inequality);
    dereference_callback.dereference_failure("array bounds",
...
inline static unsigned bv_width(const typet &type, const namespacet &ns) {
...
bool value_set_dereferencet::memory_model(exprt &value, const typet &to_type,
                                          const guardt &guard,
                                          const exprt &offset) {
  ...
  if (is_a_bv_type(from_type) && is_a_bv_type(to_type)) {
  ...
  if (from_type.id() == ID_pointer && to_type.id() == ID_pointer) {
    if (bv_width(from_type, ns) == bv_width(to_type, ns))
      return memory_model_conversion(value, to_type, guard, offset);
  }
...
bool value_set_dereferencet::memory_model_conversion(exprt &value,
  ...
  if (options.get_bool_option("pointer-check")) {
    ...
    dereference_callback.dereference_failure("word bounds", "offset not zero",
                                             tmp_guard);
  }
...
bool value_set_dereferencet::memory_model_bytes(exprt &value,
  ...
  if (options.get_bool_option("pointer-check")) {
    // upper bound
    {
      ...
      binary_relation_exprt offset_upper_bound(offset, ID_gt, bound);
    ...
    // lower bound is easy
    if (!offset.is_zero()) {
      binary_relation_exprt offset_lower_bound(offset, ID_lt,
