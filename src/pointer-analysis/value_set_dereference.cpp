  ...
  const modet mode)
{
  ...
  for(std::list<valuet>::const_iterator it= values.begin(); it != values.end();
      it++)
  {
  ...
  const guardt &guard)
{
  ...
  dereference_callback.dereference_failure(
    "pointer dereference", "invalid pointer", tmp_guard);
}
  ...
  const guardt &guard)
{
  ...
  if(root_object.id() == "NULL-object")
  {
    if(options.get_bool_option("pointer-check"))
    {
      ...
      if(o.offset().is_zero())
      {
        ...
        dereference_callback.dereference_failure(
          "pointer dereference", "NULL pointer", tmp_guard);
      }
      else
      {
  ...
  else
  {
    ...
    if(
      dereference_type_compare(object_type, dereference_type) &&
      o.offset().is_zero())
    {
    ...
    else
    {
      ...
      else
      {
        if(options.get_bool_option("pointer-check"))
        {
          ...
          dereference_callback.dereference_failure(
            "pointer dereference", msg, tmp_guard);
        }
  ...
  const modet mode)
{
  ...
  else if(symbol_expr.is_nil() || symbol_expr.get_bool(ID_C_invalid_object))
  {
  ...
  {
    const irep_idt identifier= is_ssa_expr(symbol_expr)
    ...
    {
      dereference_callback.dereference_failure(
        "pointer dereference", "invalid pointer", guard);
    }
  ...
  const guardt &guard)
{
  ...
  else
  {
    ...
    tmp_guard.add(inequality);
    dereference_callback.dereference_failure(
      "array bounds", name + " upper bound", tmp_guard);
  }
}
  ...
  const exprt &offset)
{
  ...
  if(options.get_bool_option("pointer-check"))
  {
    ...
    dereference_callback.dereference_failure(
      "word bounds", "offset not zero", tmp_guard);
  }
  ...
  const exprt &offset)
{
  ...
  if(options.get_bool_option("pointer-check"))
  {
    // upper bound
    {
      ...
      dereference_callback.dereference_failure(
        "pointer dereference", "object upper bound", tmp_guard);
    }

    // lower bound is easy
    if(!offset.is_zero())
    {
      binary_relation_exprt offset_lower_bound(
        offset, ID_lt, from_integer(0, offset.type()));

      guardt tmp_guard(guard);
      tmp_guard.add(offset_lower_bound);
      dereference_callback.dereference_failure(
        "pointer dereference", "object lower bound", tmp_guard);
    }
  }
