...
#define forall_objects(it, map)                                                \
  for (object_map_dt::const_iterator(it) = (map).begin(); (it) != (map).end(); \
       (it)++)
...
#define Forall_valid_objects(it, map)                                          \
  for (object_map_dt::iterator(it) = (map).begin(); (it) != (map).end();       \
       (it)++)                                                                 \
    if ((map).is_valid_at((it)->first, from_function,                          \
                          from_target_index)) /* NOLINT(*) */
...
void value_set_fivrt::output(const namespacet &ns, std::ostream &out) const {
  for (valuest::const_iterator v_it = values.begin(); v_it != values.end();
       v_it++) {
...
void value_set_fivrt::flatten(const entryt &e, object_mapt &dest) const {
#if 0
...
void value_set_fivrt::flatten_rec(const entryt &e, object_mapt &dest,
                                  flatten_seent &seen, unsigned at_function,
      ...
      if (fi == values.end()) {
        ...
        if (ranges_it != e.object_map.read().validity_ranges.end()) {
          for (object_map_dt::vrange_listt::const_iterator r_it =
               ...
               r_it != ranges_it->second.end(); r_it++) {
            ...
            if (r_it->function == at_function) {
              ...
              for (object_map_dt::iterator t_it = temp.write().begin();
                   t_it != temp.write().end(); t_it++) {
                if (t_it->second.offset_is_set && it->second.offset_is_set) {
                  t_it->second.offset += it->second.offset;
                } else
                  t_it->second.offset_is_set = false;
              }

              forall_objects(oit, temp.read()) insert_from(dest, oit);
            }
          }
        }
      }
  ...
  if (generalize_index) // this means we had recursive symbols in there
  {
    Forall_objects(it, dest.write()) {
      ...
      for (std::list<const object_map_dt::vrange_listt *>::const_iterator vit =
               add_ranges.begin();
           vit != add_ranges.end(); vit++) {
        for (object_map_dt::vrange_listt::const_iterator lit = (*vit)->begin();
             lit != (*vit)->end(); lit++)
...
exprt value_set_fivrt::to_expr(object_map_dt::const_iterator it) const {
  ...
  if (object.id() == ID_invalid || object.id() == ID_unknown)
    return object;
...
void value_set_fivrt::copy_objects(object_mapt &dest,
  ...
  forall_valid_objects(it, src.read()) {
    ...
    dest.write().validity_ranges[it->first].push_back(
        object_map_dt::validity_ranget(from_function, from_target_index,
                                       from_target_index));
  }
}
...
void value_set_fivrt::get_value_set(const exprt &expr,
  ...
  forall_objects(fit, flat_map.read()) value_set.push_back(to_expr(fit));
...
void value_set_fivrt::get_value_set(const exprt &expr, object_mapt &dest,
                                    const namespacet &ns) const {
  exprt tmp(expr);
  simplify(tmp, ns);
...
void value_set_fivrt::get_value_set_rec(
    const exprt &expr, object_mapt &dest, const std::string &suffix,
    const typet &original_type, const namespacet &ns,
  ...
  if (expr.type().id() == "#REF#") {
    ...
    // check if NULL
    if (expr.get(ID_value) == ID_NULL && expr.type().id() == ID_pointer) {
    ...
    if (expr.type().id() == ID_pointer) {
      ...
      forall_operands(it, expr) if (it->type().id() == ID_pointer) {
        if (ptr_operand == NULL)
          ptr_operand = &(*it);
        else
      ...
      get_value_set_rec(*ptr_operand, pointer_expr_set, "", ptr_operand->type(),
                        ns, recursion_set);

      forall_objects(it, pointer_expr_set.read()) {
        objectt object = it->second;

        if (object.offset_is_zero() && expr.operands().size() == 2) {
    ...
    if (statement == ID_function_call) {
      ...
      dynamic_object.set_instance((from_function << 16) | from_target_index);
    ...
    return;
  } else if (expr.id() == ID_with || expr.id() == ID_array_of ||
             expr.id() == ID_array) {
    // these are supposed to be done by assign()
    throw "unexpected value in get_value_set: " + expr.id_string();
  } else if (expr.id() == ID_dynamic_object) {
    const dynamic_object_exprt &dynamic_object = to_dynamic_object_expr(expr);

    const std::string name = "value_set::dynamic_object" +
...
void value_set_fivrt::dereference_rec(const exprt &src, exprt &dest) const {
...
void value_set_fivrt::get_reference_set(const exprt &expr, expr_sett &dest,
                                        const namespacet &ns) const {
  object_mapt object_map;
  ...
  forall_objects(it, object_map.read()) {
    ...
    if (expr.type().id() == "#REF#") {
      ...
      if (vit == values.end()) {
        ...
        forall_objects(it, omt.read()) dest.insert(to_expr(it));
      }
    } else
...
void value_set_fivrt::get_reference_set_sharing(const exprt &expr,
  ...
  forall_objects(it, object_map.read()) dest.insert(to_expr(it));
}
...
void value_set_fivrt::get_reference_set_sharing_rec(
    const exprt &expr, object_mapt &dest, const namespacet &ns) const {
#if 0
  ...
  if (expr.type().id() == "#REF#") {
    ...
    if (expr.type().id() == ID_array && expr.type().subtype().id() == ID_array)
    ...
    forall_objects(it, temp.read()) {
      ...
      if (obj.type().id() == "#REF#") {
        ...
        if (v_it != values.end()) {
          ...
          forall_objects(it2, t2.read()) insert_from(dest, it2);
        } else
    ...
    forall_objects(it, object_map) {
      ...
      if (object.id() == ID_unknown)
        insert_from(dest, exprt(ID_unknown, expr.type()));
      else if (object.id() == ID_dynamic_object && obj_type.id() != ID_struct &&
...
void value_set_fivrt::assign(const exprt &lhs, const exprt &rhs,
                             const namespacet &ns, bool add_to_sets) {
#if 0
  ...
  if (type.id() == ID_struct || type.id() == ID_union) {
    ...
    for (struct_typet::componentst::const_iterator
         ...
         c_it != struct_type.components().end(); c_it++, no++) {
    ...
    if (rhs.id() == ID_unknown || rhs.id() == ID_invalid) {
      ...
      } else if (rhs.id() == ID_array || rhs.id() == ID_constant) {
...
void value_set_fivrt::do_free(const exprt &op, const namespacet &ns) {
  ...
  for (valuest::iterator v_it = values.begin(); v_it != values.end(); v_it++) {
    object_mapt new_object_map;

    const object_map_dt &old_object_map = v_it->second.object_map.read();
    ...
    if (changed) {
      entryt &temp_entry =
          get_temporary_entry(v_it->second.identifier, v_it->second.suffix);
...
void value_set_fivrt::assign_rec(const exprt &lhs,
  ...
  if (lhs.type().id() == "#REF#") {
    ...
    if (recursion_set.find(ident) != recursion_set.end()) {
      ...
      forall_objects(it, temp.read())
          assign_rec(object_numbering[it->first], values_rhs, suffix, ns,
                     recursion_set, add_to_sets);
    ...
    if (has_prefix(id2string(identifier), "value_set::dynamic_object") ||
        has_prefix(id2string(identifier), "value_set::return_value") ||
    ...
    // otherwise we don't track this value
    {
      ...
      if (add_to_sets || recursive_find(identifier, values_rhs, recset)) {
    ...
    const dynamic_object_exprt &dynamic_object = to_dynamic_object_expr(lhs);
    ...
    recfind_recursion_sett recset;
    if (add_to_sets || recursive_find(name, values_rhs, recset)) {
    ...
    assign_rec(lhs.op0(), values_rhs, "." + component_name + suffix, ns,
               recursion_set, add_to_sets);
  } else if (lhs.id() == "valid_object" || lhs.id() == "dynamic_size" ||
             lhs.id() == "dynamic_type") {
    // we ignore this here
    ...
    assign_rec(typecast_expr.op(), values_rhs, suffix, ns, recursion_set,
               add_to_sets);
  } else if (lhs.id() == "zero_string" || lhs.id() == "is_zero_string" ||
...
void value_set_fivrt::do_function_call(const irep_idt &function,
  ...
  for (code_typet::parameterst::const_iterator it = parameter_types.begin();
    ...
    const exprt v_expr =
        symbol_exprt("value_set::" + id2string(function) + "::" + "argument$" +
                         std::to_string(i),
                     it->type());
...
void value_set_fivrt::do_end_function(const exprt &lhs, const namespacet &ns) {
  if (lhs.is_nil())
    return;
...
void value_set_fivrt::apply_code(const exprt &code, const namespacet &ns) {
  const irep_idt &statement = code.get(ID_statement);

  if (statement == ID_block) {
    forall_operands(it, code) apply_code(*it, ns);
  } else if (statement == ID_function_call) {
    // shouldn't be here
    assert(false);
  } else if (statement == ID_assign || statement == ID_init) {
  ...
  } else if (statement == ID_specc_notify || statement == ID_specc_wait) {
...
bool value_set_fivrt::insert_to(object_mapt &dest, unsigned n,
...
bool value_set_fivrt::insert_from(object_mapt &dest, unsigned n,
                                  const objectt &object) const {
  object_map_dt &map = dest.write();
  if (map.find(n) == map.end()) {
    //    std::cout << "NEW(" << n << "): " << object_numbering[n] << std::endl;
...
bool value_set_fivrt::object_map_dt::set_valid_at(unsigned inx, unsigned f,
                                                  unsigned line) {
  ...
  while (it->function != f && it != ranges.end())
    it++; // ffw to function block

  for (; it != ranges.end() && it->function == f && it->from <= line; it++) {
    if (it->function == f) {
      if (line == it->to + 1) {
        it->to++;

        // by any chance: does the next one connect to this one?
        vrange_listt::iterator n_it = it;
        n_it++;
        if (n_it != ranges.end() && it->function == n_it->function &&
            it->to + 1 == n_it->from) {
  ...
  if (it != ranges.end()) {
    if (it->function == f) {
      if (line == it->from - 1) {
        ...
        if (it != ranges.begin()) {
          vrange_listt::iterator p_it = it;
          p_it--;
          if (p_it->function == it->function && p_it->to + 1 == it->from) {
...
bool value_set_fivrt::object_map_dt::is_valid_at(unsigned inx, unsigned f,
                                                 unsigned line) const {
#if 0
...
bool value_set_fivrt::recursive_find(
    const irep_idt &ident, const object_mapt &rhs,
...
bool value_set_fivrt::handover(void) {
  ...
  for (valuest::iterator it = values.begin(); it != values.end(); it++) {
    ...
    if (t_it == temporary_values.end()) {
      ...
      Forall_valid_objects(o_it, state_map.write()) {
        if (state_map.write().set_valid_at(o_it->first, to_function,
                                           to_target_index))
          changed = true;
      }
    } else {
