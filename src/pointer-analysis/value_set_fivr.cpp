...
bool value_set_fivrt::make_union(object_mapt &dest, const object_mapt &src)
  const
{
  bool result= false;
...
void value_set_fivrt::copy_objects(object_mapt &dest, const object_mapt &src)
  const
{
  forall_valid_objects(it, src.read())
  {
    ...
    dest.write().validity_ranges[it->first].push_back(
      object_map_dt::validity_ranget(
        from_function, from_target_index, from_target_index));
  }
}
  ...
  gvs_recursion_sett &recursion_set) const
{
  ...
  else if(expr.id() == ID_index)
  {
    ...
    assert(
      type.id() == ID_array || type.id() == ID_incomplete_array ||
      type.id() == "#REF#");
  ...
  else if(expr.id() == ID_member)
  {
    ...
    if(expr.op0().is_not_nil())
    {
      ...
      assert(
        ...
        type.id() == ID_incomplete_struct || type.id() == ID_incomplete_union);
  ...
  else if(expr.id() == ID_if)
  {
    ...
    get_value_set_rec(
      expr.op1(), dest, suffix, original_type, ns, recursion_set);
    get_value_set_rec(
      expr.op2(), dest, suffix, original_type, ns, recursion_set);
  ...
  else if(expr.id() == ID_dereference)
  {
    ...
    if(object_map.begin() != object_map.end())
    {
      forall_objects(it1, object_map)
      {
        ...
        get_value_set_rec(
          object, dest, suffix, original_type, ns, recursion_set);
      }
  ...
  else if(expr.id() == ID_struct)
  {
    ...
    expr.id() == ID_with || expr.id() == ID_array_of || expr.id() == ID_array)
  {
  ...
  const namespacet &ns) const
{
  ...
  if(expr.type().id() == "#REF#")
  {
    ...
    expr.id() == ID_symbol || expr.id() == ID_dynamic_object ||
  ...
  else if(expr.id() == ID_index)
  {
    ...
    assert(
      array_type.id() == ID_array || array_type.id() == ID_incomplete_array);
        ...
        c_it++, no++)
    {
      ...
      else
      {
        ...
        else if(rhs.id() == ID_with)
        {
          ...
          const irep_idt &component_name= member_operand.get(ID_component_name);
  ...
  bool add_to_sets)
{
  ...
  else if(lhs.id() == ID_symbol)
  {
    ...
    if(
      has_prefix(id2string(identifier), "value_set::dynamic_object") ||
  ...
  else if(lhs.id() == ID_member)
  {
    ...
    lhs.id() == "valid_object" || lhs.id() == "dynamic_size" ||
    lhs.id() == "dynamic_type")
  {
    // we ignore this here
  }
  ...
  else if(lhs.id() == ID_typecast)
  {
    ...
    assign_rec(
      typecast_expr.op(), values_rhs, suffix, ns, recursion_set, add_to_sets);
  }
  else if(
  ...
  const namespacet &ns)
{
  ...
  for(unsigned i= 0; i < arguments.size(); i++)
  {
    ...
    make_union(
      values[identifier].object_map, temporary_values[identifier].object_map);
  }
      ...
      it++)
  {
    ...
    const exprt v_expr= symbol_exprt(
      "value_set::" + id2string(function) + "::" + "argument$" +
        std::to_string(i),
      it->type());
  ...
  for(; it != ranges.end() && it->function == f && it->from <= line; it++)
  {
    if(it->function == f)
    {
      if(line == it->to + 1)
      {
        ...
        n_it++;
        if(
          n_it != ranges.end() && it->function == n_it->function &&
          it->to + 1 == n_it->from)
        {
