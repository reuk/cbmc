...
void value_set_analysis_fivrt::initialize(const goto_programt &goto_program) {
...
void value_set_analysis_fivrt::add_vars(const goto_programt &goto_program) {
...
void value_set_analysis_fivrt::get_entries(
    const symbolt &symbol, std::list<value_set_fivrt::entryt> &dest) {
...
void value_set_analysis_fivrt::get_entries_rec(
    const irep_idt &identifier, const std::string &suffix, const typet &type,
  ...
  if (t.id() == ID_struct || t.id() == ID_union) {
    ...
    for (struct_typet::componentst::const_iterator it = c.begin();
         it != c.end(); it++) {
      get_entries_rec(identifier, suffix + "." + it->get_string(ID_name),
                      it->type(), dest);
    }
...
void value_set_analysis_fivrt::add_vars(const goto_functionst &goto_functions) {
  // get the globals
