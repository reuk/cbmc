...
bool value_set_fit::make_union(const value_set_fit::valuest &new_values)
{
      ...
      it++)
  {
    ...
    if(it2 == values.end())
    {
      ...
      if(
        has_prefix(
          id2string(it->second.identifier), "value_set::dynamic_object") ||
        has_prefix(id2string(it->second.identifier), "value_set::return_value"))
      {
        values.insert(*it);
        result= true;
      }
  ...
  assign_recursion_sett &recursion_set)
{
    ...
    if(recursion_set.find(ident) != recursion_set.end())
    {
      ...
      forall_objects(it, temp.read())
        if(object_numbering[it->first].id() != ID_unknown)
          assign_rec(
            object_numbering[it->first], values_rhs, suffix, ns, recursion_set);
  ...
  else if(lhs.id() == ID_member)
  {
    ...
    assign_rec(
      lhs.op0(), values_rhs, "." + component_name + suffix, ns, recursion_set);
  }
  else if(
