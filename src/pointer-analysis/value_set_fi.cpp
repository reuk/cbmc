...
void value_set_fit::output(const namespacet &ns, std::ostream &out) const {
...
void value_set_fit::flatten(const entryt &e, object_mapt &dest) const {
#if 0
...
void value_set_fit::flatten_rec(const entryt &e, object_mapt &dest,
  ...
  forall_objects(it, e.object_map.read()) {
    ...
    if (o.type().id() == "#REF#") {
      ...
      if (fi == values.end()) {
        ...
        forall_objects(oit, temp.read()) insert(dest, oit);
      }
    } else
      insert(dest, it);
  }
  ...
  {
    Forall_objects(it, dest.write()) it->second.offset_is_set = false;
  }
...
bool value_set_fit::make_union(const value_set_fit::valuest &new_values) {
  ...
  for (valuest::const_iterator it = new_values.begin(); it != new_values.end();
       it++) {
...
void value_set_fit::get_value_set(const exprt &expr, object_mapt &dest,
                                  const namespacet &ns) const {
  exprt tmp(expr);
  simplify(tmp, ns);
...
void value_set_fit::get_value_set_rec(const exprt &expr, object_mapt &dest,
  ...
  } else if (expr.id() == ID_array_of || expr.id() == ID_array) {
...
void value_set_fit::dereference_rec(const exprt &src, exprt &dest) const {
...
void value_set_fit::get_reference_set(const exprt &expr, expr_sett &dest,
                                      const namespacet &ns) const {
  object_mapt object_map;
    ...
    forall_objects(it, temp.read()) {
      ...
      if (obj.type().id() == "#REF#") {
        ...
        if (v_it != values.end()) {
          ...
          forall_objects(it2, t2.read()) insert(dest, it2);
        } else
...
void value_set_fit::assign(const exprt &lhs, const exprt &rhs,
                           const namespacet &ns) {
#if 0
  ...
  if (type.id() == ID_struct || type.id() == ID_union) {
    ...
    if (rhs.id() == ID_unknown || rhs.id() == ID_invalid) {
      ...
      if (rhs.id() == ID_array_of) {
        ...
        forall_operands(o_it, rhs) { assign(lhs_index, *o_it, ns); }
      } else if (rhs.id() == ID_with) {
        assert(rhs.operands().size() == 3);
...
void value_set_fit::do_free(const exprt &op, const namespacet &ns) {
...
void value_set_fit::assign_rec(const exprt &lhs, const object_mapt &values_rhs,
  ...
  if (lhs.type().id() == "#REF#") {
    ...
    if (recursion_set.find(ident) != recursion_set.end()) {
      ...
      forall_objects(it, temp.read()) if (object_numbering[it->first].id() !=
                                          ID_unknown)
...
void value_set_fit::do_end_function(const exprt &lhs, const namespacet &ns) {
  if (lhs.is_nil())
    return;
...
void value_set_fit::apply_code(const exprt &code, const namespacet &ns) {
