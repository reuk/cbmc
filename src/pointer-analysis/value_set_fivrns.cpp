...
bool value_set_fivrnst::make_union(object_mapt &dest, const object_mapt &src)
  const
{
  bool result= false;
...
void value_set_fivrnst::copy_objects(object_mapt &dest, const object_mapt &src)
  const
{
  forall_valid_objects(it, src.read())
  {
...
void value_set_fivrnst::do_end_function(const exprt &lhs, const namespacet &ns)
{
  if(lhs.is_nil())
    return;
