...
void value_set_fivrnst::output(const namespacet &ns, std::ostream &out) const {
...
void value_set_fivrnst::output_entry(const entryt &e, const namespacet &ns,
...
void value_set_fivrnst::get_value_set(const exprt &expr,
  ...
  forall_objects(it, object_map.read()) value_set.push_back(to_expr(it));
...
void value_set_fivrnst::get_value_set(const exprt &expr, object_mapt &dest,
                                      const namespacet &ns) const {
  exprt tmp(expr);
  simplify(tmp, ns);
...
void value_set_fivrnst::get_value_set_rec(const exprt &expr, object_mapt &dest,
...
void value_set_fivrnst::dereference_rec(const exprt &src, exprt &dest) const {
...
void value_set_fivrnst::get_reference_set(const exprt &expr, expr_sett &dest,
                                          const namespacet &ns) const {
  object_mapt object_map;
...
void value_set_fivrnst::get_reference_set_rec(const exprt &expr,
                                              object_mapt &dest,
                                              const namespacet &ns) const {
#if 0
...
void value_set_fivrnst::assign(const exprt &lhs, const exprt &rhs,
...
void value_set_fivrnst::do_free(const exprt &op, const namespacet &ns) {
...
void value_set_fivrnst::assign_rec(const exprt &lhs,
...
void value_set_fivrnst::do_end_function(const exprt &lhs,
  ...
  irep_idt rvs =
      std::string("value_set::return_value") + std::to_string(from_function);
  add_var(rvs, "");
...
void value_set_fivrnst::apply_code(const exprt &code, const namespacet &ns) {
...
bool value_set_fivrnst::insert_to(object_mapt &dest, unsigned n,
...
bool value_set_fivrnst::insert_from(object_mapt &dest, unsigned n,
                                    const objectt &object) const {
  object_map_dt &map = dest.write();
  if (map.find(n) == map.end()) {
    //    std::cout << "NEW(" << n << "): " << object_numbering[n] << std::endl;
...
bool value_set_fivrnst::object_map_dt::set_valid_at(unsigned inx, unsigned f,
                                                    unsigned line) {
...
bool value_set_fivrnst::object_map_dt::is_valid_at(unsigned inx, unsigned f,
                                                   unsigned line) const {
#if 0
