...
bool goto_program_dereferencet::has_failed_symbol(const exprt &expr,
  ...
  if (expr.id() == ID_symbol) {
    ...
    const irep_idt &failed_symbol = ptr_symbol.type.get("#failed_symbol");
...
void goto_program_dereferencet::dereference_rec(
  ...
  if (expr.id() == ID_address_of || expr.id() == "reference_to") {
    // turn &*p to p
  ...
  Forall_operands(it, expr) dereference_rec(*it, guard, mode);
  ...
  if (expr.id() == ID_dereference) {
    ...
    exprt tmp = dereference.dereference(expr.op0(), guard, mode);
...
void goto_program_dereferencet::dereference_expr(
    exprt &expr, const bool checks_only,
...
void goto_program_dereferencet::dereference_program(
    goto_functionst &goto_functions, bool checks_only) {
...
void goto_program_dereferencet::dereference_instruction(
    goto_programt::targett target, bool checks_only) {
...
void goto_program_dereferencet::dereference_expression(
    goto_programt::const_targett target, exprt &expr) {
  current_target = target;
#if 0
  valid_local_variables=&target->local_variables;
#endif
...
void remove_pointers(goto_programt &goto_program, symbol_tablet &symbol_table,
                     value_setst &value_sets) {
  namespacet ns(symbol_table);

  optionst options;

  goto_program_dereferencet goto_program_dereference(ns, symbol_table, options,
                                                     value_sets);
...
void remove_pointers(goto_functionst &goto_functions,
                     symbol_tablet &symbol_table, value_setst &value_sets) {
  namespacet ns(symbol_table);
...
void pointer_checks(goto_programt &goto_program, symbol_tablet &symbol_table,
                    const optionst &options, value_setst &value_sets) {
  namespacet ns(symbol_table);
  goto_program_dereferencet goto_program_dereference(ns, symbol_table, options,
                                                     value_sets);
...
void pointer_checks(goto_functionst &goto_functions,
                    symbol_tablet &symbol_table, const optionst &options,
                    value_setst &value_sets) {
  namespacet ns(symbol_table);
  goto_program_dereferencet goto_program_dereference(ns, symbol_table, options,
                                                     value_sets);
...
void dereference(goto_programt::const_targett target, exprt &expr,
                 const namespacet &ns, value_setst &value_sets) {
  optionst options;
  symbol_tablet new_symbol_table;
  goto_program_dereferencet goto_program_dereference(ns, new_symbol_table,
                                                     options, value_sets);
