...
public:
  ...
  public:
    ...
    mp_integer offset;
    bool offset_is_set;
    bool offset_is_zero() const { return offset_is_set && offset.is_zero(); }
  };
  ...
  public:
    ...
    objectt &operator[](unsigned k) { return objmap[k]; }
  ...
  bool insert_to(object_mapt &dest, const exprt &src,
  ...
  bool insert_to(object_mapt &dest, const exprt &expr,
  ...
  bool insert_from(object_mapt &dest, const exprt &src,
                   const mp_integer &offset) const {
  ...
  bool insert_from(object_mapt &dest, const exprt &expr,
                   const objectt &object) const {
  ...
  void get_value_set(const exprt &expr, std::list<exprt> &expr_set,
                     const namespacet &ns) const;

  expr_sett &get(const idt &identifier, const std::string &suffix);

  void make_any() { values.clear(); }
  ...
  void add_var(const entryt &e) { get_entry(e.identifier, e.suffix); }
  ...
  void do_function_call(const irep_idt &function,
...
protected:
  void get_reference_set_sharing(const exprt &expr, expr_sett &expr_set,
                                 const namespacet &ns) const;

  void get_value_set_rec(const exprt &expr, object_mapt &dest,
                         const std::string &suffix, const typet &original_type,
                         const namespacet &ns,
                         gvs_recursion_sett &recursion_set) const;

  void get_value_set(const exprt &expr, object_mapt &dest,
                     const namespacet &ns) const;

  void get_reference_set_sharing(const exprt &expr, object_mapt &dest,
                                 const namespacet &ns) const {
    get_reference_set_sharing_rec(expr, dest, ns);
  }

  void get_reference_set_sharing_rec(const exprt &expr, object_mapt &dest,
                                     const namespacet &ns) const;

  void dereference_rec(const exprt &src, exprt &dest) const;

  void assign_rec(const exprt &lhs, const object_mapt &values_rhs,
                  const std::string &suffix, const namespacet &ns,
                  assign_recursion_sett &recursion_set, bool add_to_sets);

  void do_free(const exprt &op, const namespacet &ns);

  void flatten(const entryt &e, object_mapt &dest) const;

  void flatten_rec(const entryt &, object_mapt &, flatten_seent &,
                   unsigned from_function, unsigned from_index) const;

  bool recursive_find(const irep_idt &ident, const object_mapt &rhs,
