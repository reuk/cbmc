  ...
  const namespacet &ns)
{
  ...
  if(
    has_prefix(id2string(id), "value_set::dynamic_object") ||
...
value_sett::entryt &
value_sett::get_entry(const entryt &e, const typet &type, const namespacet &ns)
{
  irep_idt index;
...
bool value_sett::insert(object_mapt &dest, unsigned n, const objectt &object)
  const
{
  ...
  const namespacet &ns) const
{
  ...
  else if(expr.id() == ID_member)
  {
    ...
    get_value_set_rec(
      expr.op0(), dest, "." + component_name + suffix, original_type, ns);
  }
  else if(expr.id() == ID_symbol)
  {
    ...
    {
      // look it up
      valuest::const_iterator v_it= values.find(id2string(identifier) + suffix);
  ...
  else if(expr.is_constant())
  {
    ...
    else if(expr_type.id() == ID_unsignedbv || expr_type.id() == ID_signedbv)
    {
    ...
    expr.id() == ID_byte_extract_big_endian)
  {
    ...
    if(op0_type.id() == ID_union)
    {
      ...
      for(union_typet::componentst::const_iterator c_it=
          ...
          c_it != union_type.components().end();
          c_it++)
      {
        const irep_idt &name= c_it->get_name();
        member_exprt member(expr.op0(), name, c_it->type());
  ...
  else if(expr.id() == ID_member)
  {
        ...
        it++)
    {
      ...
      else
      {
        ...
        if(
          final_object_type.id() == ID_struct ||
