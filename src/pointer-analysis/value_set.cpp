...
bool value_sett::insert(object_mapt &dest, unsigned n,
  ...
  else if (object.offset_is_set && entry->second.offset == object.offset)
    return false; // no change
  else {
...
bool value_sett::make_union(const value_sett::valuest &new_values) {
  ...
  for (valuest::const_iterator it = new_values.begin();
       it != new_values.end();) // no it++
  {
...
bool value_sett::eval_pointer_offset(exprt &expr, const namespacet &ns) const {
  bool mod = false;
  ...
  if (expr.id() == ID_pointer_offset) {
    ...
    for (object_map_dt::const_iterator it = object_map.begin();
    ...
    Forall_operands(it, expr) mod = eval_pointer_offset(*it, ns) || mod;
  }
...
void value_sett::get_value_set(const exprt &expr, value_setst::valuest &dest,
...
void value_sett::get_value_set(const exprt &expr, object_mapt &dest,
                               const namespacet &ns, bool is_simplified) const {
  exprt tmp(expr);
  if (!is_simplified)
    simplify(tmp, ns);
...
void value_sett::get_value_set_rec(const exprt &expr, object_mapt &dest,
  ...
  if (expr.id() == ID_unknown || expr.id() == ID_invalid) {
    ...
    if (expr_type.id() == ID_pointer || expr_type.id() == ID_signedbv ||
      ...
      if (v_it == values.end() &&
          (expr_type.id() == ID_struct || expr_type.id() == ID_union)) {
        ...
        v_it = values.find(id2string(identifier) + "." + first_component_name +
                           suffix);
      }
    ...
    if (object_map.begin() == object_map.end())
      ...
      for (object_map_dt::const_iterator it1 = object_map.begin();
           it1 != object_map.end(); it1++) {
    ...
    // check if NULL
    if (expr.get(ID_value) == ID_NULL && expr_type.id() == ID_pointer) {
    ...
    if (op_type.id() == ID_pointer) {
      ...
      get_value_set_rec(expr.op0(), dest, suffix, original_type, ns);
    } else if (op_type.id() == ID_unsignedbv || op_type.id() == ID_signedbv) {
      // integer-to-pointer
      ...
      if (expr.op0().is_zero())
        ...
        if (tmp.read().size() == 0) {
          ...
          insert(dest, exprt(ID_integer_address, unsigned_char_type()));
                   ...
                   object_numbering[tmp.read().begin()->first].id() ==
                       ID_unknown) {
          // if not, throw in integer
          insert(dest, exprt(ID_integer_address, unsigned_char_type()));
        } else {
    ...
    if (expr.operands().size() == 2 && expr_type.id() == ID_pointer) {
      exprt ptr_operand;

      if (expr.op0().type().id() != ID_pointer && expr.op0().is_constant()) {
      ...
      forall_operands(it, expr) {
        get_value_set_rec(*it, pointer_expr_set, "", it->type(), ns);
      }
    }

    for (object_map_dt::const_iterator it = pointer_expr_set.read().begin();
         it != pointer_expr_set.read().end(); it++) {
    ...
    const std::string prefix = "value_set::dynamic_object" +
    ...
    if (!to_integer(op1, op1_offset) && op0_type.id() == ID_struct) {
      ...
      for (struct_union_typet::componentst::const_iterator c_it =
               struct_type.components().begin();
           !found && c_it != struct_type.components().end(); c_it++) {
    ...
    if (op0_type.id() == ID_union) {
      ...
      for (union_typet::componentst::const_iterator c_it =
...
void value_sett::dereference_rec(const exprt &src, exprt &dest) const {
...
void value_sett::get_reference_set_rec(const exprt &expr, object_mapt &dest,
                                       const namespacet &ns) const {
#if 0
  ...
  if (expr.id() == ID_symbol || expr.id() == ID_dynamic_object ||
    ...
    for (object_map_dt::const_iterator a_it = object_map.begin();
         a_it != object_map.end(); a_it++) {
...
void value_sett::assign(const exprt &lhs, const exprt &rhs,
                        const namespacet &ns, bool is_simplified,
                        bool add_to_sets) {
#if 0
  ...
  if (type.id() == ID_struct || type.id() == ID_union) {
    ...
    if (rhs.id() == ID_unknown || rhs.id() == ID_invalid) {
      assign(lhs_index, exprt(rhs.id(), type.subtype()), ns, is_simplified,
             add_to_sets);
    } else {
      if (!base_type_eq(rhs.type(), type, ns))
        throw "value_sett::assign type mismatch: "
...
void value_sett::do_free(const exprt &op, const namespacet &ns) {
  ...
  for (valuest::iterator v_it = values.begin(); v_it != values.end(); v_it++) {
    ...
    for (object_map_dt::const_iterator o_it = old_object_map.begin();
         o_it != old_object_map.end(); o_it++) {
...
void value_sett::assign_rec(const exprt &lhs, const object_mapt &values_rhs,
                            const std::string &suffix, const namespacet &ns,
                            bool add_to_sets) {
#if 0
  ...
  if (lhs.id() == ID_symbol) {
    ...
    for (object_map_dt::const_iterator it = reference_set.read().begin();
         it != reference_set.read().end(); it++) {
      const exprt &object = object_numbering[it->first];
    ...
    assign_rec(lhs.op0(), values_rhs, "." + component_name + suffix, ns,
             ...
             lhs.id() == "dynamic_type" || lhs.id() == "is_zero_string" ||
             lhs.id() == "zero_string" || lhs.id() == "zero_string_length") {
    // we ignore this here
...
void value_sett::do_end_function(const exprt &lhs, const namespacet &ns) {
  if (lhs.is_nil())
    return;
...
void value_sett::apply_code(const codet &code, const namespacet &ns) {
  const irep_idt &statement = code.get_statement();

  if (statement == ID_block) {
    forall_operands(it, code) apply_code(to_code(*it), ns);
  ...
  } else if (statement == "specc_notify" || statement == "specc_wait") {
...
void value_sett::guard(const exprt &expr, const namespacet &ns) {
  if (expr.id() == ID_and) {
    forall_operands(it, expr) guard(*it, ns);
...
exprt value_sett::make_member(const exprt &src, const irep_idt &component_name,
                              const namespacet &ns) {
  const struct_union_typet &struct_union_type =
      to_struct_union_type(ns.follow(src.type()));

  if (src.id() == ID_struct || src.id() == ID_constant) {
