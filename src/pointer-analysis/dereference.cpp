...
exprt dereferencet::operator()(const exprt &pointer) {
  ...
  return dereference_rec(pointer, from_integer(0, index_type()), // offset
                         type);
}
...
exprt dereferencet::read_object(const exprt &object, const exprt &offset,
                                const typet &type) {
  ...
  if (object.id() == ID_index) {
    ...
    if (compound_type.id() == ID_struct) {
      const struct_typet &struct_type = to_struct_type(compound_type);
...
exprt dereferencet::dereference_rec(const exprt &address, const exprt &offset,
                                    const typet &type) {
  if (address.id() == ID_address_of) {
    ...
    if (to_constant_expr(address).get_value() == ID_NULL) // NULL
    {
      ...
      return dereference_rec(typecast_exprt(zero, address.type()), offset,
                             type);
    } else
...
exprt dereferencet::dereference_if(const if_exprt &expr, const exprt &offset,
                                   const typet &type) {
...
exprt dereferencet::dereference_plus(const exprt &expr, const exprt &offset,
                                     const typet &type) {
  ...
  if (expr.operands().size() > 2) {
    ...
    exprt::operandst plus_ops(++expr.operands().begin(), expr.operands().end());
...
exprt dereferencet::dereference_typecast(const typecast_exprt &expr,
  ...
  if (op_type.id() == ID_pointer)
    ...
    if (!offset.is_zero())
      integer = plus_exprt(offset, typecast_exprt(op, offset.type()));

    exprt new_typecast = typecast_exprt(integer, pointer_typet(type));
...
bool dereferencet::type_compatible(const typet &object_type,
  ...
  if (object_type.id() == ID_pointer && dereference_type.id() == ID_pointer)
    return true;
