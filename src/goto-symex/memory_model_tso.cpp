...
void memory_model_tsot::program_order(symex_target_equationt &equation)
{
      ...
      t_it++)
  {
        ...
        e_it++)
    {
      ...
      for(event_listt::const_iterator e_it2= next; e_it2 != events.end();
          e_it2++)
      {
        if(
          ((*e_it)->is_spawn() && !(*e_it2)->is_memory_barrier()) ||
          (*e_it2)->is_spawn())
        {
          add_constraint(
            equation, before(*e_it, *e_it2), "po", (*e_it)->source);
        ...
        if((*e_it2)->is_memory_barrier())
        {
          ...
          if(
            (*e_it)->is_shared_read() && !code.get_bool(ID_RRfence) &&
            !code.get_bool(ID_RWfence))
            continue;
          else if(
            (*e_it)->is_shared_write() && !code.get_bool(ID_WRfence) &&
        ...
        if(!cond.is_false())
        {
          if(ordering.is_nil())
            ordering=
              partial_order_concurrencyt::before(*e_it, *e_it2, AX_PROPAGATION);

          add_constraint(
            equation, implies_exprt(cond, ordering), "po", (*e_it)->source);
        }
      }
    }
  }
