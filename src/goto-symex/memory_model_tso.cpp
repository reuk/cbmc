    ...
    for (event_listt::const_iterator e_it = events.begin();
      ...
      for (event_listt::const_iterator e_it2 = next; e_it2 != events.end();
        ...
        if (((*e_it)->is_spawn() && !(*e_it2)->is_memory_barrier()) ||
            (*e_it2)->is_spawn()) {
          add_constraint(equation, before(*e_it, *e_it2), "po",
                         (*e_it)->source);
        ...
        if ((*e_it2)->is_memory_barrier()) {
          ...
          if ((*e_it)->is_shared_read() && !code.get_bool(ID_RRfence) &&
              !code.get_bool(ID_RWfence))
            continue;
          else if ((*e_it)->is_shared_write() && !code.get_bool(ID_WRfence) &&
        ...
        if (address(*e_it) == address(*e_it2)) {
          ordering = partial_order_concurrencyt::before(*e_it, *e_it2,
                                                        AX_SC_PER_LOCATION);
        ...
        if (!cond.is_false()) {
          ...
          add_constraint(equation, implies_exprt(cond, ordering), "po",
                         (*e_it)->source);
        }
      }
    }
