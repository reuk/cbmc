...
symex_target_equationt::symex_target_equationt(const namespacet &_ns)
    : ns(_ns) {}
...
void symex_target_equationt::assignment(
    const exprt &guard, const ssa_exprt &ssa_lhs, const exprt &ssa_full_lhs,
    const exprt &original_full_lhs, const exprt &ssa_rhs, const sourcet &source,
  ...
  SSA_step.hidden =
      (assignment_type != STATE && assignment_type != VISIBLE_ACTUAL_PARAMETER);
  SSA_step.source = source;
...
void symex_target_equationt::decl(const exprt &guard, const ssa_exprt &ssa_lhs,
                                  const sourcet &source,
                                  assignment_typet assignment_type) {
  assert(ssa_lhs.is_not_nil());
...
void symex_target_equationt::dead(const exprt &guard, const ssa_exprt &ssa_lhs,
                                  const sourcet &source) {
...
void symex_target_equationt::output(const exprt &guard, const sourcet &source,
...
void symex_target_equationt::input(const exprt &guard, const sourcet &source,
...
void symex_target_equationt::assumption(const exprt &guard, const exprt &cond,
                                        const sourcet &source) {
  SSA_steps.push_back(SSA_stept());
  SSA_stept &SSA_step = SSA_steps.back();
...
void symex_target_equationt::assertion(const exprt &guard, const exprt &cond,
  ...
  for (auto &step : SSA_steps) {
    if (step.is_assert()) {
      ...
      if (assumption.id() == ID_and)
        ...
        assumption = and_exprt(assumption, literal_exprt(step.cond_literal));
    }
  }
...
void symex_target_equationt::SSA_stept::output(const namespacet &ns,
  ...
  case goto_trace_stept::ASSIGNMENT:
    ...
    case STATE:
      out << "STATE";
      break;
    case VISIBLE_ACTUAL_PARAMETER:
      out << "VISIBLE_ACTUAL_PARAMETER";
      break;
    case HIDDEN_ACTUAL_PARAMETER:
