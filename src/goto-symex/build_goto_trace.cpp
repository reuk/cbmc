...
exprt build_full_lhs_rec(const prop_convt &prop_conv, const namespacet &ns,
                         ...
                         const exprt &src_ssa)      // renamed identifiers
{
  ...
  if (id == ID_index) {
    ...
    if (index_value.is_not_nil()) {
      ...
      tmp.array() =
          build_full_lhs_rec(prop_conv, ns, to_index_expr(src_original).array(),
    ...
    tmp.struct_op() = build_full_lhs_rec(
        prop_conv, ns, to_member_expr(src_original).struct_op(),
    ...
    tmp.op() =
        build_full_lhs_rec(prop_conv, ns, to_typecast_expr(src_original).op(),
...
exprt adjust_lhs_object(const prop_convt &prop_conv, const namespacet &ns,
                        const exprt &src) {
  return nil_exprt();
}
...
void build_goto_trace(
    ...
    const prop_convt &prop_conv, const namespacet &ns,
    ...
    if (it->is_constraint() || it->is_spawn())
      ...
      if (time_before < 0) {
        time_mapt::iterator entry =
            time_map.insert(std::make_pair(current_time, goto_tracet::stepst()))
                .first;
...
void build_goto_trace(const symex_target_equationt &target,
                      const prop_convt &prop_conv, const namespacet &ns,
                      goto_tracet &goto_trace) {
  build_goto_trace(target, target.SSA_steps.end(), prop_conv, ns, goto_trace);
