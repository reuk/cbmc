...
void goto_symext::symex_assign_rec(statet &state, const code_assignt &code) {
...
exprt goto_symext::add_to_lhs(const exprt &lhs, const exprt &what) {
...
void goto_symext::symex_assign_rec(statet &state, const exprt &lhs,
                                   const exprt &full_lhs, const exprt &rhs,
                                   guardt &guard,
                                   assignment_typet assignment_type) {
  if (lhs.id() == ID_symbol && lhs.get_bool(ID_C_SSA_symbol))
    symex_assign_symbol(state, to_ssa_expr(lhs), full_lhs, rhs, guard,
                        assignment_type);
  else if (lhs.id() == ID_index)
    symex_assign_array(state, to_index_expr(lhs), full_lhs, rhs, guard,
                       assignment_type);
  else if (lhs.id() == ID_member) {
    const typet &type = ns.follow(to_member_expr(lhs).struct_op().type());
    if (type.id() == ID_struct)
      symex_assign_struct_member(state, to_member_expr(lhs), full_lhs, rhs,
      ...
      throw "symex_assign_rec: unexpected assignment to member of `" +
          type.id_string() + "'";
  } else if (lhs.id() == ID_if)
    symex_assign_if(state, to_if_expr(lhs), full_lhs, rhs, guard,
                    assignment_type);
  else if (lhs.id() == ID_typecast)
    symex_assign_typecast(state, to_typecast_expr(lhs), full_lhs, rhs, guard,
                          assignment_type);
  else if (lhs.id() == ID_string_constant || lhs.id() == "NULL-object" ||
           lhs.id() == "zero_string" || lhs.id() == "is_zero_string" ||
           lhs.id() == "zero_string_length") {
    // ignore
  } else if (lhs.id() == ID_byte_extract_little_endian ||
             lhs.id() == ID_byte_extract_big_endian) {
    symex_assign_byte_extract(state, to_byte_extract_expr(lhs), full_lhs, rhs,
                              guard, assignment_type);
    ...
    symex_assign_rec(state, lhs.op0(), full_lhs, new_rhs, guard,
                     assignment_type);
  } else
...
void goto_symext::symex_assign_symbol(statet &state,
  ...
  state.assignment(ssa_lhs, ssa_rhs, ns, options.get_bool_option("simplify"),
  ...
  target.assignment(tmp_guard.as_expr(), ssa_lhs, ssa_full_lhs,
                    add_to_lhs(full_lhs, ssa_lhs.get_original_expr()), ssa_rhs,
                    state.source, assignment_type);
}
...
void goto_symext::symex_assign_typecast(statet &state,
  ...
  symex_assign_rec(state, lhs.op0(), new_full_lhs, rhs_typecasted, guard,
                   assignment_type);
}
...
void goto_symext::symex_assign_array(statet &state, const index_exprt &lhs,
                                     const exprt &full_lhs, const exprt &rhs,
                                     guardt &guard,
                                     assignment_typet assignment_type) {
  ...
  symex_assign_rec(state, lhs_array, new_full_lhs, new_rhs, guard,
                   assignment_type);
...
void goto_symext::symex_assign_struct_member(statet &state,
  ...
  symex_assign_rec(state, lhs_struct, new_full_lhs, new_rhs, guard,
                   assignment_type);
...
void goto_symext::symex_assign_if(statet &state, const if_exprt &lhs,
                                  const exprt &full_lhs, const exprt &rhs,
                                  guardt &guard,
                                  assignment_typet assignment_type) {
  // we have (c?a:b)=e;
  ...
  if (!renamed_guard.is_false()) {
    ...
    symex_assign_rec(state, lhs.true_case(), full_lhs, rhs, guard,
                     assignment_type);
  ...
  if (!renamed_guard.is_true()) {
    ...
    symex_assign_rec(state, lhs.false_case(), full_lhs, rhs, guard,
                     assignment_type);
    guard.swap(old_guard);
  }
}
...
void goto_symext::symex_assign_byte_extract(statet &state,
  ...
  symex_assign_rec(state, lhs.op(), new_full_lhs, new_rhs, guard,
                   assignment_type);
}
