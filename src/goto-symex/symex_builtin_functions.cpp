    ...
    // special treatment for sizeof(T)*x
    {
      ...
      if(tmp_type.is_not_nil())
      {
        ...
        if(elem_size < 0)
        {
          ...
          to_integer(tmp_size, alloc_size) && tmp_size.id() == ID_mult &&
...
irep_idt get_string_argument_rec(const exprt &src)
{
  ...
  else if(src.id() == ID_address_of)
  {
    ...
    if(src.op0().id() == ID_index)
    {
      ...
      if(
        src.op0().op0().id() == ID_string_constant && src.op0().op1().is_zero())
      {
  ...
  const exprt &rhs)
{
  ...
  if(format_string != "")
    target.output_fmt(
      state.guard.as_expr(), state.source, "printf", format_string, args);
}
  ...
  const side_effect_exprt &code)
{
  ...
  if(do_array)
  {
    ...
    index_expr.copy_to_operands(
      symbol.symbol_expr(), from_integer(0, index_type()));
