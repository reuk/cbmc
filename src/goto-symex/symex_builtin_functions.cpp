...
void goto_symext::symex_malloc(statet &state, const exprt &lhs,
...
irep_idt get_symbol(const exprt &src) {
  if (src.id() == ID_typecast)
    ...
    if (op.id() == ID_symbol && op.get_bool(ID_C_SSA_symbol))
...
void goto_symext::symex_gcc_builtin_va_arg_next(statet &state, const exprt &lhs,
                                                const side_effect_exprt &code) {
  if (code.operands().size() != 1)
...
void goto_symext::symex_printf(statet &state, const exprt &lhs,
                               const exprt &rhs) {
  ...
  const irep_idt format_string = get_string_argument(operands[0], ns);

  if (format_string != "")
    target.output_fmt(state.guard.as_expr(), state.source, "printf",
                      format_string, args);
}
...
void goto_symext::symex_input(statet &state, const codet &code) {
...
void goto_symext::symex_output(statet &state, const codet &code) {
  if (code.operands().size() < 2)
...
void goto_symext::symex_cpp_new(statet &state, const exprt &lhs,
                                const side_effect_exprt &code) {
  bool do_array;
  ...
  if (do_array) {
    ...
    index_expr.copy_to_operands(symbol.symbol_expr(),
...
void goto_symext::symex_cpp_delete(statet &state, const codet &code) {
...
void goto_symext::symex_trace(statet &state, const code_function_callt &code) {
...
void goto_symext::symex_fkt(statet &state, const code_function_callt &code) {
#if 0
...
void goto_symext::symex_macro(statet &state, const code_function_callt &code) {
