  ...
  symex_target_equationt &equation)
{
  ...
  for(std::set<exprt>::iterator i= sliced_guards.begin();
      i != sliced_guards.end();
      i++)
  {
    exprt g_copy(*i);
...
{
  if(
    (read_line == ":") || (read_line == ":exact") || (read_line == ":suffix") ||
    (read_line == ":exact-suffix") || (read_line == ":prefix"))
  {
    semantics= read_line;
    return true;
  }
...
void symex_slice_by_tracet::parse_events(std::string read_line)
{
  ...
  for(size_t vidx= 0; vidx < vlength; vidx++)
  {
    ...
    eis.insert(event);
    if((!alphabet.empty()) && ((alphabet.count(event) != 0) != alphabet_parity))
...
void symex_slice_by_tracet::compute_ts_back(symex_target_equationt &equation)
{
  ...
  for(symex_target_equationt::SSA_stepst::reverse_iterator i=
        equation.SSA_steps.rbegin();
      i != equation.SSA_steps.rend();
      i++)
  {
    if(
      i->is_output() && !i->io_args.empty() &&
...
bool symex_slice_by_tracet::matches(event_sett s, irep_idt event)
{
...
std::set<exprt> symex_slice_by_tracet::implied_guards(exprt e)
{
  ...
  { // Descend into and
    Forall_operands(it, e)
    {
      ...
      for(std::set<exprt>::iterator i= r.begin(); i != r.end(); i++)
      {
        s.insert(*i);
      }
    }
  ...
  { // Descend into or
    ...
    for(std::set<exprt>::iterator i= rs.front().begin(); i != rs.front().end();)
    {
      for(std::vector<std::set<exprt>>::iterator j= rs.begin(); j != rs.end();
          j++)
      {
        if(j == rs.begin())
          j++;
...
bool symex_slice_by_tracet::implies_false(const exprt e)
{
  ...
  for(std::set<exprt>::iterator i= imps.begin(); i != imps.end(); i++)
  {
    exprt i_copy(*i);
    i_copy.make_not();
    simplify(i_copy, ns);
