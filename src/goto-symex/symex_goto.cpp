...
void goto_symext::symex_goto(statet &state) {
  ...
  if (new_guard.is_true()) {
    ...
    if (new_guard.id() == ID_symbol ||
        (new_guard.id() == ID_not && new_guard.operands().size() == 1 &&
      ...
      target.assignment(guard.as_expr(), new_lhs, new_lhs, guard_symbol_expr,
                        new_rhs, original_source, symex_targett::GUARD);
...
void goto_symext::merge_gotos(statet &state) {
  ...
  for (statet::goto_state_listt::reverse_iterator list_it = state_list.rbegin();
       list_it != state_list.rend(); list_it++)
...
void goto_symext::phi_function(const statet::goto_statet &goto_state,
  ...
  for (std::unordered_set<ssa_exprt, irep_hash>::const_iterator it =
           variables.begin();
       it != variables.end(); it++) {
    ...
    // shared?
    if (dest_state.atomic_section_id == 0 && dest_state.threads.size() >= 2 &&
    ...
    target.assignment(true_exprt(), new_lhs, new_lhs,
                      new_lhs.get_original_expr(), rhs, dest_state.source,
                      symex_targett::PHI);
  }
}
...
void goto_symext::loop_bound_exceeded(statet &state, const exprt &guard) {
  ...
  bool unwinding_assertions = options.get_bool_option("unwinding-assertions");

  bool partial_loops = options.get_bool_option("partial-loops");

  if (!partial_loops) {
    if (unwinding_assertions) {
      // Generate VCC for unwinding assertion.
      vcc(negated_cond,
          "unwinding assertion loop " + std::to_string(loop_number), state);
