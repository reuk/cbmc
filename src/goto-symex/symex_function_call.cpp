...
void goto_symext::parameter_assignments(
    ...
    const goto_functionst::goto_functiont &goto_function, statet &state,
      ...
      if (!base_type_eq(parameter_type, rhs.type(), ns)) {
        ...
        if ((f_parameter_type.id() == ID_signedbv ||
            ...
            (f_rhs_type.id() == ID_signedbv ||
             ...
             f_rhs_type.id() == ID_c_enum_tag || f_rhs_type.id() == ID_bool ||
             f_rhs_type.id() == ID_pointer || f_rhs_type.id() == ID_union)) {
          rhs =
              byte_extract_exprt(byte_extract_id(), rhs,
                                 from_integer(0, index_type()), parameter_type);
        } else {
  ...
  if (function_type.has_ellipsis()) {
    ...
    while (!ns.lookup(id2string(function_identifier) + "::va_arg" +
...
void goto_symext::symex_function_call(const goto_functionst &goto_functions,
  ...
  if (function.id() == ID_symbol)
    ...
    throw "unexpected function for symex_function_call: " +
        function.id_string();
}
...
void goto_symext::symex_function_call_code(
  ...
  const bool stop_recursing =
      get_unwind_recursion(identifier, state.source.thread_nr,
  ...
  for (goto_symex_statet::framet::loop_iterationst::const_iterator it =
           p_frame.loop_iterations.begin();
       it != p_frame.loop_iterations.end(); ++it)
...
void goto_symext::pop_frame(statet &state) {
...

  {
    ...
    assert(state.dirty);
    for (goto_symex_statet::renaming_levelt::current_namest::iterator c_it =
         ...
         c_it != state.level2.current_names.end();) // no ++c_it
    {
      ...
      goto_symex_statet::renaming_levelt::current_namest::iterator cur = c_it;
      ++c_it;
...
void goto_symext::symex_end_of_function(statet &state) {
  ...
  target.function_return(state.guard.as_expr(), state.source.pc->function,
                         state.source);
...
void goto_symext::locality(
    const irep_idt function_identifier, statet &state,
  ...
  for (std::set<irep_idt>::const_iterator it = local_identifiers.begin();
    ...
    state.level1.current_names[l0_name] = std::make_pair(ssa, frame_nr);
...
void goto_symext::return_assignment(statet &state) {
  ...
  if (code.operands().size() == 1) {
    ...
    if (frame.return_value.is_not_nil()) {
      ...
      if (!base_type_eq(assignment.lhs().type(), assignment.rhs().type(), ns))
