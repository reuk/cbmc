...
goto_symex_statet::goto_symex_statet()
    : depth(0), symex_target(NULL), atomic_section_id(0), record_events(true),
      dirty(0) {
  threads.resize(1);
  new_frame();
}
...
void goto_symex_statet::level0t::
...
bool goto_symex_statet::constant_propagation(const exprt &expr) const {
  ...
  if (expr.id() == ID_address_of) {
    ...
    forall_operands(it, expr) if (!constant_propagation(*it)) return false;

    return true;
  } else if (expr.id() == ID_mult) {
    // propagate stuff with sizeof in it
    forall_operands(
        it, expr) if (it->find(ID_C_c_sizeof_type).is_not_nil()) return true;
...
static bool check_renaming_l1(const exprt &expr) {
  ...
  if (expr.id() == ID_symbol) {
    ...
    forall_operands(it, expr) if (check_renaming_l1(*it)) return true;
  }
...
static bool check_renaming(const exprt &expr) {
  ...
  if (expr.id() == ID_address_of && expr.op0().id() == ID_symbol)
    return check_renaming_l1(expr.op0());
  else if (expr.id() == ID_address_of && expr.op0().id() == ID_index)
    ...
    forall_operands(it, expr) if (check_renaming(*it)) return true;
  }
...
void goto_symex_statet::assignment(ssa_exprt &lhs,   // L0/L1
                                   const exprt &rhs, // L2
...
void goto_symex_statet::propagationt::operator()(exprt &expr) {
  if (expr.id() == ID_symbol) {
    valuest::const_iterator it = values.find(expr.get(ID_identifier));
    if (it != values.end())
      expr = it->second;
    ...
    // do this recursively
    Forall_operands(it, expr) operator()(*it);
  }
}
...
void goto_symex_statet::set_ssa_indices(ssa_exprt &ssa_expr,
                                        const namespacet &ns, levelt level) {
  switch (level) {
  case L0:
...
void goto_symex_statet::rename(exprt &expr, const namespacet &ns,
  ...
  if (expr.id() == ID_symbol && expr.get_bool(ID_C_SSA_symbol)) {
    ...
    // do this recursively
    Forall_operands(it, expr) rename(*it, ns, level);
...
bool goto_symex_statet::l2_thread_read_encoding(ssa_exprt &expr,
  ...
  if (atomic_section_id != 0) {
    ...
    if (a_s_read.second.empty()) {
      if (level2.current_names.find(l1_identifier) ==
          level2.current_names.end())
    ...
    symex_target->assignment(guard.as_expr(), ssa_l1, ssa_l1,
                             ssa_l1.get_original_expr(), tmp, source,
                             symex_targett::PHI);
...
void goto_symex_statet::rename_address(exprt &expr, const namespacet &ns,
  ...
  if (expr.id() == ID_symbol && expr.get_bool(ID_C_SSA_symbol)) {
    ...
    if (expr.id() == ID_index) {
      ...
      Forall_operands(it, expr) rename_address(*it, ns, level);
    }
  }
}
...
void goto_symex_statet::rename(typet &type, const irep_idt &l1_identifier,
                               const namespacet &ns, levelt level) {
  // rename all the symbols with their last known value
  // to the given level
  ...
  if (level == L2 && !l1_identifier.empty()) {
    ...
    if (!l1_type_entry.second) // was already in map
    {
      ...
      if (type.id() != ID_array || type_prev.id() != ID_array ||
...
void goto_symex_statet::get_original_name(exprt &expr) const {
  ...
  if (expr.id() == ID_symbol && expr.get_bool(ID_C_SSA_symbol))
    ...
    Forall_operands(it, expr) get_original_name(*it);
}
...
void goto_symex_statet::get_l1_name(exprt &expr) const {
  ...
  if (expr.id() == ID_symbol && expr.get_bool(ID_C_SSA_symbol))
    ...
    Forall_operands(it, expr) get_l1_name(*it);
}
