...
void goto_symext::symex_decl(statet &state, const symbol_exprt &expr) {
  ...
  if (ns.follow(expr.type()).id() == ID_pointer) {
    exprt failed = get_failed_symbol(expr, ns);
  ...
  target.decl(state.guard.as_expr(), ssa, state.source,
              hidden ? symex_targett::HIDDEN : symex_targett::STATE);

  assert(state.dirty);
  if ((*state.dirty)(ssa.get_object_name()) && state.atomic_section_id == 0)
    target.shared_write(state.guard.as_expr(), ssa, state.atomic_section_id,
                        state.source);
}
