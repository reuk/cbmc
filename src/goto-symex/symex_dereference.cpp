...
void goto_symext::dereference_rec_address_of(exprt &expr, statet &state,
                                             guardt &guard) {
  // Could be member, could be if, could be index.

  if (expr.id() == ID_member)
    dereference_rec_address_of(to_member_expr(expr).struct_op(), state, guard);
  else if (expr.id() == ID_if) {
    // the condition is not an address
    dereference_rec(to_if_expr(expr).cond(), state, guard, false);

    // add to guard?
    dereference_rec_address_of(to_if_expr(expr).true_case(), state, guard);
    dereference_rec_address_of(to_if_expr(expr).false_case(), state, guard);
  } else if (expr.id() == ID_index) {
    // the index is not an address
    dereference_rec(to_index_expr(expr).index(), state, guard, false);

    // the array _is_ an address
    dereference_rec_address_of(to_index_expr(expr).array(), state, guard);
  } else {
...
bool goto_symext::is_index_member_symbol_if(const exprt &expr) {
  ...
  if (expr.id() == ID_member) {
    return is_index_member_symbol_if(to_member_expr(expr).struct_op());
  } else if (expr.id() == ID_if) {
    return is_index_member_symbol_if(to_if_expr(expr).true_case()) &&
...
exprt goto_symext::address_arithmetic(const exprt &expr, statet &state,
                                      guardt &guard, bool keep_array) {
  exprt result;
  ...
  if (expr.id() == ID_byte_extract_little_endian ||
    ...
    result = if_expr;
  } else if (expr.id() == ID_symbol || expr.id() == ID_string_constant ||
...
void goto_symext::dereference_rec(exprt &expr, statet &state, guardt &guard,
  ...
  if (expr.id() == ID_dereference) {
    ...
    value_set_dereferencet dereference(ns, new_symbol_table, options,
                                       symex_dereference_state, language_mode);

    // std::cout << "**** " << from_expr(ns, "", tmp1) << std::endl;
    exprt tmp2 = dereference.dereference(tmp1, guard,
                                         write ? value_set_dereferencet::WRITE
                                               : value_set_dereferencet::READ);
    ...
    if (tc_op.id() == ID_address_of &&
        ...
        base_type_eq(
            ...
            ns)) {
      expr = address_of_exprt(index_exprt(to_address_of_expr(tc_op).object(),
    ...
    Forall_operands(it, expr) dereference_rec(*it, state, guard, write);
  }
}
...
void goto_symext::dereference(exprt &expr, statet &state, const bool write) {
