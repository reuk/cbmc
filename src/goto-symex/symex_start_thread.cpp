...
void goto_symext::symex_start_thread(statet &state) {
  ...
  for (goto_symex_statet::renaming_levelt::current_namest::const_iterator c_it =
       ...
       c_it != state.level2.current_names.end(); ++c_it) {
    ...
    if (!state.level1.current_names
             .insert(std::make_pair(lhs.get_l1_object_identifier(),
                                    std::make_pair(lhs, 0)))
             .second)
      assert(false);
    ...
    symex_assign_symbol(state, lhs, nil_exprt(), rhs, guard,
  ...
  forall_symbols(it, symbol_table.symbols) {
    ...
    if (!symbol.is_thread_local || !symbol.is_static_lifetime ||
