...
static bool have_to_adjust_float_expressions(const exprt &expr,
  ...
  forall_operands(
      it, expr) if (have_to_adjust_float_expressions(*it, ns)) return true;
...
void adjust_float_expressions(exprt &expr, const namespacet &ns) {
  ...
  Forall_operands(it, expr) adjust_float_expressions(*it, ns);
...
static void
adjust_float_expressions(goto_functionst::goto_functiont &goto_function,
                         const namespacet &ns) {
