...
void partial_order_concurrencyt::build_event_lists(
  ...
  for (address_mapt::const_iterator a_it = address_map.begin();
    ...
    statistics() << "Shared " << a_it->first << ": " << a_rec.reads.size()
                 << "R/" << a_rec.writes.size() << "W" << eom;
  }
}
...
exprt partial_order_concurrencyt::before(event_it e1, event_it e2,
                                         unsigned axioms) {
  const axiomt axiom_bits[] = {AX_SC_PER_LOCATION, AX_NO_THINAIR,
                               AX_OBSERVATION, AX_PROPAGATION};
...
void partial_order_concurrencyt::add_constraint(
    symex_target_equationt &equation, const exprt &cond, const std::string &msg,
