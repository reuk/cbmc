  ...
  const symbolt *&symbol)
{
  ...
  if(expr.id() == ID_symbol && expr.get_bool(ID_C_SSA_symbol))
  {
    ...
    const symbolt &ptr_symbol= ns.lookup(to_ssa_expr(expr).get_object_name());

    const irep_idt &failed_symbol= ptr_symbol.type.get("#failed_symbol");

    if(failed_symbol != "" && !ns.lookup(failed_symbol, symbol))
    {
      symbolt sym= *symbol;
      symbolt *sym_ptr= 0;
  ...
  {
    const symbolt &ptr_symbol= ns.lookup(to_symbol_expr(expr).get_identifier());
