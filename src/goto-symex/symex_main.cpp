...
void goto_symext::vcc(const exprt &vcc_expr, const std::string &msg,
                      statet &state) {
  total_vccs++;
...
void goto_symext::operator()(statet &state,
...
void goto_symext::symex_step(const goto_functionst &goto_functions,
                             statet &state) {
#if 0
  ...
  case ASSERT:
    if (!state.guard.is_false()) {
      std::string msg =
          id2string(state.source.pc->source_location.get_comment());
      if (msg == "")
        msg = "assertion";
      exprt tmp(instruction.guard);
      clean_expr(tmp, state, false);
  ...
  case FUNCTION_CALL:
    if (!state.guard.is_false()) {
      ...
      Forall_expr(it, deref_code.arguments()) clean_expr(*it, state, false);
