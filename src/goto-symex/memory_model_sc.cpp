...
void memory_model_sct::build_per_thread_map(
    const symex_target_equationt &equation, per_thread_mapt &dest) const {
  ...
  for (eventst::const_iterator e_it = equation.SSA_steps.begin();
    ...
    if (e_it->is_spawn()) {
      ...
      for (event_listt::const_iterator n_it = next_thread->second.begin();
           n_it != next_thread->second.end(); n_it++) {
  ...
  for (address_mapt::const_iterator a_it = address_map.begin();
    ...
    for (event_listt::const_iterator w_it1 = a_rec.writes.begin();
      ...
      for (event_listt::const_iterator w_it2 = next;
        ...
        // write-to-write edge
        add_constraint(equation, implies_exprt(s, before(*w_it1, *w_it2)),
                       "ws-ext", (*w_it1)->source);
...
void memory_model_sct::from_read(symex_target_equationt &equation) {
  ...
  for (address_mapt::const_iterator a_it = address_map.begin();
    ...
    for (event_listt::const_iterator w_prime = a_rec.writes.begin();
         w_prime != a_rec.writes.end(); ++w_prime) {
      ...
      for (event_listt::const_iterator w = next; w != a_rec.writes.end(); ++w) {
        ...
        for (choice_symbolst::const_iterator c_it = choice_symbols.begin();
          ...
          if (c_it->first.second == *w_prime && !ws1.is_false()) {
            ...
            cond = implies_exprt(and_exprt(r->guard, (*w)->guard, ws1, rf), fr);
            ...
            // it would even be wrong to add this guard
            cond = implies_exprt(
                and_exprt(r->guard, (*w_prime)->guard, ws2, rf), fr);
          }
